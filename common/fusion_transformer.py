...
class QuantTransformerBackbone(nn.Module):
    ...
    def __init__(self, state_dim=44, hidden_dim=256, n_layers=2, n_heads=4, seq_len=60, dropout=0.1, mode='tactical'):
        super().__init__()
        self.mode = mode
...
class StrategyInteractionLayer(nn.Module):
    ...
    def __init__(self, strategy_dim=8, embedding_dim=32):
        ...
        self.out_proj = nn.Sequential(nn.Linear(strategy_dim * embedding_dim, 64), nn.GELU(), nn.Dropout(0.1))
    ...
    def forward(self, strategies):
        ...
        K = self.key(x)
        V = self.value(x)
        attn_weights = F.softmax(torch.matmul(Q, K.transpose(-2, -1)) / np.sqrt(self.embedding_dim), dim=-1)
