...
const MODEL_INDICATOR_DETAIL = {
  v_rebound: "[계산] **매 5분봉마다** 22개 캔들/오더플로우/모멘텀 피쳐(Tier0)+RSI를 계산해 바닥쪽·천장쪽 양방향으로 TabPFN(사전학습된 트랜스포머가 in-context로 추론하는 표형 파운데이션 모델 — 데이터셋별 재학습이 없음)에 입력하고, 둘 중 확률이 높은 쪽을 그 봉의 판정으로 씁니다. 학습 컨텍스트는 전체 봉 TRAIN 182,969건 중 무작위 18,000건에 고정(자연 라벨비율 14.6% 그대로 보존·재균형 안 함, 라이브에서도 매번 이 컨텍스트를 그대로 재사용, 최신 데이터로 자동 갱신되지 않음).\n" +
    ...
    "[기준선을 50%→60%로 올린 이유, 2026-09-01] 실제 배포된 확률로 트레일링 스톱 경제성을 재측정했더니 **50% 기준은 게이트를 통과하지 못했습니다** — 방향을 반대로 뒤집은 대조군이, 잡음수확 아티팩트가 닿지 않는 구간(ARM=1.5)에서조차 정방향을 이겼습니다(VAL 정4/뒤6, OOS 정10/뒤13). 55%부터 역전되고 60%가 가장 깨끗합니다(VAL 정35/뒤0, OOS 정22/뒤9 · 선택셀 VAL +16.05bp·OOS +11.30bp · precision 0.713/0.683으로 가장 높음). 빈도 손실도 없습니다 — 화면 기준 하루 11~12건, 신호 간격 중앙값 약 1시간, 신호 없는 날 0%(50%일 때는 하루 18건으로 오히려 과했습니다).\n" +
    "[유의] 위 precision(0.713/0.683)은 **결과 라벨이 붙은 봉(전체의 52.9%) 기준**입니다. 화면은 라벨 유무와 무관하게 모든 봉을 채점하므로, 뜨는 신호의 상당수는 결과가 '급반등'도 '횡보'도 아니었던 애매한 봉에 앉습니다 — 라벨 설계에서 오는 구조적 한계이고 모든 기준선에 똑같이 적용됩니다. 참고로 게이트 시절의 '+9.28bp HOLDOUT 통과' 기록은 진입시점을 비현실적으로 가정한 값이라(local_extreme 확정에 30분이 걸리는데 5분 뒤 진입으로 계산) 보정하면 +4.75bp였고, 이 재설계는 그 비현실성 자체를 없앤 것입니다. 봇 내부 상태가 아니라 대시보드 서버가 별도로 계산합니다 — 실제 매매 결정(trading_bot.py)에는 연결되지 않았고, 재량 판단의 한 재료로만 쓰세요.",
