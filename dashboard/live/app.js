...
function mashtFadeIndicatorItem() {
  ...
  const liveText = arm.live_bp_mean != null
    ? `라이브 건당 ${arm.live_bp_mean > 0 ? "+" : ""}${arm.live_bp_mean.toFixed(2)}bp (마감 ${arm.closed}, 시간청산 ${arm.timeouts})`
    : "마감 거래 없음";
  const accText = arm.accuracy != null
    ? `참고·해소분 적중 ${(arm.accuracy * 100).toFixed(1)}% (${arm.wins}/${arm.resolved})` : "";
  const bpText = "";
  const stateTitle = [
    `마지막 앵커 p ${lp != null ? lp.toFixed(4) : "-"} / 되돌림 임계 ≤${thrF.toFixed(4)} · 보유 ${arm.open || 0}`,
    `${liveText}${guard}`,
    accText,
    "판정: 라이브 건당 bp · CI 하한 > 0 · 사전등록 기대 **−3.2bp**",
    "⚠️두 팔 다 기대치 음수 · 주문 없음 -- 가상 원장만",
  ].filter(Boolean).join("\n");
...
function mashtAnchorIndicatorItem() {
  ...
  const m = p.measured || {};
  const lr = m.live_realistic || {};
  const refText = lr.cont_bp != null
    ? `사전등록 라이브 ${lr.cont_bp > 0 ? "+" : ""}${lr.cont_bp}bp [${lr.cont_ci ? lr.cont_ci.join(", ") : "-"}]` : "";
  if (!p.available) {
  ...
  const days = Number(p.days_running || 0);
  const guard = days < 30 ? ` · ⚠️계측 ${Math.floor(days)}/30일` : "";
  // 🔴2026-09-08 판정 잣대 교체: 헤드라인은 **라이브 건당 bp**(시간청산 포함).
  // 해소분 적중률은 결과 선택으로 부풀려진다(백테스트 +12.0bp vs 라이브 −1.46bp).
  const armC = (p.arms && p.arms.cont) || {};
  const liveText = armC.live_bp_mean != null
    ? `라이브 건당 ${armC.live_bp_mean > 0 ? "+" : ""}${armC.live_bp_mean.toFixed(2)}bp (마감 ${armC.closed}, 시간청산 ${armC.timeouts})`
    : "마감 거래 없음";
  const accText = armC.accuracy != null
    ? `참고·해소분 적중 ${(armC.accuracy * 100).toFixed(1)}% (${armC.wins}/${armC.resolved}, 해소율 ${armC.resolve_rate != null ? (armC.resolve_rate * 100).toFixed(0) : "-"}%)`
    : "";
  const bpText = "";
  const lastText = p.last_p_cont != null
    ? `마지막 앵커 p ${Number(p.last_p_cont).toFixed(4)} / 임계 ${thr != null ? thr.toFixed(4) : "-"}`
    : "앵커 대기";
  const stateTitle = [`${lastText} · 보유 ${p.open_positions}`,
                      `${liveText}${guard}`,
                      accText,
                      "판정: 라이브 건당 bp · CI 하한 > 0 · 사전등록 기대 **−1.46bp** [−7.53, +5.93]",
                      "⚠️해소분 59.88%/+12.0bp 는 결과선택으로 부풀려진 값 — 판정에 쓰지 않음",
