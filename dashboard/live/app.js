...
const MODEL_INDICATOR_MEANING = {
  v_rebound: {
    ...
    "급등": "방금 유동성 스윕(지지·저항선을 살짝 뚫었다 되돌아온 것)이 나왔고, TabPFN 모델이 두 경우 중 하나로 판단했어요 — (1) '하락스윕 후 진짜 반등': 60분 안에 가격이 위쪽으로 뚜렷하게 튈 확률이 높다는 판정(정밀도 검증구간 62~74%, 이 라벨 평균발생률의 1.7배로 비교적 신뢰 가능), (2) '상승스윕 후 미반등': 아래로 꺾이는 반전 시도 자체가 없었다는 판정일 뿐, 계속 위로 간다고 확인된 건 아니에요(이쪽은 정밀도 별도 검증 안 함). 두 경우 모두 초록('급등')으로 묶어서 보여드리는 거예요 — 자세한 계산 방식은 '자세히'를 확인하세요.",
    "급락": "방금 유동성 스윕이 나왔고, TabPFN 모델이 두 경우 중 하나로 판단했어요 — (1) '상승스윕 후 진짜 반전': 60분 안에 가격이 아래쪽으로 뚜렷하게 튈 확률이 높다는 판정(정밀도 검증구간 62~74%, 1.7배로 비교적 신뢰 가능), (2) '하락스윕 후 미반등': 위로 꺾이는 반전 시도 자체가 없었다는 판정일 뿐, 계속 아래로 간다고 확인된 건 아니에요(정밀도 70~75%지만 이 라벨의 다수쪽이라 추가정보는 1.2배 정도로 약함). 두 경우 모두 빨강('급락')으로 묶어서 보여드리는 거예요 — 자세한 계산 방식은 '자세히'를 확인하세요.",
  },
  liq_pressure: {
...
const MODEL_INDICATOR_DETAIL = {
  v_rebound: "[계산] 유동성스윕(48봉 causal 스윙 고/저 이탈 후 종가 재진입, 이 대시보드의 liquidity_sweep과 동일 정의)이 발생하면 그 순간 22개 캔들/오더플로우/모멘텀 피쳐(Tier0)+RSI를 계산해 TabPFN(사전학습된 트랜스포머가 in-context로 추론하는 표형 파운데이션 모델 — 데이터셋별 재학습이 없음)에 입력합니다. 학습 컨텍스트는 2024-01~2025-08 스윕 중 '확실한' 3,783건에 고정(라이브에서도 매번 이 컨텍스트를 그대로 재사용, 최신 데이터로 자동 갱신되지 않음).\n" +
    ...
    "[배지 표시: 반등/미반등 콜 → 급등/급락] 2026-08-31부터, 이 '콜'(스윕 방향 대비 반전됐는가)을 스윕 방향과 조합해 실제 예상 가격방향으로 바로 보여드립니다 — 하락스윕 후 반등 콜, 또는 상승스윕 후 미반등 콜이면 '급등'(초록), 상승스윕 후 반등 콜, 또는 하락스윕 후 미반등 콜이면 '급락'(빨강)으로 표시됩니다. 예전엔 '반등'/'반락'을 콜 그대로 노출해서 상승스윕 후 반등 콜처럼 실제로는 하락이 예상되는데도 '반등'이라고 표시되며 빨간색이 뜨는 경우가 있었습니다(사용자 지적, 2026-08-31) — 지금은 색깔과 단어가 항상 일치합니다. **주의**: 미반등 콜로 인한 급등/급락은 '반등 시도 자체가 없었다'는 뜻일 뿐입니다(예: 상승스윕 후 미반등→'급등'이어도 '계속 오른다'가 확인된 게 아니라 '아래로 꺾이는 조짐이 없었다'는 뜻) — 반등 콜로 인한 급등/급락보다 근거가 약합니다. '반락 콜'이라는 예전 이름은 마치 반대방향으로 결정적으로 움직였다는 뜻처럼 들려 실제 정의와 어긋났고, 이를 지적받아 '미반등 콜'로 정정했습니다(사용자 지적, 2026-08-31).\n" +
