...
function extremeDetectorIndicatorItem() {
  ...
  const base = { key: "extreme_detector", label: "극점 탐지기", probaSlot: true,
                 ...
                 derivedTitle: "봇 내부 상태가 아니라 대시보드 서버가 동결 모델로 매 봉 계산합니다(워커). "
                   ...
                   + "채점한다»는 모집단 제약을 둘 다 뺐습니다. 직전 배포본(TabPFN)과 **같은 콜 "
                   + "빈도·같은 채점 절차**로 비교하면 정밀도가 강 61.2→69.7% · 중 50.0→56.2% · "
                   + "약 38.5→50.8% 로 올랐습니다(워크포워드 2창×시드 2셋 4/4 재현). 제약이 후보 "
                   + "봉을 12배 줄여 더 좋은 자리를 고를 기회를 뺏고 있었습니다. "
