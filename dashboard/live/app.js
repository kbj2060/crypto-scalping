...
const MODEL_INDICATOR_MEANING = {
  ...
  masht_anchor: {
    "지속 롱": "천장 앵커에서 상승이 계속된다에 걸어 가상 보유 중입니다. 주문은 내지 않습니다.",
    "지속 숏": "바닥 앵커에서 하락이 계속된다에 걸어 가상 보유 중입니다. 주문은 내지 않습니다.",
    "혼재 보유": "양방향 가상 포지션이 동시에 열려 있습니다.",
    "대기": "마지막 앵커는 진입 조건(지속 확률 ≥0.5372)을 만족했지만 지금 보유 중인 건 없습니다 — 이미 마감됐거나 동시보유 한도에 걸린 경우입니다.",
    "미발동": "마지막 앵커가 이 팔의 진입 조건(지속 확률 ≥0.5372)에 못 미쳤거나, 아직 앵커가 없습니다.",
  ...
  masht_fade: {
    "되돌림 롱": "바닥 앵커에서 반등에 걸어 가상 보유 중입니다. ⚠️이 팔은 워크포워드 실측 적중 52.31%로 손익분기 53.90%에 미달합니다.",
    "되돌림 숏": "천장 앵커에서 반락에 걸어 가상 보유 중입니다. ⚠️이 팔은 워크포워드 실측 적중 52.31%로 손익분기 53.90%에 미달합니다.",
    "혼재 보유": "양방향 가상 포지션이 동시에 열려 있습니다.",
    "대기": "마지막 앵커는 진입 조건(지속 확률 ≤0.5216)을 만족했지만 지금 보유 중인 건 없습니다 — 이미 마감됐거나 동시보유 한도에 걸린 경우입니다.",
    "미발동": "마지막 앵커가 이 팔의 진입 조건(지속 확률 ≤0.5216)을 넘지 못했거나, 아직 앵커가 없습니다.",
    "웜업": "섀도우 러너가 아직 첫 사이클을 돌지 않았습니다.",
...
const MODEL_INDICATOR_DETAIL = {
  masht_fade: "[규칙] 지속 팔과 같은 모델·같은 앵커·같은 청산입니다. 다른 것은 진입 조건과 방향뿐입니다 — 지속 확률이 0.5216 이하(표본외 예측 하위 30%)일 때, 지속의 반대 방향으로 가상 진입합니다. 바닥 앵커면 반등(롱), 천장 앵커면 반락(숏).\n" +
    "[⚠️왜 기대치가 음수인가] 워크포워드 실측에서 두 팔은 대칭이 아닙니다. 지속 팔은 적중 59.88% [53.94, 64.56]으로 건당 +11.6bp인데, 되돌림 팔은 52.31% [47.01, 57.38]로 건당 −3.2bp입니다. 손익분기가 53.90%인데 그 아래입니다.\n" +
  ...
  masht_anchor: "[규칙] 증거신호 8종 중 3종 이상이 3봉(15분) 안에 발동하고 마지막 발동이 그 봉이면 앵커로 봅니다(GAP 12봉 중복제거). 그 앵커 봉을 포함한 48봉 창 × 8채널(로그수익·ATR정규화 경로·DeMarker·%K백분위·테이커델타z·3봉수익z·칼만편차z·고저폭)을 만들고, MultiRocket 2,016열 + Hydra 768열 = 2,784열의 랜덤 합성곱 피쳐로 바꿔 TabPFN(사전학습 표형 파운데이션 모델, in-context)에 넣습니다. 문맥은 3,555개 앵커(2024-01-04~2026-07-29)로 동결돼 있고 라이브에서 갱신하지 않습니다.\n" +
  ...
  v_rebound: "[계산] 매 5분봉마다 22개 캔들/오더플로우/모멘텀 피쳐(Tier0)+RSI를 계산해 바닥쪽·천장쪽 양방향으로 TabPFN(사전학습된 트랜스포머가 in-context로 추론하는 표형 파운데이션 모델 — 데이터셋별 재학습이 없음)에 입력하고, 둘 중 확률이 높은 쪽을 그 봉의 판정으로 씁니다. 학습 컨텍스트는 전체 봉 TRAIN 182,969건 중 무작위 18,000건에 고정(자연 라벨비율 14.6% 그대로 보존·재균형 안 함, 라이브에서도 매번 이 컨텍스트를 그대로 재사용, 최신 데이터로 자동 갱신되지 않음).\n" +
    ...
    "[의미] 매 봉 채점판의 VAL AUC는 0.6942(TabPFN 3시드, std 0.0023)입니다. 게이트가 있던 시절의 헤드라인(VAL 0.829 / OOS 0.813 / HOLDOUT 0.847)보다 낮아 보이지만, 그 수치들이 위에서 설명한 공짜 크레딧을 포함한 값이었습니다 — 같은 모델을 얽힘 조건으로 층화해 다시 재면 내부 AUC가 0.66~0.69로 내려앉고, 매 봉 재학습판이 독립적으로 그 대역에 수렴합니다. 라벨 정의가 달라졌으므로 두 숫자의 직접 비교는 성립하지 않습니다(다른 난이도의 문제를 푼 두 모델의 AUC를 나란히 놓고 우열을 판정하면 안 된다는 이 저장소의 규칙). 0.6942는 같은 문제를 정직하게 푼 점수로 읽으세요. 전체 방법론: docs/homer/README.md + docs/homer/v_rebound_open_issues_20260901.md.\n" +
    ...
    "[⚠️매매 신호로 쓰지 마세요 — 2026-09-01 확인] 이 칩의 경제성 수치는 전부 결과 라벨이 붙은 봉(전체의 약 53%) 기준입니다. 그런데 화면은 라벨 유무와 무관하게 모든 봉을 채점하므로, 뜨는 신호의 상당수는 결과가 '급반등'도 '횡보'도 아니었던 애매한 봉에 앉습니다. 그 전체 모집단에서 방향뒤집기 대조군을 돌려보니(ARM≥1.0 80셀 전수): VAL은 정방향 28 / 뒤집기 0으로 방향이 맞았지만, OOS에서는 정방향 21 / 뒤집기 31로 역전됩니다(아티팩트 무영향 구간만 보면 4 대 16). 즉 화면에 뜨는 콜을 그대로 매매로 옮기면 최근 구간에서는 오히려 반대가 나았습니다.\n" +
  ...
  liq_pressure: "[계산] basis_raw = (선물 종가 − 현물 종가) / 현물 종가 (ETHUSDT, fapi.binance.com 선물 vs api.binance.com 현물). basis_z48 = basis_raw를 직전 48봉(4시간) 평균·표준편차로 정규화한 z값.\n" +
    ...
    "[유의] 이 청산크라우딩 검증은 약 1개월치 탐색적 표본입니다(청산 데이터의 신뢰 가능 구간이 2026-07-18부터 시작) — 이 저장소가 다른 모든 신호에 쓰는 VAL/OOS 3-split 재현성 검증은 아직 못 거쳤습니다. 표본이 더 쌓이면 정식 재검증 예정. 가격이 오르내린다는 뜻이 아니라 '어느 쪽 포지션이 청산 압박을 더 받을 가능성'만 알려주는 리스크 정보입니다. 봇 내부 상태가 아니라 대시보드 서버가 매 사이클 spot/perp klines를 직접 fetch해 계산합니다 — 아직 실제 매매 결정에는 연결되지 않았습니다.",
...
function renderEvidenceSignals(payload) {
  ...
  setH("evidenceSignalList", signals.map((s) => {
    ...
    const probaNote = nowProba != null
      ? `이 확률은 지금 봉에서 재계산한 값입니다 -- "남은 ${leftBars}봉 안에 목표에 닿는가". 발동 봉에서 계산된 원래 값은 ${modelPctText || "-"}였습니다. 시간이 갈수록 남은 창이 짧아지므로 같은 발동이라도 숫자는 내려갑니다.`
      : (isStaleProba || isResolvedByTp
          ? `옆의 확률은 발동 봉에서 계산된 값이고 이후 갱신되지 않습니다 -- 시간이 지날수록 과대평가입니다(실측: 9~12봉 뒤 실제의 약 1.5배).`
          : null);
...
function mashtFadeIndicatorItem() {
  // ⭐라벨칩이 `대조군`인 이유: 이 팔은 매매 후보가 아니라 "모델 정보가 한쪽 꼬리에만 있다"를
  // 전방 검정하려고 도는 반대 꼬리다(아래 derivedTitle·DETAIL이 그렇게 밝힌다). 사전등록 기대치가
  // 이미 **음수**로 측정돼 있어서 "검증 중"이라고 쓰면 "아직 결과 없음"으로 반대로 읽힌다.
  const base = { key: "masht_fade", label: "앵커 되돌림(MASHT)", derivedTag: "= 대시보드 자체계산·대조군",
    derivedTitle: "봇 내부 상태가 아니라 대시보드 쪽 러너가 계산합니다. 지속 팔과 같은 모델의 반대 꼬리입니다. 지속 확률이 하위 30%(≤0.5216)일 때"
      + " 지속의 반대 방향으로 가상 진입합니다.\n\n"
      + "⚠️워크포워드 실측 적중 52.31% [47.01, 57.38] · 건당 −3.2bp 로 손익분기 53.90% 미달입니다."
  ...
  // ⭐2026-09-08 (사용자 요청): `lp <= thrF`는 **되돌림 진입 조건 그 자체**인데 "되돌림 약함"으로
  // 나가고 있었다 -- 가장 강한 되돌림 신호가 화면에 "약함"으로 표시됐다. 그리고 thrF(0.5216)가
  // 0.5보다 크므로 그 다음 `lp < 0.5` 분기는 앞 분기에 이미 걸려 **절대 실행되지 않는 죽은 코드**
  // 였다. 지속 팔과 같은 형태로 맞춘다: 보유 / 대기(조건 충족·보유 없음) / 미발동.
  let subText;
  if (arm.open) subText = hasL && hasS ? "혼재 보유" : hasS ? "되돌림 숏" : "되돌림 롱";
  else if (lp == null) subText = "미발동";
  else if (lp <= thrF) subText = "대기";
  else subText = "미발동";
...
function mashtAnchorIndicatorItem() {
  // 라벨칩은 이 목록에서 **출처**를 말하는 자리다(다른 행: `= 대시보드 자체계산` / `·탐색적` /
  // `= ETH 전용`). 2026-09-08까지 두 앵커 카드가 글자까지 같은 `= 모델 · 섀도우 검증 중`을 달아
  // 기대치가 정반대인 두 팔(+11.6bp vs −3.2bp)이 칩으로 구분되지 않았다.
  const base = { key: "masht_anchor", label: "앵커 방향(MASHT)", derivedTag: "= 대시보드 자체계산·섀도우",
    derivedTitle: "봇 내부 상태가 아니라 대시보드 쪽 러너가 계산합니다. 증거신호 3종 이상이 겹친 앵커에서 48봉 창을 랜덤 합성곱(MultiRocket+Hydra)으로 2,784열 피쳐로 바꿔"
  ...
  //
  // ⭐2026-09-08 (사용자 요청 "카드 로직대로 정확하게"): 상태 판정을 **이 팔의 실제 진입 임계**로
  // 갈랐다. 그 전에는 `lp >= 0.5`라는 하드코딩 기준을 썼는데, 진입 임계는 0.5372다 -- 즉
  // 0.5~0.5372 구간이 "지속 약함"으로 뭉쳐지면서 화면의 판정 기준과 바로 아래 툴팁이 인용하는
  // 임계(thr)가 서로 다른 잣대를 쓰고 있었다.
  // "되돌림 우세"도 뺐다. 2026-09-08에 되돌림 전용 카드가 생겨서 이 카드가 남의 팔을 대신
  // 말할 이유가 없고, 두 카드가 상대어("약함"/"우세")를 각자 다른 기준으로 붙이면 같은 봉에서
  // 서로 어긋나 보인다. 기울기는 두 카드의 게이지(지속 %와 되돌림 %)가 이미 나란히 보여준다.
  const lp = p.last_p_cont != null ? Number(p.last_p_cont) : null;
  let subText;
  if (p.open_positions) subText = hasL && hasS ? "혼재 보유" : hasS ? "지속 숏" : "지속 롱";
  else if (lp == null) subText = "미발동";
  else if (thr != null && lp >= thr) subText = "대기";   // 마지막 앵커는 조건 충족, 지금 보유는 없음
  else subText = "미발동";
