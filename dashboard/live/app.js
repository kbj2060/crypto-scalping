...
const MODEL_INDICATOR_MEANING = {
  v_rebound: {
    ...
    "미반등": "방금 마감된 5분봉에서 TabPFN 모델이 V자반등의 근거를 찾지 못했어요 — 대부분의 봉이 여기 해당하는 **평상시 상태**입니다(전체 봉의 약 85%). '반대방향으로 움직인다'는 뜻이 아니라 '지금은 반등/반전을 말할 근거가 없다'는 뜻이에요. 2026-09-01 매 봉 채점으로 바뀌기 전에는 트리거가 발동한 봉에서만 나오던 판정이라 지금보다 훨씬 드물었어요.",
  },
  liq_pressure: {
...
const MODEL_INDICATOR_DETAIL = {
  v_rebound: "[계산] **매 5분봉마다** 22개 캔들/오더플로우/모멘텀 피쳐(Tier0)+RSI를 계산해 바닥쪽·천장쪽 양방향으로 TabPFN(사전학습된 트랜스포머가 in-context로 추론하는 표형 파운데이션 모델 — 데이터셋별 재학습이 없음)에 입력하고, 둘 중 확률이 높은 쪽을 그 봉의 판정으로 씁니다. 학습 컨텍스트는 전체 봉 TRAIN 182,969건 중 무작위 18,000건에 고정(자연 라벨비율 14.6% 그대로 보존·재균형 안 함, 라이브에서도 매번 이 컨텍스트를 그대로 재사용, 최신 데이터로 자동 갱신되지 않음).\n" +
    ...
    "[기준] 확률≥50%면 '반등 콜'(이후 30분 내 종가로 ATR(직전 기준) 1.5배 이상 반등 AND 60분 전체에서 정점 대비 20% 이하만 반납), 미만이면 '미반등 콜'(반등 근거 없음 — 반대방향으로 뚜렷하게 움직인다는 뜻은 아닙니다). 이 둘 사이(애매한 경우)는 라벨 자체가 없어 **학습에서 통째로 제외** — 라벨 정의(giveback 방식) 자체는 재설계 이후에도 안 바뀌었습니다. 매 봉이 채점되므로 이제 대부분(약 85%)의 봉은 '미반등'입니다.\n" +
    ...
    "[유의] ⚠️**화면의 50% 기준은 '분류' 운영점이지 '매매 가능' 기준이 아닙니다.** 실제 배포된 확률로 트레일링 스톱 경제성을 재측정한 결과(2026-09-01), 방향을 반대로 뒤집은 대조군이 **50% 이하 구간에서는 정방향을 이깁니다** — 즉 그 구간의 급등/급락 콜을 그대로 매매로 옮기면 손실입니다. 55% 이상부터 역전돼 깨끗이 통과합니다(55%: VAL +9.28bp·OOS +11.93bp, 60%: VAL +16.05bp·OOS +11.30bp). 그래서 화면에 뜨는 급등/급락 중에서도 **확률이 55%를 넘는 것만 매매 후보로 보는 게 맞습니다**(칩에 표시되는 확률 숫자를 함께 보세요). 참고로 게이트 시절의 '+9.28bp HOLDOUT 통과' 기록은 진입시점을 비현실적으로 가정한 값이라(local_extreme 확정에 30분이 걸리는데 5분 뒤 진입으로 계산) 보정하면 +4.75bp였고, 이 재설계는 그 비현실성 자체를 없앤 것입니다. 봇 내부 상태가 아니라 대시보드 서버가 별도로 계산합니다 — 실제 매매 결정(trading_bot.py)에는 연결되지 않았고, 재량 판단의 한 재료로만 쓰세요.",
...
const SIGNAL_HORIZON = {
  // -- model indicators --
  v_rebound: { text: "60분", title: "매 5분봉을 채점해 이후 60분(12봉) 안 실제 가격방향(급등/급락)을 예측 -- 확률>=50%인 '반등 콜'은 30분 내 종가로 1.5xATR 반등 후 60분 전체에서 정점 대비 20% 이하만 반납을 요구, 바닥쪽/천장쪽 중 확률 높은 방향과 조합해 급등/급락으로 표시(2026-09-01 트리거 게이트 제거)" },
