...
const MODEL_INDICATOR_MEANING = {
  ...
  vol_level: {
    "안정": "앞으로 4시간 예상 변동폭이 **평소 이하**입니다(학습창 하위 80%, 평소의 1.39배 미만). 조용할수록 같은 위험에서 수량을 **키울 수 있습니다** — 정확한 배수는 이 줄에 마우스를 올리면 나옵니다.",
    "주의": "앞으로 4시간 예상 변동폭이 **평소보다 뚜렷이 큽니다**(평소의 1.39~1.88배, 학습창 상위 20~5%). 같은 위험을 지려면 수량을 **줄여야** 합니다.",
    "위험": "앞으로 4시간 예상 변동폭이 **평소의 1.9배 이상**입니다(학습창 상위 5%). 방향 경고가 아닙니다 — 이 모델은 방향을 예측하지 않습니다.",
...
const MODEL_INDICATOR_DETAIL = {
  vol_level:
    ...
    + "자릅니다. 학습창 비율 분위 실측이 80%=1.392 · 90%=1.646 · 95%=1.882 였고, 상위 20%와 "
    + "상위 5%를 경계로 잡았습니다.\n\n"
    + "🔴 처음에는 40%/80% 분위를 경계로 썼는데, 그러면 **평소(40~80%)를 「주의」라고 부르고** "
    + "시간의 40%를 주황으로 칠하게 됩니다. 위험도 어휘는 «지금 위험한가»를 말하는 자리이므로 "
    + "평소는 안정으로 옮겼습니다. 최근 구간 점유율은 OOS 74.7/15.6/9.7%, TEST 94.0/4.4/1.5% 입니다.",
  breakout_detector:
