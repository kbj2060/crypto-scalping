...
const MODEL_INDICATOR_MEANING = {
  ...
  breakout_rev: {
    ...
    "돌파 ↑": "앞으로 **오른다**는 판정입니다 — 상승으로 발현했고 그 상승이 이어진다고 봅니다. 주문은 내지 않습니다.",
    "돌파 ↓": "앞으로 **내린다**는 판정입니다 — 하락으로 발현했고 그 하락이 이어진다고 봅니다. 주문은 내지 않습니다.",
    "되돌림 ↑": "앞으로 **오른다**는 판정입니다 — 하락으로 발현했지만 되돌아온다고 봅니다. 주문은 내지 않습니다.",
    "되돌림 ↓": "앞으로 **내린다**는 판정입니다 — 상승으로 발현했지만 되돌아온다고 봅니다. 주문은 내지 않습니다.",
    "혼재 보유": "방향이 반대인 가상 포지션이 동시에 열려 있습니다.",
    "직전 돌파↑": "직전 판정은 앞으로 **오른다**였습니다(상승 발현 → 그대로 이어진다). 지금은 보유 중이 아닙니다 — 배리어가 중앙값 5분에 해소돼 포지션은 짧게만 열립니다.",
    "직전 돌파↓": "직전 판정은 앞으로 **내린다**였습니다(하락 발현 → 그대로 이어진다). 지금은 보유 중이 아닙니다 — 배리어가 중앙값 5분에 해소돼 포지션은 짧게만 열립니다.",
    "직전 되돌림↑": "직전 판정은 앞으로 **오른다**였습니다(하락 발현 → 되돌아온다). 지금은 보유 중이 아닙니다 — 배리어가 중앙값 5분에 해소돼 포지션은 짧게만 열립니다.",
    "직전 되돌림↓": "직전 판정은 앞으로 **내린다**였습니다(상승 발현 → 되돌아온다). 지금은 보유 중이 아닙니다 — 배리어가 중앙값 5분에 해소돼 포지션은 짧게만 열립니다.",
...
const MODEL_INDICATOR_DETAIL = {
  breakout_rev: "[규칙] 증거신호 8종 중 **어느 하나가 처음 발동**하면 앵커입니다(신호별 GAP 중복제거, 25.2건/일). " +
    ...
    "발현한 그 순간부터 **1시간 안에 진입가 ±0.8×ATR**(사건별, 중앙값 0.212%) 중 어느 쪽에 먼저 닿는지를 맞힙니다. 발현 방향 쪽이면 " +
    ...
    "[검정] 시드 20개 워크포워드에서 전건 VAL .5643 / OOS .5934 / HOLDOUT .5823, " +
    "셔플 귀무 .5371 / .5490 / .5312 대비 초과 +2.7 / +4.4 / +5.1pp. **20개 시드 전부 세 창 동시에 귀무 위**입니다" +
...
const SIGNAL_HORIZON = {
  // -- model indicators --
  breakout_rev: { text: "1시간", title: "발현(트리거) 시점부터 ±0.8×ATR 배리어를 1분봉 first-touch 로 판정하고, 12봉(1시간) 안에 어느 쪽도 닿지 않으면 12봉 뒤 종가 부호로 정합니다. 배리어는 사건마다 그 시점 ATR 로 정해집니다(중앙값 0.212%) -- 2026-09-08 개정 전에는 절대 ±0.25% 였습니다." },
...
function breakoutRevIndicatorItem() {
  const base = { key: "breakout_rev", label: "앵커 돌파/되돌림", derivedTag: "= 모델 · 섀도우 검증 중",
    derivedTitle: "증거신호가 처음 발동한 뒤 15분 안에 ±0.75×ATR 로 **발현**하면(방향은 관측값),"
      + " 그 방향으로 계속 가는지(돌파) 되돌아오는지(되돌림)를 1시간 ±0.8×ATR 배리어로 판정합니다"
      + "(사건별, 중앙값 0.212%). 2026-09-08부터 가상 원장(주문 없음)으로 검증 중입니다.\n\n"
      + "시드 20개 워크포워드 전건 VAL .5643 / OOS .5934 / HOLDOUT .5823 (셔플 귀무 .5371 / .5490 / .5312"
      + " 대비 +2.7 / +4.4 / +5.1pp). 20개 시드 전부 세 창 동시에 귀무 위(B=32).\n"
  ...
  if (showNum && ageMin != null) {
    ...
    if (p.open_positions) {
      ...
      meterNoteTitle = "발현 시점부터 경과한 시간 / 시간청산까지의 지평(1시간). 배리어(±0.8×ATR)에 닿으면 그 전에 끝납니다.";
    } else {
