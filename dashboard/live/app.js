...
const MODEL_INDICATOR_DETAIL = {
  v_rebound: "[계산] **매 5분봉마다** 22개 캔들/오더플로우/모멘텀 피쳐(Tier0)+RSI를 계산해 바닥쪽·천장쪽 양방향으로 TabPFN(사전학습된 트랜스포머가 in-context로 추론하는 표형 파운데이션 모델 — 데이터셋별 재학습이 없음)에 입력하고, 둘 중 확률이 높은 쪽을 그 봉의 판정으로 씁니다. 학습 컨텍스트는 전체 봉 TRAIN 182,969건 중 무작위 18,000건에 고정(자연 라벨비율 14.6% 그대로 보존·재균형 안 함, 라이브에서도 매번 이 컨텍스트를 그대로 재사용, 최신 데이터로 자동 갱신되지 않음).\n" +
    ...
    "[유의] ⚠️**경제성 게이트는 아직 재확인 중입니다** — 매 봉 재설계판의 트레일링 스톱 경제성은 GBM 프록시 확률로 +8.01bp를 봤지만, 실제 배포된 TabPFN 확률로 다시 돌리는 작업이 남아 있습니다. 게이트 시절의 '+9.28bp HOLDOUT 통과' 기록은 진입시점을 비현실적으로 가정한 값이라(local_extreme 확정에 30분이 걸리는데 5분 뒤 진입으로 계산) 보정하면 +4.75bp였고, 이 재설계는 그 비현실성 자체를 없앤 것입니다. 또한 라벨 발생률이 32.5%→14.6%로 바뀌어 **50% 임계값을 그대로 쓰는 게 맞는지도 아직 판단 전**입니다. 봇 내부 상태가 아니라 대시보드 서버가 별도로 계산합니다 — 실제 매매 결정(trading_bot.py)에는 연결되지 않았고, 재량 판단의 한 재료로만 쓰세요.",
