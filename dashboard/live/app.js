...
const MODEL_INDICATOR_DETAIL = {
  masht_fade: "[규칙] 지속 팔과 **같은 모델·같은 앵커·같은 청산**입니다. 다른 것은 진입 조건과 방향뿐입니다 — 지속 확률이 **0.5216 이하**(표본외 예측 하위 30%)일 때, 지속의 **반대 방향**으로 가상 진입합니다. 바닥 앵커면 반등(롱), 천장 앵커면 반락(숏).\n" +
    "[⚠️왜 기대치가 음수인가] 워크포워드 실측에서 두 팔은 **대칭이 아닙니다**. 지속 팔은 적중 59.88% [53.94, 64.56]으로 건당 +11.6bp인데, 되돌림 팔은 **52.31% [47.01, 57.38]**로 건당 **−3.2bp**입니다. 손익분기가 53.90%인데 그 아래입니다.\n" +
...
function mashtFadeIndicatorItem() {
  ...
  const base = { key: "masht_fade", label: "앵커 되돌림(MASHT)", derivedTag: "= 대시보드 자체계산·대조군",
    derivedTitle: "봇 내부 상태가 아니라 대시보드 쪽 러너가 계산합니다. 지속 팔과 같은 모델의 **반대 꼬리**입니다. 지속 확률이 하위 30%(≤0.5216)일 때"
