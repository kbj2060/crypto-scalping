...
$port = 8787
$serverScript = "dashboard\server.py"

$existing = Get-CimInstance Win32_Process |
  Where-Object { $_.CommandLine -like "*$serverScript*" -and $_.CommandLine -like "*--port $port*" } |
...
$proc = Start-Process `
  -FilePath python `
  -ArgumentList "$serverScript --host 0.0.0.0 --port $port" `
