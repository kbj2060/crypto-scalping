...
CARD30_REACH_PATH = REPO_ROOT / "tmp" / "card30_direction_20260926" / "model_reach.joblib"   # 닿을 확률(표 AUC .72 → .78)
...
def make_app() -> web.Application:
    ...
    # 2026-09-23 K 를 2.0 으로 올렸다가 **되돌렸다**(사용자 「수익은 무시하고 추세만」).
    # 두 축이 정반대를 가리킨다 --
    #  ① 꼬리 수익(배리어 없는 앞 24h 부호수익): K=1.0 −0.2bp -> 2.0 **+2.9** -> 3.0 +5.0
    #     (차 +3.01bp · 일블록 CI [+0.29,+5.93]). 넓은 밴드가 잡음을 견뎌 꼬리를 먹는다.
    #  ② **추세 자체**(지그재그 라벨 채점, 수익 무관) -- 여기선 넓힐수록 **전부 단조로 나빠진다**.
    #     θ=2% 기준 일치율 .6378 -> .6213 · 전환 포착률 .8362 -> .7932 · 전환 지연 150 -> 180분.
    #     θ=1%/3% 도 같은 방향. 좋아지는 건 **유령 뒤집힘**(.3238 -> .2525) 하나뿐인데,
    #     그게 바로 히스테리시스의 존재 이유다.
    # 화면의 veto 는 «지금 어느 쪽 추세인가»를 말하므로 ②를 따른다. K=1.0 이 균형점이다 --
    # 밴드를 아예 빼면(K=0) 추세 지표는 최고지만(일치율 .6471) 유령이 .4480 · 뒤집힘 13.3회/일 로
    # 화면에 못 쓴다. K=1.0 은 유령을 .40 -> .22 로 반 토막 내면서 일치율은 .588 -> .567 만 준다.
    # 🔴어느 K 에서도 «맞힌다»고 말하면 안 된다 -- 앞 24h 방향 정확도가 전 구간 0.47~0.48 로
    #   무조건부 상승률 0.5056 보다 낮다.
    TREND_VETO_K = 1.0          # 밴드 = K x ATR. 실측 폭 중앙 +-0.223%
    ...
    def _card30_pred(bars: list[dict[str, Any]]) -> dict[str, float]:
        ...
        if card30["m"] is None:
            try:
                import joblib
                card30["m"] = {"dir": joblib.load(CARD30_MODEL_PATH), "reach": joblib.load(CARD30_REACH_PATH)}
