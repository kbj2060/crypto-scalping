...
# 🔴660 인 이유(2026-09-20). 화면 창은 5분인데 누적은 **5분 벽시계 경계**에서 0으로
# 돌아간다. 그래서 창 왼쪽 끝이 속한 구간의 누적을 제대로 그리려면 그 구간의 경계까지
# 필요한데, 최악의 경우 now-600 이다(창 시작 300 + 그 앞 구간 300). 360 이면 왼쪽 구간의
# 누적이 «있는 데이터부터» 세어져 낮게 나온다 -- 조용히 틀린 숫자가 되는 쪽이라 늘린다.
# 660 초 x 7 float 는 메모리로 무시할 만하다. 대가는 재기동 후 완전히 차는 데 11분인 것.
