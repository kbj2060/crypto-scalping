...
# 🔴🔴2026-09-22 **재가중** (사용자 지시). 이 표는 트레이더 읽기를 옮긴 것이고 **한 번도 안 쟀다.**
#   카드 자신의 타깃(추세 구간 B=지속, Y_sym)으로 4.7년 29,508 판정봉 · 6봉마다 = 지평 비중복 ·
#   월블록 CI · 연도 부호로 감사한 결과(scripts 아님, 세션 스크래치 rules.py):
#     활발     {B:5,C:5}   ON−OFF **+3.18pp** [+1.78,+4.62] 5/6  ✅
#     추세정렬  {B:3}        **+2.22pp** [+1.21,+3.33] 6/6       ✅
#     거부봉    {A:5,C:10}  **−2.20pp** [−3.54,−0.88] 6/6        ✅ (되돌림 = 엔진 의도와 일치)
#     클라이맥스 {A:15,C:5}  +1.12 [−0.35,+2.47] 4/6  — 효과 없음 (부호도 의도와 반대쪽)
#     스퀴즈    {A:20,C:5}  **+0.16** [−0.94,+1.34] 4/6 — 효과 없음
#     신규유입  {B:20}       **−0.28** [−1.42,+0.83] 3/6 — 효과 없음
#   ⭐⭐**점수 크기와 실측 효과가 반비례했다** -- 20 점짜리 둘이 0 이고 일하는 셋은 3~5 점짜리였다.
#     그 둘은 발동률 46%/53% 로 거의 항상 켜진 채 40 점을 휘둘렀다. 09-22 원장 감사의
#     「1순위가 레짐 안에서 상수」가 정확히 이 모양이다. 카드는 참 확률보다 **약 7배 과신**하고 있었다.
#   **새 눈금: 1 점 = 실측 1pp** (BASE 가 100 점 정규화라 확률 pp 와 직접 대응).
#     효과 없음인 것은 **의도 방향 CI 한계**(0.35 / 0.94 / 0.83pp)로 깎아 1 점.
#   ⚠️**안 잰 것은 안 건드렸다**: A:C 분할은 내 라벨(B vs 나머지)이 말하지 못한다 -- 옛 비율을 그대로
#     두고 크기만 줄였다. 스퀴즈/신규유입 재구성은 30분 OI **부호**뿐이라 엔진 정의(oi_agree ·
#     청산 동반)보다 거칠다 -- 확정된 사실은 「OI 방향 자체는 0」까지다. 청산 데이터는 유효 63일뿐.
#   2차 감사(같은 날, 같은 채점기) -- 역사적으로 재구성 가능한 넷을 더 쟀다. **전부 효과 없음**:
#     CVD_역행     {A:5}   −1.13pp [−2.83,+0.61] 2/6  -> {A:3}  (부호는 의도와 일치, CI 0 포함)
#     BTC_동행     {B:8}   +0.22   [−0.95,+1.37] 2/5  -> {B:1}
#     BTC_단독     {A:8}   −0.22   [−1.30,+0.98] 3/5  -> {A:1}
#     펀딩_반대쏠림 {A:10}  +0.83   [−1.96,+3.12] 1/2  -> {A:2}  🔴부호가 의도와 **반대쪽**이다
#   같은 눈금(효과 없음 = 의도 방향 CI 한계)을 적용했다. 펀딩은 이력이 ~2년뿐이라 n=9,916.
#   ⚠️**가치영역_밖 {A:5} 는 안 건드렸다 -- 측정 실패다.** 엔진은 풋프린트 **가격 셀**로 가치영역을
#     잡는데 내 재구성은 5분봉 대표가 하나씩이라 창당 레벨이 <=6 개다(발동률 76% 가 그 지문).
#     대리변수가 정의와 다른 물건이므로 «효과 없음(−0.67 [−1.93,+0.50])» 을 근거로 쓸 수 없다.
#     정밀 재측정은 풋프린트 이력(최근 구간)으로 가능하다.
#   ⚠️**분배 {A:10,C:10} · 축적 {B:10} · 벽 4종 · 저항/지지/쿠션 · 전환탐지는 여전히 미측정이다.**
#     고래/리테일 플로우는 2026-09-19 부터만 쌓이고(supply_data_accumulation_state), 호가·청산맵도
#     최근 구간뿐이다. **남은 최대 점수가 여기 몰려 있다** -- 표본이 쌓이면 같은 채점기로 잰다.
#   되돌리려면 이 커밋 하나만 revert 하면 된다(숫자만 바뀌었다).
SCORES: dict[str, dict[str, int]] = {
    "스퀴즈": {"A": 1}, "신규유입": {"B": 1}, "클라이맥스": {"A": 1},
    "전환탐지": {"A": 5, "C": 5},
    ...
    "추세정렬": {"B": 2}, "추세역행": {"A": 2}, "분배": {"A": 10, "C": 10}, "축적": {"B": 10}, "거부봉": {"A": 2, "C": 4},
    ...
    "가치영역_밖": {"A": 5}, "저항근접": {"A": 5, "B": -5}, "쿠션없음": {"C": 5}, "활발": {"B": 3, "C": 3},
    ...
    "펀딩_반대쏠림": {"A": 2}, "선물주도": {"A": 8}, "현물주도": {"B": 8},
    # BTC 같은 창 이동: 시장 전체가 같이 갔으면 지속, ETH 만 갔으면 되돌림 쪽
    "BTC_동행": {"B": 1}, "BTC_단독": {"A": 1},
...
if __name__ == "__main__":
    ...
    # 양 끝은 엄격히, 가운데는 >= -- 2점짜리라 100 정규화 반올림에서 대조와 동점이 날 수 있다.
    assert ra["prob"]["B"] > rv["prob"]["B"], (ra["prob"], rv["prob"])
    ...
    assert r6["evidence"]["btc_rel"] == "동행" and any("시장 전체" in x for x in r6["labels"])
    r7 = classify(dict(inp, btc=dict(move_bp=5.0, range_bp=60.0)))
    assert r7["evidence"]["btc_rel"] == "단독" and any("단독" in x for x in r7["labels"])
    # 2026-09-22 재가중으로 둘 다 1점이라 100 정규화 반올림에서 큰 칸(A~53)은 동점이 난다.
    # 작은 칸(B~17)에서만 2점 차가 살아남으므로 **B 로 대조**한다. A 는 >= 로 둔다.
    assert r6["prob"]["B"] > r7["prob"]["B"], (r6["prob"], r7["prob"])
    assert r7["prob"]["A"] >= r6["prob"]["A"] and r6["prob"]["B"] >= r["prob"]["B"] >= r7["prob"]["B"], \
        (r6["prob"], r["prob"], r7["prob"])
