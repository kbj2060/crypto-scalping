...
| 레짐인지형 가드 라이브 섀도우 | `scripts/live_eth_regime_aware_exit_guard_shadow_20260814.py` | 서버 상시 실행 중, h48qual 가드만 관찰 — **zig075 진입거부는 아직 이 프로세스에 없음(다음 점검 대상 #1)** |

## 신규 리소스 (섀도우 배포, 2026-08-14)

| 파일 | 용도 | 상태 |
|---|---|---|
| `scripts/live_eth_odyssey4_zig075_entry_veto_shadow_20260814.py` | Odyssey4 통합 섀도우 — h48qual 레짐인지형 exit 가드(오디세이3, 무변경) + zig075 SHORT 지속상승장 진입거부(오디세이4 #1) 단일 프로세스. `live_eth_jmlam4_regime_swap_shadow_20260809.py`·`live_eth_exithead_asymmetric_shadow_20260813.py` 대체 목적 | 서버 push+import/adapter/guard 생성 검증 완료, **systemd 활성화는 사용자 sudo 실행 대기** |
| `scripts/ops/systemd/eth-odyssey4-shadow.service` | 신규 systemd 유닛(기존 두 유닛과 동일 패턴) | 서버 push 완료 |
| `scripts/ops/systemd/install_and_cutover_odyssey4_shadow_20260814.sh` | 1회 cutover 스크립트 — 신규 유닛 설치 + 기존 두 섀도우 stop/disable + 신규 섀도우 enable/start | 서버 push 완료, **root sudo로 사용자가 직접 실행해야 함**(`deploy_watcher_sudoers`가 stop/disable/enable을 의도적으로 미허용 — 코딩 에이전트 실행 불가) |
