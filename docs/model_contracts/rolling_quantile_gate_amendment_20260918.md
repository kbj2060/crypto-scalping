# ⛔이 문서는 «해소»됐다 — 개정은 필요 없었다 (2026-09-18 최종)

제안했던 면제 조항은 **필요 없었다.** 계약 원문은 부모 의무를 이미 따로 걸어 두었고,
빠져 있던 것은 조문이 아니라 **그 의무를 검사하는 구현**이었다. 그 자리를 «면제»가 아니라
«검사»로 채우니(`--parent-only`, 우회 방지 포함) v3·v4 모두 서버에서
**exit 0 · promotion_pass=true · 15/15 통과**가 나왔다. 상세: `docs/zeus/README.md §3.7`.

아래는 그 과정의 기록이다 — **초판의 전제(qXXX 규약)가 틀렸다는 정정을 포함해** 남긴다.
이 문서를 근거로 조문을 고칠 필요는 없다.
