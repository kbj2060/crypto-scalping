...
Status: `regime_spine_adopted_perf_inconclusive` — **레짐 척추 balnobb K=0 확정·채택**. 성과 검증(Phase 2/3)은 **무신호로 종료**(측정 방식에 따라 부호가 뒤집힘). 2026-09-09 사용자 결정으로 TabPFN 축으로 복귀.

Last updated: 2026-09-09 KST (Phase 2/3 종료, TabPFN 축 복귀)
...
| 방향 정보 (OOS h48, bp) | +18.2 | +17.4 | +9.4 — 격차 CI 0 포함, **무승부** |
...
**방향 축은 무승부로 정정됐다** — 7일 블록 짝지은 부트스트랩에서 HMM−balgbm 격차 CI 가 6셀 중
5셀에서 0 을 포함하고, 유일하게 구분된 셀(VAL h48 −14.3 [−21.8,−6.8])은 부호가 balgbm 쪽이다
(`docs/experiments/omega461_regimegbm_balancedish_model_swap_20260909.md` §4 정정).
따라서 **하이브리드(HMM 방향 + GBM chop)는 전제가 없어 만들지 않는다.**
...
**채택: `balancedish_adx16_slope15_bb012` 라벨 + HistGradientBoosting (arm `balgbm`).**
전신 HMM 폐기(사용자 결정), s12k3 arm 종료.
전문: `docs/experiments/omega461_regime_label_ab_balgbm_vs_s12k3_20260909.md`

라벨 A/B 는 **모델급을 완전히 통제**한 상태에서 수행했다 — 동일 HGB HP/136피쳐/컷오프/시드/OOF,
차이는 학습 타깃 라벨 하나뿐.

| 축 | balgbm | s12k3 | 판정 |
|---|---|---|---|
| **변동성 분리력**(OOS h12/h48/h288) | 1.776 / 1.391 / 1.224 | 1.120 / 1.086 / **0.924** | **balgbm** — 6셀 부호일치, **5셀 CI 0 배제** |
| 방향 판별력 | — | — | 무승부 (6/6 구분 불가) |
| 안정성(OOS) | flip **0.0497** · 8.5봉 | flip 0.0978 · 7봉 | **balgbm** |
| 라우팅 MDD | +3.73 / +5.24pp | +2.65 / −1.50pp | **balgbm** (두 창 개선) |
| 피쳐 대비 신규성(재구성 κ) | 0.962 (중복 높음) | **0.765** | s12k3 — 단 **측정된 성과 없음** |

s12k3 은 OOS h288 에서 변동성비 **0.924, CI[0.897,0.967]** 로 **1 미만이 유의** — chop 태그가
뒤집혀 있다(chop 이라 부른 봉이 더 변동적). 이것이 s12k3 종료의 결정적 근거다.

s12k3 의 유일한 우위인 "피쳐 대비 신규성"은 라벨 무관 두 축 어디에서도 성과로 이어지지 않았다.
그것을 근거로 고르는 것은 **측정된 증거를 미측정 가설로 이기려는 것**이라 채택하지 않는다.

⚠️ 두 라벨 **모두 오메가 라우팅용으로 선택된 적이 없다**(balancedish 는 2026-05-30 HMM 라인,
S12_K3 은 대시보드 표시/증거신호 게이트용). "둘 중 낫다"이지 "오메가에 최적"이 아니다 —
라벨을 오메가 목적함수로 재설계하는 축은 **미탐색**으로 남는다.

## ✅ 레짐 척추 최종 확정 (2026-09-09, 사용자 결정)

**채택: `balnobb` — balancedish 라벨에서 BB 덮어쓰기 제거 + HGB, debounce 없음(K=0).**
아티팩트 `tmp/omega461_regimegbm_rebuild_20260909/regime_balnobb_cut2509_model.joblib`,
사이드카 접두사 `regime3_balnobb_cut2509_`.

선택 근거 (사용자 기준: "진입과 청산은 어차피 딥러닝 모델이 측정하고 우린 차트에 맞게 레짐만
잘 분류하면 돼" → **분류 충실도 우선**):

| | balgbm(원본) | **balnobb(채택)** |
|---|---|---|
| 강추세를 chop 으로 오분류 | **추세 후보의 42.2%** ❌ | 없음 ✅ |
| 클래스 배분(OOS) | 14.2 / 16.3 / **69.5** | **24.6 / 27.5 / 47.9** ✅ |
| OOS 전환 / 평균지속 | 845 / 20.1봉 | 1,674 / 10.1봉 |
| ≤5봉 비율 | 38.1% | 48.0% |

**받아들이는 대가(명시)**: K=0 이므로 전환이 하루 약 28회, 상태의 48%가 25분 이내에 끝난다.
전문가 서브넷이 그만큼 자주 교체된다. debounce 로 줄일 수 있으나(K=6 이면 656회·≤5봉 0.2%,
진입지연 19봉) **분류 충실도와 반응속도를 우선한 사용자 판단**으로 K=0 을 쓴다. 되돌리려면
예측 위에 `_debounce(pred, K)` 를 거는 것만으로 충분하다(라벨/모델 재학습 불필요).

**폐기/종료된 대안**: wide24 HMM(전신, 사용자 폐기) · s12k3(closed-negative, OOS h288 변동성비
0.924 로 chop 태그 역전이 유의) · fwd48 전향 라벨(K1 통과했으나 **K2 실패** — 6셀 중 1셀만
통과, OOS h48 부호 반전).

### ⚠️ Phase 2 로 넘길 때 반드시 지킬 것 — 컬럼 이름

전신 부모의 `base_cols` 마지막 6개는 `regime3_current_sensitive_wide24_*` 이고, **라이브
어댑터(`omega4_6_1_live.py`)가 그 이름으로 값을 자체 계산해 넣는다.** 새 값을 같은 이름에
덮어써서 학습하면, 라이브에서는 여전히 wide24 HMM 값이 들어가 **조용한 train/inference
불일치**가 된다(이 저장소가 반복해서 당한 결함 유형). 따라서 Phase 2 는
**새 접두사 `regime3_balnobb_cut2509_` 를 base_cols 에 그대로 쓰고**, 라이브 어댑터도 같은
접두사를 계산하도록 함께 바꿔야 한다. 둘 중 하나만 바꾸면 안 된다.

## Phase 2 / 3 결과 — 성과 검증 **무신호로 종료** (2026-09-09)

### 실행한 것
- **Phase 2**: 부모 TabM 재학습 20회 (2컴포넌트 × 5시드 × 2arm). balnobb 스파인 vs wide24 대조,
  같은 트레이너·시드·라벨·HP. 사이징은 BASE_TEMPLATE 고정(notional 0.45 / leverage 2.0).
- **Phase 3-a**: 부모 20개 각각에 리스크 사이드카 적합. 설정은 배포본 `report.json` 에서 복원
  (⚠️`max_validation_mdd_abs=25.0`·`live_exposure_grid`·`tail_penalty 0.5`·
  `min/max_validation_avg_notional 0.45/0.95`. 기본값 8.0 으로 돌리면 전부 실패한다).
  **20개 중 6개가 선택 격자 제약에서 실패** — balnobb 2 / **wide24 대조군 4**. 편향은 아니고
  이 라인의 부모들이 제약을 자주 못 맞춘다는 구조적 사실.
- **Phase 3-b**: greedy_replay 결합 판정 — h48qual 사이드카 결측으로 완전한 짝이 2개뿐이라
  5시드 게이트 충족 불가. **사용자 결정으로 중단.**

### 결과 — 측정 방식이 바뀌면 승자가 바뀐다

| 측정 | zig075 | h48qual |
|---|---|---|
| Phase 1 (동결 부모, 라우팅만) | VAL −24.10pp / OOS +80.33pp (부호 반대) | 〃 |
| Phase 2 (재학습, **고정** 사이징) | 무신호 — 둘 다 1/5, 중앙 **+0.17pp** | 좋음 — 둘 다 3/5, 중앙 **+7.02pp** |
| Phase 3-a (재학습, **사이드카** 사이징) | 좋음 — 둘 다 3/4, 중앙 **+12.84pp** | 나쁨 — 중앙 **−8.59pp** |

Phase 2 ↔ 3-a 에서 두 컴포넌트의 우열이 **정확히 맞바뀐다.** 부모는 그대로이고 사이징 방식
하나만 달라졌다. Phase 2 안에서도 VAL/OOS 부호가 반대였다(h48qual VAL 4/5 vs OOS 1/5).
**사이징 방식 하나로 부호가 뒤집힐 만큼 신호가 약하다** — 거래 28~35건/창의 필연적 귀결.

### 그럼에도 확정된 것 — 분류 품질

측정 방식에 흔들리지 않은 축은 **레짐 분류 품질**이다:

| | balgbm(원본) | **balnobb(채택)** |
|---|---|---|
| 강추세를 chop 으로 오분류 | 추세 후보의 **42.2%** | 없음 |
| 클래스 배분(OOS) | 14.2/16.3/**69.5** | **24.6/27.5/47.9** |
| 변동성 분리력(vs s12k3) | — | 6셀 부호일치·**5셀 유의** |

사용자가 정한 기준("차트에 맞게 레짐만 잘 분류하면 된다")으로는 balnobb 채택이 옳고 근거도
견고하다. **다만 그것이 성과로 이어진다는 증거는 만들지 못했다** — 이 라인의 정직한 최종 위치다.

### 재개 조건
표본 희소(창당 28~35건)가 이 라인의 구속조건이다. 성과 검증을 재개하려면 그것부터 해소돼야
한다(거래 빈도를 늘리는 별개 축). 사이드카 제약을 완화해 짝을 복구하는 것은 **결과를 본 뒤의
사후조정**이므로 채택하지 않는다.
