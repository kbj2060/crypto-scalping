...
- Evening session: PnL-maximization diagnostics + multi-slot shadow line.
  Sizing lever CLOSED by measurement: multiplier sweep on the promoted gated ledgers shows current sizing already at the growth optimum (OOS peaks at ~1.25x +12.9% then falls -- 2x +4.6%, 3x -5.7% -- while MDD balloons; cap-3.6 variants destroy PnL/MDD ratio). Reproduces the 2026-07-13 "multiplier doesn't help BTC" finding on the new model.
  Slot-occupancy diagnosis: OOS occupancy 92%; 625 duration-gated quality-passing signal clusters discarded while holding (vs 41 taken).
  Multi-slot equal-budget replay (`scripts/research_btc_swingtransition_multislot_20260807.py`, pre-registered: VAL selects N with MDD>=-8%; OOS adopt iff PnL>=+13.8 AND MDD>=-16.5 AND worst quarter>=-4): N=1 regression reproduces promoted numbers to <0.05pp; VAL selected N=3; OOS gated +16.72%/MDD -7.27%/124tr vs +10.75%/-12.41%/41tr single-slot, quarters +9.9/+6.9/-0.6. ADOPTED to shadow-first validation.
  New standalone shadow loop `scripts/run_btc_multislot_shadow_loop_20260807.py` (omega5 live-only loop pattern; in-bot single-slot BTC shadow untouched -- live-forward A/B). Supervised via `scripts/ops/supervisor_btc_multislot_shadow.sh`, state `data/ensemble/omega4_6_1_btc_multislot_shadow_state_20260807.json`, ledger `..._ledger_20260807.csv`. Real-execution caveat recorded: concurrent opposite-side slots need hedge mode or a same-side-only rule.
  Multiplier extension on the multislot stream (pre-registered sweep {1.25,1.5,1.75,2.0}, VAL selects s.t. VAL MDD>=-8%, OOS adopt iff PnL>=+19.7 AND MDD>=-12.4 AND worst quarter>=-4): VAL selected 1.5x; OOS gated +19.98%/MDD -10.40%/worst quarter -0.92% -> adopted into the shadow loop as `BTC_MULTISLOT_SHADOW_MARGIN_MULT=1.5` (set 1.0 to fall back). Rationale: slot diversification lowers volatility drag, moving the growth-optimal multiplier above the single-slot ~1.0-1.25x Kelly point. Third OOS look of the day on this window -- the shadow live-forward record remains the binding referee before any real-execution decision.
  ETH policy-debt resolution: documented-exception decision recorded at `docs/audits/omega461_eth_frozen_policy_debt_decision_20260807.md` -- frozen 07-06 ETH artifacts are grandfathered (core checks clean; lineage backfill would fabricate provenance the drift bisection proved unknowable; retraining is non-reproducible); ALL future ETH/BTC candidates must pass the full generic gate with dataset_snapshot-registered lineage.
  End-of-session integrity sweep: BTC asset-specific audit re-run promotion_pass=true; ETH live stack (omega4_6_1_extended_oos_20260706, both components) passes ALL core integrity checks (exact-threshold precomputed predictions + train/validation/oos timestamp match vs runtime frame) but fails the generic 20260630 audit on the six post-2026-07-30 policy fields (constraint declaration + dataset_lineage missing from the FROZEN 07-06 reports) -- same gate-version drift class as BTC's. Do NOT retrain the frozen ETH artifacts to fix report fields (ETH parent retraining is known non-reproducible); treat as documented policy debt or backfill report fields without touching artifacts. Also noted: the in-bot BTC shadow carried over an OPEN LONG from 2026-07-18 (entered by the previous bundle, now exit-managed by the promoted bundle -- mixed attribution for that one trade); workbench 32/32 and omega461 contract/manifest tests green; swing parity re-verified 0.0 diff on final code.
  Two live-wiring bugs found+fixed during the loop smoke test: (1) swing_transition provider ran before the adapter's regime3 append -- layerA needs 4 regime3_current_* cols; trading_bot.py now pre-appends regime3 before the swing feature (same-day latent bug in the morning wiring, would have fired on next bot restart); (2) the bot's causally-trimmed kline buffer is ~500 bars right after boot, far short of the 1h overlay's windows -- the provider now self-fetches a 14-day 5m kline cache from the Binance futures public REST when the passed buffer is short (offline parity re-verified 0.0 diff after both fixes).
