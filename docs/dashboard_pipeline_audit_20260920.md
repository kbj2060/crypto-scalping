...
### 실측 (헤드리스 20초, 라이브 페이로드 · 목서버라 **비압축** 기준)
| 창 | 배포본 | 증분 | |
|---|---|---|---|
| 1h (12봉) | 15,198 B/s | **1,932 B/s** | −87% |
| 4h (48봉) | 57,479 B/s | **1,987 B/s** | −97% (29배) |

요청 수는 그대로(20초에 35~40회 = 2/s). 단건 payload **32,829B → 1,104B**.
`ScriptDuration` 도 0.199→0.155s(1h)로 덤으로 줄었다(파싱할 JSON 이 줄어서).
⚠️실서버는 gzip 이 걸리므로 절대값은 더 작다 — 배포 후 재측정할 것.
