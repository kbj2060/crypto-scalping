...
## 15단계 · XRP 내부지표 3종 실시간 표시 확정 ✅

사용자: *"xrp도 내부 지표 3개 실시간으로 대시보드에서 볼 수 있게해줘"*

14단계에서 배선했는데도 화면에 안 보였다. 원인 **3건**을 찾아 고쳤다.

### ⚠️원인 1 — 툴팁이 "ETH 고정"이라고 안내하고 있었다

코인 탭의 title이 *"증거신호·레짐·특화감지기·수급흐름·리테일수급·청산캐스케이드는 **ETH 고정**입니다"*
였다. 이미 사실이 아닌데 **UI가 사용자에게 미지원이라고 말하고 있었다.**
실제 지원 범위로 갱신했다(아직 ETH 고정인 건 특화감지기 하나뿐).

### ⚠️원인 2 — `MODEL_INDICATOR_POLL_MS`가 **정의돼 있지 않았다**

`refreshCoinIndicators`의 스로틀이 `now - x < undefined` → 항상 false라 **매 틱(2.5초)마다**
요청하고 있었다. 서버 캐시가 20초라 무해했지만 낭비였다. 20000으로 정의했다.

### ⚠️원인 3 — 최신 1행만 보면 **whale이 절반은 빈 값**

XRP 실측(24시간): `nif_retail` **93.7%** / `nif_whale` **49.7%**만 비null이다.
고래 흐름은 **대형 체결이 있는 분에만** 계산되므로 구조적이다.
최신 행 하나만 읽으면 화면이 자주 "값 없음"이 된다.

⇒ **최근 15분 안의 마지막 비null 값**을 쓰고 **몇 분 전 값인지 함께** 내려준다.
오래된 값을 "지금 값"인 것처럼 보여주지 않기 위해서다.

### 실측 (배포 후)

```
수급 흐름       nif_whale  = -1.000  (3분 전)
리테일 수급     nif_retail =  0.028  (방금)
청산 캐스케이드  Z = 2.035  → "주의" 티어
최신 봉: 2026-09-03 06:15:00 KST
```

검증(dukpy 실행): 방금 값 → `유출 -1.000 · 방금` / 2분 전 값 → `유출 -1.000 · 2분 전` /
창 내 없음 → `최근 15분 내 값 없음`.
...
## 17단계 · 라벨 칩을 ETH와 동일하게 통일 ✅

사용자: *"수급흐름·리테일수급·청산캐스케이드 라벨 칩도 이더리움과 나머지가 조금 다른거 같아"*

### ⚠️원인 — subText에 자유 문구를 넣은 게 3가지를 동시에 깨뜨렸다

내가 코인 경로의 `subText`를 `"유입 0.552 · 방금"`처럼 **자유 문구**로 만들었다.
그런데 ETH는 `directionalCaution()`이 만드는 고정 어휘 **"롱 진입"/"숏 진입"/"중립"**을 쓰고,
`renderModelIndicatorList`가 그 문자열을 **두 곳에서 그대로 사용**한다:

1. **칩 문구**: `stateEl.textContent = arrow + it.subText` → ETH는 `▲ 롱 진입`, 내 쪽은 `▲ 유입 0.552 · 방금`
2. **의미 사전**: `MODEL_INDICATOR_MEANING[key][subText]` → 자유 문구는 **사전에 없어 설명이 통째로 사라짐**
3. **톤 스트립**: `history: []`로 비워 둬서 ETH엔 있는 4시간 막대가 안 나옴

### 수정 — ETH와 **같은 함수·같은 모양**으로

| 요소 | 전 | 후 |
|---|---|---|
| `subText` | `유입 0.552 · 방금` | **`directionalCaution(v, 0.05)`** → `롱 진입`/`숏 진입`/`중립` |
| 값 설명 | (없음) | **`flowRead()`/`retailFlowRead()`** → `큰손 매수가 강하게 들어옴` |
| 숫자·경과 | subText에 섞임 | **`liveText`로 분리** → `큰손 매수가 강하게 들어옴 (0.552 · 방금)` |
| 톤 스트립 | 없음 | **48샘플 × 5분 = 4시간**(ETH `MICRO_HISTORY_MAX`와 동일) |
| 캐스케이드 | `평온 (Z:-0.3)` | **`안정`/`주의`**(ETH `liqCascadeHint` 어휘) + Z는 `liveText`로 |

서버가 톤 스트립을 만든다 — 1분 테이블에서 **5분마다 하나씩** 뽑고 임계값은
`classifyIndicators`와 동일한 ±0.05를 쓴다.

### 검증 (ETH 경로와 직접 대조)

```
v= 0.552   eth {sub:"롱 진입",  val:"큰손 매수가 강하게 들어옴"}
           coin{sub:"롱 진입",  live:"큰손 매수가 강하게 들어옴 (0.552 · 방금)"}  ✅
v=-0.283   eth {sub:"숏 진입"}  coin{sub:"숏 진입"}   ✅
v= 0.01    eth {sub:"중립"}     coin{sub:"중립"}      ✅
값 없음     coin{sub:"중립", live:"최근 15분 내 값 없음"}
```

서빙 확인: btc/xrp 모두 `whale_hist 48 / retail_hist 48 / cascade_hist 48`.
