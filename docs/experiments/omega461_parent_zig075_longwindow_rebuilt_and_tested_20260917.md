...
- 남은 레버는 **품질 머리 보정**이다(통과율 3.5% vs 8.3% 가 성과 차의 대부분으로 보인다).
  ...
  하려면 검정력부터 세야 한다.
  → **실행했다**: 보정은 base 에만 듣고(+3.80→**+5.56bp**, 배포본 +4.94 를 넘는다) deep 은
  못 살린다(순위 문제). 🔴그런데 **7/7 모델이 숏 다리 우위**고 **「모델−항상숏」 초과는
  0/7 이 0 을 배제**한다 — 이 창의 성과는 숏 편향과 분리되지 않는다.
  [omega461_parent_quality_calibration_and_position_bias_20260917](omega461_parent_quality_calibration_and_position_bias_20260917.md)
