...
### 후보 9. 텍스트 뉴스/소셜 센티먼트(NLP) — ❌ 닫힘 (검증 완료, 2026-08-31)

**배경**: 이 문서와 별개로 진행 중이던 `docs/eth_rl_autotrading_agent_design_20260831.md`의 entry
게이트 조사(quality_head와 정확히 같은 질문 — "이미 direction이 고른 후보가 팔로우스루할지")가
7가지 접근(지도재순위 5회+분류기 1회+selective classification 1회) 전부 실패로 소진되자, 사용자가
"완전히 새로운 데이터"로 LOB/오더북과 나란히 뉴스/센티먼트를 지목해 처음으로 실제 조사함(§"완전
신규 구축 필요" 항목 8행 참고 — 이 문서가 이미 2026-08-11에 "존재하지 않음, 완전 신규 구축 필요"로
플래그해뒀던 바로 그 항목).

**해상도 자체가 구조적으로 안 맞음**: 조사한 어떤 소스도 5분 주기로 갱신되지 않는다 — 최선이
시간봉 집계를 5분봉에 forward-fill하거나, 이벤트 트리거형(특정 기사/트윗 발생시에만 갱신)이다.
"센티먼트로 5분봉 진입타이밍을 개선한다"는 질문 자체가 이 데이터 성격과 안 맞는 프레이밍이라는
게 첫 번째 결론.

**소스별 실사**(전부 실측, 추측 아님): CryptoPanic — 2026년 초 무료 티어 폐지, 유료전환(정확한
가격은 공식 문서가 자동조회를 막아 미확인). GDELT — 무료·2015년부터 전체이력(BigQuery bulk
`gdelt-bq.gdeltv2.gkg_partitioned`, 15분마다 갱신)이라 2025년 TRAIN 백필은 실제로 가능하나
크립토 특정 커버리지가 전혀 없는 일반 글로벌뉴스라 자체 엔티티필터 구축이 선행돼야 함. NewsAPI.org
— 실제 감성필드 자체가 없음(원문 기사만), 상용 티어 월$449. Glassnode — "소셜/센티먼트" 카테고리가
현재 제품에 아예 없음(24개 카테고리 공식 목록 직접 확인). Twitter/X — 전체이력 검색은 Enterprise급
(월$42,000~50,000), 이 조사 목적에 비대칭적으로 비쌈. Reddit — 2026-06 갱신된 Data API 약관이
**ML 모델 학습 목적의 데이터 사용을 상업 라이선스 없이는 명시적으로 금지**(비용 이전에 법적
차단), pushshift.io는 2023-05부터 대중 접근 중단, 후속 PullPush는 불안정.

**외부 문헌도 부정적**: 이 프로젝트 호라이즌에 가장 가까운 연구(arXiv:2401.00603, 15분 인트라데이
트위터 트레이딩)는 센티먼트가 "가격 움직임에 식별 가능한 영향 없음"이라고 결론. 가장 긍정적인
문헌(일봉 호라이즌)조차 저자 스스로 "복수 시장 레짐에 걸쳐 강건한 우위를 보인 동료검토 연구
없음"이라고 명시 — 일봉에서도 약한 근거를 5분봉으로 두 자릿수 배율 외삽할 근거가 없음.

**판정**: 이미 무신호로 확정된 기존 원천(AUC~0.51~0.52 direction head 후보 풀)에, 해상도가
구조적으로 안 맞고 문헌 근거도 얇으며 무료 대안은 소진되고 유료 대안은 비용 대비 근거가 부족한
데이터를 추가하는 건 근거 있는 다음 수가 아니다. **비권고, 재제안 금지** — 유일한 예외는 GDELT를
"느린 매크로 뉴스쇼크 플래그"(기존 `news_shock_guard.py` 계열, 텍스트 아닌 합성 신호와 같은
카테고리)로 격을 낮춰 쓰는 것인데, 이것도 이 프로젝트가 이미 여러 번 확인한 `corr(price)` 오염
게이트를 먼저 통과해야 하고 이 문서의 우선순위 최하단(5분 entry 문제가 아니라 별개의 느린 문제).

**하지 말아야 할 것에 추가**: 크립토 뉴스/소셜 센티먼트(무료·유료 불문)를 5분봉 entry-quality
개선 후보로 재제안하는 것.
