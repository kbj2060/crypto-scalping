...
# 5부 — 종료 상태 표기 정리 (2026-09-03, 사용자 신고)

사용자: *"갑자기 익절하고 나면 **미발동 82% 익절 2391.71 도달** 이런 식으로 텍스트가 떠"*

## 원인 — 상태는 껐는데 부속 텍스트는 그대로 뒀다

3부에서 라벨 확정 시 `fired=False`로 만들면서, **칩이 왜 꺼졌는지 설명하려고**
`tp_price` / `tp_touched` / `proba`를 일부러 남겼다. 그런데 **프론트를 같이 안 고쳤다.**
그래서 "미발동"(state) + "82%"(발동 당시 확률) + "익절 … 도달"이 한 줄에 나왔다 —
서로 모순되는 문구다.

⚠️이건 3부 변경이 만든 **회귀**다. 서버 필드를 남겨두는 선택 자체는 옳았지만,
그 필드를 읽는 쪽을 함께 손보지 않으면 이런 조합이 나온다.

## 수정

| 요소 | 전 | 후 |
|---|---|---|
| 상태 | `미발동` | **`목표 도달 · 종료`** (진짜 미발동과 구분) |
| 확률 | `82%` | **`(82%)`** — 괄호로 **발동 당시** 값임을 표시 |
| 가격 | `익절 2391.71 도달` | **`바닥 2391.71`** (state가 이미 도달을 말하므로 중복 제거) |
| 색조 | 방향색(good/bad) | **neutral** — 방향색을 두면 아직 유효한 신호처럼 보인다 |
| 배지 | `🎯 목표 도달 · 15봉 전 발동` | **`🎯 15봉 전 발동`** (state와 중복 제거) |

`isResolvedByTp = !bottom_fired && !top_fired && model_tp_touched === true`로
**"라벨 확정으로 꺼짐"과 "애초에 안 뜸"을 구분**한다.

## 검증 (렌더 문구 직접 실행)

```
종료    → 목표 도달 · 종료 (82%) 바닥 2391.71  [🎯 15봉 전 발동]
발동중  → 바닥 발동 74% 익절 2402.13
미발동  → 미발동 0%
```

세 상태가 서로 섞이지 않고, 종료 상태에서 "미발동"과 "도달"이 동시에 나오지 않는 것을 확인했다.

## 교훈

⭐**표시 상태를 끄면서 부속 필드를 남길 때는 그 필드를 읽는 쪽을 같이 고친다.**
"왜 꺼졌는지 설명하려고 남긴 데이터"가 "아직 살아있다는 신호"로 읽히면 원래 문제보다 나쁘다.


---

# 6부 — 배포 절차 결함: `git checkout`이 만든 **옛 코드 서빙 창** (2026-09-03)

사용자: *"demarker가 **아직도** 미발동 / 82% / 익절 2391.71 도달 이렇게 나와"*

5부에서 문구를 고치고 푸시했는데도 증상이 그대로였다. 원인은 코드가 아니라 **내 배포 절차**였다.

## 무슨 일이 있었나

```
1) 로컬 수정 → handoff.sh push (서버에 새 app.js 반영, 서빙 정상)
2) git commit + git push (origin/main = 4ee97c4)
3) ⚠️드리프트를 없애려고 서버에서 git checkout -- dashboard/live/app.js
   → 서버 HEAD는 아직 768a721이므로 **옛 코드로 되돌아감**
4) watcher는 10분 주기 + CI 대기 → 그동안 계속 옛 코드 서빙
```

실측: `isResolvedByTp: 0`, `index 버스터: 20260903-tp-touched`(옛 값),
`Last-Modified: 18:59`. **드리프트를 지우려다 사용자에게 옛 화면을 10분간 보여줬다.**

## 왜 이런 습관이 생겼나

CLAUDE.md의 2채널 계약이 "머지 전 드리프트 0"을 요구해서, 커밋 직후 서버 파일을
`git checkout`으로 되돌리는 걸 루틴으로 삼았다. **푸시 전에는 옳지만 푸시 후에는 정반대다** --
푸시 후의 드리프트는 "서버가 미래(=이미 푸시된 커밋)를 앞서 갖고 있는" 상태라 되돌리면 퇴행이다.

## 규칙 (수정)

| 시점 | 서버 서빙파일이 로컬과 다를 때 | 조치 |
|---|---|---|
| **git push 전** | 서버 내용이 커밋에 없음 | ✅ 커밋하거나 `git checkout`으로 정리 |
| **git push 후, watcher 머지 전** | 서버 내용 == 방금 푸시한 커밋 | ⭐**그대로 둔다.** 되돌리면 옛 코드가 서빙된다 |
| 언제든 | 서버 내용이 로컬/커밋과 **다름** | ⚠️다른 세션 작업 가능 — md5 대조 후 판단 |

⭐**푸시 후 드리프트는 watcher가 스스로 해소한다** -- 이번에 실측으로 확인했다.
stash한 diff와 머지된 내용이 **동일**하므로 `git stash pop`이 충돌 없이 끝난다:

```
[04:10:08] stashed local changes before pulling
[04:10:08] pulled to 4ee97c4...
[04:10:09] restored stashed local changes    ← 충돌 없음
[04:10:09] restarting dashboard
```

머지 후 워킹트리 == HEAD가 되어 드리프트가 **자동으로 0**이 된다(`git status --porcelain
dashboard/live/` 빈 출력 확인).

⚠️단 이건 **rsync한 내용 == 커밋한 내용**일 때만 성립한다. 둘이 다르면 2026-08-24·09-01의
충돌 사고가 그대로 재현된다. 그래서 **`handoff.sh push`할 파일과 커밋할 파일을 반드시
같은 바이트로 유지**해야 한다.

## 캐시 관련 (참고)

`index.html`은 `Cache-Control: no-cache`, `app.js`/`styles.css`는 `max-age=31536000, immutable`이다.
즉 **캐시버스터만 바꾸면 새로고침으로 반영된다** -- index를 매번 새로 받으므로.
(2026-09-01 저ATR 오버플로우 때 Cloudflare 엣지캐시로 고생한 것과 같은 구조.)
