...
⚠️ **이 저장소를 동시에 건드리는 다른 세션과의 충돌이 이 문서 작업 중에만 4회 발생**(BTC 배포 2회 + 라이브탭 제거 1회 + 이번 XRP 1회, 매번 호메로스 증거신호 프로젝트 관련) — 매번 커밋 직전 `git diff`로 훅 단위까지 확인해 분리 커밋 처리함. 이 devmachine에서 대시보드 파일을 커밋하기 전엔 이 확인을 습관화할 것.
