...
**다음 단계(미착수)**: demarker_extreme에 절대 bp 하한을 도입할지는 **이 진단이 아니라 경제성
게이트로 판단해야 한다**(위 2번 원칙, V_REBOUND에서 분류 +0.021이 경제성으로 전이되지 않은 전례).
demarker의 96/96 경제성 통과 기록을 저ATR 구간 제외 조건으로 재실행하는 것이 정확한 다음 검정이다.
⚠️demarker/kalman 수치는 로컬 재구성이므로, 실제 조치 전에 서버의 원본 train context로 재확인할 것.
