...
Last updated: 2026-09-06 KST — **5.31 신설: 물타기의 이득은 전부 크기 효과였고, 건당 기대값 격자는 자본구조를 판정할 수 없다(통과 셀 10/10이 복리에서 로그성장 음수).**
...
- ⭐⭐⭐**건당 기대값 격자는 자본구조를 판정할 수 없다.** §5.30 4편의 확장 격자에서 **126/243 셀이 세 창 전부 계좌 양수**이고 다중성 귀무(평균 41.9 · 95분위 65)의 **100 백분위**로 "우연이 아니다" 판정을 받았다. 그 통과 셀 상위 10개를 **같은 시뮬 함수 그대로**(파리티 |Δ|=0.00e+00) 예약 사다리·동시 5·복리로 태우면 — 건당 계좌%는 열 셀 전부 세 창 양수(+0.56~+2.92%)인데 **로그성장은 TRAIN 10/10이 음수**이고 세 창 전부 양수인 셀은 **0/10**, MDD는 전부 **−95.7% 이하**다. 그 갭이 정확히 **분산 드래그(σ²/2)** 다. ⇒ 격자 통과·귀무 통과는 필요조건일 뿐이고, **로그성장과 MDD를 함께 내기 전에는 자본구조를 주장하지 않는다.**
...
- (a) **격자 경계는 해소되지 않았다**: 물타기 하한을 100 → {50,75}bp로 확장하자 통과 셀 질량이 **새 하한으로 이동**(50: 53 · 75: 47 · 100: 25 · 150: 1)하고 max_adds는 여전히 자본 상한(4: 66 · 9: 42). 격자가 고르는 건 파라미터가 아니라 **명목 크기**다. (b) **다중성 귀무는 통과**(실측 126 vs 귀무 평균 41.9, 100 백분위) — 칩은 무작위보다 실제로 낫다. **동시에 무작위도 평균 42셀을 통과시킨다.**
