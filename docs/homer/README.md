...
**임계값 0.5는 유지가 맞다**(라벨률이 32.5%→14.6%로 바뀌어 재검토가 필요했던 항목):

| thr | VAL 호출률/precision/lift | OOS 호출률/precision/lift |
|---|---|---|
...
0.5에서 VAL/OOS가 거의 겹친다. 0.70은 precision을 사지만 VAL 0.8242 vs OOS 0.7302로 **벌어져
과적합 신호**가 보이고 호출도 0.7%로 줄어 화면 신호로 못 쓴다.
...
**미결**: 경제성 게이트가 아직 GBM 프록시 확률 기준(+8.01bp)이고 TabPFN 확률 재실행이 남아 있다.
