...
미탐색 축("모델이 배우는 라벨 자체를 바꾸기")을 실행해 **11축 전수 실패**. ⭐결정타는 **방향
반전 0/8** — 피쳐에 방향 정보가 틀린 부호로라도 있었다면 뒤집는 것만으로 수익이 났어야 한다.
...
#### ⭐⭐결정타는 축 11이다 — 방향 반전이 0/8

피쳐에 방향 정보가 **틀린 부호로라도** 들어 있었다면 뒤집는 것만으로 수익이 났어야 한다.
8종 전부 유의하지 않았다. ⇒ **신호가 반대인 것도 아니고, 방향에 대해 아무 정보가 없다.**
"모델을 더 잘 만들면"이나 "라벨을 더 잘 정의하면"으로 풀리는 종류가 아니라는 뜻이다.
