...
§5.22 자기정정 참조, 직접 측정은 서버 복구 후 미완).
...
⏳**직접 측정은 미완이다** -- `scripts/research_eth_evidence8_direction_auc_20260905.py`가
비용 **전** 방향 수익 부호를 타깃으로 TabPFN/HGB 분류 AUC를 일 단위 군집 CI와 함께 재도록
작성돼 있으나, 2026-09-05 서버 다운으로 실행하지 못했다. **CI가 0.5를 포함하면 (b) 확립,
0.5를 넘으면 방향 정보는 있고 병목이 비용·사이징 쪽이라는 뜻**이므로 결론이 갈린다.
⇒ 서버 복구 후 이 스크립트를 돌리기 전까지, **(b)는 "강하게 시사됨"이지 "증명됨"이 아니다.**
