...
function run({ asset = "eth", worker = true, spike = false, session = 1.02, mobile = false } = {}) {
  const els = [];
  ...
  const latestBreakoutDetector = worker ? { rvol: { base_days: 14, bar, times, session } } : null;
  const fn = new Function(
    ...
    "fmtUsdCompact", "fmtDateTick", BLOCK);
  const GROUP = mk("g");
...
{
  const els = run();
  ck(texts(els).some((t) => t.includes("─ RVOL")), "ETH·워커정상 -> 범례 «─ RVOL»");
  ...
  ck(texts(els).some((t) => t.includes("오늘 누적 1.02배")), "세션 누적 RVOL 표시");
}
// ③ 워커 부재 -> 조용히 거래대금 폴백. **빈 선이 되면 안 된다**
{
  ...
  ck(!texts(els).some((t) => t.includes("오늘 누적")), "워커 부재 -> 세션 줄 없음");
}
...
// ⑥ 모바일 한 줄 범례
{
  ...
  ck(texts(els).some((t) => t.includes("─ RVOL 축")), "모바일 한 줄 범례에 RVOL");
  ck(texts(els).some((t) => t.includes("오늘 1.02배")), "모바일 한 줄에 세션값");
}
