...
- ⚠️**차트 예외:** 캔들 SVG 안 글씨(축·꼬리표·델타·풋프린트 셀)는 밀도 때문에 9~10.5px 을 허용하고, 풋프린트 셀 숫자는 행 높이에 맞춰 6~9px 까지 줄어든다(app.js 풋프린트 `fontPx`). 사용자가 매일 읽는 밀도라 바꾸지 않았다 — 새 차트 글씨는 9px 아래로 만들지 않는다.
...
### Order Buttons — 확인 규칙 (2026-09-26 확정)
- **마우스:** 0.4초 누르고 있으면 **바로 발주**(채움 막대가 곧 확인). 미리보기가 늦게 와도 오는 즉시 나간다 — 네트워크 속도에 따라 한 단계/두 단계가 갈리지 않는다. 떼면 취소.
- **터치·키보드:** 탭/Enter → 미리보기 → 확인 버튼(30초) → 확인. 스크롤로 끝난 터치는 탭이 아니다.
- 상한에 걸리면 막지 않고 **여유까지만** 채우고(«요청 25% → …까지 22%만»), 여유 0 일 때만 흐린 «진입 불가» + 그 쪽 버튼만 흐림.
- 안내문(«0.4초 누르고 있으면 바로 나갑니다 — 떼면 취소»)은 진입·청산 두 레인에 같다. 동작을 바꾸면 이 문구부터 고친다.
...
- 계좌 카드가 화면에 보이는 동안엔 **접혀 있다**(같은 조작부가 카드 안에 있고, 떠 있으면 «지금 닫으면»·미실현을 덮는다). 모바일 기본 자리는 하단 가운데.
...
- **Don't** 방향이 아닌 것(«방금 계산됨», 신선함)을 초록으로 칠한다 — 신선함은 흐린 «N초 전» 글자로만. 1·2위 차이가 5pp 미만이면 1위를 내세우지 말고 «비등».
- **Don't** 연구 원문(TRAIN bp·표본 수·p값)을 매매 화면 본문에 상주시킨다 — «근거» 접기 안으로(`whyFold`).
