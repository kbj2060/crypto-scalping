...
# Default False: BTC/SOL get the per-asset prealloc budget (PortfolioRiskManager) on their decision
# path only when a real executor exists (the flag above). When True the same budget is applied to
# their shadow-only sizing too, so the trade journal records what the real path would have sized
# even if real execution is later switched off -- the A4 cross-symbol cap line
# (docs/eth_cross_symbol_exposure_cap_design_20260831.md) is evaluated from that journal.
FINAL_GOVERNOR_OMEGA4_6_1_SOL_BTC_SHADOW_PORTFOLIO_CAP_ENABLE = _env_flag(
    "FINAL_GOVERNOR_OMEGA4_6_1_SOL_BTC_SHADOW_PORTFOLIO_CAP_ENABLE", False
)
...
FINAL_GOVERNOR_LIFECYCLE_V1_EXIT_HARD_STOP = float(
    os.getenv(
        ...
        "0.0" if FINAL_GOVERNOR_CONFORMAL_VETO_V1_5_ENABLE else "0.025",
    )
)
