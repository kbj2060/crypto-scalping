...
Entry-Model Layer Reconstruction Gate (ETH)
새 진입/청산 라벨 파이프라인은 층 출력을 동결(sha256)하기 전에 `scripts/gate_eth_entry_layers_20260903.py --pipeline <dir>`를 통과해야 한다. 게이트는 정적 스캐너가 아니라 **재구성**이다 — 각 층을 다른 경로로 다시 계산해 파이프라인 숫자와 대조한다: L4 known_ts 계약(진입·라벨 시작이 트리거가 *알려진* 봉 이후, 저장 인덱스↔타임스탬프 정합) · L1 발동 인과성(known_ts에서 잘라 재계산 + known_ts 직전 절단에서만 존재하는 유령 발동) · L2 라벨 1분 재구성(체결 5분봉은 1분봉으로 갈라 체결 *이후*만 크레딧) · L2P 백테스트 라벨 함수↔라이브/섀도우 채점기 파리티(합성 경로) · L3 피쳐 누수(단일피쳐 AUC≥0.95 · 모델 VAL AUC≥0.99 · 스태킹 OOF 출처 · **조인 시점**: 결정 시각 행의 수익률이 마감봉이 아니라 다음 봉과 상관 1.0이면 한 봉 미래참조 — 2026-09-01 exit 스모크가 체크포인트 맥락을 다음 봉에서 조인해 AUC +0.10 부풀었던 사고, 이 저장소 피쳐 프레임은 행 τ가 봉 τ 자신의 종가를 담는다) · T1 너무 완전한 통과(대조군 전부 통과 + DSR≈1/PBO≈0 → 재구성 PASS 전 진행 금지) · T2 수치 취약성(float32/64·컬럼순서 변형에 부호 반전=FAIL, 변형간 폭이 |추정치|의 50% 초과=FLAG).
`<dir>/gate_config.json`이 계약이다: known_ts를 명시 선언(raw 트리거면 `assumption` 한 줄, 확정지연 트리거면 `fires` 표), `build_fires(kl)→[timestamp,…,known_ts]` 노출, 라벨의 청산·비용·명목 파라미터(게이트가 라벨을 1e-9 이내로 재현 못 하면 FAIL), 라이브/섀도우 채점기가 생기는 즉시 `scoring_parity` 필수, 봉 인덱스 피쳐 프레임 저장(한 봉 밀기 트립와이어용).
**FAIL 층 위에 만든 결과는 전부 provisional이다** — 승격·동결·보고에 쓰지 않는다. FLAG(T1/T2)는 L1/L2 재구성 PASS로만 해소된다. 매 기능 추가마다가 아니라 **층 출력이 바뀔 때 그 층 게이트를 다시** 돌린다.
근거: 2026-09-03 철회 v1(`tmp/eth_entry_limit_fade_v1_20260903`)에 걸어 L2가 전수조사와 같은 숫자로 FAIL(전체 후보 PF 2.87→0.96, 평균 +20.6→−1.0bp, 승률 84.5→69.9%), L2P가 백테스트(체결봉부터) vs 섀도우(L3 규약) 불일치 95.7%로 FAIL, T1이 대조군 8/8+DSR 1.0000을 FLAG했다. 합성 cluster_dedup 트리거는 L1이 **유령 발동**으로 잡는다(자기 known_ts에서는 앵커가 그대로 나오므로 "발동 시점 재계산"만으론 못 잡는다). 표준 대조군 5종은 A~D 넷 다 못 잡는다(docs/homer/README.md §5.21). 게이트는 A~D 재발과 인접 부류를 막을 뿐 5번째 메커니즘은 못 막는다 — 그 방어는 "헤드라인 수치를 다른 경로로 한 번 더 계산하기 전엔 그 위에 쌓지 않는다"는 규율이다. 검정력(독립 일수 42~45일, HOLDOUT 소진)도 게이트가 바꾸지 못한다. 상세: docs/eth_entry_layer_reconstruction_gates_20260903.md.
