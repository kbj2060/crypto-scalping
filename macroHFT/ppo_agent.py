...
class PPOAgent:
    ...
    def select_action(self, state, action_mask=None, mode='router', expert_idx=0, deterministic=False):
        ...
        with torch.no_grad():
            ...
            action_logits, value_mean, _, proj_context = out  # proj_context 추가
            ...
            # 🔥 행동 인덱스 → (direction, scale) 디코딩 (9개 행동: 3방향 × 3레버리지)
            direction = action_idx % 3          # 0:HOLD, 1:LONG, 2:SHORT
            leverage_idx = action_idx // 3      # 0,1,2 → 각각 [1,5,10]배
            scale = config.LEVERAGE_CANDIDATES[leverage_idx] / config.MAX_LEVERAGE  # 0.1, 0.5, 1.0
            action = (direction, scale)

        # proj_context를 추가로 반환 (ICM용)
        return action, log_prob, value, selected_expert, router_log_prob, router_value, proj_context
    ...
    def train_net(self, episode=1, mode='router', expert_idx=0):
        ...
        leverage_values = torch.tensor(config.LEVERAGE_CANDIDATES, device=self.device)  # [1,5,10]
        ...
        action_idx = a_dir + leverage_idx * 3   # 0~8
        ...
        masks = torch.tensor(np.array(batch_data[8]), dtype=torch.float32, device=self.device)  # (B, 9)
        ...
        if mode == 'router' and len(self.router_data) > 0:
            ...
            #         logits, _ = self.router(router_states[i:i+1])
            #         prob = F.softmax(logits, dim=-1).squeeze(0)
            #         prob_action = prob[router_actions[i]].item()
            #         prob_action = max(prob_action, self.router_exp3_eps)  # 하한 설정
            #         estimated_reward = exp3_reward / prob_action
            #         estimated_reward = np.clip(estimated_reward, -10.0, 10.0)  # 이상치 클리핑
        ...
        # 🔥 LR Decay (조건부 적용)
        # 에피소드 1000회 이전에는 1.0(감쇠 없음), 이후부터 0.9995 적용
        lr_decay = 1.0 if episode < 500 else 0.9995
        
        for opt in self.opt_experts + [self.opt_router]:
            for param_group in opt.param_groups:
                param_group['lr'] = max(1e-6, param_group['lr'] * lr_decay)
