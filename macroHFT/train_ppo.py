...
class PPOTrainer:
    ...
    def _prepare_curriculum_indices(self):
        ...
        if 'chop_index' in df.columns and 'volatility_z' in df.columns:
            ...
            if len(self.indices_trend) < 100: self.indices_trend = valid_indices
            ...
            if len(self.indices_vol) < 100: self.indices_vol = valid_indices
            ...
            if len(self.indices_chop) < 100: self.indices_chop = valid_indices

            self.idx_map = [self.indices_trend, self.indices_vol, self.indices_chop]
            self.idx_all = self.indices_all
            logger.info("📊 Curriculum Split: T:%d V:%d S:%d", len(self.indices_trend), len(self.indices_vol), len(self.indices_chop))
        else:
    ...
    def train_episode(self, episode_num, max_steps=None):
        ...
        # [Pure RL] No Teacher Forcing
        
        # [Curriculum] Expert vs Router Training
        ...
        for step in pbar:
            ...
            if current_idx >= self.train_end_idx - 1: break
            ...
            if should_exit and current_position is not None:
                if current_position == 'LONG': action = 2
                elif current_position == 'SHORT': action = 1
                if exit_reason:
            ...
            # [Update] Save Data without Oracle
            self.agent.put_data((state, action, reward, next_state, prob, done, val, volatility_label, action_mask, selected_expert))
