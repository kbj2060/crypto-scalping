...
def rank_features_by_variance(
    df: pd.DataFrame,
    feature_cols: List[str],
    must_include: List[str] | None = None,
) -> List[str]:
    required = list(must_include or [])
    valid_cols: List[str] = []
    for c in required + feature_cols:
    ...
    required_present = [c for c in required if c in variances.index]
    ranked_rest = [c for c in variances.sort_values(ascending=False).index.tolist() if c not in required_present]
    return required_present + ranked_rest
