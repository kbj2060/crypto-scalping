...
DSAC_STATE_DIM = 18
DSAC_ACTION_DIM = 2
DSAC_STATE_SCHEMA = "baseline29_plus_ai1_meta_state_ablate18_v3_dir_exit"
...
class DSACCompactTradingEnv(_BaseSACTradingEnv):
    ...
    def step(self, action):
        if not _futures_exec_enabled(default=False):
            arr = np.asarray(action, dtype=np.float32).reshape(-1)
            dir_action = float(np.clip(arr[0] if arr.size else 0.0, -1.0, 1.0))
            exit_raw = float(np.clip(arr[1] if arr.size > 1 else -1.0, -1.0, 1.0))
            exit_gate = 0.5 * (exit_raw + 1.0)
            ...
            if self.pos is None:
                if dir_action > 0.0 and dir_action < long_entry_thresh:
                    dir_action = 0.0
                elif dir_action < 0.0 and abs(dir_action) < short_entry_thresh:
                    dir_action = 0.0
            if self.pos == "LONG" and (dir_action < -_REV_EXIT_THRESH or exit_gate >= _EXIT_GATE_THRESH) and self.hold_count >= self.min_hold_bars:
                dir_action = 0.0
            elif self.pos == "SHORT" and (dir_action > _REV_EXIT_THRESH or exit_gate >= _EXIT_GATE_THRESH) and self.hold_count >= self.min_hold_bars:
                dir_action = 0.0
            if self.pos is None and self._bars_since_close < self.reentry_cooldown_bars:
                if abs(dir_action) > self.pos_thresh:
                    dir_action = 0.0
            next_state, reward, done, info = super().step(dir_action)
        ...
        if force_close:
            ...
            if self.hold_count >= self.min_hold_bars and (abs(direction) < _HOLD_CLOSE_THRESH or target_exposure <= 1e-8 or exit_gate >= _EXIT_GATE_THRESH):
                is_closing = True
...
class DSACRouter:
    ...
    def decide(self, features, pos):
        ...
        action_val = float(act_np[0]) if act_np.size else 0.0
        exit_raw = float(act_np[1]) if act_np.size > 1 else -1.0
        exit_gate = 0.5 * (float(np.clip(exit_raw, -1.0, 1.0)) + 1.0)
        ...
        if not _futures_exec_enabled(default=False):
            ...
            if cur_pos is not None:
                if abs_action < _HOLD_CLOSE_THRESH or exit_gate >= _EXIT_GATE_THRESH:
            ...
            info = {
                ...
                "raw_exit": round(exit_raw, 4),
                "exit_gate": float(exit_gate),
