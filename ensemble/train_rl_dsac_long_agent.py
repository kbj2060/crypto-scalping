"""
DSAC Long Specialist 에이전트
================================================================
롱 방향 전용 DSAC. action ∈ [0, 1]:
  0.0        = 관망 / 청산
  0.0 ~ 0.15 = 데드존 (현상 유지)
  0.15 ~ 1.0 = 롱 진입/유지 (크기 = action)

원본 both-side DSAC에서 발생하던 롱 편향 문제를 구조적으로 분리.
Short specialist(train_rl_dsac_short.py)와 쌍으로 사용.

State: 28D compact (원본 30D에서 entry offset 전체 제거, side_flag 추가)
  Block A: Market Prediction Meta (17) — entry offset 제거, hour_cos/garch_vol_z 추가
  Block B: Immediate Tick Context (6)
  Block C: Agent Private State (5)  — pos_sign 제거(항상 롱), side_flag 추가 불필요
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# Long Specialist Environment
# ─────────────────────────────────────────────────────────────────────────────
class LongSpecialistEnv(_BaseSACTradingEnv):
    """롱 전용 환경.

    action ∈ [0, 1]:
      action > POS_THRESH  → 롱 진입/유지
      action < CLOSE_THRESH → 청산/관망
      그 사이 → 데드존 (현상 유지)

    숏 진입은 절대 불가. reward도 롱 거래에 최적화.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # ── 사전 계산 배열 (원본 DSACCompactTradingEnv와 동일) ──
    ...
    def _build_state(self, idx):
        ...
        # Block C: Agent Private State (5) — 포지션은 항상 None 또는 LONG
        in_position = 1.0 if self.pos == "LONG" else 0.0
    ...
    # ── step() 오버라이드: action [0,1] → 롱만 ──
    def step(self, action: float):
        """action ∈ [0, 1]: 0=관망/청산, 1=풀 롱."""
        ...
        if self.pos is None:
            ...
            # bear(1)/chop(2)/whipsaw(3) 일 때 관망은 페널티 없음
            if regime_idx == 0:  # bull
                r5_idle = -0.0005  # bull에서만 아주 약한 관망 페널티
            else:
...
class SigmoidActor(nn.Module):
    ...
    def sample(self, state):
        ...
        # log_prob with sigmoid correction
...
class DSACLongAgent:
    def __init__(
        ...
        # target_entropy for sigmoid output: -0.5 * action_dim (heuristic for [0,1])
...
# Router (라이브 추론)
# ─────────────────────────────────────────────────────────────────────────────
class DSACLongRouter:
    """롱 specialist 라이브 라우터.

    decide() → (action_int, leverage, info)
      action_int: 0=관망, 1=LONG
    """
    ...
    def _build_compact_state(self, features: dict[str, Any], pos: dict[str, Any]) -> np.ndarray:
        ...
        in_position = 1.0 if pos_type == "LONG" else 0.0
    ...
    def decide(self, features, pos):
        ...
        if cur_pos == "LONG":
            if action_val < _CLOSE_THRESH:
                ...
                action_int, leverage = 1, action_val
...
def train(
    ...
    if "timestamp" in df.columns:
        ...
        if ts.notna().any():
            ...
            logger.info("[LONG] ts_range=%s -> %s | years=%s", ts.min(), ts.max(), years)
    ...
    env = LongSpecialistEnv(df_train, phase="train", hmm_detector=train_hmm, mtf_features=mtf_train)
    agent = DSACLongAgent(STATE_DIM, hidden_dim=256, n_quantiles=32, cvar_frac=float(cvar_frac), device=device)
    ...
    logger.info(
        "[LONG][TRAIN CFG] val_interval=%d | lr_sched=%s (factor=%.3f patience=%d min_lr=%.1e) | early_stop_patience=%d | cvar_frac=%.2f | fallback_gain=%.2f",
        int(val_interval),
    ...
    ckpt_path = "data/ensemble/ckpt/dsac_long_checkpoint.pth"
    best_path = "data/ensemble/ckpt/best_dsac_long_agents.pth"
    ...
    if (not fresh_start) and os.path.exists(ckpt_path):
        try:
            ...
            logger.info(
                "[LONG] ♻️ 복원 ep=%d | global_step=%d | best_pnl=%.2f%% | best_score=%.2f | bad_val=%d",
                start_ep - 1,
                global_step,
                best_val_pnl,
                best_val_score,
            ...
            if use_lr_scheduler:
                try:
                    ...
                    logger.warning("[LONG] LR scheduler 복원 실패: %s", e)
            if len(agent.memory) < min_buffer:
                refill_steps = max(warmup_steps, min_buffer)
                logger.info("[LONG] WARMUP 재실행 -> %d 스텝", refill_steps)
                warmup_env = LongSpecialistEnv(df_train, phase="train", hmm_detector=copy.deepcopy(hmm_detector), mtf_features=mtf_train)
                ws = warmup_env.reset()
                for _ in range(refill_steps):
                    ...
                    if wd:
                        ws = warmup_env.reset()
                logger.info("[LONG] WARMUP 완료 | 버퍼: %d", len(agent.memory))
        except Exception as e:
            logger.warning("[LONG] 체크포인트 복원 실패: %s", e)
    elif fresh_start:
        logger.info("[LONG] 🧹 FRESH START")
    ...
    try:
        for ep in range(start_ep, nep + 1):
            ...
            logger.info(
                "[LONG] Ep %04d | PnL:%6.1f%% Tr:%4d WR:%4.0f%% Rew:%7.3f | buf:%6d | α:%.4f | CVaR:%+.4f",
            ...
            if ep % max(1, int(val_interval)) == 0:
                ...
                val_env = LongSpecialistEnv(df_val, phase="val", hmm_detector=val_hmm, mtf_features=mtf_val)
                val_state = val_env.reset()
                val_done = False
                val_peak_eq = float(val_env.initial_balance)
                val_mdd_pct = 0.0
                val_entries = 0
                val_force_close_long = 0
                val_hold_sum_long = 0
                val_hold_n_long = 0
                ...
                while not val_done:
                    ...
                    if prev_pos is None and val_env.pos == "LONG":
                        val_entries += 1
                    if bool(val_info.get("force_closed", False)) and str(val_info.get("closed_side", "") or "") == "LONG":
                        val_force_close_long += 1
                    closed_hold_count = int(val_info.get("closed_hold_count", 0) or 0)
                    closed_side = str(val_info.get("closed_side", "") or "")
                    if closed_hold_count > 0 and closed_side == "LONG":
                        val_hold_sum_long += closed_hold_count
                        val_hold_n_long += 1
                ...
                avg_hold_long = float(val_hold_sum_long / max(val_hold_n_long, 1)) if val_hold_n_long > 0 else 0.0
                ...
                logger.info(
                    "[LONG]   [VAL] PnL:%6.2f%% | Tr:%4d | WR:%.0f%% | MDD:%.2f%% | Entries:%4d | FCL:%3d | AvgHold:%4.1f | Score:%.2f",
                    val_pnl, val_env.total_trades, val_wr * 100, val_mdd_pct, val_entries, val_force_close_long, avg_hold_long, val_score,
                )
                ...
                if improved:
                    ...
                    logger.info("[LONG]   🎉 NEW BEST (PnL:%.2f%%)", best_val_pnl)
                else:
                    bad_val_count += 1
                ...
                if use_lr_scheduler and actor_scheduler and critic_scheduler:
                    ...
                    if (new_actor_lr < prev_actor_lr) or (new_critic_lr < prev_critic_lr):
                        logger.info(
                            "[LONG]   📉 [LR DROP] actor %.3e -> %.3e | critic %.3e -> %.3e",
                            prev_actor_lr,
                            new_actor_lr,
                            prev_critic_lr,
                            new_critic_lr,
                        ...
                        logger.info(
                            "[LONG]   [LR] actor %.3e | critic %.3e | bad_val=%d",
                            new_actor_lr,
                            new_critic_lr,
                            bad_val_count,
                        )
                ...
                if int(early_stop_patience) > 0 and bad_val_count >= int(early_stop_patience):
                    logger.info(
                        "[LONG] ⏹️ EARLY STOP bad_val_count=%d >= patience=%d | best_score=%.2f | best_pnl=%.2f%%",
                        bad_val_count,
                        int(early_stop_patience),
                        best_val_score,
                        best_val_pnl,
        ...
        logger.info("[LONG] ⚠️ 학습 중단.")
        _save_checkpoint(ep)


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train DSAC Long Specialist")
...
if __name__ == "__main__":
    ...
    if args.startup_check_only:
        logger.info("startup check ok: train_rl_dsac_long")
        raise SystemExit(0)
    train(
