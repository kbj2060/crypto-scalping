...
class LongSpecialistEnv(_BaseSACTradingEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._m7_entry_long_offset_np = self._require_col("m7_entry_long_offset")
...
class DSACLongRouter:
    ...
    def _build_compact_state(self, features: dict[str, Any], pos: dict[str, Any]) -> np.ndarray:
        """28D 롱 전용 state 구성 (라이브). 누락·비유한 피처는 즉시 예외 발생."""
...
def train(
    ...
    logger.info(
        "[LONG][TRAIN CFG] val_interval=%d | lr_sched=%s (factor=%.3f patience=%d min_lr=%.1e)"
