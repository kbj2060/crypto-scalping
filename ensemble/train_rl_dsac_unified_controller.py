"""
DSAC Controller 에이전트 (Distributional Soft Actor-Critic)
===========================================================
기존 unified 알파 정책을 메인으로 두고, base DSAC를 필터로 사용해
실행 승인/레버리지 버킷만 제어하는 controller RL 실험 버전.

Controller State (33D)
  Block A: M7 + Regime + MTF 문맥 (13)
  Block B: execution context (2)
  Block C: leveraged position state (6)
  Block D: AI meta insights (4)
  Block E: unified/base shadow policy outputs (8)
"""
...
from ensemble.train_rl_dsac_agent import (  # noqa: E402
    GaussianActor as BaseShadowActor,
    DSACRouter as BaseShadowRouter,
    DSAC_STATE_DIM as BASE_SHADOW_STATE_DIM,
)
from ensemble.train_rl_dsac_unified_2025 import (  # noqa: E402
    GaussianActor as UnifiedShadowActor,
    DSACRouter as UnifiedShadowRouter,
    DSAC_STATE_DIM as UNIFIED_SHADOW_STATE_DIM,
)
...
# ─────────────────────────────────────────────────────────────────────────────
# DSAC Controller State Spec
# ─────────────────────────────────────────────────────────────────────────────
DSAC_STATE_DIM = 33
DSAC_ACTION_DIM = 1
DSAC_STATE_SCHEMA = "unified_main_base_filter_controller33_v2"

CONTROLLER_BUCKETS = np.asarray([0.0, 0.5, 0.75, 1.0, 1.5, 2.0, 2.5, 3.0], dtype=np.float32)
CONTROLLER_MAIN_CKPT = "data/ensemble/ckpt/best_dsac_unified.pth"
CONTROLLER_FILTER_CKPT = "data/ensemble/ckpt/best_dsac_agents.pth"
...
def _same_side_resize_enabled(default: bool = True) -> bool:
...
def _controller_bucket_from_action(action: Any) -> tuple[float, str]:
    raw = float(np.clip(np.asarray(action, dtype=np.float32).reshape(-1)[0], -1.0, 1.0))
    magnitude = abs(raw)
    edges = np.linspace(0.0, 1.0, len(CONTROLLER_BUCKETS) + 1)
    idx = int(np.clip(np.searchsorted(edges[1:], magnitude, side="right"), 0, len(CONTROLLER_BUCKETS) - 1))
    bucket = float(CONTROLLER_BUCKETS[idx])
    return bucket, f"ctrl_bucket_{bucket:.2f}x"


def _load_shadow_actor(ckpt_path: str, actor_cls, state_dim: int, device: str):
    ckpt = torch.load(ckpt_path, map_location=device, weights_only=False)
    actor_state = ckpt.get("actor")
    if not isinstance(actor_state, dict):
        raise KeyError(f"actor state missing in {ckpt_path}")
...
class DSACCompactTradingEnv(_BaseSACTradingEnv):
    ...
    def __init__(
        ...
        self._controller_device = "cuda" if torch.cuda.is_available() else "cpu"
        self._controller_shadow_unified_actor = _load_shadow_actor(
            os.path.join(_ROOT_DIR, CONTROLLER_MAIN_CKPT),
            UnifiedShadowActor,
            UNIFIED_SHADOW_STATE_DIM,
            self._controller_device,
        )
        self._controller_shadow_base_actor = _load_shadow_actor(
            os.path.join(_ROOT_DIR, CONTROLLER_FILTER_CKPT),
            BaseShadowActor,
            BASE_SHADOW_STATE_DIM,
            self._controller_device,
        )
        self._controller_shadow_unified_router = None
        self._controller_shadow_base_router = None
        self._controller_cache_idx = -1
        self._controller_cache_pos_sig = None
        self._controller_cache = None
        self.same_side_resize_enable = _same_side_resize_enabled(default=False)
        self.same_side_add_min_delta = max(0.0, _env_float("RL_SAME_SIDE_ADD_MIN_DELTA", 0.35))
        self.same_side_add_min_signal = float(np.clip(_env_float("RL_SAME_SIDE_ADD_MIN_SIGNAL", 0.65), 0.0, 1.0))
        self.same_side_add_cooldown = max(0, int(_env_float("RL_SAME_SIDE_ADD_COOLDOWN", 12)))
        self._lots: list[dict[str, float | int | str]] = []
        self._compact_ready = True
        self._reset_controller_shadows()
    ...
    def reset(self, start_idx=None):
        ...
        self.current_notional_exposure = 0.0
        self._lots = []
        if hasattr(self, "_controller_shadow_unified_actor") and hasattr(self, "_controller_shadow_base_actor"):
            self._reset_controller_shadows()
        return st

    def _reset_controller_shadows(self):
        self._controller_shadow_unified_router = UnifiedShadowRouter(
            self._controller_shadow_unified_actor,
            device=self._controller_device,
            hmm_detector=None,
        )
        self._controller_shadow_base_router = BaseShadowRouter(
            self._controller_shadow_base_actor,
            device=self._controller_device,
            hmm_detector=None,
        )
        self._controller_cache_idx = -1
    ...
    def _controller_pos_dict(self) -> dict[str, float | str]:
        current_exp = float(np.clip(getattr(self, "current_notional_exposure", 0.0), 0.0, self.max_leverage))
        unr_unlev = _unlevered_pnl_from_exposure(float(self.unrealized_pnl), current_exp) if self.pos is not None else 0.0
        mdd_unlev = _unlevered_pnl_from_exposure(float(self.max_drawdown), current_exp) if self.pos is not None else 0.0
        return {
            "type": self.pos,
            "unrealized": float(unr_unlev),
            "mdd": float(mdd_unlev),
            "hold_count": float(self.hold_count),
            "margin_fraction": float(np.clip(getattr(self, "current_margin_fraction", 0.0), 0.0, 1.0)),
            "leverage": float(max(0.0, getattr(self, "current_leverage", 0.0))),
        }

    def _controller_signal_bundle(self, idx: int) -> dict[str, float]:
        pos_sig = (
            str(self.pos or "FLAT"),
            round(float(self.unrealized_pnl), 8),
            round(float(getattr(self, "current_margin_fraction", 0.0)), 8),
            round(float(getattr(self, "current_leverage", 0.0)), 8),
            int(self.hold_count),
        )
        if idx == self._controller_cache_idx and pos_sig == self._controller_cache_pos_sig and self._controller_cache is not None:
            return dict(self._controller_cache)

        row = self.df.iloc[idx].to_dict()
        pos = self._controller_pos_dict()
        u_action, u_lev, u_info = self._controller_shadow_unified_router.decide(row, pos)
        b_action, b_lev, b_info = self._controller_shadow_base_router.decide(row, pos)
        u_signed = float(np.clip(_safe_float(u_info.get("raw_action", 0.0)), -1.0, 1.0))
        b_signed = float(np.clip(_safe_float(b_info.get("raw_action", 0.0)), -1.0, 1.0))
        agreement = 1.0 if (u_action == b_action and u_action in (1, 2)) else (-1.0 if (u_action in (1, 2) and b_action in (1, 2) and u_action != b_action) else 0.0)
        disagreement = float(np.clip(abs(u_signed - b_signed) * 0.5, 0.0, 1.0))
        base_veto = 1.0 if (agreement < 0.0 and float(b_lev) >= 0.20) else 0.0
        kelly_gap = float(np.clip(float(u_lev) - float(b_lev), -1.0, 1.0))
        bundle = {
            "u_action": float(u_action),
            "u_signed": u_signed,
            "u_kelly": float(np.clip(float(u_lev), 0.0, 1.0)),
            "b_action": float(b_action),
            "b_signed": b_signed,
            "b_kelly": float(np.clip(float(b_lev), 0.0, 1.0)),
            "agreement": agreement,
            "disagreement": disagreement,
            "base_veto": base_veto,
            "kelly_gap": kelly_gap,
        }
        self._controller_cache_idx = idx
        self._controller_cache_pos_sig = pos_sig
        self._controller_cache = dict(bundle)
        return bundle
    ...
    def _lot_total_exposure(self) -> float:
        return float(sum(float(x.get("exposure", 0.0) or 0.0) for x in self._lots))

    def _lot_weighted_entry(self) -> float:
        total = self._lot_total_exposure()
        if total <= 1e-12:
            return 0.0
        return float(
            sum(
                float(x.get("entry_price", 0.0) or 0.0) * float(x.get("exposure", 0.0) or 0.0)
                for x in self._lots
            ) / total
        )
    ...
    def _sync_position_from_lots(self) -> None:
        total = self._lot_total_exposure()
        if total <= 1e-12 or not self._lots:
            self.pos = None
            ...
            return
        side = str(self._lots[0].get("side", "")).upper()
        self.pos = side if side in {"LONG", "SHORT"} else None
        self.entry_price = self._lot_weighted_entry()
        self.current_notional_exposure = float(np.clip(total, 0.0, self.max_leverage))
        self.current_margin_fraction = float(np.clip(min(total, 1.0), 0.0, 1.0))
        self.current_leverage = float(
            np.clip(
                self.current_notional_exposure / max(self.current_margin_fraction, 1e-8),
                1.0,
                self.max_leverage,
            )
        ) if self.current_margin_fraction > 1e-8 else 0.0
    ...
    def _lot_trade_pnl(self, side: str, entry_price: float, exit_price: float, exposure: float) -> float:
        ...
        lev = float(np.clip(float(exposure or 0.0), 0.0, self.max_leverage))
        if side_u not in {"LONG", "SHORT"} or entry <= 0.0 or exit_raw <= 0.0 or lev <= 0.0:
            return 0.0
        if side_u == "LONG":
            exit_exec = exit_raw * (1.0 - self.slip)
            gross = (exit_exec - entry) / max(entry, 1e-8)
        else:
            exit_exec = exit_raw * (1.0 + self.slip)
            gross = (entry - exit_exec) / max(abs(entry), 1e-8)
    ...
    def _lot_add(self, side: str, price: float, exposure: float, idx: int) -> None:
        exp = float(np.clip(exposure, 0.0, self.max_leverage))
        if exp <= 1e-12:
            return
        entry_px = float(price) * (1.0 + self.slip) if str(side).upper() == "LONG" else float(price) * (1.0 - self.slip)
        self.balance -= self.balance * self.fee * exp
        self._lots.append({
            "side": str(side).upper(),
    ...
    def _lot_reduce(self, reduce_exposure: float, price: float) -> float:
        remaining = float(max(reduce_exposure, 0.0))
        realized = 0.0
    ...
    def step(self, action: float):
        ...
        ctrl = self._controller_signal_bundle(decision_step)
        bucket_mult, bucket_tag = _controller_bucket_from_action(action)
        unified_action = int(ctrl["u_action"])
        base_action = int(ctrl["b_action"])
        unified_kelly = float(np.clip(ctrl["u_kelly"], 0.0, 1.0))
        base_veto = bool(ctrl["base_veto"] >= 0.5)

        direction = 0.0
        target_exposure = 0.0
        if unified_action == 1:
            direction = 1.0
            target_exposure = unified_kelly * bucket_mult
        elif unified_action == 2:
            direction = -1.0
            target_exposure = unified_kelly * bucket_mult

        if base_veto and bucket_mult > 1.0:
            target_exposure = unified_kelly * 1.0
        if base_action == 0 and bucket_mult > 2.0:
            target_exposure = unified_kelly * 1.5
        if self.pos is not None and self.unrealized_pnl < 0.0:
        ...
        target_margin = float(np.clip(min(target_exposure, 1.0), 0.0, 1.0))
        target_leverage = float(np.clip(target_exposure / max(target_margin, 1e-8), 1.0, self.max_leverage)) if target_margin > 1e-8 else 0.0
        ...
        if force_close:
            ...
            if abs(direction) < self.close_thresh or target_exposure <= 1e-8:
                ...
                is_closing = True
            elif self.same_side_resize_enable:
                current_exp = float(np.clip(getattr(self, "current_notional_exposure", 0.0), 0.0, self.max_leverage))
                delta_add = max(0.0, float(target_exposure) - current_exp)
                if (
                    delta_add >= self.same_side_add_min_delta
                    and unified_kelly >= self.same_side_add_min_signal
                    and bucket_mult >= 1.5
                    and (not base_veto)
                    and self.unrealized_pnl >= 0.0
                    and self.hold_count >= self.same_side_add_cooldown
                ):
                    is_adjusting = True
        ...
        if is_entering_long:
            self._lot_add("LONG", fill_price, target_exposure, fill_step)
        elif is_entering_short:
            self._lot_add("SHORT", fill_price, target_exposure, fill_step)
            ...
            if exp_delta >= self.same_side_add_min_delta:
                if exp_delta > 1e-12:
                    self._lot_add(str(self.pos), fill_price, exp_delta, fill_step)
        elif is_closing and self.pos is not None:
            closed_side = str(self.pos)
            closed_hold_count = int(self.hold_count)
            fee_base = self.balance
            close_exp = self._lot_total_exposure()
            realized_pnl = self._lot_reduce(close_exp, fill_price)
            self.balance *= (1.0 + realized_pnl)
            self.balance -= fee_base * self.fee * close_exp
            self.total_trades += 1
            if realized_pnl > 0:
                self.win_trades += 1
        ...
        if self.pos is not None:
            ...
            self.unrealized_pnl = self._lot_mark_pnl(float(next_price))
        ...
        info = {
            ...
            "exposure_bucket": float(bucket_mult),
            "controller_exposure_bucket": float(bucket_mult),
            "controller_exposure_bucket_tag": str(bucket_tag),
            "controller_bucket": float(bucket_mult),
            "controller_bucket_tag": str(bucket_tag),
            "unified_action": int(unified_action),
    ...
    def _build_state(self, idx):
        ...
        # ── Block C: Agent Private State (unlevered PnL + direct exposure) ──
        ...
        notional_exposure_norm = float(np.clip(current_exp / 1.5, 0.0, 1.0))
        ...
        unrealized_norm = _norm_tanh(unrealized_unlev if self.pos is not None else 0.0, 0.02)
        ...
        drawdown_norm = float(np.clip((drawdown_unlev if self.pos is not None else 0.0) / 0.05, -1.0, 1.0))
        ...
        ctrl = self._controller_signal_bundle(idx)
        u_long = 1.0 if int(ctrl["u_action"]) == 1 else 0.0
        u_short = 1.0 if int(ctrl["u_action"]) == 2 else 0.0
        b_long = 1.0 if int(ctrl["b_action"]) == 1 else 0.0
        b_short = 1.0 if int(ctrl["b_action"]) == 2 else 0.0
        state = np.array(
            [
                # Block A (13)
                up * _M7_DIR_SCALE,
                ...
                float(ctrl["b_signed"]),
                float(ctrl["b_kelly"]),
                float(ctrl["agreement"]),
                float(ctrl["disagreement"]),
                float(ctrl["base_veto"]),
                float(ctrl["kelly_gap"]),
            ],
            dtype=np.float32,
        )
...
class DSACRouter:
    ...
    def __init__(self, actor, device="cuda", hmm_detector=None, mtf_features=None):
        ...
        self._shadow_unified_actor = _load_shadow_actor(
            os.path.join(_ROOT_DIR, CONTROLLER_MAIN_CKPT),
            UnifiedShadowActor,
            UNIFIED_SHADOW_STATE_DIM,
            device,
        )
        self._shadow_base_actor = _load_shadow_actor(
            os.path.join(_ROOT_DIR, CONTROLLER_FILTER_CKPT),
            BaseShadowActor,
            BASE_SHADOW_STATE_DIM,
            device,
        )
        self._shadow_unified_router = UnifiedShadowRouter(self._shadow_unified_actor, device=device, hmm_detector=None)
        self._shadow_base_router = BaseShadowRouter(self._shadow_base_actor, device=device, hmm_detector=None)
    ...
    def _build_compact_state(self, features: dict[str, Any], pos: dict[str, Any]) -> np.ndarray:
        ...
        unrealized_norm = _norm_tanh(unrealized_unlev if pos_sign != 0.0 else 0.0, 0.02)
        drawdown_unlev = _unlevered_pnl_from_exposure(mdd, current_exp) if pos_sign != 0.0 else 0.0
        drawdown_norm = float(np.clip((drawdown_unlev if pos_sign != 0.0 else 0.0) / 0.05, -1.0, 1.0))
        ...
        shadow_pos = {
            "type": pos_type,
            "unrealized": unrealized_unlev,
            "mdd": drawdown_unlev,
            "hold_count": hold_count_proxy,
            "margin_fraction": margin_usage,
            "leverage": current_lev,
        }
        u_action, u_lev, u_info = self._shadow_unified_router.decide(features or {}, shadow_pos)
        b_action, b_lev, b_info = self._shadow_base_router.decide(features or {}, shadow_pos)
        u_signed = float(np.clip(_safe_float(u_info.get("raw_action", 0.0)), -1.0, 1.0))
        b_signed = float(np.clip(_safe_float(b_info.get("raw_action", 0.0)), -1.0, 1.0))
        agreement = 1.0 if (u_action == b_action and u_action in (1, 2)) else (-1.0 if (u_action in (1, 2) and b_action in (1, 2) and u_action != b_action) else 0.0)
        disagreement = float(np.clip(abs(u_signed - b_signed) * 0.5, 0.0, 1.0))
        ...
        state = np.array(
            [
                ...
                ai_flow_flip,
                float(u_signed),
                float(np.clip(float(u_lev), 0.0, 1.0)),
                float(b_signed),
                float(np.clip(float(b_lev), 0.0, 1.0)),
                float(agreement),
                float(disagreement),
                float(base_veto),
                float(kelly_gap),
...
def train(
    ...
    config_json_path: str = "data/ensemble/ckpt/dsac_unified_controller_train_config.json",
    checkpoint_path: str = "data/ensemble/ckpt/dsac_unified_controller_checkpoint.pth",
    best_path: str = "data/ensemble/ckpt/best_dsac_unified_controller.pth",
    hmm_cache_path: str = "data/ensemble/ckpt/hmm_init_cache_dsac_unified_controller.npz",
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--hmm-cache-path", default="data/ensemble/ckpt/hmm_init_cache_dsac_unified_controller.npz")
    p.add_argument("--hmm-force-refit", action="store_true", default=False)
    p.add_argument("--config-json-path", default="data/ensemble/ckpt/dsac_unified_controller_train_config.json")
    p.add_argument("--checkpoint-path", default="data/ensemble/ckpt/dsac_unified_controller_checkpoint.pth")
    p.add_argument("--best-path", default="data/ensemble/ckpt/best_dsac_unified_controller.pth")
...
if __name__ == "__main__":
    ...
    if args.startup_check_only:
        logger.info("startup check ok: train_rl_dsac_unified_controller")
        raise SystemExit(0)
    train(
