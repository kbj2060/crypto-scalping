...
DSAC_STATE_DIM = 18
DSAC_ACTION_DIM = 5
DSAC_STATE_SCHEMA = "baseline29_plus_ai1_meta_state_ablate18_v4_discrete"
ACT_FLAT = 0
ACT_LONG = 1
ACT_SHORT = 2
ACT_CLOSE = 3
ACT_HOLD = 4
_POSITION_STATE_IDX = 14
...
def _action_mask_from_state_tensor(state: torch.Tensor) -> torch.Tensor:
    pos = state[..., _POSITION_STATE_IDX]
    up = state[..., 0]
    dn = state[..., 1]
    quality = state[..., 4]
    flat = pos.abs() < 0.5
    long_pos = pos > 0.5
    short_pos = pos < -0.5
    strong_entry = flat & ((torch.maximum(up, dn) >= 0.33) & (quality >= 0.10))
    mask = torch.zeros((*state.shape[:-1], DSAC_ACTION_DIM), dtype=torch.bool, device=state.device)
    # flat: FLAT / LONG / SHORT
    mask[..., ACT_FLAT] |= flat
    mask[..., ACT_LONG] |= flat
    mask[..., ACT_SHORT] |= flat
    mask[..., ACT_FLAT] &= ~strong_entry
    # long: HOLD / CLOSE / SHORT
    mask[..., ACT_HOLD] |= long_pos
    mask[..., ACT_CLOSE] |= long_pos
    mask[..., ACT_SHORT] |= long_pos
    # short: HOLD / CLOSE / LONG
    mask[..., ACT_HOLD] |= short_pos
    mask[..., ACT_CLOSE] |= short_pos
    mask[..., ACT_LONG] |= short_pos
    return mask
...
class DSACCompactTradingEnv(_BaseSACTradingEnv):
    ...
    def step(self, action):
        ...
        action_idx = int(act_arr[0]) if act_arr.size else ACT_FLAT
        action_idx = int(np.clip(action_idx, 0, DSAC_ACTION_DIM - 1))
        if not _futures_exec_enabled(default=False):
            decision_step = self.current_step
            missed_entry = False
            if self.pos is None and action_idx == ACT_FLAT:
                up = float(np.clip(_safe_float(self._m7_prob_up_np[decision_step], 0.0), 0.0, 1.0))
                dn = float(np.clip(_safe_float(self._m7_prob_dn_np[decision_step], 0.0), 0.0, 1.0))
                quality = float(np.clip(_safe_float(self._m7_quality_np[decision_step], 0.0), -1.0, 1.0))
                missed_entry = max(up, dn) >= 0.62 and quality >= 0.10
            mapped = 0.0
            if self.pos is None:
                if action_idx == ACT_LONG:
                    mapped = 1.0
                elif action_idx == ACT_SHORT:
                    mapped = -1.0
            else:
                if action_idx == ACT_CLOSE:
                    mapped = 0.0
                elif action_idx == ACT_LONG:
                    mapped = 1.0 if self.pos == "LONG" else 0.0
                elif action_idx == ACT_SHORT:
                    mapped = -1.0 if self.pos == "SHORT" else 0.0
                else:
                    mapped = 1.0 if self.pos == "LONG" else (-1.0 if self.pos == "SHORT" else 0.0)
            next_state, reward, done, info = super().step(mapped)
            if bool(info.get("force_closed", False)):
                reward -= _FORCE_CLOSE_HEAVY_PENALTY
            if missed_entry:
                reward -= _MISSED_ENTRY_PENALTY
        ...
        target_margin = 1.0 if action_idx in (ACT_LONG, ACT_SHORT) else 0.0
        target_leverage = target_margin
        target_exposure = target_margin
        ...
        if force_close:
            ...
            if self._bars_since_close >= self.reentry_cooldown_bars and action_idx == ACT_LONG:
                is_entering_long = True
            elif self._bars_since_close >= self.reentry_cooldown_bars and action_idx == ACT_SHORT:
                is_entering_short = True
        else:
            if action_idx == ACT_CLOSE:
                is_closing = True
            elif self.pos == "LONG" and action_idx == ACT_SHORT and self.hold_count >= self.min_hold_bars:
                is_closing = True
            elif self.pos == "SHORT" and action_idx == ACT_LONG and self.hold_count >= self.min_hold_bars:
                is_closing = True
...
class DiscreteActor(nn.Module):
    ...
    def __init__(self, state_dim=DSAC_STATE_DIM, hidden_dim=256, action_dim=DSAC_ACTION_DIM):
        ...
        self.logits = nn.Linear(hidden_dim, int(action_dim))

    def forward(self, state: torch.Tensor) -> torch.Tensor:
        feat = self.feat(state)
        return self.logits(feat)

    def probs(self, state: torch.Tensor, mask: torch.Tensor | None = None) -> torch.Tensor:
        logits = self.forward(state)
        if mask is not None:
            logits = _masked_logits(logits, mask)
        return torch.softmax(logits, dim=-1)

    def sample(self, state: torch.Tensor, mask: torch.Tensor | None = None) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        logits = self.forward(state)
        if mask is not None:
            logits = _masked_logits(logits, mask)
    ...
    def deterministic(self, state: torch.Tensor, mask: torch.Tensor | None = None) -> torch.Tensor:
        logits = self.forward(state)
        if mask is not None:
            logits = _masked_logits(logits, mask)
...
class DSACAgent:
    """Discrete entropy-regularized actor-critic."""
    ...
    def __init__(
        ...
        logn = float(np.log(self.effective_action_dim))
        if float(entropy_min) <= 0.0 or float(entropy_max) <= 0.0:
            self.entropy_min = 0.75 * logn
            self.entropy_max = 0.98 * logn
        else:
        ...
        self.actor = DiscreteActor(state_dim, hidden_dim, self.action_dim).to(device)
        self.critic = TwinQCritic(state_dim, hidden_dim, self.action_dim).to(device)
        ...
        self.target_entropy = float(np.clip(0.90 * logn, self.entropy_min, self.entropy_max))
    ...
    def act(self, state: np.ndarray, deterministic: bool = False) -> np.ndarray:
        ...
        mask = _action_mask_from_state_tensor(state_ts)
        with torch.no_grad():
            if deterministic:
                action = self.actor.deterministic(state_ts, mask=mask)
            else:
                action, _, _ = self.actor.sample(state_ts, mask=mask)
        return action.cpu().numpy().reshape(-1).astype(np.int64)
    ...
    def update(self, batch_size=256) -> dict:
        ...
        a = torch.LongTensor(a.reshape(-1)).to(self.device)
        r = torch.FloatTensor(r).unsqueeze(1).to(self.device)
        ns = torch.FloatTensor(ns).to(self.device)
        d = torch.FloatTensor(d).unsqueeze(1).to(self.device)
        s_mask = _action_mask_from_state_tensor(s)
        ns_mask = _action_mask_from_state_tensor(ns)

        with torch.no_grad():
            next_logits = self.actor.forward(ns)
            next_logits = _masked_logits(next_logits, ns_mask)
            ...
            next_v = (next_probs * (tq_min - alpha * next_log_probs)).sum(dim=1, keepdim=True)
        ...
        logits = _masked_logits(logits, s_mask)
        probs = torch.softmax(logits, dim=-1)
        log_probs = torch.log(probs + 1e-8)
        q1_pi, q2_pi = self.critic(s)
        q_pi = torch.minimum(q1_pi, q2_pi)
        alpha = torch.clamp(self.log_alpha.exp().detach(), min=self.alpha_min)
        long_prob = probs[:, ACT_LONG]
        short_prob = probs[:, ACT_SHORT]
        bias_reg = (long_prob - short_prob).mean().pow(2)
        ...
        det_action_batch = self.actor.deterministic(s, mask=s_mask)
        entry_prob = probs[:, ACT_LONG] + probs[:, ACT_SHORT]
        action_abs_mean = entry_prob.mean()
        det_entry = ((det_action_batch == ACT_LONG) | (det_action_batch == ACT_SHORT)).float()
        det_action_abs_mean = det_entry.mean()
        anti_flat_pen = torch.relu(torch.tensor(self.anti_flat_min_abs, device=self.device) - det_action_abs_mean)
        side_balance_pen = (long_prob.mean() - short_prob.mean()).abs()
        actor_loss = (
            (probs * (alpha * log_probs - q_pi)).sum(dim=1).mean()
        ...
        if self.dynamic_entropy:
            action_std = float(probs.detach().std().item())
            policy_entropy = float(entropy.detach().mean().item())
            det_np = det_action_batch.detach().cpu().numpy()
            n_pos = float(np.sum(det_np == ACT_LONG))
            n_neg = float(np.sum(det_np == ACT_SHORT))
            sign_imbalance = abs(n_pos - n_neg) / max(n_pos + n_neg, 1.0)
            n_entry = float(np.sum((det_np == ACT_LONG) | (det_np == ACT_SHORT)))
        ...
        return {
            ...
            "cvar_q": float(q_pi.mean().item()),
            "target_entropy": float(self.target_entropy),
            "target_std": float(probs.std().item()),
            "var_w": 1.0,
            ...
            "q_disagree": float((q1_pi - q2_pi).abs().mean().item()),
...
class DSACRouter:
    ...
    def decide(self, features, pos):
        ...
        self.actor.eval()
        with torch.no_grad():
            mask = _action_mask_from_state_tensor(state)
        ...
        score = float(np.max(probs_np)) if probs_np.size else 0.0
        if not _futures_exec_enabled(default=False):
            cur_pos = pos.get("type") if isinstance(pos, dict) else None
            if cur_pos is None:
                if action_idx == ACT_LONG:
                    action_int, leverage = 1, 1.0
                elif action_idx == ACT_SHORT:
                    action_int, leverage = 2, 1.0
                else:
                ...
                if action_idx == ACT_CLOSE:
                    ...
                    action_int, leverage = (2, 1.0) if action_idx == ACT_SHORT else (1, 1.0)
                else:
                    action_int, leverage = (1, 1.0) if action_idx == ACT_LONG else (2, 1.0)
            info = {
                "agent": "DSAC_DISCRETE",
                "raw_action": int(action_idx),
                "action_probs": [float(x) for x in probs_np.tolist()],
                "kelly": float(leverage),
                "leverage": float(leverage),
                "long_edge": float(probs_np[ACT_LONG]) if probs_np.size > ACT_LONG else 0.0,
                "short_edge": float(probs_np[ACT_SHORT]) if probs_np.size > ACT_SHORT else 0.0,
                "score": score,
                "state_dim": DSAC_STATE_DIM,
        ...
        if cur_pos is None:
            if action_idx == ACT_LONG:
                action_int, leverage, margin_fraction, notional_exposure = 1, 1.0, 1.0, 1.0
            elif action_idx == ACT_SHORT:
                action_int, leverage, margin_fraction, notional_exposure = 2, 1.0, 1.0, 1.0
            else:
                action_int, leverage, margin_fraction, notional_exposure = 0, 0.0, 0.0, 0.0
        else:
            if action_idx == ACT_CLOSE:
                action_int, leverage, margin_fraction, notional_exposure = 0, 0.0, 0.0, 0.0
            elif cur_pos == "LONG":
                if action_idx == ACT_SHORT:
                    action_int, leverage, margin_fraction, notional_exposure = 2, 1.0, 1.0, 1.0
                else:
                    action_int, leverage, margin_fraction, notional_exposure = 1, 1.0, 1.0, 1.0
            else:
...
def train(
    ...
    config_json_path: str = "data/ensemble/ckpt/dsac_unified_train_config.json",
    checkpoint_path: str = "data/ensemble/ckpt/dsac_unified_checkpoint.pth",
    best_path: str = "data/ensemble/ckpt/best_dsac_unified.pth",
    hmm_cache_path: str = "data/ensemble/ckpt/hmm_init_cache_dsac_unified.npz",
        ...
        for ep in range(start_ep, nep + 1):
            ...
            while not done:
                ...
                if global_step < warmup_steps:
                    pos_state = float(state[_POSITION_STATE_IDX]) if len(state) > _POSITION_STATE_IDX else 0.0
                    if abs(pos_state) < 0.5:
                        strong_entry = max(float(state[0]), float(state[1])) >= 0.33 and float(state[4]) >= 0.10
                        valid_actions = [ACT_LONG, ACT_SHORT] if strong_entry else [ACT_FLAT, ACT_LONG, ACT_SHORT]
                    elif pos_state > 0.5:
                        valid_actions = [ACT_HOLD, ACT_CLOSE, ACT_SHORT]
                    else:
                        valid_actions = [ACT_HOLD, ACT_CLOSE, ACT_LONG]
                    action = np.array([int(np.random.choice(valid_actions))], dtype=np.int64)
                else:
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--checkpoint-path", default="data/ensemble/ckpt/dsac_unified_checkpoint.pth")
    p.add_argument("--best-path", default="data/ensemble/ckpt/best_dsac_unified.pth")
