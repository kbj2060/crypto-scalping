...
MODEL_ID = "alpha3_regime4_state24_v2_plus_pred_full_retrain_20260526"
CLEAN_PREFIX = "clean_regime_2024_unsup_v4_"
SOURCE_STATE24_PREFIX = "clean_regime4_2024_unsup_v1_"
TARGET_STATE24_PREFIX = "clean_regime4_state24_sticky090_v2_"
PRED4_PREFIX = "regime4_pred_"
SIDE_CLEAN4_2025 = ROOT / "data/ensemble/supervised/clean_regime4_state24_sticky090_v2_20260517/training_features_2025_clean_regime4_state24_sticky090_v2.csv"
SIDE_CLEAN4_2026 = ROOT / "data/ensemble/supervised/clean_regime4_state24_sticky090_v2_20260517/training_features_2026_rebuilt_clean_regime4_state24_sticky090_v2.csv"
SIDE_PRED4_2025 = ROOT / "data/ensemble/supervised/regime4_pred_tft_h12_nomdjd_all74_20260517/training_features_2025_regime4_pred_tft_vsn_selected.csv"
SIDE_PRED4_2026 = ROOT / "data/ensemble/supervised/regime4_pred_tft_h12_nomdjd_all74_20260517/training_features_2026_rebuilt_regime4_pred_tft_vsn_selected.csv"
OUT_DIR = ROOT / "data/ensemble/supervised/alpha3_regime4_state24_v2_plus_pred_full_retrain_20260526"
REPORT_OUT = ROOT / "data/ensemble/reports/alpha3_regime4_state24_v2_plus_pred_full_retrain_20260526_summary.json"
AUDIT_OUT = ROOT / "data/ensemble/reports/alpha3_regime4_state24_v2_plus_pred_full_retrain_20260526_audit.json"
GRID_OUT = ROOT / "data/ensemble/reports/alpha3_regime4_state24_v2_plus_pred_full_retrain_20260526_grid.csv"
...
def _rename_state24_sidecar(side: pd.DataFrame) -> pd.DataFrame:
    mapping = {
        c: TARGET_STATE24_PREFIX + c[len(SOURCE_STATE24_PREFIX) :]
        for c in side.columns
        if c.startswith(SOURCE_STATE24_PREFIX)
    }
    renamed = side.rename(columns=mapping)
    v2_cols = [c for c in renamed.columns if c.startswith(TARGET_STATE24_PREFIX)]
    if not v2_cols:
        raise RuntimeError("state24 sidecar rename failed: no v2 columns were produced")
    return renamed
...
def _merge_sidecar(base: pd.DataFrame, side: pd.DataFrame, cols: list[str], *, tag: str) -> tuple[pd.DataFrame, list[str]]:
    base_norm = _normalize_ts(base)
    side_norm = _normalize_ts(side)
    if not cols:
        raise RuntimeError(f"{tag} sidecar selected columns are empty")
    merged = base_norm.merge(side_norm[["timestamp", *cols]], on="timestamp", how="left", suffixes=("", f"__{tag}"))
    missing = [c for c in cols if c not in merged.columns]
    if missing:
        raise RuntimeError(f"{tag} merge failed; missing merged columns: {missing[:20]}")
    na_cols = [c for c in cols if merged[c].isna().any()]
    if na_cols:
        raise RuntimeError(f"{tag} merge produced NaN values: {na_cols[:20]}")
    return merged, cols


def _merge_state24(base: pd.DataFrame, side: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
    v2_cols = [c for c in side.columns if c.startswith(TARGET_STATE24_PREFIX)]
    return _merge_sidecar(base, side, v2_cols, tag="state24_v2")


def _merge_pred4(base: pd.DataFrame, side: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
    pred_cols = [c for c in side.columns if c.startswith(PRED4_PREFIX)]
    return _merge_sidecar(base, side, pred_cols, tag="pred4")


def _build_feature_cols(cols: list[str], state24_v2_cols: list[str], pred4_cols: list[str]) -> tuple[list[str], list[str]]:
    removed = [c for c in cols if c.startswith(CLEAN_PREFIX)]
    kept = [c for c in cols if not c.startswith(CLEAN_PREFIX)]
    kept = [c for c in kept if not c.startswith(SOURCE_STATE24_PREFIX)]
    kept.extend(state24_v2_cols)
    kept = [c for c in kept if not c.startswith(PRED4_PREFIX)]
    kept.extend(pred4_cols)
...
def _safe_seq_cols(df: pd.DataFrame) -> list[str]:
    base_cols = [c for c in v27._seq_cols(df) if not c.startswith(CLEAN_PREFIX) and not c.startswith(SOURCE_STATE24_PREFIX)]
    pred_cols = [c for c in df.columns if c.startswith(PRED4_PREFIX)]
    state_cols = [c for c in df.columns if c.startswith(TARGET_STATE24_PREFIX)]
    rest = [c for c in base_cols if c not in pred_cols and c not in state_cols]
    cols = list(dict.fromkeys(pred_cols + state_cols + rest))
    bad = [
        c
        for c in cols
...
def main() -> int:
    p = argparse.ArgumentParser(description="Full-stack Alpha3 retune with Regime4 state24 sticky090 v2 current-regime context.")
    ...
    clean4_2025 = _rename_state24_sidecar(_read(SIDE_CLEAN4_2025))
    clean4_2026 = _rename_state24_sidecar(_read(SIDE_CLEAN4_2026))
    pred4_2025 = _read(SIDE_PRED4_2025)
    pred4_2026 = _read(SIDE_PRED4_2026)
    train_all, v2_cols_2025 = _merge_state24(train_all, clean4_2025)
    eval_df, v2_cols_2026 = _merge_state24(eval_df, clean4_2026)
    train_all, pred4_cols_2025 = _merge_pred4(train_all, pred4_2025)
    eval_df, pred4_cols_2026 = _merge_pred4(eval_df, pred4_2026)
    if sorted(v2_cols_2025) != sorted(v2_cols_2026):
        raise RuntimeError("state24 v2 columns mismatch between 2025 and 2026 sidecars")
    if sorted(pred4_cols_2025) != sorted(pred4_cols_2026):
        raise RuntimeError("pred4 columns mismatch between 2025 and 2026 sidecars")
    state24_v2_cols = sorted(v2_cols_2025)
    pred4_cols = sorted(pred4_cols_2025)
    ...
    feature_cols, removed_cols = _build_feature_cols(raw_cols, state24_v2_cols, pred4_cols)
        ...
        json.dumps(
            {
                ...
                "added_state24_v2_count": len(state24_v2_cols),
                "added_pred4_count": len(pred4_cols),
    ...
    parent_bundle["candidate_name"] = "state24_v2_full_retrain_parent"
    parent_bundle["feature_ablation"] = {
        "removed_legacy_prefix": CLEAN_PREFIX,
        "removed_legacy_features": removed_cols,
        "added_prefix": TARGET_STATE24_PREFIX,
        "added_state24_v2_features": state24_v2_cols,
        "added_pred4_prefix": PRED4_PREFIX,
        "added_pred4_features": pred4_cols,
        "raw_feature_count": len(raw_cols),
        "candidate_feature_count": len(feature_cols),
    }
    parent_path = OUT_DIR / "parent_state24_v2.pkl"
    ...
    teacher_path = OUT_DIR / "teacher_state24_v2.pt"
    torch.save(
        {
            "model_id": MODEL_ID,
    ...
    scout_path = OUT_DIR / "deep_scout_state24_v2.pt"
    torch.save(
        {
            "model_id": MODEL_ID,
    ...
    runner_path = OUT_DIR / "v21_runner_state24_v2.pkl"
    ...
    candidate = {
        "name": "alpha3_regime4_state24_v2_full_retrain",
    ...
    audit = {
        ...
        "removed_legacy_prefix": CLEAN_PREFIX,
        "removed_legacy_feature_count": len(removed_cols),
        "added_state24_v2_prefix": TARGET_STATE24_PREFIX,
        "added_state24_v2_count": len(state24_v2_cols),
        "added_pred4_prefix": PRED4_PREFIX,
        "added_pred4_count": len(pred4_cols),
    ...
    report = {
        "model_id": MODEL_ID,
        "design": "Full-stack Alpha3 retune with legacy clean_regime_2024_unsup_v4_* removed and current Regime4 features replaced by clean_regime4_state24_sticky090_v2_* (renamed from state24 sidecars before merge). Parent, teacher gate, V21.2 runner, V27-style deep scout, and V31 overlay selection are rebuilt; execution/accounting/data remain frozen.",
        ...
        "feature_contract": {
            ...
            "added_prefix": TARGET_STATE24_PREFIX,
            "added_columns": state24_v2_cols,
            "added_pred4_prefix": PRED4_PREFIX,
            "added_pred4_columns": pred4_cols,
            "source_sidecars": {
                "clean4_2025": str(SIDE_CLEAN4_2025),
                "clean4_2026": str(SIDE_CLEAN4_2026),
                "pred4_2025": str(SIDE_PRED4_2025),
                "pred4_2026": str(SIDE_PRED4_2026),
            },
        },
