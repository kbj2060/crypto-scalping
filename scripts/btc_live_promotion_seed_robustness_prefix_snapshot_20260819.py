#!/usr/bin/env python3
"""git HEAD(현재 7bf0b3f, 이 파일을 마지막으로 건드린 커밋은 e36908d/2026-08-14) 기준
scripts/train_eval_omega4_3head_parent72_loose_entry_quality_btc_swingtransition_20260806.py를
그대로 스냅샷한 파일 -- 작업트리의 그 파일은 2026-08-18 exit_head pos_tp/pos_sl/pos_notional
버그수정(CLAUDE.md "Position-Feature Train/Inference Parity Contract")이 미커밋 상태로 적용돼
있어(git diff HEAD로 확인, ETH 원본과 동일 패턴의 수정), import하면 수정본이 로드된다. 이
스냅샷은 실제 라이브 BTC h48qual+swingtransition 번들(tmp/causal_regen_20260516/btc_omega4_
3head_parent72_loose_entry_quality_swingtransition_20260806_h48qual_20260806_swingtransition,
2026-08-06 학습)을 만들어낸 정확한 코드 상태를 보존해, "라이브로 승격된 그 모델 자체가 시드에
강건한가"를 재현학습으로 검증하기 위한 용도(ETH eth_live_promotion_seed_robustness_prefix_
snapshot_20260819.py와 동일 기법, N=5로 BTC에 적용). scripts/btc_live_promotion_seed_
robustness_seed_variant_20260819.py가 이 모듈을 import해서 쓴다.

이 파일 내용 자체는 git show HEAD:<원본경로> 리다이렉트 그대로이며(diff로 바이트 동일 확인),
바뀐 건 이 docstring뿐이다. 원본 모듈이 스스로 설명하던 내용(ETH omega 모듈과 달리 train_fixed
정렬을 안 쓴다는 것 등)은 아래 본문에 그대로 남아 있다:

  BTC copy of train_eval_omega4_3head_parent72_loose_entry_quality_20260620.py. Imports the BTC
  `omega` module (train_eval_omega1_2_tabm_diffusion_risk_btc_swingtransition_20260806.py) instead
  of the ETH one. The only structural change vs the ETH script is in _prepare_frames(): the ETH
  version builds `train_fixed`/`train_df` by aligning to a frozen upstream Omega1.2 TabM prediction
  file (omega.TABM_2025) that this BTC build does not have. That block is only consumed by the
  `independent_entry_hold_offsets` exit-head mode; the production recipe uses
  `entry_label_terminal_giveback` instead (verified against the live bundle's own report.json
  `exit_label.mode`), which builds exit-head examples directly from zigzag_action segments and
  never touches `train_fixed`. So here `train_df` is simply `train_raw` (no external alignment
  needed) and `train_fixed` is left unset - callers must pass --exit-label-mode
  entry_label_terminal_giveback (or entry_label_path_optimal) to match the real production recipe;
  the default independent_entry_hold_offsets mode is NOT supported by this BTC copy.

BTC-vs-ETH 사전점검 차이(2026-08-19, scripts/btc_live_promotion_seed_robustness_precheck_
20260819.py 참고): ETH는 legacy CSV의 7주치 피쳐엔지니어링 누적 드리프트 때문에 canonicaldata
오버라이드+102-pin이 필요했다. BTC omega 모듈의 TRAIN_CSV/EVAL_CSV 기본값(data/splits/year_oos/
btc_features_{2025,2026}_swingtransition.csv) 자체는 라이브 번들과 같은 날(2026-08-06)의 정적
CSV라 그 종류의 드리프트는 구조적으로 있을 수 없다고 예상되나(이유는 ETH와 다름 -- 최종 실측결과는
precheck 실행 로그 참고). 다만 이와 별개로, omega 모듈이 요구하는 REGIME3_CURRENT_2025/2026
오버레이(6개 wide24 HMM regime 컬럼)는 공유 정식 경로에서 2025년 파일이 아예 없고 2026년 파일도
백업만 남아있는 것으로 드러났다(dev/server 둘 다 동일, 원인 불명) -- 이 스냅샷 자체는 그대로 두고,
btc_live_promotion_seed_robustness_canonicaldata_20260819.py가 그 오버레이만 별도 scratch
경로에 재생성해서 감싼다 (같은 날 SOL 쪽에서 먼저 발견/해결한 것과 동일 패턴).
"""
