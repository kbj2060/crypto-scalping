...
MODEL_ID = "alpha3_rl_exit_owner_fulltrain_20260514"
OUT_DIR = ROOT / "data/ensemble/supervised/alpha3_rl_exit_owner_fulltrain_20260514"
MODEL_OUT = OUT_DIR / "offline_rl_exit_q.pt"
REPORT_OUT = ROOT / "data/ensemble/reports/alpha3_rl_exit_owner_fulltrain_20260514_summary.json"
AUDIT_OUT = ROOT / "data/ensemble/reports/alpha3_rl_exit_owner_fulltrain_20260514_audit.json"
GRID_OUT = ROOT / "data/ensemble/reports/alpha3_rl_exit_owner_fulltrain_20260514_grid.csv"
DATASET_OUT = ROOT / "data/ensemble/reports/alpha3_rl_exit_owner_fulltrain_20260514_dataset.json"
...
def _q_from_model(model: ExitQNet, x: np.ndarray, mean: np.ndarray, std: np.ndarray) -> np.ndarray:
    ...
    with torch.no_grad():
        return model(torch.from_numpy(z[None, :]).to(device)).cpu().numpy()[0].astype(np.float64)
...
def _train_q_model(x: np.ndarray, y: np.ndarray, seed: int = 20260514) -> tuple[ExitQNet, dict[str, Any]]:
    ...
    model = ExitQNet(x.shape[1], y.shape[1]).to(device)
    opt = torch.optim.AdamW(model.parameters(), lr=7e-4, weight_decay=2e-4)
    loss_fn = nn.SmoothL1Loss()
    x_fit_t = torch.from_numpy(x_fit_z).to(device)
    y_fit_t = torch.from_numpy(y_fit).to(device)
    x_hold_t = torch.from_numpy(x_hold_z).to(device)
    y_hold_t = torch.from_numpy(y_hold).to(device)
    best_loss = float("inf")
    ...
    for epoch in range(1, 161):
        model.train()
        order = torch.randperm(len(x_fit_t), device=device)
        total = 0.0
...
def main() -> int:
    ...
    report = {
        ...
        "date": "2026-05-14",
        "design": {
            "algorithm": "compact discrete fitted-Q / CQL-style conservative offline RL exit owner",
            "scope": "Alpha3 entry stack frozen; RL is allowed to close the active position early and select reduce-only exit placement. First fulltrain version closes 100%, no partial sizing yet.",
    ...
    audit = {
        ...
        "limitations": [
            ...
            "This first exit-owner policy can close 100% early and select exit placement, but does not yet perform partial close/TWAP sizing.",
