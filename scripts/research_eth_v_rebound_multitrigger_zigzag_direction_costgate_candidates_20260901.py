#!/usr/bin/env python3
"""ZDC(wick-앵커) 9트리거 통합 V자반등류 모델의 trade-candidate population 빌드 — 계획서
(swift-doodling-grove.md) Step E 1/2. `research_eth_sweep_v_rebound_v7b_costgate_candidates_
20260830.py`(giveback v7b판)와 정확히 같은 방법론: VAL+OOS(out-of-sample) 후보에 대해 모델 자신의
4시드 평균 proba로 실제 "콜"(proba>=0.5)한 것만 남기고(라벨 자체가 아니라 라이브에서 실제로
보게 될 모집단), 원인이 된 idx+1봉 open을 entry로, forward OHLC를 붙인다.

FORWARD_BARS 재검토(계획서 Step E 필수사항): v7b는 자기 라벨창(60분=12봉)의 ~16.7배인 200봉을
버퍼로 썼다. ZDC는 자기 라벨(entry 확정, idx->confirm_idx)만으로도 p50=24봉/p90=87봉/p99=208봉/
max=288봉(2026-09-01 실측)로 giveback보다 훨씬 느리게 해상되므로, 동일 비율을 적용하면 비현실적으로
커진다 — 대신 "entry 해상p99(208봉)를 충분히 덮고, 그 뒤 트레일링이 전개될 여유"를 기준으로
FORWARD_BARS=400(33.3h)을 사용한다. 트레일링스톱 자체는 라벨 해상과 무관하게 SL/ARM/Trail 조건만
보고 독립적으로 청산되므로 대부분의 경우 훨씬 일찍 끝난다 — 버퍼는 "한 번도 안 멈춘" 꼬리 케이스의
안전장치일 뿐. 버퍼 소진 비율은 산출물에 명시적으로 기록한다(계획서 요구사항).

Entry 컨벤션은 giveback과 동일(entry=트리거idx+1봉 open, downside트리거=LONG/upside트리거=SHORT)
— v7b 스크립트에서 그대로 재사용, 재구현 아님. HOLDOUT은 여기서 안 건드림(VAL+OOS만).
"""
...
HOLDOUT_START = pd.Timestamp("2026-04-01", tz="UTC")  # untouched by this script -- VAL+OOS only
LABEL_WINDOW = pd.Timedelta(hours=24)  # matches Step D embargo (MAX_LOOKFORWARD_BARS=288)
...
def main() -> int:
    ...
    print(f"ZDC called (proba>=0.5): {len(called)} / {len(combined)} "
    ...
    result.to_pickle(OUT_DIR / "zdc_costgate_candidates.pkl")
    result.drop(columns=["fwd_open", "fwd_high", "fwd_low", "fwd_close"]).to_csv(
        OUT_DIR / "zdc_costgate_candidates_summary.csv", index=False)
