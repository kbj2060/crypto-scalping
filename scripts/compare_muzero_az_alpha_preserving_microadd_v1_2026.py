...
MODEL_ID = "muzero_az_alpha_preserving_microadd_v1_2026"
DEFAULT_REPORT_OUT = ROOT / "data/ensemble/reports/muzero_az_alpha_preserving_microadd_v1_2026.json"
DEFAULT_MODEL_DIR = ROOT / "data/ensemble/supervised/muzero_az_alpha_preserving_microadd_v1"
BASELINE_TARGET = {
    "pnl": 467.64425635219607,
    "mdd": -25.91296925261518,
    "trades": 369,
    "trades_per_day": 6.2897727272727275,
    "avg_leverage": 1.5949813894973917,
}
MICROADD_NOTIONAL_CAPS = (0.10, 0.15, 0.20, 0.25, 0.35)
MICROADD_LEVERAGE_CAPS = (1.0, 1.2, 1.4, 1.6)
        ...
        if pos != 0:
            ...
            if age >= int(exit_cfg["min_exit_age"]):
                ...
                if _exit_probability_vec(exit_model, row_vec) >= float(exit_cfg["exit_threshold"]):
                    ...
                    model_cooldown = 0
                    loss_streak = loss_streak + 1 if unreal < 0.0 else 0
                    if unreal < 0.0:
    ...
    out = pd.DataFrame(
        {
            ...
            "baseline_entry_rows_approx": entry_rows,
            "baseline_exit_rows_approx": exit_rows,
        }
    )
    audit = {
        "method": "sequential_replay_approx_with_exit_governor_probabilities_and_simplified_risk_blocks",
        "rows": int(len(df)),
        "position_open_before_rows": int(position_open_before.sum()),
        "flat_for_microadd_rows": int(flat_for_microadd.sum()),
        "approx_entry_rows": int(entry_rows.sum()),
        "approx_exit_rows": int(exit_rows.sum()),
        "block_counts": block_counts,
        "exact_backtest_state_claimed": False,
    }
    return out, audit
...
def _microadd_grid_configs(args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    for margin in (0.20, 0.30, 0.40):
        for agreement in (3, 4):
            for survival in (0.62, 0.70, 0.78):
                for edge_floor in (0.0010, 0.0020, 0.0035):
                    for max_cvar_loss in (0.006, 0.010, 0.015):
...
def _selection_score(candidate: dict[str, Any], baseline: dict[str, Any], cost3_candidate: dict[str, Any], cost3_baseline: dict[str, Any]) -> float:
    ...
    return float(pnl_delta - mdd_penalty + 0.25 * tpd_delta - 0.35 * cost3_degradation)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Compare frozen MuZero/AZ with alpha-preserving defensive + micro-add sleeve v1.")
    ...
    p.add_argument("--max-grid-configs", type=int, default=60)
...
def main() -> int:
    ...
    target_idx, target, defense_target_meta = _diagnostic_targets(train_df, train_dec, cfg=def_cfg, fee=float(args.fee), slip=float(args.slip))
    defense_models, defense_train_meta = _train_sleeve(train_x_def, target_idx, target, def_cfg)
    val_pred_def = _predict_sleeve(defense_models, val_x_def)
    eval_pred_def = _predict_sleeve(defense_models, eval_x_def)

    defense_grid: list[dict[str, Any]] = []
    for i, sleeve_cfg in enumerate(_grid_configs(args)):
        val_def_dec, telemetry = _apply_defensive_sleeve(val_dec, val_pred_def, val_x_def, sleeve_cfg, def_cfg, max_notional=max_notional, leverage_cap=float(args.leverage_cap))
        row = _run(f"alpha_preserving_defense_grid_{i:03d}_val", val_df, policy, az_exit, entry_cfg, risk_cfg, zero_exit_cfg, (val_feat, val_def_dec, val_close, val_fill), fee=args.fee, slip=args.slip, mdd_weight=args.mdd_weight)
        row["config"] = sleeve_cfg
        row["telemetry"] = telemetry
        defense_grid.append(row)
    selected_defense = sorted(defense_grid, key=lambda r: float(r["score"]), reverse=True)[0]
    selected_defense_cfg = dict(selected_defense["config"])
    ...
    val_def_dec, val_def_telemetry = _apply_defensive_sleeve(val_dec, val_pred_def, val_x_def, selected_defense_cfg, def_cfg, max_notional=max_notional, leverage_cap=float(args.leverage_cap))
    eval_def_dec, eval_def_telemetry = _apply_defensive_sleeve(eval_dec, eval_pred_def, eval_x_def, selected_defense_cfg, def_cfg, max_notional=max_notional, leverage_cap=float(args.leverage_cap))
    ...
    loop_success_gates = {
        "reference": "limited_eval_current_muzero_az" if smoke_mode else "full_oos_current_muzero_az_target",
    ...
    joblib.dump(
        ...
        args.model_dir / "alpha_preserving_microadd_v1.pkl",
    )
    selector_payload = {
    ...
    report = {
        ...
        "note": "Frozen Current MuZero/AZ baseline plus defensive monotonic active-row sleeve and deterministic current-bar-vote micro-adds only when sequential baseline replay is flat.",
        ...
        "candidate_config": {
            "architecture": ["Alpha-preserving defensive gate", "Current-bar deterministic vote micro-add generator", "HGB survival/edge/CVaR scorer", "Validation-only threshold/quota selector"],
        ...
        "validation": {
            ...
            "selection_trace": {"top_configs": sorted(micro_grid, key=lambda r: float(r["selection_score"]), reverse=True)[:20], "ineligible_counts": ineligible_counts, "eligible_count": int(len(eligible_grid)), "grid_count": int(len(micro_grid))},
        },
        "eval": {
            "baseline_muzero_az": zero_eval,
            "alpha_preserving_microadd_v1": candidate_eval,
        ...
        "calibration_audit": {"selector_artifact": str(args.model_dir / "threshold_selector.json"), "model_artifact": str(args.model_dir / "alpha_preserving_microadd_v1.pkl")},
        ...
        "limitations": ["Funding, liquidation, and maintenance-margin path are not first-class in this loop-1 implementation.", "Sequential flat state is an approximation audited separately from final accounting."],
    }
