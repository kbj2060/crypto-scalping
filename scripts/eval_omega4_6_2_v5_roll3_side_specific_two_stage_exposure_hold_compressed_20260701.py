#!/usr/bin/env python3
"""3h hold-compressed two-stage exposure branch for Omega 4.6.2 v5."""
...
MODEL_ID = "omega4_6_2_v5_roll3_side_specific_two_stage_exposure_hold_compressed_20260701"
REFERENCE_MODEL_ID = "omega4_6_2_v5_roll4_side_specific_two_stage_exposure_hold_compressed_20260701"
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll3_side_specific_two_stage_exposure_hold_compressed_20260701.md"
MAX_ROLL_HOURS = 3.0
...
def load_roll5() -> Any:
    roll5 = load_module("omega462_roll5_for_roll3_eval", ROLL5_PATH)
...
def write_markdown(report: dict[str, Any]) -> None:
    ...
    text = f"""# Omega 4.6.2 v5 Roll3 Side-Specific Two-Stage Exposure Hold Compressed - 2026-07-01
...
- Max roll hold: `{selected["roll3_max_hours"]}h`
- Research gate pass: `{selected["research_roll3_hold_compressed_gate_pass"]}`
...
def main() -> int:
    ...
    roll7 = roll6.load_module("omega462_roll7_for_roll3_eval", roll6.ROLL7_PATH)
    ...
    for long_factor in [0.00, 0.05, 0.10, 0.20, 0.30, 0.40, 0.50, 0.60, 0.70, 0.80]:
        for short_factor in [0.40, 0.50, 0.60, 0.70, 0.80, 0.90, 1.00, 1.10, 1.20, 1.30, 1.40]:
            for cap_notional in [2.00, 2.40, 2.80, 3.20, 3.60, 4.00, 4.40, 4.80, 5.00]:
                ...
                row = {
                    ...
                    "roll3_max_hours": MAX_ROLL_HOURS,
                    "roll3_long_tp_move": 0.0200,
                    "roll3_long_sl_move": 0.0300,
                    "roll3_short_tp_move": 0.0250,
                    "roll3_short_sl_move": 0.0385,
                ...
                row["validation_roll3_hold_compressed_gate_pass"] = validation_gate
                row["oos_safety_gate_pass"] = oos_gate
                row["research_roll3_hold_compressed_gate_pass"] = validation_gate and oos_gate
                rows.append(row)

    ranking = pd.DataFrame(rows).sort_values(
        [
            "research_roll3_hold_compressed_gate_pass",
            "validation_pnl",
            "oos_pnl",
    ...
    status = (
        "RESEARCH_ROLL3_TWO_STAGE_EXPOSURE_HOLD_COMPRESSED_PASS"
        if bool(selected["research_roll3_hold_compressed_gate_pass"])
        else "NO_ROLL3_TWO_STAGE_EXPOSURE_HOLD_COMPRESSED_PASSING_CANDIDATE"
    )
    safe = str(selected["exposure_spec"]).replace(".", "p").replace("/", "_")
    ranking_path = OUT_DIR / "roll3_two_stage_exposure_hold_compressed_ranking.csv"
    top20_path = OUT_DIR / "roll3_two_stage_exposure_hold_compressed_top20.csv"
    ...
    report = {
        ...
        "selection_scope": "validation_primary_roll3_two_stage_veto_exposure_overlay_with_oos_safety_gate; fresh_holdout_required",
        "selection_rule": "regenerate 3h roll path with current two-stage veto rules; among research-gated exposure overlays, sort by validation_pnl, oos_pnl, validation_avg_hold",
        ...
        json.dumps(
            {
                ...
                "selected_validation": {
                    ...
                    "gate": bool(selected["validation_roll3_hold_compressed_gate_pass"]),
                },
                "selected_oos": {
