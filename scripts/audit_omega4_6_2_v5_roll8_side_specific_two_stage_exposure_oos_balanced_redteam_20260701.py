#!/usr/bin/env python3
"""Red-team audit for Omega 4.6.2 v5 roll8 two-stage OOS-balanced exposure branch."""
...
BUFFERED_AUDIT_PATH = ROOT / "scripts/audit_omega4_6_2_v5_roll8_side_specific_two_stage_exposure_buffered_redteam_20260701.py"
EVAL_MODULE_PATH = ROOT / "scripts/eval_omega4_6_2_v5_roll8_side_specific_two_stage_exposure_buffered_20260701.py"
...
RANKING = OUT_DIR / "roll8_two_stage_exposure_oos_balanced_ranking.csv"
PRIOR_RUNTIME_AUDIT = (
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll8_side_specific_two_stage_exposure_oos_balanced_redteam_20260701.md"
TOL = 1.0e-8
...
def write_markdown(payload: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- Best buffered validation PnL: `{payload['best_buffered_validation_pnl']:.4f}%`",
        f"- Nearmax tolerance: `{payload['validation_nearmax_tolerance_pp']}pp`",
        "",
    ...
    for split in ["validation", "oos"]:
        lines.append(
            ...
            f"{observed[split]['avg_hold_hours']:.4f}h | {observed[split]['max_hold_hours']:.4f}h |"
        )
    ...
    lines.extend(
        [
            ...
            "## Replay Checks",
            "",
            f"- Validation exposure replay: `{payload['exposure_replay']['validation']['pass']}`",
            f"- OOS exposure replay: `{payload['exposure_replay']['oos']['pass']}`",
            "",
            "## Artifacts",
            "",
...
def main() -> None:
    audit = load_module("omega462_loss_cluster_audit_for_oos_balanced_exposure", AUDIT_HELPER_PATH)
    buffered_audit = load_module("omega462_buffered_exposure_audit_for_oos_balanced", BUFFERED_AUDIT_PATH)
    eval_mod = load_module("omega462_buffered_exposure_eval_for_oos_balanced_audit", EVAL_MODULE_PATH)
    report = read_json(REPORT)
    reference_report = read_json(REFERENCE_REPORT)
    buffered_report = read_json(BUFFERED_REPORT)
    selected = report["selected_variant"]
    reference = report["reference_variant"]
    buffered_selected = buffered_report["selected_variant"]
    ...
    audit.add_check(
        ...
        == "nearmax_validation_two_stage_exposure_overlay_with_oos_balanced_selection; fresh_holdout_required",
        "research_blocker",
        {"observed": report.get("selection_scope")},
    )
    audit.add_check(
        checks,
        "selection_rule_declared",
        report.get("selection_rule")
        == "reuse buffered exposure grid; require research gate and validation_pnl within 1.0pp of best buffered validation_pnl; select highest oos_pnl, then validation_pnl",
        "research_blocker",
        {"observed": report.get("selection_rule")},
    )
    audit.add_check(
        checks,
        "nearmax_oos_balanced_contract",
        float(selected["validation_pnl"])
        >= float(report["best_buffered_validation_pnl"]) - float(report["validation_nearmax_tolerance_pp"])
        and float(selected["oos_pnl"]) > float(buffered_selected["oos_pnl"])
        and float(selected["validation_pnl"]) <= float(buffered_selected["validation_pnl"]) + TOL,
        "research_blocker",
        {"selected": selected, "buffered_selected": buffered_selected},
    )
    ...
    val_replay = buffered_audit.exposure_replay(ref_validation_df, validation_df, selected, eval_mod)
    oos_replay = buffered_audit.exposure_replay(ref_oos_df, oos_df, selected, eval_mod)
    audit.add_check(
        checks,
        "exposure_replay_parity",
    ...
    verdict = (
        "FULL_LIVE_PASS"
        if full_live_pass
        else "RESEARCH_ROLL8_TWO_STAGE_EXPOSURE_OOS_BALANCED_PASS_FULL_LIVE_BLOCKED"
        if research_pass
        else "REDTEAM_FAIL"
    )
    payload = {
        "audit_id": "omega4_6_2_v5_roll8_side_specific_two_stage_exposure_oos_balanced_redteam_20260701",
        ...
        "validation_nearmax_tolerance_pp": report["validation_nearmax_tolerance_pp"],
        "best_buffered_validation_pnl": report["best_buffered_validation_pnl"],
        "selected_variant": selected,
        "reference_variant": reference,
        "buffered_reference_variant": buffered_selected,
