...
MODEL_ID = "hf_v13_deep_residual_scout_preempt_v27_2_20260511"
...
DEFAULT_OUT_DIR = ROOT / "data/ensemble/supervised/hf_v13_deep_residual_scout_preempt_v27_2_20260511"
DEFAULT_REPORT = ROOT / "data/ensemble/reports/hf_v13_deep_residual_scout_preempt_v27_2_20260511_summary.json"
DEFAULT_AUDIT = ROOT / "data/ensemble/reports/hf_v13_deep_residual_scout_preempt_v27_2_20260511_audit.json"
DEFAULT_GRID = ROOT / "data/ensemble/reports/hf_v13_deep_residual_scout_preempt_v27_2_20260511_grid.csv"
HORIZONS = (12, 24, 48)
    ...
    for i in range(0, len(df) - 2):
        ...
        if pos != 0:
            ...
            if owner == "deep_alpha" and not reason:
                parent_dec = decisions.iloc[i]
                if int(parent_dec.action) != ACTION_CASH and int(parent_dec.side) != 0:
                    reason = "deep_alpha_parent_preempt"
                    parent_preempt = True
            ...
            if reason:
                ...
                pos = 0
                owner = ""
                closed_cooldown = int(next_cooldown)
                next_cooldown = 0
                if parent_preempt:
                    cooldown = 0
                    deep_cooldown = 0
                else:
                    cooldown = closed_cooldown
                    deep_cooldown = max(deep_cooldown, int(cfg.cooldown))
                add_done = False
                open_record = None
                if not parent_preempt:
                    continue
        if pos != 0:
            continue
        if cooldown > 0:
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="V27.2 deep residual scout sleeve with parent-preempt mask.")
...
def main() -> int:
    ...
    model_path = args.out_dir / "v27_2_deep_residual_scout_preempt.pt"
