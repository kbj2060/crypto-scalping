...
MODEL_ID = "alpha3_dsac_exit_owner_v7_20260515"
OUT_DIR = ROOT / "data/ensemble/supervised/alpha3_dsac_exit_owner_v7_20260515"
MODEL_OUT = OUT_DIR / "dsac_exit_owner_v7.pt"
REPORT_OUT = ROOT / "data/ensemble/reports/alpha3_dsac_exit_owner_v7_20260515_summary.json"
AUDIT_OUT = ROOT / "data/ensemble/reports/alpha3_dsac_exit_owner_v7_20260515_audit.json"
GRID_OUT = ROOT / "data/ensemble/reports/alpha3_dsac_exit_owner_v7_20260515_grid.csv"
DATASET_OUT = ROOT / "data/ensemble/reports/alpha3_dsac_exit_owner_v7_20260515_dataset.json"
...
EXPANDED_ACTIONS = ["hold", "close_100"]
...
def _runtime_policies() -> list[base_exit.OfflineRLPolicy]:
    ...
    for min_hold in (6, 12, 18, 24, 32):
        for partial_mode in ("none", "narrow", "strict"):
            suffix = f"partial{partial_mode}"
            fallback = "next_open_limit_touch0_fee20"
            rows.append(
                base_exit.OfflineRLPolicy(
                    name=f"dsac_exit_owner_minhold{min_hold}_{suffix}_fb_{fallback}",
                    q_margin=0.0,
...
def _partial_mode(policy: base_exit.OfflineRLPolicy) -> str:
    name = str(policy.name)
    if "partialstrict" in name:
        return "strict"
    if "partialnarrow" in name:
        return "narrow"
    return "none"


def _should_partial_profit_lock(
    policy: base_exit.OfflineRLPolicy,
    *,
    owner: str,
    hold: int,
    ...
    mode = _partial_mode(policy)
    if mode == "none" or reduce_steps >= 1 or hold - last_reduce_hold < 16:
        return False
    if hold < max(int(policy.min_hold), 8):
        return False
    giveback = max(0.0, float(mfe - unreal))
    tp_anchor = max(float(effective_tp), 0.020)
    if mode == "strict":
        return (
            owner == "deep_alpha"
            and unreal >= max(0.036, 0.78 * tp_anchor)
            and mfe >= max(0.052, 1.05 * tp_anchor)
            and giveback >= max(0.010, 0.22 * max(mfe, 0.0))
            and mae > -0.020
        )
    return (
        unreal >= max(0.030, 0.70 * tp_anchor)
        and mfe >= max(0.045, 0.95 * tp_anchor)
        and giveback >= max(0.008, 0.20 * max(mfe, 0.0))
        and mae > -0.024
    )
    ...
    *,
    ...
    for i in range(0, len(df) - 2):
        ...
        if pos != 0:
            ...
            reason = base_reason
            if not base_reason and _should_partial_profit_lock(
                policy,
                owner=owner,
                hold=hold,
                unreal=unreal,
                ...
                filled, cash, notional, _, route = _apply_exit_fill(
                    ...
                    notional=notional,
                    fraction=0.5,
                    action_name="partial_profit_lock",
                ...
                if filled:
                    ...
                    partial_exits += 1
                    runner_actions["partial_profit_lock"] = runner_actions.get("partial_profit_lock", 0) + 1
                    continue
                else:
                    runner_actions["partial_profit_lock_miss"] = runner_actions.get("partial_profit_lock_miss", 0) + 1
...
def main() -> int:
    ...
    dataset_summary = {
        **dataset_meta,
        ...
        "reward_design": {
            ...
            "partial_profit_lock": "Not a DSAC action. Runtime deterministic one-shot 50% reduce for narrow profit-lock/giveback contexts.",
        },
    }
    ...
    for mode_i, mode in enumerate(FEATURE_MODES):
        ...
        model_out = OUT_DIR / f"dsac_exit_owner_v7_{mode}.pt"
    ...
    experiments = [
        ...
        {
            "name": f"alpha3_dsac_exit_owner_v7::{selected_mode}::{selected_policy.name}",
    ...
    report = {
        ...
        "design": {
            ...
            "scope": "Alpha3 corrected entry stack frozen. DSAC exit owner chooses only hold or close_100. A deterministic one-shot partial-profit-lock overlay may close 50% before DSAC full-exit decisions.",
            "feature_design": "Feature mode is fixed to state16_topbase20 from Alpha3 exit-owner feature analysis 20260515.",
            "limitations": "No L2/orderbook/queue inputs or labels by request. Maker fills use existing OHLC immediate-limit proxy. Partial close is rule-gated, not learned by DSAC.",
    ...
    audit = {
        ...
        "notes": [
            ...
            "Only hold and close_100 DSAC actions are included. Execution placement is not learned by DSAC.",
