#!/usr/bin/env python3
"""RESEARCH ONLY -- ONE-SHOT OOS confirmation of the TCN h48qual exit_head (Odyssey2 priority #5,
last item in the priority queue), GATED on
scripts/research_eth_omega461_tcn_exit_head_val_20260813.py's VAL gate. This script reads that
script's report.json and REFUSES TO RUN (raises before loading any OOS data) if gate_pass is not
True -- per this project's "VAL 못 이기면 OOS는 절대 보지 말고 부정 결과로 문서화" discipline,
enforced in code, not left to operator discretion. Same pattern as priority #4's
research_eth_omega461_gbdt_exit_head_oos_20260813.py. No retuning after seeing this result: if run,
...
exit-head research line's standing OOS convention, matching priority #4's OOS script and
...
(docs/experiments/eth_omega461_oos_selection_bias_scope_and_resolution_20260813.md), same as
priority #4's OOS script: the deployed quality_threshold values h48qual (0.50) and zig075 (0.75)
both run live with -- shared identically by ALL FOUR variants compared here (baseline / TabM
live-ATR / GBDT / TCN all freeze direction_head / quality_head / quality_threshold, only exit_head
differs) -- were themselves OOS-pnl-primary selected against a frame spanning
2026-01-01..2026-02-28, the first two of this run's three OOS months. Because that contamination
applies EQUALLY to all variants (identical frozen direction/quality/quality_threshold), the RELATIVE
comparison (does TCN beat the TabM live-ATR baseline within this run) remains meaningful. The
ABSOLUTE OOS PnL/MDD figures must NOT be read as clean, unbiased forward performance.

Portfolio-level only (matches priority #4's OOS scope -- no component-level OOS table exists for
the TabM live-ATR candidate either).

fresh_forward_bar_by_bar=true (greedy_replay_windowed: single causal forward pass).
trade_ledgers_used_as_input=false. saved_parent_exit_timestamps_used=false.
future_rows_used_for_entry=false. direction_head/quality_head frozen (unchanged across all
variants); only h48qual's exit_head differs. No duration-gate post-filter applied (matches the
VAL-side comparison and current live's gate-off setting).
...
import research_eth_omega461_tcn_exit_head_val_20260813 as tcn_val  # noqa: E402

OUT_DIR = ROOT / "tmp/causal_regen_20260516/eth_omega461_tcn_exit_head_oos_20260813"
VAL_REPORT_PATH = tcn_val.OUT_DIR / "report.json"

CAVEAT_TEXT = (
    "quality_threshold (h48qual=0.50, zig075=0.75), shared identically by all variants compared "
    "here (baseline / TabM live-ATR / TCN), was OOS-pnl-primary selected against a frame spanning "
    "exactly 2026-01-01..2026-02-28 -- the first two of this run's three OOS months (see "
    "docs/experiments/eth_omega461_oos_selection_bias_scope_and_resolution_20260813.md). The "
    "relative comparison (TCN vs TabM live-ATR baseline within this run) remains meaningful because "
    "all variants share the identical contaminated entry-selection layer; the absolute OOS PnL/MDD "
    "figures below are not clean unbiased forward performance and must not be over-interpreted as "
    "such."
)
...
def _run_variant_oos_tcn(
    oos_frame: pd.DataFrame, aligned_pred_paths: dict[str, Path], tcn_bundle: dict[str, Any], *, fee: float, slip: float,
) -> dict[str, Any]:
    ...
    # matching priority #4's OOS script's identical use of greedy.prepare_component here).
    h48qual_prepped = greedy.prepare_component(oos_frame, aligned_pred_paths["h48qual"], h48qual_cfg, portfolio.DEVICE)
    h48qual_tcn = tcn_val._inject_tcn_exit_runtime(h48qual_prepped, tcn_bundle, portfolio.DEVICE, base_cols)
    ...
    _diag, ledger = tcn_val.greedy_replay_windowed(
        oos_frame, components, fee=fee, slip=slip, cost_mult=sweep.COST_MULT, device=portfolio.DEVICE, window=int(tcn_bundle["window"]),
    )
    ledger.to_csv(OUT_DIR / "portfolio_ledger_oos_asymmetric_h48qual_tcn_zig075_original.csv", index=False)
...
def main() -> int:
    ...
    if not VAL_REPORT_PATH.exists():
        raise RuntimeError(
            f"VAL report not found -- run research_eth_omega461_tcn_exit_head_val_20260813.py first: {VAL_REPORT_PATH}"
        )
    val_report = json.loads(VAL_REPORT_PATH.read_text(encoding="utf-8"))
    if not bool(val_report.get("gate_pass")):
        raise RuntimeError(
            "VAL gate_pass=False -- TCN did not beat the TabM live-ATR baseline on VAL "
    ...
    print("stage=load_tcn_bundle", flush=True)
    tcn_bundle = tcn_val._load_tcn_bundle(tcn_val.TCN_BUNDLE)
    ...
    print("stage=run_variant_oos name=asymmetric_h48qual_tcn_zig075_original", flush=True)
    oos_results["asymmetric_h48qual_tcn_zig075_original"] = _run_variant_oos_tcn(oos_frame, aligned_pred_paths, tcn_bundle, fee=fee, slip=slip)
    ...
    val_by_name = {
        ...
        "asymmetric_h48qual_tcn_zig075_original": val_results_portfolio["asymmetric_h48qual_tcn_zig075_original"],
    }
    for name in ("baseline_both_original", "asymmetric_h48qual_liveatr_zig075_original", "asymmetric_h48qual_tcn_zig075_original"):
    ...
    tcn_oos = oos_results["asymmetric_h48qual_tcn_zig075_original"]
    tabm_oos = oos_results["asymmetric_h48qual_liveatr_zig075_original"]
    oos_tcn_nonworse = bool(float(tcn_oos["pnl"]) >= float(tabm_oos["pnl"]) and float(tcn_oos["mdd"]) >= float(tabm_oos["mdd"]))
    print(f"stage=oos_relative_result tcn_nonworse_than_tabm_liveatr={oos_tcn_nonworse}", flush=True)
    ...
    report = {
        ...
        "tcn_bundle": str(tcn_val.TCN_BUNDLE),
        "val_results_source": str(VAL_REPORT_PATH),
        "val_results": val_by_name,
        "oos_results": oos_results,
        "oos_tcn_nonworse_than_tabm_liveatr": oos_tcn_nonworse,
    }
