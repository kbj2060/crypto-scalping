#!/usr/bin/env python3
"""Tier0 features for the FEEDER-ROLE-OPTIMIZED 8-trigger V자반등 label. Verbatim mirror of
build_eth_5m_v_rebound_multitrigger_features_tier0_20260831.py (same feature set/formulas, same
build_indicator_frame() reuse, same timestamp-join convention) -- only the source label CSV and
output dir point at the feeder-optimized pool (build_eth_5m_v_rebound_multitrigger_labels_feeder_
optimized_20260901.py's output) instead of the original 9-trigger one.

Run with the quant_ai conda env:
  ~/anaconda3/envs/quant_ai/bin/python3 scripts/build_eth_5m_v_rebound_multitrigger_feeder_optimized_features_tier0_20260901.py
"""
...
_spec = importlib.util.spec_from_file_location("tier0_features_20260901feeder", TIER0_SCRIPT)
_tier0 = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(_tier0)

LABEL_CSV = ROOT / "data/labels/eth_5m_v_rebound_multitrigger_feeder_optimized_20260901/eth_5m_v_rebound_multitrigger_feeder_optimized_labels.csv"
...
def main() -> int:
    ...
    print(f"라벨 {len(labels)}건 -> 피처merge후 {len(merged)}건 (드롭 {dropped}건, "
          f"그중 2024-01-01 이전 {n_before_2024}건)")
    ...
    out_path = OUT_DIR / "eth_5m_v_rebound_multitrigger_feeder_optimized_features_tier0.csv"
    ...
    report = {
        "rows": int(len(result)), "rows_dropped_at_merge": dropped,
