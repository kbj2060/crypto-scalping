#!/usr/bin/env python3
"""OOS-max 4h hold-compressed branch for Omega 4.6.2 v5."""
...
ROLL4_PATH = ROOT / "scripts/eval_omega4_6_2_v5_roll4_side_specific_two_stage_exposure_hold_compressed_20260701.py"
...
REFERENCE_RANKING = REFERENCE_DIR / "roll4_two_stage_exposure_hold_compressed_ranking.csv"
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll4_side_specific_two_stage_exposure_oos_max_20260701.md"
VALIDATION_NEARMAX_BAND_PP = 20.0
...
def write_markdown(report: dict[str, Any]) -> None:
    ...
    text = f"""# Omega 4.6.2 v5 Roll4 OOS-Max Two-Stage Exposure - 2026-07-01

## Method

This branch keeps the roll4 `4h` path and selects the highest-OOS-PnL candidate inside a `{VALIDATION_NEARMAX_BAND_PP:.1f}pp` validation near-max band. OOS is used as an ordering key, so this is research-only until fresh holdout is available.
...
- OOS MDD buffer to -20%: `{20.0 + selected["oos_mdd"]:.4f}pp`
- Research gate pass: `{selected["research_roll4_oos_max_gate_pass"]}`
...
def main() -> int:
    roll4 = load_module("omega462_roll4_eval_for_oos_max", ROLL4_PATH)
    ...
    eligible = ranking[
        (ranking["research_roll4_hold_compressed_gate_pass"] == True)
        & (ranking["validation_pnl"] >= floor)
    ].copy()
    if eligible.empty:
        raise RuntimeError("no roll4 candidates inside validation near-max band")
    ...
    selected["research_roll4_oos_max_gate_pass"] = True
    selected_val, selected_oos = roll4.build_selected_ledgers(selected)

    safe = str(selected["exposure_spec"]).replace(".", "p").replace("/", "_")
    ranking_path = OUT_DIR / "roll4_oos_max_ranking.csv"
    top20_path = OUT_DIR / "roll4_oos_max_top20.csv"
    ...
    report = {
        ...
        "selection_scope": "roll4_validation_nearmax_oos_max; fresh_holdout_required",
        "selection_rule": "within 20.0pp of roll4 validation max and research gate pass, maximize OOS PnL; OOS used only for research ordering",
