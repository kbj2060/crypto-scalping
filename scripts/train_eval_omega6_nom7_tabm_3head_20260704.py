#!/usr/bin/env python3
# Forked from train_eval_omega6_tabm_3head_20260703.py (the ORIGINAL zigzag-label frozen-winner
# trainer, NOT the failed barrier-matched fork). Only change: m7_* columns (44 of the 172 input
# features) are dropped from the feature set. Reason: the live SevenModelEnsemble stack that
# generates m7_* has since been restructured (gmm/hdb/iso/vae unsupervised heads removed) and the
# uncommitted intermediate state that built the ORIGINAL m7 features is unrecoverable (verified
# 2026-07-04: neither the current working tree nor the last commit reproduces the original
# candidates file's m7_* values -- see docs/model_contracts/omega6_synthesis_v1_20260703_contract.md
# "Data extension" section). Per user: m7 was itself later abandoned project-wide because it was
# fit on 2024-and-predicts-2025 (making 2024 unusable as direct training data), so dropping it
# here is consistent with the project's own direction, not a regression.
# All OTHER original 128 non-m7 features (including sig_whale/sig_oi_divergence/sig_ai_squeeze
# from strategies/elite_builder.py, which is unchanged and confirmed reproducible, and the NF
# forecaster ai_*/patchtst_*/tide_*/dlinear_* columns from ensemble/ensemble_router.py, also
# confirmed reproducible) are kept. train/val split, SPLIT_TS, architecture, and the zigzag_action
# label are all unchanged from the frozen winner's original trainer.
...
MODEL_ID = "omega6_nom7_3head_tabm_20260704"
...
def _prepare_frames(*, disable_tp_sl: bool) -> dict[str, Any]:
    ...
    dropped_m7 = [c for c in feature_cols if c.startswith("m7_")]
    # Also drift-confirmed unreproducible on the extended 2026-03..06 window (verified
    # 2026-07-04 via mismatch-rate scan against the original candidates file's Jan-Feb overlap,
    # tmp/causal_regen_20260516/extended_eval_frame_nom7_20260704/full_mismatch_scan.json):
    # conf_patchtst/pred_patchtst (PatchTST NF forecaster confidence formula changed, ~100%
    # mismatch), ou_halflife (99.8% mismatch), garch_vol_z (97.1% mismatch), kel (49% mismatch,
    # sign flipped), dual_momentum (11.9% mismatch). All other 113 compared columns matched
    # within <5% mismatch (mostly <3%, consistent with harmless rolling-window edge noise).
    DRIFTED_NON_M7 = {"conf_patchtst", "pred_patchtst", "ou_halflife", "garch_vol_z", "kel", "dual_momentum"}
    dropped_drifted = [c for c in feature_cols if c in DRIFTED_NON_M7]
    feature_cols = [c for c in feature_cols if not c.startswith("m7_") and c not in DRIFTED_NON_M7]
    if not dropped_m7:
        raise RuntimeError("expected to drop m7_* columns but found none -- feature source changed?")
    print(f"dropped {len(dropped_m7)} m7_* + {len(dropped_drifted)} drifted columns; {len(feature_cols)} features remain", flush=True)
...
def main() -> int:
    ...
    report = {
        "model_id": MODEL_ID,
        "lineage": "Forked from omega1_2_true_3head_tabm_20260603 for Omega6 synthesis L2 parent (docs/model_contracts/omega6_synthesis_design_20260703.md). Architecture/bundle contract unchanged.",
