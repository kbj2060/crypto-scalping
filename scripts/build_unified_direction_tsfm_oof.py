...
from ensemble.train_unified_direction_catboost_tsfm import FEATURE_COLS

DEFAULT_CSV = "/home/llewyn/crypto-scalping/data/rl_training_2025_unified.csv"
DEFAULT_OUT_CSV = "/home/llewyn/crypto-scalping/data/rl_training_2025_unified_dircat_tsfm_oof.csv"
DEFAULT_OUT_JSON = "/home/llewyn/crypto-scalping/data/ensemble/supervised/unified_direction_catboost_tsfm_oof_report.json"
...
def build_folds(n: int, train_end: int, min_train_frac: float, val_frac: float, step_frac: float) -> list[FoldSpec]:
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Build OOF TS-model-augmented direction probabilities with walk-forward folds")
    ...
    ap.add_argument("--od-wait", type=int, default=60)
    ...
    out = work.copy()
    out["ud_tsfm_short_prob"] = oof_probs[:, 0]
    out["ud_tsfm_flat_prob"] = oof_probs[:, 1]
    out["ud_tsfm_long_prob"] = oof_probs[:, 2]
    ...
    out["ud_tsfm_prob_max"] = prob_max
    out["ud_tsfm_pred_class"] = pred_class
    out["ud_tsfm_oof_fold"] = oof_fold
    out["ud_tsfm_is_holdout"] = (np.arange(len(out)) >= test_start).astype(np.int8)
    out["ud_tsfm_tb_label"] = y.astype(np.int64)
