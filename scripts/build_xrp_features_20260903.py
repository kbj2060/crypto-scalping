...
`build_btc_features_20260708.py`의 자산 상수만 바꾼 포팅. 교차자산 슬롯엔 BTC가 들어간다
(`build_xrp_raw_frame_20260903.py` 참조).
"""
...
RAW_PATH = ROOT / "data/splits/year_oos/xrp_raw_frame_2024_2026.csv"
OUT_PATH = ROOT / "data/splits/year_oos/xrp_features_2024_2026.csv"
...
def main() -> int:
    ...
    print(f"XRP raw frame: {len(raw)} rows {raw['timestamp'].iloc[0]}..{raw['timestamp'].iloc[-1]}", flush=True)
    ...
    print(f"\nXRP engineered features: {len(features)} rows, {len(features.columns)} columns", flush=True)
