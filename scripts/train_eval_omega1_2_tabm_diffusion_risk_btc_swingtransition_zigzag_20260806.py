#!/usr/bin/env python3
"""BTC copy of the shared `omega` utility module
(train_eval_omega1_2_tabm_diffusion_risk_20260603.py), imported by the BTC
parent trainer as `omega`.

Changes vs the ETH original:
- TRAIN_CSV/EVAL_CSV point at BTC's own FeatureEngineer output
  (data/splits/year_oos/btc_features_{2025,2026}.csv), not the ETH
  alpha6/alpha7 accreted trade-candidate lineage.
- REGIME3_CURRENT_2025/2026 point at BTC's own wide24 HMM overlay.
- The TABM_2025/2026 (frozen Omega1.2 regime3-routed expert direction/quality
  ...
  building BTC equivalents of those two extra regime3 sidecars would be
  wasted work. This keeps the BTC build faithful to the actual production
...
MODEL_ID = "btc_omega1_2_softfloor00_tabm_diffusion_risk_20260708"
OUT_DIR = ROOT / "tmp/causal_regen_20260516" / MODEL_ID

TRAIN_CSV = ROOT / "data/splits/year_oos/btc_features_2025_swingtransition_zigzag.csv"
EVAL_CSV = ROOT / "data/splits/year_oos/btc_features_2026_swingtransition_zigzag.csv"
REGIME3_CURRENT_2025 = ROOT / "data/ensemble/supervised/btc_regime3_current_hmm_sensitive_wide24_20260708/btc_features_2025_regime3_current_sensitive_hmm_wide24.csv"
REGIME3_CURRENT_2026 = ROOT / "data/ensemble/supervised/btc_regime3_current_hmm_sensitive_wide24_20260708/btc_features_2026_regime3_current_sensitive_hmm_wide24.csv"
...
def main() -> int:  # pragma: no cover - unused by the BTC parent trainer; kept for import parity only
    raise SystemExit("this BTC omega module is a library only; the diffusion-policy standalone experiment was not ported")
