...
def _validate_m7_training_columns(df: pd.DataFrame, tag: str = "DSAC") -> None:
    ...
    if missing:
        raise ValueError(f"[{tag}] missing required M7 columns: {', '.join(sorted(missing))}")
...
def _load_rl_frame(csv_path: str, start: str | None = None, end: str | None = None) -> pd.DataFrame:
    ...
    df = prune_to_feature_keep(df, include_entry_price=False, extra_keep=["timestamp", "ai_ready"])
...
def _load_training_hmm(train_cfg: dict[str, Any]) -> OnlineHMMDetector:
    csv_path = str(train_cfg.get("csv_path", "data/splits/year_oos/rl_training_2025_m7.csv"))
    train_ratio = float(train_cfg.get("train_ratio", 0.8))
    hmm_cache_path = str(train_cfg.get("hmm_cache_path", "data/ensemble/ckpt/hmm_init_cache_dsac.npz"))
    hmm_fit_iter = 30
    ...
    if hmm_detector is not None:
        ...
        return hmm_detector
    print("[HMM] cache miss; refitting from training CSV...")
...
def _apply_normal_soft_gate_exact(action: float, state_vec: np.ndarray, regime: str, gate_scale: float) -> float:
    a = float(action)
    ...
    def _mix(mult: float) -> float:
        return float(a * (1.0 - gs * (1.0 - float(mult))))
    ...
    try:
        ...
        return a
    if regime == "whipsaw":
        return _mix(0.80) if trend_entropy > 0.24 else a
    if regime == "chop":
...
def _build_agent_from_ckpt(ckpt_path: str, train_cfg: dict[str, Any], device: str) -> tuple[DSACAgent, dict[str, Any]]:
    ckpt = torch.load(ckpt_path, map_location=device, weights_only=False)
    if int(ckpt.get("state_dim", -1)) != int(DSAC_STATE_DIM):
        raise ValueError(f"checkpoint state_dim={ckpt.get('state_dim')} != current state_dim={DSAC_STATE_DIM}")
    agent = DSACAgent(
        DSAC_STATE_DIM,
        hidden_dim=256,
...
def _eval_policy_exact(eval_df: pd.DataFrame, agent: DSACAgent, hmm_detector: OnlineHMMDetector, train_cfg: dict[str, Any], ckpt_epoch: int) -> dict[str, float]:
    if len(eval_df) < 32:
        return {"pnl": 0.0, "wr": 0.0, "mdd": 0.0, "tr": 0, "long_entries": 0, "short_entries": 0, "fcl": 0, "fcs": 0, "avg_hold_long": 0.0, "avg_hold_short": 0.0, "side_balance": 0.0, "side_pen": 0.0, "score": -5.0}
    ...
    while not done:
        ...
        a = _apply_normal_soft_gate_exact(a, st, env.regime_bucket(), gate_scale=gate_scale)
        st, _, done, info = env.step(a)
    ...
    wr = env.win_rate
    trade_score = -5.0 if env.total_trades == 0 else (min(env.total_trades / 30.0, 1.0) * 5.0 if pnl > 0 else -min(env.total_trades / 30.0, 1.0) * 10.0)
    ...
    side_pen = float(train_cfg.get("val_side_bias_penalty", 80.0)) * side_imbalance
    score = pnl * 3.0 + wr * 60.0 + trade_score + mdd_pct * 2.0 - side_pen
    return {"pnl": float(pnl), "wr": float(wr), "mdd": float(mdd_pct), "tr": int(env.total_trades), "long_entries": int(le), "short_entries": int(se), "fcl": int(fcl), "fcs": int(fcs), "avg_hold_long": float(hs_l / max(hn_l, 1)) if hn_l > 0 else 0.0, "avg_hold_short": float(hs_s / max(hn_s, 1)) if hn_s > 0 else 0.0, "side_balance": float(side_balance), "side_pen": float(side_pen), "score": float(score)}


def _regime_breakdown(eval_df: pd.DataFrame, agent: DSACAgent, hmm_detector: OnlineHMMDetector, train_cfg: dict[str, Any], ckpt_epoch: int):
    ...
    for i, rc in enumerate(reg_cols):
        ...
        w_score += w * float(rs["score"]); w_sum += w
        reg_logs.append((rname, len(sub), float(rs["score"]), float(rs["pnl"]), int(rs["tr"])))
    return reg_logs, float(w_score / w_sum) if w_sum > 0 else 0.0


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument("--csv-path", default="data/rl_training_2026_unified.csv")
    ap.add_argument("--ckpt-path", default="data/ensemble/ckpt/best_dsac_agents.pth")
    ap.add_argument("--config-path", default="data/ensemble/ckpt/dsac_train_config_latest.json")
    ...
    payload = {
        ...
        "overall": overall,
        "regime_logs": [{"regime": rn, "rows": n, "score": sc, "pnl": pnl, "trades": tr} for rn, n, sc, pnl, tr in reg_logs],
        "val_score": float(val_score),
    }
    out_json = args.out_json or os.path.join(_ROOT_DIR, "data/ensemble/reports", f"eval_best_dsac_agent_2026_exact_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
    os.makedirs(os.path.dirname(out_json), exist_ok=True)
    with open(out_json, "w", encoding="utf-8") as f:
        json.dump(payload, f, indent=2, ensure_ascii=False)
    print("[VAL] PnL:{pnl:.2f}% | Tr:{tr} | WR:{wr:.0f}% | MDD:{mdd:.2f}% | L:{le} S:{se} | SideBal:{sb:.3f} | SidePen:{sp:.2f} | FCL:{fcl} FCS:{fcs} | AvgHoldL:{ahl:.1f} AvgHoldS:{ahs:.1f} | Score:{score:.2f}".format(
        pnl=float(overall["pnl"]), tr=int(overall["tr"]), wr=float(overall["wr"])*100.0, mdd=float(overall["mdd"]), le=int(overall["long_entries"]), se=int(overall["short_entries"]), sb=float(overall["side_balance"]), sp=float(overall["side_pen"]), fcl=int(overall["fcl"]), fcs=int(overall["fcs"]), ahl=float(overall["avg_hold_long"]), ahs=float(overall["avg_hold_short"]), score=float(val_score)))
    if reg_logs:
        print("[VAL REGIME] " + " | ".join([f"{rn}:n={n} score={sc:.1f} pnl={pnl:.1f}% tr={tr}" for rn, n, sc, pnl, tr in reg_logs]))
    print(f"\\nSaved: {out_json}")
