...
def _build_exit_dataset_entry_label_path_optimal(
    ...
    *,
    ...
    exit_head_liveatr_barrier_and_label_reaudit_20260818.md. The per-segment `row_notional` also
    feeds `_exit_fill_net`'s cash simulation that derives exit_net/edge/label, unchanged in scope --
    that's a legitimate backtest-sizing assumption, not the train/inference feature-parity bug this
    fix targets."""
...
def _build_exit_dataset_entry_label_terminal_giveback(
    ...
    *,
    ...
    training, no sidecar for this bundle can exist yet). None falls back to the fixed BASE_TEMPLATE
    constant, recorded as risk_sizing_source. 2026-08-18 fix, see docs/experiments/eth_odyssey4_
    exit_head_liveatr_barrier_and_label_reaudit_20260818.md."""
    required = {"timestamp", "zigzag_action", "open", "high", "low", "close"}
    missing = sorted(required - set(frame.columns))
    if missing:
