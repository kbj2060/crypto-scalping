#!/usr/bin/env python3
"""ZDC(완전wick) 저장된 라벨 파일 기반 육안검증 -- HIT 10건 + MISS 10건, 한 이미지.

wick-앵커판 렌더 스크립트(render_eth_5m_v_rebound_multitrigger_zigzag_direction_wick_anchor_
20examples_20260901.py)와 동일한 레이아웃/색상 관례를 재사용, 라벨 CSV만 완전wick판으로 교체.
"""
...
OUT_PATH = ROOT / "data/labels/eth_5m_v_rebound_multitrigger_zigzag_direction_full_wick_20260901/full_wick_hit_miss_20examples.png"
LOOKBACK_BARS = 12
...
def main() -> int:
    ...
    fig.suptitle(
        "ZDC 완전wick 라벨 육안검증(저장된 라벨 파일 기준, TabPFN 학습에 실제로 쓰일 그 파일) -- "
        "HIT 10건(위 2줄)/MISS 10건(아래 2줄), 무작위 샘플(seed=20260901)\n"
        "회색 점선=트리거 idx, 회색 가로선=wick 앵커(bottom→low[idx], top→high[idx]). subsequent 봉도 "
        "고가/저가로 추적(종가 아님, wick-앵커판과의 유일한 차이). 굵은 주황선=실제 확정봉. "
        "가는 보라 점선=피벗 기록봉(idx와 다를 때만 표시).",
        fontsize=14, y=0.999,
    )
