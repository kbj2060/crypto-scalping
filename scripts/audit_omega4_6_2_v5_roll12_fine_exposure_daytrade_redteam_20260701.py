#!/usr/bin/env python3
"""Red-team audit for Omega 4.6.2 v5 roll12 fine exposure branch."""
...
RANKING = OUT_DIR / "roll12_fine_exposure_daytrade_ranking.csv"
EVAL_SCRIPT = ROOT / "scripts/eval_omega4_6_2_v5_roll12_fine_exposure_daytrade_20260701.py"
PRIOR_RUNTIME_AUDIT = (
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll12_fine_exposure_daytrade_redteam_20260701.md"
TOL = 1.0e-8
...
def write_markdown(payload: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- TP/SL: `{selected['roll12_tp_move']}` / `{selected['roll12_sl_move']}`",
        "",
...
def main() -> None:
    audit = load_module("omega462_loss_cluster_audit_for_roll12_fine", AUDIT_V1_PATH)
    segment_audit = load_module("omega462_segment_audit_for_roll12_fine", AUDIT_SEGMENT_PATH)
    ...
    audit.add_check(checks, "selected_daytrade_gate_pass", selected.get("research_daytrade_gate_pass") is True, "research_blocker", {"observed": selected.get("research_daytrade_gate_pass")})
    ...
    audit.add_check(
        checks,
        "improves_roll12_reference_pnl_without_longer_hold",
        ...
        and validation["avg_hold_hours"] <= reference["validation_avg_hold_hours"] + 1.0e-12
        and oos["avg_hold_hours"] <= reference["oos_avg_hold_hours"] + 1.0e-12,
        "research_blocker",
    ...
    audit.add_check(
        ...
        and float(selected["roll12_tp_move"]) > 0.0
        and float(selected["roll12_sl_move"]) > 0.0
        and {"roll12_bracket_tp", "roll12_bracket_sl"}.issubset(set(validation_df["reason"].astype(str))),
        "research_blocker",
        {"tp": selected.get("roll12_tp_move"), "sl": selected.get("roll12_sl_move")},
    )
    ...
    verdict = (
        "FULL_LIVE_PASS"
        if full_live_pass
        else "RESEARCH_ROLL12_FINE_EXPOSURE_PASS_FULL_LIVE_BLOCKED"
        if research_pass
        else "REDTEAM_FAIL"
    )
    payload = {
        "audit_id": "omega4_6_2_v5_roll12_fine_exposure_daytrade_redteam_20260701",
