...
XRP 주체 + **BTC를 교차자산**으로 하는 raw 프레임. XRP 레짐 Phase 2가 이미 BTC를
파트너로 썼으므로 학습/추론 파리티를 위해 여기서도 BTC를 넣는다.

⭐**교차자산 컬럼 명명 함정**: `FeatureEngineer`는 교차자산 컬럼을 `close_btc`/`volume_btc`/
`quote_volume_btc`로 **하드코딩**한다. 어떤 자산을 넣든 이름은 `_btc`다. 여기서는 그 슬롯에
BTC가 들어가므로 이름과 내용이 우연히 일치하지만, **의미는 "교차자산 슬롯"이지 "BTC 전용"이
아니다**. XRP 라이브 스코어러도 같은 슬롯에 BTC를 넣어야 파리티가 맞는다.
(BTC 캐노니컬은 ETH가 그 슬롯에 들어가 있다 -- `live_regime_btc_signal_20260902.py` docstring 참조.)
"""
...
OUT_PATH = ROOT / "data/splits/year_oos/xrp_raw_frame_2024_2026.csv"
