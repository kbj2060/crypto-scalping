...
MODEL_ID = "omega1_2_8m_full_retrain_numeric_cash_sleeve_primary_half_price_width_20260617"
...
def _primary_half_row(row: pd.Series) -> pd.Series:
    out = row.copy()
    out["take_profit"] = float(row.get("take_profit", 0.0) or 0.0) * PRIMARY_TP_SL_MULT
    out["stop_loss"] = abs(float(row.get("stop_loss", 0.0) or 0.0)) * PRIMARY_TP_SL_MULT
    return out


def _metrics_with_primary_half_and_fallback(
    frame: pd.DataFrame,
    dec: pd.DataFrame,
    ...
    *,
    ...
    for i in range(0, len(frame) - 2):
        ...
        if bool(active[i]):
            ...
            if side != 0:
                half_row = _primary_half_row(row)
                cash, pos, entered = sleeve._open_position(cash, arrays, i, side, "primary", None, half_row, fee_eff, slip_eff)
                if entered:
                    primary_entries += 1
...
def main() -> int:
    ...
    val_cash_action = np.zeros(len(val_payload["frame"]), dtype=np.int64)
    val_cash_conf = np.zeros(len(val_payload["frame"]), dtype=np.float64)
    oos_cash_action = np.zeros(len(oos_payload["frame"]), dtype=np.int64)
    oos_cash_conf = np.zeros(len(oos_payload["frame"]), dtype=np.float64)
    original_base_val = omega._metrics(val_payload["frame"], val_payload["dec"], fee=fee, slip=slip, cost_mult=3.0)
    original_base_oos = omega._metrics(oos_payload["frame"], oos_payload["dec"], fee=fee, slip=slip, cost_mult=3.0)
    base_val_sleeve = _metrics_with_primary_half_and_fallback(val_payload["frame"], val_payload["dec"], RISK, val_cash_action, val_cash_conf, 1.0, fee=fee, slip=slip, cost_mult=3.0)
    base_oos_sleeve = _metrics_with_primary_half_and_fallback(oos_payload["frame"], oos_payload["dec"], RISK, oos_cash_action, oos_cash_conf, 1.0, fee=fee, slip=slip, cost_mult=3.0)
    ...
    diagnostics: dict[str, Any] = {
        "mode": "full_retrain_parent_numeric_cash_sleeve_primary_half_price_width",
        "baseline_model_id": BASELINE_ID,
        "parent_artifact": meta["parent_dir"],
        "risk": asdict(RISK),
        "risk_semantics": "fallback TP/SL remains omega1_2_8b account-PnL thresholds; primary TP/SL thresholds are multiplied by 0.5, so primary price distance is halved at unchanged notional_exposure",
        "primary_tp_sl_multiplier": PRIMARY_TP_SL_MULT,
        "feature_count": int(x_val.shape[1]),
        "features": list(x_val.columns),
        "baseline": {"validation": base_val_sleeve, "oos": base_oos_sleeve},
        "original_parent_baseline": {"validation": original_base_val, "oos": original_base_oos},
    ...
    for cal_q in (0.50, 0.65, 0.80):
        ...
        for ev_min in (0.001, 0.002, 0.003, 0.004):
            ...
            val_m = _metrics_with_primary_half_and_fallback(val_payload["frame"], val_payload["dec"], RISK, val_ev_a, val_ev_c, 0.0, fee=fee, slip=slip, cost_mult=3.0)
            oos_m = _metrics_with_primary_half_and_fallback(oos_payload["frame"], oos_payload["dec"], RISK, oos_ev_a, oos_ev_c, 0.0, fee=fee, slip=slip, cost_mult=3.0)
            ...
            for cfg_id, (uvl, uvs, uol, uos) in utility_preds.items():
                for utility_min in (-0.001, 0.0, 0.001, 0.002):
                    for margin_min in (0.0, 0.001, 0.002):
                        ...
                        val_m = _metrics_with_primary_half_and_fallback(val_payload["frame"], val_payload["dec"], RISK, val_a, val_c, 0.0, fee=fee, slip=slip, cost_mult=3.0)
                        oos_m = _metrics_with_primary_half_and_fallback(oos_payload["frame"], oos_payload["dec"], RISK, oos_a, oos_c, 0.0, fee=fee, slip=slip, cost_mult=3.0)
    ...
    report = {
        ...
        "method": "Full-retrained 3-head parent artifact is used as parent. Primary parent positions use half-width account-PnL TP/SL thresholds during metrics evaluation. Cash sleeve EV lower-bound and numeric utility agreement/veto regressors are newly fit on validation full-retrain features, while fallback risk remains the omega1_2_8b baseline contract.",
