#!/usr/bin/env python3
"""Trailing-stop cost-gate grid search for orthogonal_combo (final label config H=24/GAP=12,
v2 exclude-middle for the CLASSIFIER only), using this repo's STANDARD backtest engine
(core.causal_futures_backtest.simulate_single_position, ATR-trailing-stop exit mode) -- the same
engine/convention taker_delta_z_climax / short_term_return_z / liquidity_sweep's own trailing-stop
cost-gates used (backtest_taker_and_str_z_trailing_stop_standard_engine_20260830.py,
backtest_eth_liquidity_sweep_topdown_trailing_gridsearch_20260830.py), NOT a scratchpad simulation.

Per this project's default policy (feedback_trailing_stop_default_costgate_logic_20260830):
trailing stop is the FIRST exit structure tried, not fixed TP/SL. Tests ALL raw H=24/GAP=12 fires
(bottom=long, top=short) -- build_eth_orthogonal_combo_raw_fires_H24_GAP12_20260831.py's output,
NOT the classifier training CSV (which already dropped the exclude-middle zone) -- unconditional on
the trained model's own probability AND unconditional on the exclude-middle label decision (that
filter is a classifier-training device, not a property of what the live dashboard actually fires
on). Matches liquidity_sweep's own precedent exactly.

Same SL/ARM/Trail grid as liquidity_sweep's own grid search (no signal-specific reason yet to narrow
it). HOLDOUT (2026-04-01+) entirely excluded -- single-touch policy, not yet earned.
"""
...
MARGIN_FRACTION = 0.30  # CLAUDE.md Futures Risk Sizing Contract, matches every prior Homer cost-gate
LEVERAGE = 3.0
ROUNDTRIP_COST_RATE = 0.001  # 10bp standard cost, no fee-discount assumptions
HORIZON_BARS = 24
...
def log(msg: str) -> None:
    print(f"[orthogonal_combo_trailing_gridsearch] {msg}", flush=True)
...
def main() -> int:
    ...
    tp_moves_placeholder = np.full(len(fires), 999.0)  # unused in trailing mode, must be finite
