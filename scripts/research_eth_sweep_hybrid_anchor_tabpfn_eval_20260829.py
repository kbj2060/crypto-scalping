#!/usr/bin/env python3
"""Evaluates the hybrid-anchor V_REBOUND label (build_eth_5m_liquidity_sweep_hybrid_anchor_
labels_20260829.py: sweep level = whichever is more recent of the 48-bar rolling extreme vs a
confirmed causal zigzag pivot) under the exact same TabPFN + Tier0+rsi methodology as the
original (48-bar-only) label, for a fair, apples-to-apples comparison.

Same TRAIN(<2025-09-01)/VAL(2025-09-01..12-31)/OOS(2026-01-01..03-31) split, same 4-seed panel,
same embargo. VAL+OOS only -- reserved holdout stays untouched (same discipline as every other
test in this lineage).
"""
...
TIER0_CSV = ROOT / "data/labels/eth_5m_sweep_hybrid_anchor_v_rebound_20260829/eth_5m_sweep_hybrid_anchor_features_tier0.csv"
...
def main() -> int:
    ...
    print(f"\n=== FOR COMPARISON: original (48-bar-only) V_REBOUND label, Tier0+rsi ===")
