#!/usr/bin/env python3
"""9h side-specific fine bracket compression branch for Omega 4.6.2 v5."""
...
MODEL_ID = "omega4_6_2_v5_roll9_side_specific_fine_valmax_20260701"
REFERENCE_MODEL_ID = "omega4_6_2_v5_roll10_side_specific_fine_valmax_20260701"
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll9_side_specific_fine_valmax_20260701.md"
MAX_ROLL_HOURS = 9.0
REFERENCE_CAP = 4.20
EPS = 1.0e-12
...
def normalize_roll9(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    out = out.rename(columns={col: col.replace("roll16_", "roll9_") for col in out.columns if col.startswith("roll16_")})
    out["reason"] = out["reason"].astype(str).str.replace("roll16_", "roll9_", regex=False)
    return out
...
def write_markdown(report: dict[str, Any]) -> None:
    ...
    text = f"""# Omega 4.6.2 v5 Roll9 Side-Specific Fine Valmax - 2026-07-01
...
- Long TP/SL: `{selected["roll9_long_tp_move"]:.4f}` / `{selected["roll9_long_sl_move"]:.4f}`
- Short TP/SL: `{selected["roll9_short_tp_move"]:.4f}` / `{selected["roll9_short_sl_move"]:.4f}`
- Research gate pass: `{selected["research_roll9_gate_pass"]}`
...
def main() -> int:
    source = load_module("omega462_roll16_source_for_roll9_side", SOURCE_MODULE_PATH)
    v1 = source.load_module("omega462_loss_cluster_v1_for_roll9_side", source.V1_MODULE_PATH)
    segment_mod = source.load_module("omega462_segment_for_roll9_side", source.SEGMENT_MODULE_PATH)
    stop_mod = v1.load_module("omega462_stop_mod_for_roll9_side", v1.STOP_MODULE_PATH)
    ...
    bracket_specs = [
        ("fine9_valmax", 0.0200, 0.0450, 0.0250, 0.0400),
        ("fine9_fast", 0.0200, 0.0300, 0.0250, 0.0400),
        ("fine9_balanced", 0.0225, 0.0400, 0.0300, 0.0400),
        ("fine9_short_room", 0.0200, 0.0400, 0.0400, 0.0400),
        ("fine9_oos", 0.0250, 0.0350, 0.0400, 0.0400),
    ]
    ...
    exposure_specs = [
        ...
        for lf in [0.60, 0.65, 0.70, 0.75, 0.85, 0.90]
        for sf in [0.80, 0.85, 0.90, 0.95, 1.00, 1.02]
        for cap in [3.60, 3.80, 4.00, 4.20]
    ]
    governors = [
        segment_mod.SegmentGovernorSpec("none", 1.00, 1.00, 0.0),
        segment_mod.SegmentGovernorSpec("loss1_90_win9", 0.90, 1.00, 9.0),
        segment_mod.SegmentGovernorSpec("streak85_60_win9", 0.85, 0.60, 9.0),
    ]
        ...
        for exposure in exposure_specs:
            for governor in governors:
                ...
                row = {
                    ...
                    "roll9_max_hours": MAX_ROLL_HOURS,
                    "roll9_long_tp_move": long_tp,
                    "roll9_long_sl_move": long_sl,
                    "roll9_short_tp_move": short_tp,
                    "roll9_short_sl_move": short_sl,
                ...
                row["validation_roll9_gate_pass"] = validation_gate
                row["oos_safety_gate_pass"] = oos_gate
                row["research_roll9_gate_pass"] = validation_gate and oos_gate
                row["cap_reference_distance_abs"] = abs(float(row["exposure_cap_notional"]) - REFERENCE_CAP)
                rows.append(row)

    ranking = pd.DataFrame(rows).sort_values(
        [
            "research_roll9_gate_pass",
            "validation_pnl",
    ...
    status = (
        "RESEARCH_ROLL9_SIDE_SPECIFIC_FINE_PASS"
        if bool(selected["research_roll9_gate_pass"])
        else "NO_ROLL9_SIDE_SPECIFIC_FINE_PASSING_CANDIDATE"
    )
    safe = (
        f"{selected['side_bracket_spec']}__{selected['exposure_spec']}__"
        f"{selected['segment_governor_spec']}"
    ).replace(".", "p").replace("/", "_")
    ranking_path = OUT_DIR / "roll9_side_specific_fine_valmax_ranking.csv"
    top20_path = OUT_DIR / "roll9_side_specific_fine_valmax_top20.csv"
    ...
    report = {
        ...
        "selection_scope": "validation_primary_roll9_hold_compression_with_oos_safety_gate; fresh_holdout_required",
        "selection_rule": "among research-gated roll9 variants, sort by validation_pnl, validation_avg_hold, cap distance; OOS is not an ordering key",
        ...
        json.dumps(
            {
                ...
                "selected_validation": {
                    ...
                    "gate": bool(selected["validation_roll9_gate_pass"]),
                },
                "selected_oos": {
