#!/usr/bin/env python3
"""git HEAD 스냅샷(2026-08-19 시점)의
scripts/train_eval_omega4_3head_parent72_loose_entry_quality_sol_20260707.py 본문을 그대로 복사한
파일 -- 작업트리의 그 파일은 2026-08-18 exit_head pos_tp/pos_sl/pos_notional/risk_margin/
risk_leverage 버그수정이 미커밋 상태로 적용돼 있어(`git diff HEAD`로 확인), import하면 수정본이
로드된다. 이 스냅샷은 실제 라이브 SOL zig075 v2(adaptive_squeeze) 번들
(FINAL_GOVERNOR_OMEGA4_6_1_SOL_BUNDLE_PATH, `tmp/causal_regen_20260516/
sol_omega4_3head_parent72_loose_entry_quality_20260707_adaptive_squeeze_20260720/
true_3head_tabm_bundle.pt`, 2026-07-20 학습)을 만들어낸 정확한 코드 상태를 보존해, "라이브로
승격된 그 모델 자체가 시드에 강건한가"를 재현학습으로 검증하기 위한 용도.

ETH 쪽 동일 기법(scripts/eth_live_promotion_seed_robustness_prefix_snapshot_20260819.py,
docs/experiments/eth_live_promotion_seed_robustness_3seed_20260819.md)을 SOL에 그대로 적용한
것 -- 워킹트리의 dirty 파일은 절대 건드리지 않고, git HEAD 시점 코드를 별도 파일로 추출한다.
scripts/sol_live_promotion_seed_robustness_zig075_seed_variant_20260819.py가 이 모듈을 import해서
--seed만 바꿔 재학습한다.

원본 파일 자신의 HEAD 시점 모듈 docstring(SOL 사본이 ETH 원본과 구조적으로 다른 지점을 설명 --
_prepare_frames()에서 omega.TABM_2025 정렬을 안 쓰고 train_df=train_raw로 단순화, --exit-label-mode
선택지가 entry_label_terminal_giveback 하나뿐이라는 내용)은 이 헤더로 교체됐다 -- 원문 대조는
`git show HEAD:scripts/train_eval_omega4_3head_parent72_loose_entry_quality_sol_20260707.py`로
직접 가능하다. 아래 코드 본문(shebang 이후)은 그 커밋의 바이트 그대로다.

⚠️ ETH와 다른 점 (SOL 고유): 이 HEAD 스냅샷 자체에 이미 `--base-feature-contract-bundle` CLI
옵션이 내장돼 있다(main()에서 지정된 번들의 base_cols로 자동유도 feature_cols를 pin, 없으면
RuntimeError) -- ETH는 이 메커니즘이 없어 별도 monkey-patch(`omega._numeric_feature_cols` 교체)가
필요했지만 SOL은 CLI 플래그 하나로 해결된다. seed_variant 스크립트가 이 플래그에 라이브 번들
경로를 넘긴다."""
