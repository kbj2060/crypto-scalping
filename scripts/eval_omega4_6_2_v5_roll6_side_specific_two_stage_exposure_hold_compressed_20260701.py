#!/usr/bin/env python3
"""6h hold-compressed two-stage exposure branch for Omega 4.6.2 v5."""
...
MODEL_ID = "omega4_6_2_v5_roll6_side_specific_two_stage_exposure_hold_compressed_20260701"
REFERENCE_MODEL_ID = "omega4_6_2_v5_roll7_side_specific_two_stage_exposure_hold_compressed_20260701"
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll6_side_specific_two_stage_exposure_hold_compressed_20260701.md"
MAX_ROLL_HOURS = 6.0
...
def build_selected_ledgers(selected: dict[str, Any]) -> tuple[pd.DataFrame, pd.DataFrame]:
    roll7 = load_module("omega462_roll7_for_roll6_build", ROLL7_PATH)
    roll7.MAX_ROLL_HOURS = MAX_ROLL_HOURS
    return roll7.build_selected_ledgers(selected)


def write_markdown(report: dict[str, Any]) -> None:
    selected = report["selected_variant"]
    reference = report["reference_variant"]
    text = f"""# Omega 4.6.2 v5 Roll6 Side-Specific Two-Stage Exposure Hold Compressed - 2026-07-01
...
- Max roll hold: `{selected["roll6_max_hours"]}h`
- Research gate pass: `{selected["research_roll6_hold_compressed_gate_pass"]}`
...
def main() -> int:
    roll7 = load_module("omega462_roll7_for_roll6_eval", ROLL7_PATH)
    roll7.MAX_ROLL_HOURS = MAX_ROLL_HOURS
    mods = roll7.modules()
    helper = mods["helper"]
    ...
    for long_factor in [0.40, 0.50, 0.60, 0.70, 0.75, 0.80, 0.85, 0.90]:
        for short_factor in [0.80, 0.90, 1.00, 1.05, 1.10, 1.15, 1.20]:
            for cap_notional in [3.60, 4.00, 4.40, 4.80, 5.00]:
                ...
                row = {
                    ...
                    "roll6_max_hours": MAX_ROLL_HOURS,
                    "roll6_long_tp_move": 0.0200,
                    "roll6_long_sl_move": 0.0300,
                    "roll6_short_tp_move": 0.0250,
                    "roll6_short_sl_move": 0.0385,
                ...
                row["validation_roll6_hold_compressed_gate_pass"] = validation_gate
                row["oos_safety_gate_pass"] = oos_gate
                row["research_roll6_hold_compressed_gate_pass"] = validation_gate and oos_gate
                rows.append(row)

    ranking = pd.DataFrame(rows).sort_values(
        [
            "research_roll6_hold_compressed_gate_pass",
            "validation_pnl",
            "oos_pnl",
    ...
    status = (
        "RESEARCH_ROLL6_TWO_STAGE_EXPOSURE_HOLD_COMPRESSED_PASS"
        if bool(selected["research_roll6_hold_compressed_gate_pass"])
        else "NO_ROLL6_TWO_STAGE_EXPOSURE_HOLD_COMPRESSED_PASSING_CANDIDATE"
    )
    safe = str(selected["exposure_spec"]).replace(".", "p").replace("/", "_")
    ranking_path = OUT_DIR / "roll6_two_stage_exposure_hold_compressed_ranking.csv"
    top20_path = OUT_DIR / "roll6_two_stage_exposure_hold_compressed_top20.csv"
    ...
    report = {
        ...
        "selection_scope": "validation_primary_roll6_two_stage_veto_exposure_overlay_with_oos_safety_gate; fresh_holdout_required",
        "selection_rule": "regenerate 6h roll path with current two-stage veto rules; among research-gated exposure overlays, sort by validation_pnl, oos_pnl, validation_avg_hold",
        ...
        json.dumps(
            {
                ...
                "selected_validation": {
                    ...
                    "gate": bool(selected["validation_roll6_hold_compressed_gate_pass"]),
                },
                "selected_oos": {
