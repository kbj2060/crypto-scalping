...
MODEL_ID = "alpha7_regime3_current_moe_active_mix_per_conf_20260601"
ROUTER_NAME = "regime3_current_context"
OUT_DIR = ROOT / "tmp/causal_regen_20260516/alpha7_regime3_current_moe_active_mix_per_conf_20260601"
...
def main() -> int:
    ...
    # Fail fast if the current-regime router contract changes; this script only
    # uses the risk-aligned frames to match the active expert-source mix.
    router_train, router_eval, router_overlay = _load_router_frames(ROUTER_NAME)
    if len(train_all) != len(router_train) - 6 or len(eval_df) != len(router_eval) - 6:
        raise RuntimeError("unexpected risk/router frame alignment; expected six edge rows removed by risk sidecar")
    ...
    val_expert_dec = {
        expert: _side_constrained(_predict_combo(models[expert]["primary"], models[expert]["fallback"], val_df), expert=expert)
        for expert in SOURCE_BY_EXPERT
    }
    oos_expert_dec = {
        expert: _side_constrained(_predict_combo(models[expert]["primary"], models[expert]["fallback"], eval_df), expert=expert)
        for expert in SOURCE_BY_EXPERT
    }
    ...
    for bull_thr in [0.75, 0.80, 0.85]:
        for bear_thr in [0.70, 0.75, 0.80, 0.85]:
    ...
    report = {
        "model_id": MODEL_ID,
        "design": "Per-expert confidence threshold grid on the active expert-source mix: bull practical, bear current+risk, chop practical. Current-Regime3 MoE frame is unchanged.",
        "overlay": {"risk": risk_overlay, "router": router_overlay},
        "selected": {
