#!/usr/bin/env python3
"""Visual validation for one candidate 3-class threshold pair (T_sustain=0.15, T_fail=0.70) from
research_eth_sweep_v_rebound_3class_design_20260830.py's grid -- 4 random examples per class
(지속/횡보/실패), with 실패 sampled from BOTH its sub-types (never reached the fast-window
target, and reached it but gave back >70%) since those are mechanically different but should
both look like genuine failures by eye. Diagnostic only.
"""
...
EVENTS_CSV = ROOT / "tmp/eth_sweep_v_rebound_3class_design_20260830/events_with_3class_inputs.csv"
OUT_PATH = ROOT / "tmp/eth_sweep_v_rebound_3class_design_20260830/3class_candidate_examples.png"
WINDOW_BARS = 6
SEED = 20260830
...
def load_impl():
    spec = importlib.util.spec_from_file_location("sweep_impl_3class_render_20260830", IMPL_SCRIPT)
...
def main() -> int:
    ...
    groups = [
        ("지속 (giveback<=0.15)", sustained.sample(n=4, random_state=SEED), "#eef8f0"),
        ("횡보/지지 (0.15<giveback<=0.70)", plateau.sample(n=4, random_state=SEED), "#fff6e0"),
        ("실패-미도달 (fast target never hit)", failed_never.sample(n=2, random_state=SEED), "#fdf1ef"),
        ("실패-반납 (giveback>0.70)", failed_gaveback.sample(n=2, random_state=SEED), "#fdf1ef"),
    ]

    plt.rcParams.update({"font.size": 11})
    fig, axes = plt.subplots(4, 4, figsize=(28, 22), dpi=145)
    fig.suptitle(
        f"3-class candidate (T_sustain={T_SUSTAIN}, T_fail={T_FAIL}): row1=지속, row2=횡보/지지, "
        "row3-4=실패 (2 never reached target + 2 gave back too much)",
        fontsize=20, y=0.995,
    )
    ...
    for row, (title, sample, tint) in enumerate(groups):
        for col, (_, event) in enumerate(sample.iterrows()):
            ...
            ax.set_title(f"{title}\n{event['side']} | giveback={gb_str}", fontsize=11)
            ax.grid(alpha=0.25)
