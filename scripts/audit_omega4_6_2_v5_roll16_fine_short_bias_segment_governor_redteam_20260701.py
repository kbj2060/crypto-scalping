#!/usr/bin/env python3
"""Red-team audit for Omega 4.6.2 v5 roll16 fine short-bias branch."""
...
RANKING = OUT_DIR / "roll16_fine_short_bias_segment_governor_ranking.csv"
EVAL_SCRIPT = ROOT / "scripts/eval_omega4_6_2_v5_roll16_fine_short_bias_segment_governor_20260701.py"
PRIOR_RUNTIME_AUDIT = (
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll16_fine_short_bias_segment_governor_redteam_20260701.md"
TOL = 1.0e-8
...
def write_markdown(payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Omega 4.6.2 v5 Roll16 Fine Short-Bias Red-Team Audit - 2026-07-01",
        "",
        ...
        "",
        "| Split | Fine Best PnL | Short-Bias PnL | Fine Best MDD | Short-Bias MDD |",
        "| --- | ---: | ---: | ---: | ---: |",
    ]
...
def main() -> None:
    audit = load_module("omega462_loss_cluster_audit_for_roll16_short_bias", AUDIT_V1_PATH)
    segment_audit = load_module("omega462_segment_audit_for_roll16_short_bias", AUDIT_SEGMENT_PATH)
    ...
    audit.add_check(
        checks,
        "selection_scope_discloses_short_bias_and_oos_gate",
        report.get("selection_scope") == "validation_short_bias_branch_with_oos_safety_gate; fresh_holdout_required",
        "research_blocker",
    ...
    audit.add_check(
        checks,
        "short_bias_selection_rule_pass",
        selected.get("short_bias_branch_rule_pass") is True
        ...
        and selected["exposure_long_factor"] <= rule["max_long_factor"] + 1.0e-12
        and selected["exposure_short_factor"] >= rule["min_short_factor"] - 1.0e-12
        and selected["exposure_cap_notional"] <= rule["max_cap_notional"] + 1.0e-12,
        "research_blocker",
    ...
    audit.add_check(
        checks,
        "oos_improves_source_best_without_longer_hold",
        oos["pnl"] > source_best["oos_pnl"]
        and validation["pnl"] >= source_best["validation_pnl"] - rule["max_validation_pnl_gap_pp"]
    ...
    verdict = (
        "FULL_LIVE_PASS"
        if full_live_pass
        else "RESEARCH_ROLL16_FINE_SHORT_BIAS_PASS_FULL_LIVE_BLOCKED"
        if research_pass
        else "REDTEAM_FAIL"
    )
    payload = {
        "audit_id": "omega4_6_2_v5_roll16_fine_short_bias_segment_governor_redteam_20260701",
