#!/usr/bin/env python3
"""Research probe (2026-08-09, overnight loop idea #10): combined-feature skip-filter, this
project's best shot for the night at replicating an h48qual-style blocking effect from scratch.

Combines every feature family tried tonight (ETH momentum, BTC lead-lag, OFI, realized-vol/ATR)
into one binary win-probability model, and sweeps the skip quantile {0.10,0.20,0.30,0.40,0.50}
as the actual search (rather than fixing 30% by fiat as ideas #8-#9 did). The search itself is
falsification_audit'd on DEV before any quantile is trusted -- this is the correct place for
that instrument here, since a quantile sweep is a real multi-configuration search over the SAME
period, exactly the shape falsification_audit was built for.

If this also fails, that's a strong, comprehensively-covered negative for tonight's
"replicate blocking from generic features" line: momentum, order flow, cross-asset lead-lag,
and volatility have all been tried, alone and combined, across two framings (3-way reclassify,
binary skip-filter), and none carry enough information to beat the label's structural baseline
or even a random skip-filter of the same size.
"""
