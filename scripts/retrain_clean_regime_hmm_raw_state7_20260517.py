...
MODEL_ID = "clean_regime_2024_unsup_raw_state7_v8_20260517"
DEFAULT_OUT_DIR = ROOT / "data/ensemble/supervised/clean_regime_raw_state7_v8_20260517"
DEFAULT_REPORT = ROOT / "data/ensemble/reports/clean_regime_raw_state7_v8_20260517_report.json"
STATE7_COLS = [
    "state7_trend_score",
...
def _fit_obs(train: pd.DataFrame, pred: pd.DataFrame) -> tuple[np.ndarray, np.ndarray, RobustScaler, pd.Series]:
    x_train_raw = train[STATE7_COLS].apply(pd.to_numeric, errors="coerce").replace([np.inf, -np.inf], np.nan)
    medians = x_train_raw.median(numeric_only=True).fillna(0.0)
    x_train = x_train_raw.fillna(medians).fillna(0.0)
    x_pred = pred[STATE7_COLS].apply(pd.to_numeric, errors="coerce").replace([np.inf, -np.inf], np.nan).fillna(medians).fillna(0.0)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Retrain raw-only clean regime HMM with seven semantic state features.")
    ...
    parser.add_argument("--seed", type=int, default=80517)
    ...
    train = _with_raw_state7(_read(args.train_2024))
    ...
    train_obs, val_obs, val_scaler, val_medians = _fit_obs(train_part, val_part)
    ...
    full_obs, _, scaler, medians = _fit_obs(train, train.iloc[:1].copy())
    ...
    model_path = args.out_dir / "clean_regime_raw_state7_v8_2024.joblib"
    joblib.dump(
        {
            "model_id": MODEL_ID,
            ...
            "notes": "Raw-only HMM emission uses seven semantic engineered features and no clean_regime inputs.",
        },
        model_path,
    )
    ...
    report: dict[str, Any] = {
        ...
        "outputs": {},
        "notes": [
            "Raw-only seven semantic state features replace all clean_regime factor inputs.",
            "The script does not call clean_regime_factors and does not use clean_regime_* as HMM inputs.",
            "No risk_off, transition, cluster id, hidden-state id, factor columns, or hard label columns are written.",
        ],
    }

    for src in transforms:
        frame = _with_raw_state7(_read(src))
        ...
        sidecar = args.out_dir / f"{src.stem}_clean_regime_raw_state7_v8.csv"
