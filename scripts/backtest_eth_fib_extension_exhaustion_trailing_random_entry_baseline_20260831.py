#!/usr/bin/env python3
"""Random-entry economics baseline for fib_extension_exhaustion's leading grid-search candidate
(SL=3.5/ARM=0.5/Trail=0.1xATR, HORIZON=20) -- proactive check per the orthogonal_combo deep-audit
lesson (a tight exit structure can inflate win rate on pure noise; avg bp is the metric that
actually validates edge). Run BEFORE trusting the grid-search win rates this time (not after, as
happened for orthogonal_combo) -- ALL 9/96 passing combos in the grid search used ARM=0.5 (the
smallest tested value) with win rates 84-93%, the exact same red flag pattern that orthogonal_combo
showed (ARM=0.5, win rates 91-96%, later found to be ~83-85% even for random entries).
"""
...
def log(msg): print(f"[fib_ext_random_baseline] {msg}", flush=True)
...
def main():
    ...
    log(f"\nREAL fib_extension_exhaustion (SL=3.5/ARM=0.5/Trail=0.1): VAL +15.15bp(win93.2%)  OOS +3.00bp(win87.6%)")
