#!/usr/bin/env python3
"""Fine validation-max 12h side-specific bracket branch for Omega 4.6.2 v5."""
...
MODEL_ID = "omega4_6_2_v5_roll12_side_specific_fine_valmax_20260701"
REFERENCE_MODEL_ID = "omega4_6_2_v5_roll12_side_specific_bracket_daytrade_20260701"
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll12_side_specific_fine_valmax_20260701.md"
REFERENCE_CAP = 4.20
EPS = 1.0e-12
...
def write_markdown(report: dict[str, Any]) -> None:
    ...
    text = f"""# Omega 4.6.2 v5 Roll12 Side-Specific Fine Valmax - 2026-07-01
...
def main() -> int:
    helper = load_module("omega462_roll12_side_helper_for_fine_valmax", HELPER_PATH)
    source = helper.load_module("omega462_roll16_source_for_fine_valmax", helper.SOURCE_MODULE_PATH)
    v1 = source.load_module("omega462_loss_cluster_v1_for_fine_valmax", source.V1_MODULE_PATH)
    segment_mod = source.load_module("omega462_segment_for_fine_valmax", source.SEGMENT_MODULE_PATH)
    stop_mod = v1.load_module("omega462_stop_mod_for_fine_valmax", v1.STOP_MODULE_PATH)
    ...
    bracket_specs = [
        ("fine_val_max", 0.0225, 0.0500, 0.0400, 0.0400),
        ("fine_fast_val", 0.0200, 0.0400, 0.0400, 0.0400),
        ("fine_val_oos", 0.0200, 0.0500, 0.0400, 0.0400),
        ("fine_oos_val", 0.0300, 0.0400, 0.0400, 0.0400),
    ]
        ...
        for exposure in exposure_specs:
            for governor in governors:
                ...
                row = {
                    ...
                    "roll12_max_hours": helper.MAX_ROLL_HOURS,
    ...
    status = (
        "RESEARCH_ROLL12_SIDE_SPECIFIC_FINE_VALMAX_PASS"
        if bool(selected["research_fine_valmax_gate_pass"])
        else "NO_ROLL12_SIDE_SPECIFIC_FINE_VALMAX_PASSING_CANDIDATE"
    )
    safe = (
        f"{selected['side_bracket_spec']}__{selected['exposure_spec']}__"
        f"{selected['segment_governor_spec']}"
    ).replace(".", "p").replace("/", "_")
    ranking_path = OUT_DIR / "roll12_side_specific_fine_valmax_ranking.csv"
    top20_path = OUT_DIR / "roll12_side_specific_fine_valmax_top20.csv"
