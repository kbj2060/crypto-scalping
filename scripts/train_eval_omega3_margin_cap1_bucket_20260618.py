...
MODEL_ID = "omega3_margin_cap1_bucket_fixedlev2_20260618"
...
FIXED_LEVERAGE = 2.0
PARENT_RISK_MARGIN_FRACTION_MIN = PARENT_RISK_NOTIONAL_MIN / FIXED_LEVERAGE
PARENT_RISK_MARGIN_FRACTION_MAX = 1.0
PARENT_RISK_NOTIONAL_EFFECTIVE_MAX = FIXED_LEVERAGE * PARENT_RISK_MARGIN_FRACTION_MAX
MARGIN_BUCKET_VALUES = np.asarray([0.10, 0.20, 0.30, 0.40, 0.60, 0.80, 1.00], dtype=np.float64)
...
def _risk_from_path_row(row: pd.Series, prefix: str, default: sleeve.FallbackRisk) -> sleeve.FallbackRisk:
    ...
    quality_scale = 1.0 + 0.65 * rr_signal + 0.20 * float(net_signal) - 0.10 * float(time_signal)
    base_margin = float(default.notional) / float(FIXED_LEVERAGE)
    margin_fraction = float(
        np.clip(
            base_margin * float(np.clip(quality_scale, 0.10, 2.20)),
            float(PARENT_RISK_MARGIN_FRACTION_MIN),
            float(PARENT_RISK_MARGIN_FRACTION_MAX),
        )
    )
    leverage = float(FIXED_LEVERAGE)
    notional = float(np.clip(margin_fraction * leverage, float(PARENT_RISK_NOTIONAL_MIN), float(PARENT_RISK_NOTIONAL_EFFECTIVE_MAX)))
    margin_fraction = float(notional / max(leverage, 1.0e-12))
...
def _risk_label_table(path_labels: pd.DataFrame, default_risk: sleeve.FallbackRisk) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    diag = {
        ...
        "long_margin_fraction_min": float(labels["long_margin_fraction"].min()) if len(labels) else float(default_risk.notional / max(FIXED_LEVERAGE, 1.0e-12)),
        "long_margin_fraction_max": float(labels["long_margin_fraction"].max()) if len(labels) else float(default_risk.notional / max(FIXED_LEVERAGE, 1.0e-12)),
        "short_margin_fraction_min": float(labels["short_margin_fraction"].min()) if len(labels) else float(default_risk.notional / max(FIXED_LEVERAGE, 1.0e-12)),
        "short_margin_fraction_max": float(labels["short_margin_fraction"].max()) if len(labels) else float(default_risk.notional / max(FIXED_LEVERAGE, 1.0e-12)),
    }
    return labels, diag
...
def _nearest_margin_bucket(values: np.ndarray) -> np.ndarray:
    arr = np.asarray(values, dtype=np.float64)
    distances = np.abs(arr[:, None] - MARGIN_BUCKET_VALUES[None, :])
    return np.argmin(distances, axis=1).astype(np.int64)


def _fit_predict_margin_bucket_head(
    x_val: pd.DataFrame,
    x_oos: pd.DataFrame,
    labels: pd.DataFrame,
    long_col: str,
    ...
    *,
    ...
    y_long[idx] = _nearest_margin_bucket(y_long_value[idx])
    y_short[idx] = _nearest_margin_bucket(y_short_value[idx])
    default_long = int(np.bincount(y_long[idx], minlength=len(MARGIN_BUCKET_VALUES)).argmax()) if len(idx) else 0
    default_short = int(np.bincount(y_short[idx], minlength=len(MARGIN_BUCKET_VALUES)).argmax()) if len(idx) else 0
    ...
    val_long = MARGIN_BUCKET_VALUES[np.clip(val_long_bucket, 0, len(MARGIN_BUCKET_VALUES) - 1)]
    val_short = MARGIN_BUCKET_VALUES[np.clip(val_short_bucket, 0, len(MARGIN_BUCKET_VALUES) - 1)]
    oos_long = MARGIN_BUCKET_VALUES[np.clip(oos_long_bucket, 0, len(MARGIN_BUCKET_VALUES) - 1)]
    oos_short = MARGIN_BUCKET_VALUES[np.clip(oos_short_bucket, 0, len(MARGIN_BUCKET_VALUES) - 1)]
    diag = {
        "target_cols": [long_col, short_col],
        "model": "HistGradientBoostingClassifier side-specific margin buckets",
        "bucket_values": [float(x) for x in MARGIN_BUCKET_VALUES.tolist()],
...
def main() -> int:
    ...
    val_margin_p, val_margin_s, oos_margin_p, oos_margin_s, risk_margin_diag = _fit_predict_margin_bucket_head(
        x_val,
        x_oos,
        risk_labels,
        "long_margin_fraction",
    ...
    diagnostics: dict[str, Any] = {
        ...
        "parent_risk_source": {
            ...
            "head_contract": "tp_price_move/sl_price_move regressors plus margin_fraction bucket classifier; leverage fixed at 2; margin cap 1.0; notional and account TP/SL are derived",
        },
        "risk": {**asdict(RISK), "fixed_leverage": float(FIXED_LEVERAGE), "margin_fraction_max": float(PARENT_RISK_MARGIN_FRACTION_MAX), "effective_notional_max": float(PARENT_RISK_NOTIONAL_EFFECTIVE_MAX), "margin_bucket_values": [float(x) for x in MARGIN_BUCKET_VALUES.tolist()]},
