#!/usr/bin/env python3
"""Red-team audit for Omega 4.6.2 v5 roll9 side-specific fine branch."""
...
RANKING = OUT_DIR / "roll9_side_specific_fine_valmax_ranking.csv"
EVAL_SCRIPT = ROOT / "scripts/eval_omega4_6_2_v5_roll9_side_specific_fine_valmax_20260701.py"
PRIOR_RUNTIME_AUDIT = (
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll9_side_specific_fine_valmax_redteam_20260701.md"
TOL = 1.0e-8
...
def write_markdown(payload: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- Roll max hold: `{selected['roll9_max_hours']}`",
        f"- Long TP/SL: `{selected['roll9_long_tp_move']}` / `{selected['roll9_long_sl_move']}`",
        f"- Short TP/SL: `{selected['roll9_short_tp_move']}` / `{selected['roll9_short_sl_move']}`",
        "",
...
def main() -> None:
    audit = load_module("omega462_loss_cluster_audit_for_roll9_fine", AUDIT_V1_PATH)
    segment_audit = load_module("omega462_segment_audit_for_roll9_fine", AUDIT_SEGMENT_PATH)
    ...
    audit.add_check(
        ...
        == "validation_primary_roll9_hold_compression_with_oos_safety_gate; fresh_holdout_required",
        "research_blocker",
    ...
    audit.add_check(
        ...
        == "among research-gated roll9 variants, sort by validation_pnl, validation_avg_hold, cap distance; OOS is not an ordering key",
        "research_blocker",
        {"observed": report.get("selection_rule")},
    )
    audit.add_check(
        checks,
        "selected_roll9_gate_pass",
        selected.get("research_roll9_gate_pass") is True,
        "research_blocker",
        {"observed": selected.get("research_roll9_gate_pass")},
    )
    ...
    audit.add_check(
        checks,
        "roll9_hold_contract",
        validation["max_hold_hours"] <= 9.0
        and oos["max_hold_hours"] <= 9.0
    ...
    audit.add_check(
        ...
        float(selected["roll9_max_hours"]) == 9.0
        and float(selected["roll9_long_tp_move"]) > 0.0
        and float(selected["roll9_long_sl_move"]) > 0.0
        and float(selected["roll9_short_tp_move"]) > 0.0
        and float(selected["roll9_short_sl_move"]) > 0.0
        and {"roll9_bracket_tp", "roll9_bracket_sl"}.issubset(set(validation_df["reason"].astype(str))),
        "research_blocker",
        {
            "long_tp": selected.get("roll9_long_tp_move"),
            "long_sl": selected.get("roll9_long_sl_move"),
            "short_tp": selected.get("roll9_short_tp_move"),
            "short_sl": selected.get("roll9_short_sl_move"),
        },
    )
    ...
    verdict = (
        "FULL_LIVE_PASS"
        if full_live_pass
        else "RESEARCH_ROLL9_FINE_PASS_FULL_LIVE_BLOCKED"
        if research_pass
        else "REDTEAM_FAIL"
    )
    payload = {
        "audit_id": "omega4_6_2_v5_roll9_side_specific_fine_valmax_redteam_20260701",
