...
MODEL_ID = "hf_v13_tail_aware_runner_v21_1_20260511"
...
DEFAULT_OUT_DIR = ROOT / "data/ensemble/supervised/hf_v13_tail_aware_runner_v21_1_20260511"
DEFAULT_REPORT = ROOT / "data/ensemble/reports/hf_v13_tail_aware_runner_v21_1_20260511_summary.json"
DEFAULT_AUDIT = ROOT / "data/ensemble/reports/hf_v13_tail_aware_runner_v21_1_20260511_audit.json"
DEFAULT_GRID = ROOT / "data/ensemble/reports/hf_v13_tail_aware_runner_v21_1_20260511_grid.csv"
...
def _grid() -> list[CostRunnerConfig]:
    ...
    i = 0
    for robust_edge, jackpot_p, jackpot_q90 in ((0.002, 0.25, 0.015), (0.002, 0.35, 0.015), (0.004, 0.25, 0.025), (0.004, 0.35, 0.025), (0.006, 0.30, 0.020), (0.010, 0.25, 0.025)):
        for bad_cap in (0.35, 0.50):
            rows.append(CostRunnerConfig(f"v21_1_tail_runner_{i}", robust_edge, robust_edge, 0.60, -0.006, jackpot_p, jackpot_q90, bad_cap, 0.004, 3, 0.20, 0.10, 1.35, 2.75, 0.30))
            i += 1
    rows.append(CostRunnerConfig("v21_1_parent_noop", 99.0, 99.0, 1.0, 99.0, 99.0, 99.0, 0.0, 99.0, 999, 0.0, 0.0, 1.0, 99.0, 99.0))
    return rows
    ...
    for i in range(0, len(df) - 2):
        ...
        if pos != 0:
            ...
            if not reason and not add_done and cfg.full_add_frac > 0.0 and unreal >= cfg.min_unrealized and hold >= cfg.min_bars_since_entry and dd_abs <= cfg.dd_block:
                ...
                is_robust = edge >= cfg.full_edge_th and p >= cfg.p_th and q10 >= cfg.q10_floor and p_bad <= cfg.bad_cap
                is_half = edge >= cfg.half_edge_th and p >= cfg.p_th and q10 >= min(cfg.q10_floor, -0.006) and p_bad <= min(0.65, cfg.bad_cap + 0.10)
                if is_jackpot or is_robust:
                    action_frac = cfg.full_add_frac
                elif is_half:
                    action_frac = cfg.half_add_frac
...
def main() -> int:
    ...
    model_path = args.out_dir / "v21_1_tail_aware_runner.pkl"
    ...
    report = {"model_id": MODEL_ID, "design": "v21.1 tail-aware add-on runner. It separates robust utility, jackpot capture, bad-add-on risk, and cost3 survival gates.", "base_model": str(args.model), "model": str(model_path), "split_policy": "Tail runner trained on 2025 Jan-Sep; config selected on 2025 Oct-Dec; 2026 fixed OOS not used for selection.", "selected_config": asdict(selected), "selection_result": best, "metrics": metrics, "audit": audit, "artifacts": {"model": str(model_path), "report": str(args.report_out), "audit": str(args.audit_out), "grid": str(args.grid_out), "ledgers": ledgers}}
