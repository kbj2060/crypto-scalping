#!/usr/bin/env python3
"""Causal native 4-action portfolio router.

State receives all ETH/SOL/BTC model candidates at the current timestamp.
Actions:
- SKIP
- TAKE_ETH
- TAKE_SOL
...
The online learner updates only from trades closed before the current decision.
This is a causal contextual bandit, not an offline full-ledger learner.
"""
...
OUT_DIR = ROOT / "tmp/causal_regen_20260516/portfolio_online_bandit_4action_native_20260709"
DOC_PATH = ROOT / "docs/model_contracts/portfolio_online_bandit_4action_native_20260709.md"
ASSETS = ("eth", "sol", "btc")
ACTIONS = ("SKIP", "TAKE_ETH", "TAKE_SOL", "TAKE_BTC")
...
class OnlineActionModel:
    ...
    def update(self, state: np.ndarray, action_idx: int, trade_return: float, notional: float) -> None:
        ...
        reward = float(trade_return) - 0.25 * tail_penalty - 0.001 * float(notional)
    ...
    def choose(self, state: np.ndarray, valid_actions: list[int], candidates: dict[str, native.Candidate | None]) -> tuple[int, dict[str, float]]:
        # Before the model has enough closed samples, use a deterministic,
        # non-OOS-tuned exploration policy: take the valid candidate with the
        # highest validation-only asset score.
        if self.weights is None or len(self.ys) < self.explore_until:
            asset_actions = [a for a in valid_actions if a > 0]
            if not asset_actions:
                return 0, {ACTIONS[a]: 0.0 for a in valid_actions}
            best = max(asset_actions, key=lambda a: native.ASSET_SCORES[ASSETS[a - 1]])
            return best, {ACTIONS[a]: 0.0 for a in valid_actions}
        qs = {ACTIONS[a]: self.q(state, a) for a in valid_actions}
        best_action = max(valid_actions, key=lambda a: qs[ACTIONS[a]])
        return int(best_action), qs
...
def _replay(world: dict[str, Any], *, device: Any) -> tuple[dict[str, Any], pd.DataFrame, pd.DataFrame]:
    ...
    for ts in world["timestamps"]:
        ...
        valid_actions = [0] + [i + 1 for i, asset in enumerate(ASSETS) if candidates[asset] is not None]
        if len(valid_actions) == 1:
            continue
        drawdown = cash / max(peak, 1e-12) - 1.0
        state = _state_features(candidates, ts, lag_returns, drawdown, len(rows))
        action_idx, qs = model.choose(state, valid_actions, candidates)
        decisions.append(
            {
                "timestamp": ts,
...
def _write_doc(report: dict[str, Any]) -> None:
    lines = [
        "# Portfolio Online Bandit 4-Action Native - 2026-07-09",
        "",
        "State receives ETH/SOL/BTC candidates. Action space is `SKIP / TAKE_ETH / TAKE_SOL / TAKE_BTC` with action masking.",
        "",
...
def main() -> int:
    ...
    for split in ("validation", "oos"):
        ...
        print(f"stage=replay_4action split={split}", flush=True)
    ...
    report = {
        "method": "portfolio_online_bandit_4action_native",
