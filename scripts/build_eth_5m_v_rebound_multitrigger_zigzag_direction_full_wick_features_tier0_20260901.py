#!/usr/bin/env python3
"""Tier0 피쳐(22+rsi=23개)를 ZDC 완전wick 라벨에 결합.

build_eth_5m_v_rebound_multitrigger_zigzag_direction_features_tier0_20260901.py(wick-앵커판)와
완전히 동일한 피쳐 구성/공식(build_indicator_frame() import 등 전부 무변경) -- 유일한 차이는
라벨 소스(LABEL_CSV)뿐. Tier0 피쳐 자체는 트리거 population 레벨이라 어떤 ZDC 변형을 쓰든 무관.

Run with the quant_ai conda env (torch is a transitive dependency of compute_indicators):
  /home/kbj20/anaconda3/envs/quant_ai/bin/python scripts/build_eth_5m_v_rebound_multitrigger_zigzag_direction_full_wick_features_tier0_20260901.py
"""
...
LABEL_CSV = ROOT / "data/labels/eth_5m_v_rebound_multitrigger_zigzag_direction_full_wick_20260901/eth_5m_v_rebound_multitrigger_zigzag_direction_full_wick_labels.csv"
OUT_DIR = ROOT / "data/labels/eth_5m_v_rebound_multitrigger_zigzag_direction_full_wick_20260901"

FEATURE_COLUMNS = _tier0.FEATURE_COLUMNS + ["rsi"]  # 기존 wick-앵커판과 정확히 동일한 23개
...
def main() -> int:
    ...
    result = merged[["idx", "timestamp", "direction", "triggers", "n_triggers",
                      "pivot_type", "extreme_idx", "confirm_idx", "hit", "outcome"]].copy()
    ...
    out_path = OUT_DIR / "eth_5m_v_rebound_multitrigger_zigzag_direction_full_wick_features_tier0.csv"
