#!/usr/bin/env python3
"""OOS-max 2h hold-compressed branch for Omega 4.6.2 v5."""
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll2_side_specific_two_stage_exposure_oos_max_20260701.md"
...
def write_markdown(report: dict[str, Any]) -> None:
    ...
    text = f"""# Omega 4.6.2 v5 Roll2 OOS-Max Two-Stage Exposure - 2026-07-01

## Method

This branch keeps the roll2 `2h` path and selects the highest-OOS-PnL candidate inside a `{VALIDATION_NEARMAX_BAND_PP:.1f}pp` validation near-max band. OOS is used as an ordering key, so this is research-only until fresh holdout is available.
...
- Research gate pass: `{selected["research_roll2_oos_max_gate_pass"]}`
...
def main() -> int:
    roll2 = load_module("omega462_roll2_eval_for_oos_max", ROLL2_PATH)
    ...
    if eligible.empty:
        ordered = eligible.reset_index(drop=True)
        ...
        selected["research_roll2_oos_max_gate_pass"] = False
    else:
        ...
        selected["validation_nearmax_band_pp"] = VALIDATION_NEARMAX_BAND_PP
        selected["validation_nearmax_floor"] = floor
        selected["research_roll2_oos_max_gate_pass"] = True
    selected_val, selected_oos = roll2.build_selected_ledgers(selected)

    safe = str(selected["exposure_spec"]).replace(".", "p").replace("/", "_")
    ranking_path = OUT_DIR / "roll2_oos_max_ranking.csv"
    top20_path = OUT_DIR / "roll2_oos_max_top20.csv"
    ...
    report = {
        ...
        "selection_scope": "roll2_validation_nearmax_oos_max; fresh_holdout_required",
        "selection_rule": "within 10.0pp of roll2 validation max and research gate pass, maximize OOS PnL; OOS used only for research ordering",
        "reference_variant": reference,
        "selected_variant": selected,
        "top20": ordered.head(20).to_dict(orient="records"),
        "status": (
            "RESEARCH_ROLL2_OOS_MAX_PASS"
            if bool(selected["research_roll2_oos_max_gate_pass"])
            else "NO_ROLL2_OOS_MAX_PASSING_CANDIDATE"
        ),
        "artifacts": {
            "out_dir": str(OUT_DIR),
