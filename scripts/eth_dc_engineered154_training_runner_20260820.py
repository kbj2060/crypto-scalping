#!/usr/bin/env python3
"""정리(VIF112)+조합(RIT30)+금융ML문헌(12)=154피쳐 최종 엔지니어링 피쳐셋 TabM 단일모델
학습 CLI 러너. 레짐 라우팅 무력화 monkeypatch는 기존 unified_single_model 러너들과 완전
동일(재구현 아님) -- canonicaldata 래퍼만 eth_dc_engineered_features_canonicaldata_20260820.py로
교체. epochs=2로 DC원본(158피쳐) 베이스라인과 동일하게 맞춰 비교 기준을 유지."""
...
import eth_dc_engineered_features_canonicaldata_20260820 as canon  # noqa: E402
...
if __name__ == "__main__":
    ...
    out_suffix = f"dc_engineered154_unified_single_model_seed{known.seed}_20260820"
    sys.argv = [sys.argv[0], *BASE_ARGS, "--seed", str(known.seed), "--out-suffix", out_suffix]
    print(f"stage=start seed={known.seed} out_suffix={out_suffix} mode=engineered154_features", flush=True)
    t0 = time.time()
