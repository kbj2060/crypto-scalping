#!/usr/bin/env python3
"""SINGLE final HOLDOUT (2026-04-01+) exposure for the smt_divergence trailing-stop cost-gate --
final config SL=4.0xATR/ARM=2.0xATR/Trail=0.1xATR, picked from
backtest_eth_smt_divergence_trailing_gridsearch_20260831.py (71/96 combos passed VAL+OOS, this
was the #1 combo by min(VAL,OOS)=+6.18bp) + backtest_eth_smt_divergence_trailing_optimistic_
crosscheck_20260831.py (robust to both intrabar-ordering conventions, ~1.0-1.1bp spread, same
ranking either way) + backtest_eth_smt_divergence_trailing_random_entry_baseline_20260831.py
(random entries clearly weaker on both win rate and avg bp). Companion to the classification
HOLDOUT already touched in research_eth_smt_divergence_metalabel_final_20260831.py (AUC 0.6823) --
together these are the ONE holdout exposure for this model; do not re-run with a different
SL/ARM/Trail after seeing this result.
"""
...
def log(msg: str) -> None:
    print(f"[smt_divergence_trailing_HOLDOUT] {msg}", flush=True)
...
def main() -> int:
    ...
    log(f"(for reference: VAL was +7.00bp/win72.4%, OOS was +6.18bp/win69.6% at this exact config)")
    return 0
