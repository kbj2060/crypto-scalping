...
def entry_notional(equity: float, safe_mae_pct: float, existing_notional: float = 0.0,
    ...
    🔴2026-09-15 에 E|r| 배수 층(`× evr_q`)을 여기 얹었다가 **켜기 전에 철회**했다 --
    5주 원장 순차 검정에서 증분이 전반 +11.01bp/건 → 후반 **+0.73**(동전)으로 갈렸고
    상위 10건을 빼면 부호가 뒤집혔다. docs/homer §5.36-S."""
...
def _self_check() -> None:
    ...
    # 🔴철회된 E|r| 배수가 되돌아오지 않았는지 -- 진입 명목은 상한과 기존 포지션만의 함수다
    assert "evr_mult" not in e, "E|r| 배수 층이 다시 붙었다(§5.36-S 철회)"
