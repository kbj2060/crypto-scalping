...
DEFAULT_OUT_DIR = ROOT / "tmp/causal_regen_20260516/alpha7_fallback_replace_alpha6_logic_top3_20260525"
...
def _override_with_alpha6_logic(
    *,
    ...
    action = np.where(primary_busy, 0, action).astype(np.int64)
    quality = np.where(primary_busy, float(cash_score), quality).astype(np.float64)
    hold_idx = np.where(primary_busy, 0, hold_idx).astype(np.int64)
    y["action"] = action
    y["quality"] = quality
    trade = action != 0
    ...
    meta = {
        ...
        "primary_cash_rows": int((~primary_busy).sum()),
        "action_distribution_after_cash_mask": pd.Series(action).value_counts().sort_index().to_dict(),
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Replace Alpha7 cash fallback with Alpha6 label-logic-driven fallback candidates.")
    ...
    ap.add_argument("--seed", type=int, default=72525)
...
def main() -> int:
    ...
    for i, preset in enumerate(presets):
        y_train, label_meta = _override_with_alpha6_logic(
            train_df=train_df,
            ...
            cash_score=float(label_cfg.cash_score),
        )
        parent = train_policy(
            x_train,
            y_train,
        ...
        report = {
            "model_id": f"alpha7_fallback_replace_alpha6_logic_{preset}_20260525",
            "preset": preset,
            "design": "Primary Alpha7 is fixed. Fallback parent feature contract remains the 61-feature alpha43_no_legacy contract. Training labels are replaced with Alpha6 label-generation logic, but only on candidate rows where the fixed primary is CASH; primary-active candidate rows are forced to CASH.",
