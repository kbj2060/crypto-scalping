#!/usr/bin/env python3
"""Red-team audit for Omega 4.6.2 v5 roll7 OOS-balanced branch."""
...
ROLL7_EVAL_PATH = ROOT / "scripts/eval_omega4_6_2_v5_roll7_side_specific_two_stage_exposure_hold_compressed_20260701.py"
MODEL_ID = "omega4_6_2_v5_roll7_side_specific_two_stage_exposure_oos_balanced_20260701"
...
RANKING = OUT_DIR / "roll7_oos_balanced_ranking.csv"
TOP20 = OUT_DIR / "roll7_oos_balanced_top20.csv"
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll7_side_specific_two_stage_exposure_oos_balanced_redteam_20260701.md"
TOL = 1.0e-8
...
def main() -> None:
    audit = load_module("omega462_loss_cluster_audit_for_roll7_oos_balanced", AUDIT_HELPER_PATH)
    roll7_eval = load_module("omega462_roll7_eval_for_oos_balanced_audit", ROLL7_EVAL_PATH)
    ...
    missing = {
        ...
        for name, path in {
            ...
            "ranking": RANKING,
            "top20": TOP20,
            "roll7_eval_script": ROLL7_EVAL_PATH,
    ...
    audit.add_check(
        ...
        report.get("selection_scope") == "roll7_validation_nearmax_oos_balanced; fresh_holdout_required",
        "research_blocker",
    ...
    audit.add_check(
        checks,
        "selected_gate_pass",
        selected.get("research_roll7_oos_balanced_gate_pass") is True
        and selected.get("research_roll7_hold_compressed_gate_pass") is True,
        "research_blocker",
        {
            "oos_balanced": selected.get("research_roll7_oos_balanced_gate_pass"),
            "base_gate": selected.get("research_roll7_hold_compressed_gate_pass"),
        },
    )
    ...
    replay_val, replay_oos = roll7_eval.build_selected_ledgers(selected)
    val_replay = replay_compare(replay_val, validation_df)
    oos_replay = replay_compare(replay_oos, oos_df)
    audit.add_check(
        checks,
        "roll7_oos_balanced_replay_parity",
    ...
    audit.add_check(
        ...
        and oos["mdd"] >= -20.0
        and validation["max_hold_hours"] <= 7.0
        and oos["max_hold_hours"] <= 7.0
    ...
    verdict = (
        "FULL_LIVE_PASS"
        if full_live_pass
        else "RESEARCH_ROLL7_OOS_BALANCED_PASS_FULL_LIVE_BLOCKED"
        if research_pass
        else "REDTEAM_FAIL"
    )
    payload = {
        "audit_id": "omega4_6_2_v5_roll7_side_specific_two_stage_exposure_oos_balanced_redteam_20260701",
