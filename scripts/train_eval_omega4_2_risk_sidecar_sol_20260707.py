...
MODEL_ID = "sol_omega4_2_trade_risk_sidecar_20260707"
OUT_DIR = ROOT / "tmp/causal_regen_20260516" / MODEL_ID
BASELINE_BUNDLE = (
    ROOT
    / "tmp/causal_regen_20260516"
    / "sol_omega4_3head_parent72_loose_entry_quality_20260707_zig075_20260707"
...
def _prepare_exit_runtime(
...
) -> tuple[np.ndarray, dict[str, tuple[parent.ThreeHeadTabM, np.ndarray, np.ndarray]], list[int]]:
    ...
    return base_x.to_numpy(dtype=np.float32), runtime, pos_idx
...
def _replay_with_risk(
    ...
    *,
    ...
    route = hard._route_id(frame)
    base_np, exit_runtime, pos_idx = _prepare_exit_runtime(base_x, loaded_models)
...
def main() -> int:
    ...
    ap.add_argument("--fixed-mapping-report", type=Path, default=None)
    ...
    if args.fixed_mapping_report is not None:
        fixed_report = json.loads(Path(args.fixed_mapping_report).read_text(encoding="utf-8"))
        fixed_mapping = fixed_report["selected"]["mapping"]
        missing_mapping_keys = [key for key in MARGIN_CFG_KEYS if key not in fixed_mapping]
        unknown_mapping_keys = [key for key in fixed_mapping if key not in MARGIN_CFG_KEYS and key not in LEVERAGE_CFG_KEYS]
        if missing_mapping_keys or unknown_mapping_keys:
            raise RuntimeError(
                f"invalid fixed mapping: missing={missing_mapping_keys}, unknown={unknown_mapping_keys}"
            )
        candidates = [{key: float(value) for key, value in fixed_mapping.items()}]
    ...
    if full_eligible and str(args.selection_scope) != "validation_only":
        if str(args.selection_objective) == "log_risk":
    ...
    report = {
        ...
        "risk_model": {
            ...
            "fixed_mapping_report": str(args.fixed_mapping_report) if args.fixed_mapping_report is not None else None,
            "model_kind": str(args.model_kind),
            "risk_feature_mode": str(args.risk_feature_mode),
            "side_split_model": bool(args.side_split_model),
            "score_quality_blend": float(args.score_quality_blend),
