#!/usr/bin/env python3
"""Second loss-cluster sweep: shorter hard stops with wider capped exposure."""
...
MODEL_ID = "omega4_6_2_loss_cluster_governor_v2_20260701"
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_loss_cluster_governor_v2_20260701.md"
...
def stop_specs(v1: Any, stop_mod: Any) -> list[Any]:
    specs = []
    for hard in [72.0, 78.0, 84.0, 90.0]:
        for loss_name, loss_after, loss_stop in [
            ("loss48_4p5", 48.0, -0.045),
            ("loss60_5p0", 60.0, -0.050),
        ]:
            for stall_name, stall_after, stall_min in [
                ("stall72_lb24_min5p5", 72.0, 0.055),
                ("stall84_lb24_min6p0", 84.0, 0.060),
            ]:
                specs.append(
                    stop_mod.StopSpec(
                        name=f"hard{int(hard)}__{loss_name}__trail72_7p0_gap2p5__{stall_name}",
...
def exposure_specs(v1: Any) -> list[Any]:
    specs = []
    for factor in [1.78, 1.95, 2.10, 2.25, 2.40, 2.55, 2.70]:
    ...
    for long_factor in [0.75, 1.00, 1.20]:
        for short_factor in [1.78, 1.95, 2.10, 2.25, 2.40, 2.55, 2.70]:
...
def governor_specs(v1: Any) -> list[Any]:
    return [
        ...
        v1.GovernorSpec("loss1_70_win24", 0.70, 1.00, -1.0, 1.00, -1.0, 1.00, 24.0),
        v1.GovernorSpec("loss1_65_win24", 0.65, 1.00, -1.0, 1.00, -1.0, 1.00, 24.0),
        v1.GovernorSpec("loss1_60_win24", 0.60, 1.00, -1.0, 1.00, -1.0, 1.00, 24.0),
    ]
...
def write_markdown(report: dict[str, Any]) -> None:
    ...
    text = f"""# Omega 4.6.2 Loss-Cluster Governor v2 - 2026-07-01
...
def main() -> int:
    v1 = load_module("omega462_loss_cluster_v1", V1_MODULE_PATH)
    stop_mod = v1.load_module("omega462_stop_mod_for_v2", v1.STOP_MODULE_PATH)
    ...
    stop_list = stop_specs(v1, stop_mod)
    ...
    ranking_path = OUT_DIR / "loss_cluster_governor_v2_ranking.csv"
    top20_path = OUT_DIR / "loss_cluster_governor_v2_top20.csv"
