#!/usr/bin/env python3
"""Trailing-stop cost-gate grid search for smt_divergence (final label config HORIZON=72(6h)/
GAP=12/K=4.20), using this repo's STANDARD backtest engine
...
model's own probability -- smt_divergence uses plain single-K labeling (no exclude-middle, unlike
orthogonal_combo), so the existing classifier CSV already IS the full raw-fire population (5,441
rows, hit=0/1 covers everything).
...
def log(msg: str) -> None:
    print(f"[smt_divergence_trailing_gridsearch] {msg}", flush=True)
...
def main() -> int:
    ...
    out_dir = ROOT / "tmp/eth_smt_divergence_metalabel_tabpfn_20260831"
