#!/usr/bin/env python3
"""Fine exposure sweep around the Omega 4.6.2 loss-cluster governor v3 candidate."""
...
MODEL_ID = "omega4_6_2_loss_cluster_governor_v4_fine_exposure_20260701"
REFERENCE_MODEL_ID = "omega4_6_2_loss_cluster_governor_v3_20260701"
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_loss_cluster_governor_v4_fine_exposure_20260701.md"
...
def write_markdown(report: dict[str, Any]) -> None:
    ...
    text = f"""# Omega 4.6.2 Loss-Cluster Governor v4 Fine Exposure - 2026-07-01
...
def main() -> int:
    v1 = load_module("omega462_loss_cluster_v1_for_v4", V1_MODULE_PATH)
    stop_mod = v1.load_module("omega462_stop_mod_for_v4", v1.STOP_MODULE_PATH)
    ...
    governor = v1.GovernorSpec("loss1_55_win12", 0.55, 1.00, -1.0, 1.00, -1.0, 1.00, 12.0)
    ...
    for short_factor in [1.86, 1.87, 1.88, 1.89, 1.90, 1.91]:
        cap = round(2.15 * short_factor, 2)
        exposure = v1.ExposureSpec(
            name=f"long120_short{int(round(short_factor * 100)):03d}_cap{int(round(cap * 100)):03d}",
            long_factor=1.20,
        ...
        row["selection_score"] = pnl_gain - max(0.0, mdd_abs - 19.9) * 15.0
        rows.append(row)
    ...
    selected_short = float(selected["exposure_short_factor"])
    selected_cap = float(selected["exposure_cap_notional"])
    selected_exposure = v1.ExposureSpec(
        name=selected["exposure_spec"],
        long_factor=1.20,
        short_factor=selected_short,
        cap_notional=selected_cap,
    )
    selected_val = v1.apply_exposure_governor(val_stop, selected_exposure, governor)
    selected_oos = v1.apply_exposure_governor(oos_stop, selected_exposure, governor)
    status = (
    ...
    ranking_path = OUT_DIR / "loss_cluster_governor_v4_fine_exposure_ranking.csv"
    top20_path = OUT_DIR / "loss_cluster_governor_v4_fine_exposure_top20.csv"
