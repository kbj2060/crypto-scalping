...
# Columns visible to the exit head only -- zeroed for direction/quality/entry decisions
# (via _base_input), populated with the real per-bar value only when building exit-head
# training/replay context (via _exit_state_input / _exit_input_from_position_rows), exactly
# like POS_COLS but for a market signal rather than position state. Appended after POS_COLS
# in every feature vector so model input column order stays base_cols + POS_COLS + EXIT_ONLY_COLS.
EXIT_ONLY_COLS = ["swing_transition_prob"]
...
def _base_input(frame: pd.DataFrame, base_cols: list[str]) -> pd.DataFrame:
    ...
    for col in POS_COLS:
        x[col] = 0.0
    for col in EXIT_ONLY_COLS:
        x[col] = 0.0
    return x.astype(np.float32)


def _exit_state_input(frame: pd.DataFrame, base_cols: list[str]) -> pd.DataFrame:
    """Like _base_input, but keeps EXIT_ONLY_COLS at their real per-row value (POS_COLS still
    zeroed -- position state isn't known yet at label-building time). Used to build the `state`
    frame that _position_feature_row's cur_{col} lookup sources exit-time context from, so the
    exit head can see the real signal while direction/quality (built via _base_input) never do."""
    x = frame.reindex(columns=base_cols).apply(pd.to_numeric, errors="coerce").replace([np.inf, -np.inf], np.nan).fillna(0.0)
    for col in POS_COLS:
        x[col] = 0.0
    for col in EXIT_ONLY_COLS:
        x[col] = pd.to_numeric(frame[col], errors="raise").to_numpy(dtype=np.float32)
    return x.astype(np.float32)
