...
$existingTunnel = Get-CimInstance Win32_Process |
  Where-Object { $_.CommandLine -like '*scripts\\cloudflare_quick_tunnel.py*' } |
  Select-Object -First 1
$strayServers = Get-CimInstance Win32_Process |
  Where-Object { $_.CommandLine -like '*serve_dashboard.py*' -and $_.CommandLine -like '*8788*' } |
...
$proc = Start-Process `
  -FilePath python `
  -ArgumentList 'scripts\cloudflare_quick_tunnel.py --host 127.0.0.1 --port 8788' `
