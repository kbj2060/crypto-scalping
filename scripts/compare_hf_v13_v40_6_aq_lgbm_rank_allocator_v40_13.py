...
MODEL_ID = "hf_v13_tree_vs_foundation_aq_lgbm_rank_allocator_v40_13_20260512"
...
DEFAULT_OUT_DIR = ROOT / "data/ensemble/supervised/hf_v13_tree_vs_foundation_aq_lgbm_rank_allocator_v40_13_20260512"
DEFAULT_REPORT = ROOT / "data/ensemble/reports/hf_v13_tree_vs_foundation_aq_lgbm_rank_allocator_v40_13_20260512_summary.json"
DEFAULT_AUDIT = ROOT / "data/ensemble/reports/hf_v13_tree_vs_foundation_aq_lgbm_rank_allocator_v40_13_20260512_audit.json"
DEFAULT_GRID = ROOT / "data/ensemble/reports/hf_v13_tree_vs_foundation_aq_lgbm_rank_allocator_v40_13_20260512_grid.csv"
...
class RankRuntime:
    ...
    def name(self) -> str:
        return (
            f"rt_nm{str(self.notional_mult).replace('.', 'p')}"
            f"_floor{str(self.min_notional).replace('.', 'p')}"
            f"_cap{str(self.max_notional).replace('.', 'p')}"
            f"_{'scaletpsl' if self.tp_sl_scale else 'rawtpsl'}"
        )
...
def _rank_runtime_grid() -> list[RankRuntime]:
    return [
        RankRuntime(1.00, 0.00, 3.00, True),
        RankRuntime(1.50, 0.55, 3.00, True),
        RankRuntime(2.00, 0.80, 3.00, True),
        RankRuntime(2.50, 1.10, 3.00, True),
        RankRuntime(3.00, 1.50, 3.00, True),
        RankRuntime(2.00, 0.80, 3.00, False),
        RankRuntime(2.50, 1.10, 3.00, False),
    ]
...
def _predict_rank_two_stage(
    ...
    if runtime is not None:
        old_notional = notional.copy()
        take = old_notional > 0.0
        scaled_notional = np.clip(old_notional * float(runtime.notional_mult), float(runtime.min_notional), float(runtime.max_notional))
        scaled_notional = np.where(take, scaled_notional, 0.0)
        scale = np.divide(scaled_notional, np.maximum(old_notional, 1e-12), out=np.ones_like(scaled_notional), where=take)
        notional = scaled_notional
        if bool(runtime.tp_sl_scale):
            take_profit = take_profit * scale
            stop_loss = stop_loss * scale
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "V40.13: original v40.6 action/quality parent + side-aware LightGBM risk-adjusted rank allocator "
            "with Q4-selected conviction notional scaling."
        )
    )
...
def main() -> int:
    ...
    print(f"[{MODEL_ID}] selecting side-aware risk-adjusted rank allocator + runtime scaling on 2025 Q4", flush=True)
    ...
    for idx, hp in enumerate(_rank_hp_grid(), start=1):
        ...
        for runtime in _rank_runtime_grid():
            val_dec = _predict_rank_two_stage(final_aq_bundle, rank_bundle, val_full, runtime=runtime)
            ...
            row = {
                ...
                **asdict(hp),
                **{f"runtime_{k}": v for k, v in asdict(runtime).items()},
    ...
    selected_runtime = best["runtime"]
    eval_dec = _predict_rank_two_stage(final_aq_bundle, selected_rank, eval_full, runtime=selected_runtime)
    ...
    audit = {
        ...
        "runtime": "Q4-selected notional multiplier/floor/cap overlay",
    ...
    print(json.dumps({"report": str(args.report_out), "audit": str(args.audit_out), "grid": str(args.grid_out), "selected_rank": asdict(best["hp"]), "selected_runtime": asdict(selected_runtime), "metrics": metrics, "audit_status": audit["status"]}, ensure_ascii=False, default=_json_default))
    return 0
