#!/usr/bin/env python3
"""Red-team audit for Omega 4.6.2 v5 roll8 side-specific two-stage veto branch."""
...
RANKING = OUT_DIR / "roll8_side_specific_two_stage_veto_ranking.csv"
TOP20 = OUT_DIR / "roll8_side_specific_two_stage_veto_top20.csv"
EVAL_SCRIPT = ROOT / "scripts/eval_omega4_6_2_v5_roll8_side_specific_two_stage_veto_20260701.py"
PRIOR_RUNTIME_AUDIT = (
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll8_side_specific_two_stage_veto_redteam_20260701.md"
TOL = 1.0e-8
EPS = 1.0e-12
...
def second_stage_veto_replay(
    ...
    ref_active = reference_df["notional"].astype(float) > EPS
    is_short = reference_df["side"].astype(int) < 0
    values = pd.to_numeric(reference_df[feature], errors="coerce")
    if op == "<=":
    ...
    expected = ref_active & is_short & condition.fillna(False)
    actual_new = (
        ref_active
        & (candidate_df["notional"].astype(float) <= EPS)
        & (candidate_df["reason"].astype(str) == "feature_veto")
    )
    expected_spec = f"{feature}_{op}_{threshold:.8g}"
    spec_ok = bool((candidate_df.loc[actual_new, "feature_veto_spec"].astype(str) == expected_spec).all())
    mask_match = bool((expected.to_numpy() == actual_new.to_numpy()).all())
    zero_cols = [
        col
        for col in [
            "notional",
    ...
    veto_zero_ok = bool((candidate_df.loc[actual_new, zero_cols].astype(float).abs() <= TOL).all().all())
    common_cols = [c for c in reference_df.columns if c in candidate_df.columns]
    non_new_match = bool(
        (
            reference_df.loc[~expected, common_cols].astype(str).reset_index(drop=True)
            == candidate_df.loc[~actual_new, common_cols].astype(str).reset_index(drop=True)
        )
        .all()
        .all()
    )
    return {
        "split": split,
        "pass": bool(id_match and mask_match and spec_ok and veto_zero_ok and non_new_match),
        "id_match": id_match,
        "mask_match": mask_match,
        "spec_ok": spec_ok,
        "veto_zero_ok": veto_zero_ok,
        "non_new_match": non_new_match,
        "expected_spec": expected_spec,
        "expected_veto_count": int(expected.sum()),
        "actual_new_veto_count": int(actual_new.sum()),
        "selected_veto_count": int(selected[f"{split}_vetoed"]),
        "long_new_veto_count": int((actual_new & (reference_df["side"].astype(int) > 0)).sum()),
    }
...
def write_markdown(payload: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- Validation/OOS second-stage vetoed shorts: `{selected['validation_vetoed']}` / `{selected['oos_vetoed']}`",
    ...
    lines.extend(
        [
            ...
            "## Replay Checks",
            "",
            f"- Validation second-stage replay: `{payload['feature_veto_replay']['validation']['pass']}`",
            f"- OOS second-stage replay: `{payload['feature_veto_replay']['oos']['pass']}`",
...
def main() -> None:
    audit = load_module("omega462_loss_cluster_audit_for_roll8_two_stage_veto", AUDIT_HELPER_PATH)
    load_module("omega462_feature_veto_audit_for_two_stage", FEATURE_VETO_AUDIT_PATH)
    fold_eval = load_module("omega462_fold_eval_for_two_stage_audit", FOLDROBUST_EVAL_PATH)
    ...
    audit.add_check(
        ...
        == "validation_primary_second_stage_short_veto_with_temporal_fold_gate_and_productive_oos_safety_gate; fresh_holdout_required",
        "research_blocker",
    ...
    audit.add_check(
        ...
        == "search one additional non-lookahead-named numeric short-entry veto; require 4-fold validation robustness and productive OOS safety; among research-gated variants sort by validation_pnl, fold min pnl delta, validation_avg_hold; OOS is not an ordering key",
        "research_blocker",
        {"observed": report.get("selection_rule")},
    )
    audit.add_check(
        checks,
        "selected_two_stage_veto_gate_pass",
        selected.get("research_two_stage_veto_gate_pass") is True,
        "research_blocker",
        {"observed": selected.get("research_two_stage_veto_gate_pass")},
    )
    ...
    audit.add_check(
        checks,
        "productive_oos_gate_declared_and_met",
        int(selected["oos_vetoed"]) >= int(selected["min_oos_vetoed"])
        and float(selected["oos_pnl"]) > float(reference["oos_pnl"])
        and float(selected["oos_avg_hold_hours"])
        <= float(reference["oos_avg_hold_hours"])
        - float(selected["min_oos_avg_hold_improvement_hours"]),
        "research_blocker",
        {
            "selected": selected,
            "reference_oos_pnl": reference["oos_pnl"],
            "reference_oos_avg_hold_hours": reference["oos_avg_hold_hours"],
        },
    )
    ...
    val_replay = second_stage_veto_replay("validation", ref_validation_df, validation_df, selected)
    oos_replay = second_stage_veto_replay("oos", ref_oos_df, oos_df, selected)
    audit.add_check(
        checks,
        "second_stage_veto_replay_parity",
    ...
    audit.add_check(
        checks,
        "accounting_notional_overlap_contracts",
        validation["accounting_error_max_abs"] <= TOL
    ...
    verdict = (
        "FULL_LIVE_PASS"
        if full_live_pass
        else "RESEARCH_ROLL8_TWO_STAGE_VETO_PASS_FULL_LIVE_BLOCKED"
        if research_pass
        else "REDTEAM_FAIL"
    )
    payload = {
        "audit_id": "omega4_6_2_v5_roll8_side_specific_two_stage_veto_redteam_20260701",
