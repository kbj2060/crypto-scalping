...
MODEL_ID = "alpha3_dsac_exit_owner_v3_20260515"
OUT_DIR = ROOT / "data/ensemble/supervised/alpha3_dsac_exit_owner_v3_20260515"
MODEL_OUT = OUT_DIR / "dsac_exit_owner_v3.pt"
REPORT_OUT = ROOT / "data/ensemble/reports/alpha3_dsac_exit_owner_v3_20260515_summary.json"
AUDIT_OUT = ROOT / "data/ensemble/reports/alpha3_dsac_exit_owner_v3_20260515_audit.json"
GRID_OUT = ROOT / "data/ensemble/reports/alpha3_dsac_exit_owner_v3_20260515_grid.csv"
DATASET_OUT = ROOT / "data/ensemble/reports/alpha3_dsac_exit_owner_v3_20260515_dataset.json"
...
ACTION_SCALARS = {
    "hold_with_guard": 0.0,
    "market_close_100": -0.90,
    "maker_close_100_0": -0.58,
    "maker_close_100_2": -0.25,
    "maker_close_100_4": 0.25,
    "close_50_market": 0.55,
    "close_50_maker2": 0.78,
    "twap_maker2": 0.95,
}

ARM_BY_EXPANDED_ACTION = {
    "maker_close_100_0": "exit0_pen0",
    "maker_close_100_2": "exit2_pen0",
    "maker_close_100_4": "exit4_pen0",
    "close_50_maker2": "exit2_pen0",
    "twap_maker2": "exit2_pen0",
}
...
def _scalar_to_action_name(a: float, *, force_exit: bool) -> str:
    ...
    if -0.15 <= x <= 0.15:
        return "market_close_100" if force_exit else "hold_with_guard"
    if x < -0.65:
        return "market_close_100"
    if x < -0.35:
        return "maker_close_100_0"
    if x < -0.15:
        return "maker_close_100_2"
    if x < 0.45:
        return "maker_close_100_4"
    if x < 0.75:
        return "close_50_market"
    if x < 0.90:
        return "close_50_maker2"
    return "twap_maker2"
...
def _expanded_targets(base_y: np.ndarray, base_actions: list[str], x_full: np.ndarray, feature_names: list[str]) -> np.ndarray:
    ...
    baseline = base_y[:, idx["baseline_exit2_pen05"]]
    exit0 = base_y[:, idx["exit0_pen0"]]
    exit2 = base_y[:, idx["exit2_pen0"]]
    exit4 = base_y[:, idx["exit4_pen0"]]
    ...
    exit_adv = np.maximum.reduce([baseline, exit0, exit2, exit4]) - hold
    near_sl = (effective_sl > 1e-6) & ((-unreal) >= 0.55 * effective_sl)
    profit_lock = (unreal >= 0.006) & (giveback >= np.maximum(0.0020, 0.18 * np.maximum(mfe, 0.0)))
    strong_profit = (unreal >= 0.012) & (mfe >= 0.015)
    mature = hold_norm >= (3.0 / 64.0)
    exit_context = (exit_adv >= 0.0015) | profit_lock | near_sl
    partial_context = strong_profit & (giveback >= 0.0010) & mature
    twap_context = (unreal >= 0.018) & (mfe >= 0.024) & (giveback >= 0.0025) & mature

    market = baseline - 0.0040 - 0.020 * np.maximum(unreal, 0.0)
    maker0 = exit0 - 0.0010
    maker2 = exit2 - 0.0012
    maker4 = exit4 - 0.0018
    close50_market = 0.45 * market + 0.55 * hold - 0.0060 - 0.060 * mae
    close50_maker2 = 0.45 * maker2 + 0.55 * hold - 0.0045 - 0.050 * mae
    twap_maker2 = 0.25 * maker2 + 0.75 * hold - 0.0065 - 0.020 * hold_norm
    hold_guard = hold - np.maximum(0.0, baseline - hold) * 0.06 - 0.020 * giveback - 0.004 * hold_norm

    invalid = np.full_like(hold, -1e6, dtype=np.float32)
    market = np.where(near_sl | (exit_adv >= 0.0040) | (giveback >= 0.010), market, invalid)
    maker0 = np.where(exit_context, maker0, invalid)
    maker2 = np.where(exit_context, maker2, invalid)
    maker4 = np.where(exit_context, maker4, invalid)
    close50_market = np.where(partial_context & near_sl, close50_market, invalid)
    close50_maker2 = np.where(partial_context, close50_maker2, invalid)
    twap_maker2 = np.where(twap_context, twap_maker2, invalid)
    y = np.stack(
        [
            hold_guard,
            market,
...
def _apply_exit_fill(
    ...
    *,
    ...
    if action_name in {"market_close_100", "close_50_market"}:
        exit_px = _market_exit_price(df, i, pos, slip_base)
        exit_fee = fee_base
        route = "exit_market_direct_close"
    else:
        arm_name = ARM_BY_EXPANDED_ACTION.get(action_name, "exit2_pen0")
    ...
    *,
    ...
    if force_exit:
        if effective_sl > 0.0 and unreal <= -abs(effective_sl):
            return "market_close_100"
        return action_name if action_name != "hold_with_guard" else "maker_close_100_2"
    giveback = max(0.0, float(mfe - unreal))
    near_sl = effective_sl > 0.0 and unreal <= -0.55 * abs(effective_sl)
    profit_lock = unreal >= 0.006 and giveback >= max(0.0020, 0.18 * max(mfe, 0.0))
    if action_name == "market_close_100":
        if near_sl or giveback >= 0.010 or unreal <= -0.010:
            return action_name
        return "maker_close_100_2" if profit_lock else "hold_with_guard"
    if action_name.startswith("maker_close_100"):
        if profit_lock or near_sl or unreal >= 0.010 or mae <= -0.012:
            return action_name
        return "hold_with_guard"
    if action_name in {"close_50_market", "close_50_maker2", "twap_maker2"}:
        if reduce_steps >= 1 or hold - last_reduce_hold < 8:
            return "hold_with_guard"
        if unreal < 0.010 or mfe < 0.015 or giveback < 0.0015 or hold < 3:
            return "hold_with_guard"
        if action_name == "twap_maker2" and not (unreal >= 0.018 and mfe >= 0.024 and giveback >= 0.0025):
            return "close_50_maker2"
        if action_name == "close_50_market" and not near_sl:
            return "close_50_maker2"
        return action_name
    return action_name
    ...
    for i in range(0, len(df) - 2):
        ...
        if pos != 0:
            ...
            if base_reason or hold >= int(policy.min_hold):
                ...
                if base_reason and action_name == "hold_with_guard":
                    action_name = "market_close_100"
                if effective_sl > 0.0 and unreal <= -abs(effective_sl):
                    action_name = "market_close_100"
                if max_hold > 0 and hold >= max_hold and action_name == "hold_with_guard":
                    action_name = "market_close_100"
                rl_action_counts[action_name] = rl_action_counts.get(action_name, 0) + 1
                if action_name != "hold_with_guard":
                    frac = 1.0
                    if action_name.startswith("close_50"):
                        frac = 0.5
                        partial_exits += 1
                    elif action_name == "twap_maker2":
                        frac = min(0.34, 1.0)
                        twap_actions += 1
...
def main() -> int:
    ...
    fixed_front_cfg = alpha3.ImmediateLimitConfig(
        "entry2_exit4_pen0_fee20",
        "next_open",
        2.0,
        4.0,
        0.0,
    ...
    dataset_summary = {
        **dataset_meta,
        ...
        "reward_design": {
            "market_close_100": "baseline immediate limit fallback target minus direct taker/urgency cost.",
            "maker_close_100": "exit0/2/4 OHLC immediate-limit proxy targets minus maker offset risk cost.",
            "partial_close": "Masked unless profitable and mature; 45% realized close value plus 55% hold continuation with larger MAE/action penalty.",
            "twap_maker2": "Masked unless strong profit plus giveback; 25% maker2 close value plus 75% hold continuation with time/action penalty.",
            "hold_with_guard": "Hold continuation with mild exit-advantage/giveback/age penalty to prevent premature churn.",
        },
    }
    ...
    val_fixed_front = alpha3._metrics_signal_limit(
        val_df,
        stack["parent"],
        stack["jackpot_model"],
        stack["add_cfg"],
    ...
    rows: list[dict[str, Any]] = [
        {
            ...
            "name": "alpha3_baseline_exit2_pen05",
        ...
        {
            "feature_mode": "control",
            "name": "alpha3_fixed_front_run_exit4_pen0",
            "selection_score": _score(val_fixed_front),
            "val_cost1_pnl": val_fixed_front["cost1"]["pnl"],
            "val_cost1_mdd": val_fixed_front["cost1"]["mdd"],
            "val_cost1_trades": val_fixed_front["cost1"]["trades"],
            "val_cost2_pnl": val_fixed_front["cost2"]["pnl"],
            "val_cost3_pnl": val_fixed_front["cost3"]["pnl"],
        },
    ]
    ...
    for mode_i, mode in enumerate(FEATURE_MODES):
        ...
        model_out = OUT_DIR / f"dsac_exit_owner_v3_{mode}.pt"
            ...
            rows.append(
                {
                    ...
                    **asdict(policy),
                    ...
                    "val_cost1_twap_actions": metrics["cost1"].get("twap_actions", 0),
    ...
    fixed_front = alpha3._metrics_signal_limit(
        eval_df,
        stack["parent"],
        stack["jackpot_model"],
        stack["add_cfg"],
    ...
    experiments = [
        ...
        {"name": "alpha3_fixed_front_run_exit4_pen0", "config": asdict(fixed_front_cfg), "metrics": fixed_front, "score": _score(fixed_front)},
        {
            "name": f"alpha3_dsac_exit_owner_v3::{selected_mode}::{selected_policy.name}",
    ...
    report = {
        ...
        "design": {
            ...
            "scope": "Alpha3 entry stack frozen. DSAC exit owner chooses hold-with-guard, market close, maker close, partial close, or TWAP-style reduce-only actions.",
            "feature_design": "Feature mode is fixed to state16_topbase20 from Alpha3 exit-owner feature analysis 20260515.",
            "limitations": "No L2/orderbook/queue inputs or labels by request. Maker fills use existing OHLC immediate-limit proxy; partial/TWAP rewards are synthetic approximations and are now masked/gated to avoid churn.",
    ...
    audit = {
        ...
        "notes": [
            ...
            "Partial close, TWAP, market close, and hold-with-guard actions are included with reward masks and runtime gates; partial/TWAP labels are synthetic approximations from DP close/hold targets.",
