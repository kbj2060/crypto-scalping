"""ETH Omega4.6.1 FULL greedy router with BOTH components (h48qual + zig075) swapped to JM lambda=4
regime3, each VAL-tuned to its best quality_threshold from the {0.40,0.45,0.50,0.55,0.60} grid:
h48qual=q055 (VAL +7.45%, best of the grid on the parent's own report), zig075=q040 (VAL +17.74%).
Extends rerun_eth_greedy_router_regime_jmlam4_20260809.py's pattern (same window cap, same
common-timestamp alignment fix) to swap BOTH components instead of just h48qual.
"""
...
OUT_DIR = ROOT / "tmp/eth_greedy_router_regime_jmlam4_full_20260809"
...
def run_router(tag: str, use_jm: bool, common_ts: pd.Series) -> dict:
    ...
    if use_jm:
        ...
        h48qual_pred_csv = JM_H48QUAL_DIR / "oos_predictions_q055.csv"
        zig075_pred_csv = JM_ZIG075_DIR / "oos_predictions_q040.csv"
    else:
...
def main() -> int:
    ...
    jm_h48_ts = set(pd.to_datetime(pd.read_csv(JM_H48QUAL_DIR / "oos_predictions_q055.csv", usecols=["timestamp"])["timestamp"]))
    jm_zig_ts = set(pd.to_datetime(pd.read_csv(JM_ZIG075_DIR / "oos_predictions_q040.csv", usecols=["timestamp"])["timestamp"]))
    ...
    result_jm = run_router("jmlam4_both", use_jm=True, common_ts=common_ts_series)
    out = {
        "window": [START, END],
        "common_bars": len(common_ts),
        "note": "BOTH h48qual (q055) and zig075 (q040) swapped to JM regime3, VAL-tuned thresholds. Window capped by EVAL_CSV coverage.",
        "baseline_wide24_full_router": result_baseline,
        "jmlam4_both_components_full_router": result_jm,
    }
