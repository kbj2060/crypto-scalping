...
BTC_KLINES = ROOT / "data/btc_5m_1year.csv"
ETH_KLINES = ROOT / "data/eth_5m_1year.csv"
BTC_CANON = ROOT / "data/splits/year_oos/btc_features_2024_2026.csv"
BTC_FUNDING_DIR = ROOT / "data/research/funding_extracted/BTCUSDT"
PIVOT_CACHE = ROOT / "tmp/btc_regime_label_conditional_lift_20260902/btc_pivots.parquet"
SCALES = (6, 12, 24, 48)
...
OUT_DIR = ROOT / "tmp/btc_regime_label_conditional_lift_20260902"


def load_btc_funding_z() -> pd.DataFrame:
    frames = [pd.read_csv(p) for p in sorted(BTC_FUNDING_DIR.glob("BTCUSDT-fundingRate-*.csv"))]
...
def build_btc_pivots() -> pd.DataFrame:
    ...
    frame = pd.read_csv(BTC_CANON, usecols=["timestamp", "open", "high", "low", "close"],
...
def main() -> None:
    ...
    eth = pd.read_csv(ETH_KLINES, usecols=["timestamp", "high", "low"], parse_dates=["timestamp"])
    frame = compute_signals(raw, btc_df=eth, funding_df=load_btc_funding_z())
    pivots = build_btc_pivots()
    print(f"BTC evidence frame {len(frame):,} bars | pivots {len(pivots):,} "
