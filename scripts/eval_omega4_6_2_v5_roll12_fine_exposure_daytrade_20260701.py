#!/usr/bin/env python3
"""Fine exposure sweep for Omega 4.6.2 v5 roll12 bracket day-trade branch."""
...
ROLL12_MODULE_PATH = ROOT / "scripts/eval_omega4_6_2_v5_roll12_bracket_daytrade_20260701.py"
MODEL_ID = "omega4_6_2_v5_roll12_fine_exposure_daytrade_20260701"
REFERENCE_MODEL_ID = "omega4_6_2_v5_roll12_bracket_daytrade_20260701"
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll12_fine_exposure_daytrade_20260701.md"
MAX_ROLL_HOURS = 12.0
TP_MOVE = 0.030
SL_MOVE = 0.040
...
def write_markdown(report: dict[str, Any]) -> None:
    ...
    text = f"""# Omega 4.6.2 v5 Roll12 Fine Exposure Daytrade - 2026-07-01
...
- TP/SL: `{selected["roll12_tp_move"]:.4f}` / `{selected["roll12_sl_move"]:.4f}`
...
def main() -> int:
    roll12 = load_module("omega462_roll12_for_fine", ROLL12_MODULE_PATH)
    source = roll12.load_module("omega462_roll16_source_for_roll12_fine", roll12.SOURCE_MODULE_PATH)
    v1 = source.load_module("omega462_loss_cluster_v1_for_roll12_fine", source.V1_MODULE_PATH)
    segment_mod = source.load_module("omega462_segment_for_roll12_fine", source.SEGMENT_MODULE_PATH)
    stop_mod = v1.load_module("omega462_stop_mod_for_roll12_fine", v1.STOP_MODULE_PATH)
    ...
    for long_factor in [0.70, 0.80, 0.85, 0.90, 0.95, 1.00]:
        for short_factor in [1.00, 1.02, 1.04, 1.06, 1.08, 1.10, 1.12]:
            for cap_notional in [4.10, 4.20, 4.30, 4.40]:
                ...
                for governor in governors:
                    ...
                    row = {
                        ...
                        "roll12_tp_move": TP_MOVE,
                        "roll12_sl_move": SL_MOVE,
    ...
    ranking = pd.DataFrame(rows).sort_values(
        ["research_daytrade_gate_pass", "selection_score", "validation_pnl"],
    ...
    status = (
        "RESEARCH_ROLL12_FINE_EXPOSURE_PASS"
        if bool(selected["research_daytrade_gate_pass"])
        else "NO_ROLL12_FINE_EXPOSURE_PASSING_CANDIDATE"
    )
    safe = (
        f"{selected['exposure_spec']}__{selected['segment_governor_spec']}__"
        f"tp{float(selected['roll12_tp_move']):.3f}_sl{float(selected['roll12_sl_move']):.3f}"
    ).replace(".", "p").replace("/", "_")
    ranking_path = OUT_DIR / "roll12_fine_exposure_daytrade_ranking.csv"
    top20_path = OUT_DIR / "roll12_fine_exposure_daytrade_top20.csv"
