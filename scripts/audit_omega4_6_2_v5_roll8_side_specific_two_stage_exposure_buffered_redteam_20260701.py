#!/usr/bin/env python3
"""Red-team audit for Omega 4.6.2 v5 roll8 two-stage buffered exposure branch."""
...
RANKING = OUT_DIR / "roll8_two_stage_exposure_buffered_ranking.csv"
TOP20 = OUT_DIR / "roll8_two_stage_exposure_buffered_top20.csv"
PRIOR_RUNTIME_AUDIT = (
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll8_side_specific_two_stage_exposure_buffered_redteam_20260701.md"
TOL = 1.0e-8
...
def exposure_replay(
    ...
    spec_match = bool(
        (candidate_df["two_stage_exposure_spec"].astype(str) == str(selected["exposure_spec"])).all()
    )
    return {
...
def write_markdown(payload: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- Validation MDD buffer floor: `{selected['validation_mdd_buffer_floor']}%`",
        "",
...
def main() -> None:
    audit = load_module("omega462_loss_cluster_audit_for_two_stage_exposure", AUDIT_HELPER_PATH)
    eval_mod = load_module("omega462_two_stage_exposure_eval_for_audit", EVAL_MODULE_PATH)
    ...
    audit.add_check(
        ...
        == "validation_primary_two_stage_exposure_overlay_with_validation_mdd_buffer_and_oos_safety_gate; fresh_holdout_required",
        "research_blocker",
    ...
    audit.add_check(
        ...
        == "among research-gated exposure overlays, sort by validation_pnl, oos_pnl, validation_mdd; OOS is a safety/tie-break key only after validation gate and MDD buffer",
        "research_blocker",
        {"observed": report.get("selection_rule")},
    )
    audit.add_check(
        checks,
        "selected_exposure_gate_pass",
        selected.get("research_two_stage_exposure_gate_pass") is True,
        "research_blocker",
        {"observed": selected.get("research_two_stage_exposure_gate_pass")},
    )
    ...
    val_replay = exposure_replay(ref_validation_df, validation_df, selected, eval_mod)
    oos_replay = exposure_replay(ref_oos_df, oos_df, selected, eval_mod)
    audit.add_check(
        checks,
    ...
    audit.add_check(
        ...
        and validation["mdd"] >= float(selected["validation_mdd_buffer_floor"])
        and oos["mdd"] >= -20.0
    ...
    verdict = (
        "FULL_LIVE_PASS"
        if full_live_pass
        else "RESEARCH_ROLL8_TWO_STAGE_EXPOSURE_BUFFERED_PASS_FULL_LIVE_BLOCKED"
        if research_pass
        else "REDTEAM_FAIL"
    )
    payload = {
        "audit_id": "omega4_6_2_v5_roll8_side_specific_two_stage_exposure_buffered_redteam_20260701",
