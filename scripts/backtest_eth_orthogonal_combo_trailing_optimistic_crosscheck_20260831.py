...
(backtest_eth_orthogonal_combo_trailing_gridsearch_20260831.py: 73/96 combos passed VAL+OOS, top 4
all share ARM=0.50xATR/Trail=0.10xATR -- the narrowest trail in the grid, exactly the case
feedback_intrabar_ordering_optimistic_pessimistic_bracket_20260830.md flags as needing this check
most). The grid search used core.causal_futures_backtest's standard engine, whose
_resolve_trade_trailing already checks the adverse stop level BEFORE using the same bar's favorable
extreme to arm/tighten the trail -- i.e. it is ALREADY the "pessimistic" convention in this
project's own established terminology.

This script recomputes exit price-move for the EXACT SAME trades the grid search actually took
(same decision indices, same SL/ARM/Trail), but under the OPPOSITE ("optimistic": favorable extreme
updates the trail FIRST, stop checked after) convention -- isolating the intrabar-ordering effect
alone, not confounded by a different skip-while-open trade set. Same resolve_optimistic() as
liquidity_sweep's own crosscheck script (backtest_eth_liquidity_sweep_topdown_trailing_optimistic_
crosscheck_20260830.py), verbatim.
"""
...
CANDIDATES = [
    {"sl": 4.0, "arm": 0.5, "trail": 0.1},
    {"sl": 3.5, "arm": 0.5, "trail": 0.1},
    {"sl": 3.0, "arm": 0.5, "trail": 0.1},
    {"sl": 2.5, "arm": 0.5, "trail": 0.1},
]


def log(msg: str) -> None:
    print(f"[orthogonal_combo_optimistic_crosscheck] {msg}", flush=True)
...
def resolve_optimistic(side: int, entry: float, high: np.ndarray, low: np.ndarray, close: np.ndarray,
    ...
    swapped: this bar's favorable extreme updates peak/arm/trail FIRST, stop checked after."""
    if side > 0:
