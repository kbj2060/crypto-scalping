...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Unified DSAC 26D state ablation and saliency analysis")
    p.add_argument("--csv", default="data/rl_training_2025_unified.csv")
...
def prepare_df(path: str, train_ratio: float, use_val_split: bool, limit: int) -> pd.DataFrame:
    df = pd.read_csv(path)
...
def rollout(df: pd.DataFrame, actor: GaussianActor, device: str, mask_dim: int | None = None) -> dict:
    ...
    trades = int(getattr(env, "total_trades", 0))
    wins = int(getattr(env, "win_trades", 0))
    return {
        "pnl_pct": pnl_pct,
        "mdd_pct": mdd_pct,
        "trades": trades,
...
def main() -> int:
    args = parse_args()
    df = prepare_df(args.csv, args.train_ratio, args.use_val_split, args.limit)
