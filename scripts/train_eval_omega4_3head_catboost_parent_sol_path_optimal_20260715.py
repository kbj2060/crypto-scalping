"""Same CatBoost 3-head parent as train_eval_omega4_3head_catboost_parent_sol_20260713.py
(identical direction/quality labels, still SOL's zigzag_action -- WHERE a segment exists is
still decided by zigzag, unchanged), but the exit-head's training target is swapped from the
heuristic entry_label_terminal_giveback rule to the DP suffix-maximum optimal-stopping label
(omega4_exit_label_path_optimal_sol_20260715.build_exit_dataset_entry_label_path_optimal):
within each zigzag-identified segment, the exit label is the Bellman-optimal stop/hold decision
computed from the fully realized future price path inside that segment.

This is the "DP as a refinement of zigzag-identified segments" design (chosen after full
DP-label replacement of direction was tried and failed -- see build_sol_dp_trajectory_labels_*
_20260715.py write-ups) -- direction stays zigzag's (learnable, coarse), only the WHEN-TO-EXIT
decision is upgraded from a hand-tuned heuristic to the exact oracle solution.
"""
...
import omega4_exit_label_path_optimal_sol_20260715 as path_optimal  # noqa: E402
...
MODEL_ID = "sol_omega4_3head_catboost_parent_path_optimal_20260715"
...
def main() -> int:
    ...
    ap.add_argument("--seed", type=int, default=260715)
    ap.add_argument("--out-suffix", default="zig075_path_optimal_q070")
    args = ap.parse_args()
    ...
    report = {
        ...
        "baseline_model": "sol_omega4_3head_catboost_parent_20260713_zig075_q070",
        "research_only": True,
        ...
        "exit_label": {"mode": "entry_label_path_optimal_dp_suffix_max", "exit_edge_min": float(args.exit_edge_min), "diag": exit_diag},
