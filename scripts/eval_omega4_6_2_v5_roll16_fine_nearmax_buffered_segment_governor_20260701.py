#!/usr/bin/env python3
"""Near-max buffered branch from the Omega 4.6.2 v5 roll16 fine exposure sweep."""
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll16_fine_nearmax_buffered_segment_governor_20260701.md"
MAX_VALIDATION_PNL_GAP_PP = 0.5
MIN_VALIDATION_MDD = -19.05
...
def write_markdown(report: dict[str, Any]) -> None:
    ...
    text = f"""# Omega 4.6.2 v5 Roll16 Fine Near-Max Buffered Segment Governor - 2026-07-01
...
- validation MDD at least `{MIN_VALIDATION_MDD:.2f}%`,
...
| Max hold h | `{source_best["validation_max_hold_hours"]:.4f}` | `{selected["validation_max_hold_hours"]:.4f}` | `{source_best["oos_max_hold_hours"]:.4f}` | `{selected["oos_max_hold_hours"]:.4f}` |
...
- Near-max ranking: `{report["artifacts"]["ranking"]}`
...
def main() -> int:
    fine = load_module("omega462_roll16_fine_for_nearmax", FINE_MODULE_PATH)
    roll16 = fine.load_module("omega462_roll16_for_nearmax", fine.ROLL16_MODULE_PATH)
    v1 = roll16.load_module("omega462_loss_cluster_v1_for_nearmax", roll16.V1_MODULE_PATH)
    segment_mod = roll16.load_module("omega462_segment_for_nearmax", roll16.SEGMENT_MODULE_PATH)
    stop_mod = v1.load_module("omega462_stop_mod_for_nearmax", v1.STOP_MODULE_PATH)
    if not SOURCE_REPORT.exists() or not SOURCE_RANKING.exists():
        raise FileNotFoundError("run the roll16 fine exposure evaluator before near-max selection")
    ...
    source_best = source_report["selected_variant"]
    best_validation_pnl = float(source_best["validation_pnl"])
    eligible = passed[
        (passed["validation_pnl"] >= best_validation_pnl - MAX_VALIDATION_PNL_GAP_PP)
        & (passed["validation_mdd"] >= MIN_VALIDATION_MDD)
        & (passed["exposure_long_factor"] <= float(source_best["exposure_long_factor"]) + 1.0e-12)
        & (abs(passed["exposure_short_factor"] - float(source_best["exposure_short_factor"])) <= 1.0e-12)
        & (abs(passed["exposure_cap_notional"] - float(source_best["exposure_cap_notional"])) <= 1.0e-12)
        & (passed["segment_governor_spec"] == str(source_best["segment_governor_spec"]))
    ].copy()
    if eligible.empty:
        raise RuntimeError("no candidate met the near-max buffered validation rule")
    eligible["nearmax_selection_score"] = (
        eligible["validation_pnl"]
        + 0.2 * (eligible["validation_mdd"] - MIN_VALIDATION_MDD)
    )
    nearmax_ranking = eligible.sort_values(
        ["nearmax_selection_score", "validation_pnl"], ascending=[False, False]
    ).reset_index(drop=True)
    selected = nearmax_ranking.iloc[0].to_dict()
    ...
    selected["nearmax_buffered_branch_rule_pass"] = True
    ...
    ranking_path = OUT_DIR / "roll16_fine_nearmax_buffered_segment_governor_ranking.csv"
    val_out = OUT_DIR / f"validation_{safe}_ledger.csv"
    oos_out = OUT_DIR / f"oos_{safe}_ledger.csv"
    report_path = OUT_DIR / "report.json"
    nearmax_ranking.to_csv(ranking_path, index=False)
    ...
    report = {
        ...
        "reference_model_id": SOURCE_MODEL_ID,
        "parent_model_id": PARENT_MODEL_ID,
        "created_at_utc": datetime.now(timezone.utc).isoformat(),
        "selection_scope": "validation_nearmax_buffered_with_oos_safety_gate; fresh_holdout_required",
        "selection_rule": {
            "max_validation_pnl_gap_pp": MAX_VALIDATION_PNL_GAP_PP,
            "min_validation_mdd": MIN_VALIDATION_MDD,
            "source_short_factor": float(source_best["exposure_short_factor"]),
            "source_cap_notional": float(source_best["exposure_cap_notional"]),
            "source_segment_governor_spec": str(source_best["segment_governor_spec"]),
        },
        ...
        "variants_eligible": int(len(nearmax_ranking)),
