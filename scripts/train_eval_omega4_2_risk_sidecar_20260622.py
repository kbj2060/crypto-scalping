...
MODEL_ID = "omega4_2_trade_risk_sidecar_20260622"
...
def _risk_feature_frame(
    ...
    *,
    ...
    for col in list(numeric_src.columns):
        if numeric_src[col].dtype == object or pd.api.types.is_string_dtype(numeric_src[col]):
...
def main() -> int:
    ...
    if str(args.selection_scope) != "validation_only":
        raise RuntimeError(
            "risk sidecar promotion selection must be validation_only; "
            f"got {str(args.selection_scope)!r}"
        )
    eligible = [
        r
        for r in rows
    ...
    if not eligible:
        raise RuntimeError(
            "no eligible validation-only risk mapping: "
            f"trades >= {val_trade_floor}, validation_mdd >= {validation_mdd_floor:.4f}, "
            f"min_avg_notional={float(args.min_validation_avg_notional):.4f}, "
            f"max_avg_notional={float(args.max_validation_avg_notional):.4f}"
        )
    ...
    if not full_eligible:
        raise RuntimeError(
            "no eligible risk mapping after full validation replay: "
            f"trades >= {val_trade_floor}, validation_mdd >= {validation_mdd_floor:.4f}"
        )
    if str(args.selection_objective) == "log_risk":
        selected_full = max(
