...
MODEL_ID = "hf_v13_teacher_distilled_entry_parent_v38_1_20260512"
...
DEFAULT_OUT_DIR = ROOT / "data/ensemble/supervised/hf_v13_teacher_distilled_entry_parent_v38_1_20260512"
DEFAULT_REPORT = ROOT / "data/ensemble/reports/hf_v13_teacher_distilled_entry_parent_v38_1_20260512_summary.json"
DEFAULT_AUDIT = ROOT / "data/ensemble/reports/hf_v13_teacher_distilled_entry_parent_v38_1_20260512_audit.json"
DEFAULT_GRID = ROOT / "data/ensemble/reports/hf_v13_teacher_distilled_entry_parent_v38_1_20260512_grid.csv"
...
def _nearest_bucket_index(values: np.ndarray, buckets: tuple[float, ...]) -> np.ndarray:
...
def main() -> int:
    ...
    teacher = joblib.load(args.parent_model)
    ...
    cfg = _parent_cfg()
    print(f"[{MODEL_ID}] building teacher labels", flush=True)
    teacher_dec = predict_policy_frame(teacher, train_df, close=_close(train_df))
    valid = np.arange(SEQ_LEN - 1, len(train_df), 3, dtype=np.int64)
    y = {
        "action": teacher_dec["action"].iloc[valid].to_numpy(dtype=np.int64),
        "quality": teacher_dec["quality_score"].iloc[valid].to_numpy(dtype=np.float32),
        "notional": _nearest_bucket_index(teacher_dec["notional_exposure"].iloc[valid].to_numpy(dtype=np.float64), cfg.notional_buckets),
    }
    meta = {
        "candidates": int(len(valid)),
        "stride_bars": 3,
        "label_source": str(args.parent_model),
        "teacher_action_counts": pd.Series(y["action"]).value_counts().sort_index().to_dict(),
    }
    ...
    torch.save(
        {
            ...
            "training_meta": meta,
            "teacher_parent_model": str(args.parent_model),
        },
        model_path,
    )
    pd.DataFrame(
    ...
    report = {
        "model_id": MODEL_ID,
        "design": "Teacher-distilled Deep Entry Parent Lite. A sequence Transformer imitates the promoted margin110 parent for entry direction, with auxiliary teacher quality/notional heads. Runtime ablations add conviction gate, learned sizing, risk throttle, and dynamic exit one role at a time.",
