...
# 🔁2026-09-19 복원 (f2eb2377 ponytail-audit 에서 삭제됐다가 되살림). 되살린 이유는 V자
# 급등락이 TabPFN -> HGB 로 바뀌면서 **경제성 게이트를 다시 돌려야 해서**다 -- 더 이상 고아가
# 아니다. 시뮬레이터(simulate_trailing_scalar/_vec)는 **한 글자도 안 고쳤다**.
#
# 딱 하나 바뀐 것: 아래 `_fl` 체인(bp_floor_sweep -> feasibility -> tier0 -> variant_screen,
# 4파일)이 전부 같은 커밋에서 삭제돼 import 가 죽는다. 그 체인이 준 건 **상수 5개**뿐이라
# 직접 적었다(값은 삭제 시점 원본 그대로). 이 파일의 `main()` 은 `_feas`/`_vs` 가 필요해서
# 더는 못 돌지만, 새 드라이버
# (`backtest_eth_v_rebound_hgb_costgate_20260919.py`)가 호출 모집단을 자기가 만들어 넣는다.
_fl = _vs = _feas = None            # main() 전용. 시뮬레이터는 이 셋을 안 쓴다.
...
FORWARD_BARS = 200
PROBA_THRESHOLD = 0.5
SEEDS = [20260901, 11, 22, 33, 44]                     # _fl.SEEDS (삭제 시점 원본)
FEATURE_COLUMNS = None                                 # 드라이버가 자기 것을 쓴다
