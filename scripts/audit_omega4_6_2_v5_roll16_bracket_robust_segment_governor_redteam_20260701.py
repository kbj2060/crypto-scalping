#!/usr/bin/env python3
"""Red-team audit for Omega 4.6.2 v5 robust roll16 bracket branch."""
...
RANKING = OUT_DIR / "robust_roll16_bracket_segment_governor_ranking.csv"
EVAL_SCRIPT = (
    ROOT / "scripts/eval_omega4_6_2_v5_roll16_bracket_robust_segment_governor_20260701.py"
)
PRIOR_RUNTIME_AUDIT = (
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll16_bracket_robust_segment_governor_redteam_20260701.md"
TOL = 1.0e-8
...
def write_markdown(payload: dict[str, Any]) -> None:
    ...
    for split in ["validation", "oos"]:
        lines.append(
            "| "
            f"{split} | "
            f"{reference[f'{split}_pnl']:.4f}% | {source_best[f'{split}_pnl']:.4f}% | {observed[split]['pnl']:.4f}% | "
            f"{reference[f'{split}_mdd']:.4f}% | {source_best[f'{split}_mdd']:.4f}% | {observed[split]['mdd']:.4f}% | "
...
def main() -> None:
    audit = load_module("omega462_loss_cluster_audit_for_roll16_robust", AUDIT_V1_PATH)
    segment_audit = load_module("omega462_segment_audit_for_roll16_robust", AUDIT_SEGMENT_PATH)
    ...
    audit.add_check(
        checks,
        "selection_scope_discloses_robust_and_oos_gate",
        report.get("selection_scope") == "validation_robust_branch_with_oos_safety_gate; fresh_holdout_required",
        "research_blocker",
    ...
    audit.add_check(
        checks,
        "robust_selection_rule_pass",
        selected.get("robust_branch_rule_pass") is True
        and selected["validation_pnl"] >= source_best["validation_pnl"] - report["selection_rule"]["max_validation_pnl_gap_pp"]
        and selected["validation_mdd"] >= report["selection_rule"]["min_validation_mdd"]
        and selected["exposure_cap_notional"] <= report["selection_rule"]["max_cap_notional"] + 1.0e-12,
        "research_blocker",
        {"selection_rule": report["selection_rule"], "selected": selected},
    )
    ...
    audit.add_check(
        checks,
        "improves_24h_reference_pnl_and_hold",
    ...
    verdict = (
        "FULL_LIVE_PASS"
        if full_live_pass
        else "RESEARCH_ROBUST_PASS_FULL_LIVE_BLOCKED"
        if research_pass
        else "REDTEAM_FAIL"
    )
    payload = {
        "audit_id": "omega4_6_2_v5_roll16_bracket_robust_segment_governor_redteam_20260701",
