#!/usr/bin/env python3
"""Audit causal online native portfolio bandit gate."""
...
ART_DIR = ROOT / "tmp/causal_regen_20260516/portfolio_online_bandit_gate_native_20260709"
REPORT_PATH = ART_DIR / "report.json"
OUT_JSON = ART_DIR / "redteam_audit.json"
OUT_MD = ROOT / "docs/audits/portfolio_online_bandit_gate_native_redteam_20260709.md"
...
def main() -> int:
    ...
    res = report["results"]
    if res["oos_extended"]["pnl"] < 0.0:
        _issue(issues, "P1", "oos_negative_pnl", f"OOS PnL={res['oos_extended']['pnl']:.2f}%")
    ...
    if res["oos_extended"]["mdd"] < -25.0:
        _issue(issues, "P1", "oos_mdd_budget", f"OOS MDD={res['oos_extended']['mdd']:.2f}%")
    audit = {
        "audit_id": "portfolio_online_bandit_gate_native_redteam_20260709",
    ...
    lines = [
        "# Portfolio Online Bandit Gate Native Red-Team Audit - 2026-07-09",
        "",
