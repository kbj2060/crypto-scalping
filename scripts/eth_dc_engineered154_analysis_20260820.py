#!/usr/bin/env python3
"""정리(VIF112)+조합(RIT30)+금융ML문헌(12)=154피쳐 최종 엔지니어링 피쳐셋 5시드 분석 --
개별 조건부 방향정확도 + 5시드 딥앙상블, DC 원본(158피쳐/동일 TRAIN_CSV/EVAL_CSV/epochs=2)
베이스라인과 비교."""
...
SEEDS = [153027316, 560569608, 451695778, 931719773, 392936629]
...
def main() -> None:
    ...
    print(f"\n[engineered154 개별5시드] cond_acc: {min(accs):.1f}~{max(accs):.1f}% "
          f"(DC원본(158피쳐) 베이스라인: 48.2~51.4%)", flush=True)
    ...
    print(f"\n[engineered154 5시드 딥앙상블] n_bars={ens['n_bars']:,} LONG%={ens['long_pct']:.1f}% "
          f"cond_acc={ens['cond_acc']:.1f}% pnl={ens['pnl']:+.2f} mdd={ens['mdd']:+.2f} "
          f"trades={ens['trades']} wr={ens['wr']:.3f}", flush=True)

    out_path = ROOT / "tmp/eth_dc_engineered154_analysis_20260820.json"
