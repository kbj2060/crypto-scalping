#!/usr/bin/env python3
"""Red-team audit for Omega 4.6.2 v5 roll2 OOS-balanced branch."""
...
ROLL2_EVAL_PATH = ROOT / "scripts/eval_omega4_6_2_v5_roll2_side_specific_two_stage_exposure_hold_compressed_20260701.py"
...
RANKING = OUT_DIR / "roll2_oos_balanced_ranking.csv"
TOP20 = OUT_DIR / "roll2_oos_balanced_top20.csv"
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll2_side_specific_two_stage_exposure_oos_balanced_redteam_20260701.md"
TOL = 1.0e-8
...
def replay_compare(expected: pd.DataFrame, observed: pd.DataFrame) -> dict[str, Any]:
    ...
    return {"pass": bool(id_match and all(value <= TOL for value in diffs.values())), "id_match": id_match, "max_abs_diffs": diffs}
...
def write_markdown(payload: dict[str, Any]) -> None:
    ...
    lines.extend(
        [
            ...
            "## Replay Checks",
            "",
            f"- Validation replay: `{payload['replay']['validation']['pass']}`",
            f"- OOS replay: `{payload['replay']['oos']['pass']}`",
            "",
            "## Artifacts",
            "",
...
def main() -> None:
    audit = load_module("omega462_loss_cluster_audit_for_roll2_oos_balanced", AUDIT_HELPER_PATH)
    roll2_eval = load_module("omega462_roll2_eval_for_oos_balanced_audit", ROLL2_EVAL_PATH)
    ...
    missing = {
        ...
        for name, path in {
            ...
            "ranking": RANKING,
            "top20": TOP20,
            "roll2_eval_script": ROLL2_EVAL_PATH,
    ...
    audit.add_check(
        ...
        report.get("selection_scope") == "roll2_validation_nearmax_oos_balanced; fresh_holdout_required",
        "research_blocker",
    ...
    audit.add_check(
        checks,
        "selected_gate_pass",
        selected.get("research_roll2_oos_balanced_gate_pass") is True
        and selected.get("research_roll2_hold_compressed_gate_pass") is True,
        "research_blocker",
        {
            "oos_balanced": selected.get("research_roll2_oos_balanced_gate_pass"),
            "base_gate": selected.get("research_roll2_hold_compressed_gate_pass"),
        },
    )
    ...
    replay_val, replay_oos = roll2_eval.build_selected_ledgers(selected)
    val_replay = replay_compare(replay_val, validation_df)
    oos_replay = replay_compare(replay_oos, oos_df)
    audit.add_check(
        checks,
        "roll2_oos_balanced_replay_parity",
    ...
    audit.add_check(
        checks,
        "oos_balanced_contract",
        validation["pnl"] >= 100.0
        and oos["pnl"] >= 100.0
        and validation["mdd"] >= -20.0
        and oos["mdd"] >= -20.0
        and validation["max_hold_hours"] <= 2.0
        and oos["max_hold_hours"] <= 2.0
    ...
    audit.add_check(checks, "runtime_native_replay_complete", native_status.startswith("PASS"), "live_blocker", {"inherited_status": native_status, "prior_audit": str(PRIOR_RUNTIME_AUDIT)})
    audit.add_check(
        checks,
        "fresh_holdout_walkforward_complete",
        fresh_available,
        "live_blocker",
        {"fresh_holdout_available": fresh_available, "reason": fresh.get("fresh_holdout_unavailable_reason"), "prior_audit": str(PRIOR_RUNTIME_AUDIT)},
    )
    ...
    verdict = (
        "FULL_LIVE_PASS"
        if full_live_pass
        else "RESEARCH_ROLL2_OOS_BALANCED_PASS_FULL_LIVE_BLOCKED"
        if research_pass
        else "REDTEAM_FAIL"
    )
    payload = {
        "audit_id": "omega4_6_2_v5_roll2_side_specific_two_stage_exposure_oos_balanced_redteam_20260701",
        ...
        "checks": checks,
        "replay": {"validation": val_replay, "oos": oos_replay},
