...
MODEL_ID = "btc_omega4_3head_parent72_loose_entry_quality_exitonly_20260806"
...
def _prepare_frames(
    *,
    ...
    feature_cols = [c for c in omega._numeric_feature_cols(train_all, eval_df) if c not in ("swing_transition_prob",) and c not in parent.EXIT_ONLY_COLS]
    ...
    s_train_label = parent._exit_state_input(train_df, feature_cols)
    return {
        "train_raw": train_raw,
        "val_raw": val_raw,
...
def _build_exit_dataset_entry_label_terminal_giveback(
    ...
    *,
    ...
    # pos_tp/pos_sl must reflect the ACTUAL live ATR-adaptive barrier (LIVE_ATR_CFG), not the
    # stale fixed BASE_TEMPLATE take_profit/stop_loss -- 2026-08-18 fix, see docs/experiments/
    # eth_odyssey4_exit_head_liveatr_barrier_and_label_reaudit_20260818.md.
...
def main() -> int:
    ...
    ap.add_argument("--disable-exit-only-feature", action="store_true", help="ablation: EXIT_ONLY_COLS=[] so the exit head never sees swing_transition_prob either, isolating retrain-seed noise from the feature's own effect")
    args = ap.parse_args()
    ...
    # risk_margin/risk_leverage=None: bootstrap training, no sidecar for this bundle can exist yet
    # -- see _build_exit_dataset_entry_label_terminal_giveback's docstring.
    ...
    report = {
        ...
        "input_contract": {
            ...
            "exit_only_feature_count": len(parent.EXIT_ONLY_COLS),
            "total_features": len(base_cols) + len(parent.POS_COLS) + len(parent.EXIT_ONLY_COLS),
            "position_cols": parent.POS_COLS,
            "exit_only_cols": parent.EXIT_ONLY_COLS,
    ...
    torch.save({"models": models, "base_cols": base_cols, "pos_cols": parent.POS_COLS, "exit_only_cols": parent.EXIT_ONLY_COLS, "config": parent.CFG.__dict__}, out_dir / "true_3head_tabm_bundle.pt")
