...
HORIZON = 30            # live_evidence_signal_metalabel_20260829.py::METALABEL_SIGNALS 확립값(150분)
GAP = 12                # 같은 출처. 인과적 첫 발동 판정에 쓴다(뒤만 봄).
...
SEED = 20260904
OUT = ROOT / "data/research/eth_liq_sweep_econ_label_screen_20260904/report.json"
...
def main() -> int:
    ...
    if bcol not in sig.columns or tcol not in sig.columns:
        log(f"❌컬럼 없음. 후보: {[c for c in sig.columns if SIGNAL in c][:10]}")
        return 1
    ...
    log(f"  long (봉,측면) {len(long):,}행 · split {dict(long['split'].value_counts())}")
    ...
    log(f"  피쳐 {X.shape[1]}/{len(TIER0)}개")

    from sklearn.ensemble import HistGradientBoostingClassifier
    from sklearn.metrics import roc_auc_score

    print(f"\n{'라벨':>22s}{'양성률TR':>10s}{'양성률VA':>10s}{'평균bp':>10s}{'무작위bp':>10s}"
          f"{'VAL AUC':>10s}{'롱AUC':>8s}{'숏AUC':>8s}")
    rep = {}
    for name, net in labels.items():
        ...
        if len(np.unique(y[tr])) < 2 or len(np.unique(y[va])) < 2:
            print(f"{name:>22s}{base_tr:10.3f}{base_va:10.3f}{mean_bp:10.2f}{rand_bp:10.2f}"
                  f"{'단일클래스':>10s}")
            continue
        ...
        print(f"{name:>22s}{base_tr:10.3f}{base_va:10.3f}{mean_bp:10.2f}{rand_bp:10.2f}"
              f"{auc:10.4f}{side_auc['long']:8.3f}{side_auc['short']:8.3f}")
    ...
    best = max((v["val_auc"] for v in rep.values()), default=0.0)
    print()
    log(f"⭐최고 VAL AUC {best:.4f}")
    log("판정: " + ("❌BTC와 같은 결론(방향력 없음) -- 여기서 종료 권고"
                   if best < 0.55 else "✅유의미 -- TabPFN/정식관문으로 진행 검토"))
    OUT.parent.mkdir(parents=True, exist_ok=True)
    OUT.write_text(json.dumps({
        "signal": SIGNAL, "horizon": HORIZON, "gap": GAP, "cost_bp": COST_BP,
        "n_candidates": int(len(ii)), "n_train": int(tr.sum()), "n_val": int(va.sum()),
        "labels": rep, "best_val_auc": best, "oos_touched": False,
