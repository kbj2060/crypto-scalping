...
MODEL_ID = "omega1_2_1_dual_side_cap_sweep_20260612"
    ...
    *,
    ...
    for i in range(0, len(frame) - 2):
        if pos.side != 0:
            ...
            if route == "rotation":
                pos = hr._maybe_rotation_floor(pos, unreal, sidecap.ROT_SPEC)
            ...
            if not reason and route == "runner":
                if int(pos.side) < 0 and held >= SHORT_CAP_BARS and unreal >= SHORT_CAP_MIN_UNREAL:
                    reason = "short_static_profit_cap_exit"
                elif int(pos.side) > 0 and int(long_cap_bars) > 0 and held >= int(long_cap_bars) and unreal >= float(long_cap_min_unreal):
                    reason = "long_static_profit_cap_exit"
            if reason:
        ...
        if not bool(active[i]):
            continue
        route = "rotation" if hr._rotation_proba(rot_model, rot_cols, frame, state, dec, int(i)) >= 0.65 else "runner"
        ...
        if entered:
            ...
            if route == "rotation":
                pos = hr._apply_rotation(pos, sidecap.ROT_SPEC)
        else:
            route = "runner"
...
def main() -> int:
    ...
    configs = [{"variant": "short_cap2000_min0.035_only", "long_cap_bars": 0, "long_cap_min_unreal": 0.0}]
    for bars in (2048, 2400, 2650, 2880, 3072):
        for min_unreal in (0.00, 0.02, 0.04, 0.06, 0.08, 0.10):
            configs.append({"variant": f"short_cap2000_min0035_long_cap{bars}_min{min_unreal:.2f}", "long_cap_bars": int(bars), "long_cap_min_unreal": float(min_unreal)})
    ...
    for candidate_id, cfg in enumerate(configs):
        ...
        for split in ("validation", "oos"):
            ...
            metrics, ledger = _simulate(
                ...
                rot_model=rot_model,
                rot_cols=rot_cols,
                long_cap_bars=int(cfg["long_cap_bars"]),
                long_cap_min_unreal=float(cfg["long_cap_min_unreal"]),
            )
    ...
    baseline = ranking.loc[ranking["variant"].eq("short_cap2000_min0.035_only")].iloc[0]
    ...
    ranking.to_csv(OUT_DIR / "dual_side_cap_ranking.csv", index=False)
    ...
    balanced.to_csv(OUT_DIR / "dual_side_cap_balanced.csv", index=False)
    ...
    report = {
        "model_id": MODEL_ID,
        "purpose": "Add conservative long cap on top of current best short-cap horizon router.",
        "rot_diag": diag,
        "baseline_short_cap": baseline.to_dict(),
        ...
        "artifacts": {
            "out_dir": str(OUT_DIR),
            "ranking": str(OUT_DIR / "dual_side_cap_ranking.csv"),
            "balanced": str(OUT_DIR / "dual_side_cap_balanced.csv"),
