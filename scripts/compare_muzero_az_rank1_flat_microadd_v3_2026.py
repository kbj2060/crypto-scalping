...
MODEL_ID = "muzero_az_rank1_flat_microadd_v3_2026"
DEFAULT_REPORT_OUT = ROOT / "data/ensemble/reports/muzero_az_rank1_flat_microadd_v3_2026.json"
DEFAULT_MODEL_DIR = ROOT / "data/ensemble/supervised/muzero_az_rank1_flat_microadd_v3"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Compare current-top MuZero/AZ Stage2+AZ Exit with flat-only micro-add sleeve v3.")
...
def main() -> int:
    ...
    joblib.dump(
        ...
        args.model_dir / "rank1_flat_microadd_v3.pkl",
    )
    selector_payload = {
    ...
    report = {
        ...
        "note": "Frozen current-top MuZero/AZ Stage2+AZ Exit baseline preserved exactly; deterministic current-bar-vote micro-adds can fire only when exact sequential baseline replay is flat.",
        ...
        "eval": {
            ...
            "rank1_flat_microadd_v3": candidate_eval,
        ...
        "calibration_audit": {"selector_artifact": str(args.model_dir / "threshold_selector.json"), "model_artifact": str(args.model_dir / "rank1_flat_microadd_v3.pkl")},
        ...
        "limitations": ["Funding, liquidation, and maintenance-margin path are not first-class in this loop-3 implementation."],
    }
