...
⚠️⚠️ **경제성 주장 철회 (2026-09-01 라벨 격자 스크리닝 중 확인)**: 위 표의 bp 수치를 포함해
이 신호의 **모든 경제성 수치는 라벨이 붙은 행(전체의 ~53%) 기준**이다. 라이브는 라벨 유무와
무관하게 모든 봉을 채점하므로 이 수치를 라이브에 그대로 적용하면 안 된다. 전체 모집단에서
방향뒤집기 대조군을 ARM>=1.0 80셀 전수로 돌린 결과:

    VAL x 전체봉   정방향 28/80  뒤집기  0/80   (중앙 -0.81 / -4.75)  방향 맞음
    VAL x 라벨행만  정방향 62/80  뒤집기  0/80   (중앙 +4.71 / -18.91) 방향 맞음
    OOS x 전체봉   정방향 21/80  뒤집기 31/80   (중앙 -1.86 / -0.60)  ⚠️역전
    OOS x 라벨행만  정방향 28/80  뒤집기 23/80   (중앙 -2.65 / -6.02)  방향 유지

아티팩트 무영향 구간(ARM=1.5)만 보면 OOS 전체봉에서 **정방향 4 대 뒤집기 16**이다. 즉 경제적
엣지는 결과가 깨끗하게 분류되는 봉에만 있고, excluded-middle 봉에서는 방향성이 없으며 OOS에서는
오히려 역방향이다. **VAL 전체봉도 중앙값 -0.81** -- 전체 모집단 경제성은 애초에 강했던 적이 없다.

**분류 성능을 부정하는 건 아니다**(OOS AUC 0.7051 유지). 무너진 것은 그 판정을 트레일링 스톱
매매로 옮겼을 때의 방향성이다. 임계값 0.60은 분류 운영점(precision 0.713/0.683)으로는 여전히
근거가 있으므로 유지한다. 근거:
data/research/eth_v_rebound_label_grid_stage1_20260901/{stage3_oos,deployed_oos_flip_diagnostic}.json
