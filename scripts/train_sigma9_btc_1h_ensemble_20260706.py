#!/usr/bin/env python3
"""Sigma9 BTC leg: 5-seed HGB ensemble on BTC's own 1h trend-scanning dataset (28 OHLCV-only
features, no funding/OI -- see build_1h_trendscan_dataset_btc_20260706.py). Mirrors
train_sigma3_1h_ensemble_20260705.py exactly (same hyperparameters, same seeds, same tape
schema including zeroed fallback_* columns for apply_quality_threshold compatibility).
"""
...
DATA_DIR = ROOT / "tmp/causal_regen_20260516/sigma9_1h_btc_20260706"
...
def load_all() -> pd.DataFrame:
    frames = [pd.read_parquet(DATA_DIR / f"sigma9_btc_1h_{y}.parquet") for y in (2024, 2025, 2026)]
...
def main() -> int:
    ...
    tape = pd.DataFrame({
        ...
        "primary_action": final_action, "primary_side": side, "primary_expert": "sigma9btc",
    ...
    out_path = OUT_DIR / "tape_btc_ensemble.parquet"
    tape.to_parquet(out_path, index=False)
    print(f"wrote BTC ensemble tape {len(tape)} rows -> {out_path}", flush=True)
    print(f"nonzero pct: {(tape['primary_side'] != 0).mean():.3f}", flush=True)
    (OUT_DIR / "report_btc_ensemble.json").write_text(json.dumps({"seeds": SEEDS, "n_features": len(feat_cols)}, indent=2))
    return 0
