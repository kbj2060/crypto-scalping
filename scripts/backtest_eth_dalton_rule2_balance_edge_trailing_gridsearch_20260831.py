#!/usr/bin/env python3
"""Trailing-stop cost-gate grid search for dalton_rule2_balance_edge (final label config
HORIZON=30/GAP=12/K=1.90), using this repo's STANDARD backtest engine
(core.causal_futures_backtest.simulate_single_position, ATR-trailing-stop exit mode) -- same
engine/convention as taker/short_term_return_z/liquidity_sweep/orthogonal_combo's own cost-gates.

Per this project's default policy (feedback_trailing_stop_default_costgate_logic_20260830):
trailing stop is the FIRST exit structure tried. Tests ALL raw HORIZON=30/GAP=12 fires (bottom=
long, top=short) unconditional on the trained model's own probability -- dalton uses simple
single-K labeling (no exclude-middle), so the existing classifier CSV already IS the full raw-fire
population (4,157 rows, hit=0/1 covers everything -- unlike orthogonal_combo's exclude-middle CSV).

IMPORTANT sizing choice: uses `atr_pct_288` (288-bar mean ATR), NOT the 14-bar `atr_pct` feature,
for SL/ARM/Trail multiples -- same reasoning as the CLAUDE.md-documented label-threshold fix
(eth_dalton_rule2_balance_edge_metalabel_v1_20260830.md): dalton's own trigger condition gates on
LOW 14-bar ATR-percentile (<=30th pctile of trailing 288-bar history), so `atr_pct` at any dalton
fire bar is mechanically suppressed (verified ~5th percentile of ALL bars, not just dalton fires) --
sizing stops off this self-referentially-suppressed value would produce unrealistically tight
SL/trail distances relative to the volatility that actually unfolds afterward. `atr_pct_288` is a
representative (non-self-suppressed) volatility estimate at the same bar, already computed and
saved in the fires CSV (used for the label's own K-multiple threshold).

HOLDOUT (2026-04-01+) excluded entirely -- single-touch policy, not yet earned.
"""
...
def log(msg: str) -> None:
    print(f"[dalton_trailing_gridsearch] {msg}", flush=True)
...
def main() -> int:
    ...
    log(f"{len(klines)} klines, {len(fires)} candidate fires (excl. HOLDOUT), sizing off atr_pct_288")

    decision_indices = fires["pos"].to_numpy(dtype=np.int64)
    is_long = (fires["side"] == "bottom").to_numpy()
    scores = np.where(is_long, 1.0, -1.0)
    atr = fires["atr_pct_288"].to_numpy()
    ...
    out_dir = ROOT / "tmp/eth_dalton_rule2_balance_edge_metalabel_20260830"
