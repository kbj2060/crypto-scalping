...
MODEL_ID = "omega1_2_8e_full_retrain_numeric_cash_sleeve_sltp_regression_heads_20260616"
OUT_DIR = ROOT / "tmp/causal_regen_20260516" / MODEL_ID
RISK = sleeve.FallbackRisk("sltp_reg_heads_n081_l2_h192", 0.052, 0.028, 0.81, 2.0, 192)
BASELINE_ID = "omega1_2_true_3head_tabm_20260603_full_retrain_cash_alpha43_20260608"
TP_BOUNDS = (0.018, 0.078)
SL_BOUNDS = (0.010, 0.042)
UTILITY_CFGS = (
        ...
        rows.append(
            {
                ...
                "long_net": float(long_d["net"]),
                "short_net": float(short_d["net"]),
                "long_mfe": float(long_d["mfe"]),
                "short_mfe": float(short_d["mfe"]),
...
def _sltp_targets_from_path_labels(path_labels: pd.DataFrame, risk: sleeve.FallbackRisk) -> tuple[pd.DataFrame, dict[str, Any]]:
    labels = path_labels.copy()
    for side in ("long", "short"):
        mfe = labels[f"{side}_mfe"].astype(float).to_numpy(dtype=np.float64)
        mae = labels[f"{side}_mae"].astype(float).to_numpy(dtype=np.float64)
        net = labels[f"{side}_net"].astype(float).to_numpy(dtype=np.float64)
        tp_raw = np.where(mfe > 0.0, mfe * 0.95, float(risk.take_profit) * 0.50)
        sl_raw = np.where(mae < 0.0, -mae * 1.05, float(risk.stop_loss) * 0.50)
        tp_raw = np.where(net > 0.0, np.maximum(tp_raw, float(risk.take_profit) * 0.50), np.minimum(tp_raw, float(risk.take_profit)))
        sl_raw = np.where(net > 0.0, np.maximum(sl_raw, float(risk.stop_loss) * 0.50), np.minimum(sl_raw, float(risk.stop_loss)))
        labels[f"{side}_tp_target"] = np.clip(tp_raw, TP_BOUNDS[0], TP_BOUNDS[1])
        labels[f"{side}_sl_target"] = np.clip(sl_raw, SL_BOUNDS[0], SL_BOUNDS[1])
    diag = {
        "tp_bounds": list(TP_BOUNDS),
        "sl_bounds": list(SL_BOUNDS),
        "long_tp_mean": float(labels["long_tp_target"].mean()) if len(labels) else 0.0,
        "short_tp_mean": float(labels["short_tp_target"].mean()) if len(labels) else 0.0,
        "long_sl_mean": float(labels["long_sl_target"].mean()) if len(labels) else 0.0,
        "short_sl_mean": float(labels["short_sl_target"].mean()) if len(labels) else 0.0,
    }
    return labels, diag
...
def _fit_predict_bounded_head(
    ...
    *,
    ...
    y = np.full(len(x_val), float(default), dtype=np.float64)
    y[idx] = labels[col].to_numpy(dtype=np.float64)
    val_pred = np.full(len(x_val), float(default), dtype=np.float64)
    folds_meta: list[dict[str, Any]] = []
    for fold_id, (tr, va) in enumerate(_chron_folds(idx)):
        model = HistGradientBoostingRegressor(max_iter=120, learning_rate=0.035, max_leaf_nodes=9, l2_regularization=2.0, random_state=int(seed + fold_id * 10))
        model.fit(x_val.iloc[tr].to_numpy(dtype=np.float64), y[tr])
        pred = model.predict(x_val.iloc[va].to_numpy(dtype=np.float64)).astype(np.float64)
        val_pred[va] = np.clip(pred, bounds[0], bounds[1])
        folds_meta.append({"fold": int(fold_id), "train_rows": int(len(tr)), "val_rows": int(len(va))})

    model = HistGradientBoostingRegressor(max_iter=120, learning_rate=0.035, max_leaf_nodes=9, l2_regularization=2.0, random_state=int(seed + 101))
    model.fit(x_val.iloc[idx].to_numpy(dtype=np.float64), y[idx])
    oos_pred = np.clip(model.predict(x_oos.to_numpy(dtype=np.float64)).astype(np.float64), bounds[0], bounds[1])
    diag = {
        "target_col": col,
        "bounds": list(bounds),
        "default": float(default),
        "val_mean": float(np.mean(val_pred)),
        "oos_mean": float(np.mean(oos_pred)),
        "folds": folds_meta,
    }
    return val_pred, oos_pred, diag


def _fit_sltp_heads(x_val: pd.DataFrame, x_oos: pd.DataFrame, labels: pd.DataFrame, risk: sleeve.FallbackRisk) -> tuple[dict[str, np.ndarray], dict[str, np.ndarray], dict[str, Any]]:
    val: dict[str, np.ndarray] = {}
    oos: dict[str, np.ndarray] = {}
    diag: dict[str, Any] = {}
    specs = (
        ("long_tp", "long_tp_target", 282101, TP_BOUNDS, float(risk.take_profit)),
        ("long_sl", "long_sl_target", 282102, SL_BOUNDS, float(risk.stop_loss)),
        ("short_tp", "short_tp_target", 282201, TP_BOUNDS, float(risk.take_profit)),
        ("short_sl", "short_sl_target", 282202, SL_BOUNDS, float(risk.stop_loss)),
    )
    for name, col, seed, bounds, default in specs:
        val[name], oos[name], diag[name] = _fit_predict_bounded_head(x_val, x_oos, labels, col, seed=seed, bounds=bounds, default=default)
    return val, oos, diag


def _risk_for_row(side: int, i: int, risk: sleeve.FallbackRisk, preds: dict[str, np.ndarray]) -> sleeve.FallbackRisk:
    if int(side) > 0:
        tp = float(preds["long_tp"][int(i)])
        sl = float(preds["long_sl"][int(i)])
    else:
        tp = float(preds["short_tp"][int(i)])
        sl = float(preds["short_sl"][int(i)])
    return sleeve.FallbackRisk("sltp_regression_row", tp, abs(sl), float(risk.notional), float(risk.leverage), int(risk.max_hold_bars))


def _metrics_with_regression_sltp(
    frame: pd.DataFrame,
    dec: pd.DataFrame,
    ...
    for i in range(0, len(frame) - 2):
        ...
        if bool(active[i]):
            ...
            if side_action in (sleeve.ACTION_LONG, sleeve.ACTION_SHORT) and conf >= float(threshold):
                ...
                row_risk = _risk_for_row(side, i, risk, risk_preds)
...
def main() -> int:
    ...
    sltp_labels, sltp_diag = _sltp_targets_from_path_labels(path_labels, RISK)
    val_risk_preds, oos_risk_preds, sltp_fit_diag = _fit_sltp_heads(x_val, x_oos, sltp_labels, RISK)
    ...
    diagnostics: dict[str, Any] = {
        "mode": "full_retrain_parent_numeric_cash_sleeve_with_sltp_regression_heads",
        ...
        "sltp_label_targets": sltp_diag,
        "sltp_regression_heads": sltp_fit_diag,
        "ev_labels": ev_diag,
    }
    ...
    for cal_q in (0.50, 0.65, 0.80):
        ...
        for ev_min in (0.001, 0.002, 0.003, 0.004):
            ...
            val_m = _metrics_with_regression_sltp(val_payload["frame"], val_payload["dec"], RISK, val_risk_preds, val_ev_a, val_ev_c, 0.0, fee=fee, slip=slip, cost_mult=3.0)
            oos_m = _metrics_with_regression_sltp(oos_payload["frame"], oos_payload["dec"], RISK, oos_risk_preds, oos_ev_a, oos_ev_c, 0.0, fee=fee, slip=slip, cost_mult=3.0)
            ...
            for cfg_id, (uvl, uvs, uol, uos) in utility_preds.items():
                for utility_min in (-0.001, 0.0, 0.001, 0.002):
                    for margin_min in (0.0, 0.001, 0.002):
                        ...
                        val_m = _metrics_with_regression_sltp(val_payload["frame"], val_payload["dec"], RISK, val_risk_preds, val_a, val_c, 0.0, fee=fee, slip=slip, cost_mult=3.0)
                        oos_m = _metrics_with_regression_sltp(oos_payload["frame"], oos_payload["dec"], RISK, oos_risk_preds, oos_a, oos_c, 0.0, fee=fee, slip=slip, cost_mult=3.0)
