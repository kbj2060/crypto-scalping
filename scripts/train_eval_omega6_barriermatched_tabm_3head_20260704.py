#!/usr/bin/env python3
# Forked from train_eval_omega6_tabm_3head_20260703.py: identical architecture/bundle contract
# and training pipeline. Only change: the direction/quality head label (zigzag_action) is
# loaded from scripts/build_zigzag_action_labels_barrier_matched_20260704.py's output instead
# of hard._build_frame()'s zigzag-swing-pivot label. That label simulates the EXACT trigger
# mechanics the frozen v2 winner's backtest uses (tp=15x/sl=5x ATR(192), 24h time-stop,
# notional=0.6) so the model is trained to predict "does this specific barrier trade win"
# instead of "are we in a zigzag swing" -- see docs/model_contracts/omega6_synthesis_v1_20260703_contract.md
# "Root-cause analysis" section for why this mismatch mattered.
...
MODEL_ID = "omega6_barriermatched_3head_tabm_20260704"
OUT_DIR = ROOT / "tmp/causal_regen_20260516" / MODEL_ID
SPLIT_TS = pd.Timestamp("2025-10-01")
BARRIER_MATCHED_LABEL_DIR = ROOT / "tmp/causal_regen_20260516/zigzag_action_labels_barrier_matched_20260704"
...
def _prepare_frames(*, disable_tp_sl: bool) -> dict[str, Any]:
    ...
    label_2025 = pd.read_csv(BARRIER_MATCHED_LABEL_DIR / "zigzag_action_labels_2025.csv", usecols=["timestamp", "zigzag_action"], parse_dates=["timestamp"])
    label_2026 = pd.read_csv(BARRIER_MATCHED_LABEL_DIR / "zigzag_action_labels_2026.csv", usecols=["timestamp", "zigzag_action"], parse_dates=["timestamp"])
