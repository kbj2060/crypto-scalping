...
from scripts.train_alpha7_regime3_current_moe_feature_variants_20260601 import RISK_COLS, _overlay_required  # noqa: E402
...
CMAMBA_ROUTER_NAME = "regime3_cmamba_h6_future"
OUT_DIR = ROOT / "tmp/causal_regen_20260516/alpha7_active_clean_regime3_full_contract_moe_20260601"
...
DROP_PREFIXES = ("clean_regime4_", "regime4_pred_", "regime3_pred_")
DROP_EXACT = {TP_COL}
...
def _dropped_cols(cols: list[str]) -> list[str]:
    clean = set(_clean_cols(cols))
    return [c for c in cols if c not in clean]


def _assert_strict_feature_contract(cols: list[str], *, name: str) -> None:
    bad = [
        c for c in cols
        if c in DROP_EXACT or c.startswith(DROP_PREFIXES) or any(tok in c for tok in DROP_TOKENS)
    ]
    if bad:
        raise RuntimeError(f"{name} contains forbidden feature columns: {bad[:40]}")


def _load_frames_with_full_regime3() -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    train, eval_df, current_report = _load_router_frames(ROUTER_NAME)
    cmamba = ROUTERS[CMAMBA_ROUTER_NAME]
    cmamba_cols = [*cmamba["cols"], *cmamba["extra_cols"]]
    train, cmamba_train = _overlay_required(train, cmamba["train"], cmamba_cols, tag="train_cmamba_h6")
    eval_df, cmamba_eval = _overlay_required(eval_df, cmamba["eval"], cmamba_cols, tag="eval_cmamba_h6")
    ...
    return train, eval_df, {
        "current_router": current_report,
        "cmamba_h6": {"train": cmamba_train, "eval": cmamba_eval},
...
def _train_pair(
    *,
    ...
    _assert_strict_feature_contract(primary_cols, name=f"{name}_primary")
    _assert_strict_feature_contract(fallback_cols, name=f"{name}_fallback")
...
def main() -> int:
    ...
    cmamba_cols = [*ROUTERS[CMAMBA_ROUTER_NAME]["cols"], *ROUTERS[CMAMBA_ROUTER_NAME]["extra_cols"]]
    full_regime3_cols = [*current_cols, *cmamba_cols, *RISK_COLS]
    primary_cols = list(dict.fromkeys([*_clean_cols(raw_primary_cols), *full_regime3_cols]))
    fallback_cols = list(dict.fromkeys([*_clean_cols(raw_fallback_cols), *full_regime3_cols]))

    base = _train_pair(
        name="baseline_regime3_full",
        train_all=train_all,
        eval_df=eval_df,
    ...
    for idx, expert in enumerate(EXPERT_NAMES):
        ...
        expert_summaries[expert] = {"rows": int(mask.sum()), **pair["summary"]}
    ...
    report = {
        "model_id": MODEL_ID,
        "design": "Strict Alpha7 Regime3 full-sidecar replacement candidate. Regime4-family and tp_sl_action_score are removed; Regime3 current context, CryptoMamba h6 future-context sidecar, and Regime3 h6 stability/risk sidecar are injected into baseline plus every bull/bear/chop expert.",
        "feature_contract": {
            "primary_feature_count": len(primary_cols),
            "fallback_feature_count": len(fallback_cols),
            "regime3_added_cols": full_regime3_cols,
    ...
    print(json.dumps({"report": str(OUT_DIR / "report.json"), "selected": report["selected"], "feature_contract": report["feature_contract"]}, ensure_ascii=False, indent=2, default=_json_default))
    return 0
