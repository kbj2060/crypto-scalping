#!/usr/bin/env python3
"""돌파/되돌림 섀도우 **모델 아티팩트 동결 -- 발현창 60분** (2026-09-09, 사용자 채택).

규칙 (2026-09-08 전 축 검증 종료 후 확정):
    모집단  앵커 `first_fire` -- 8종 증거신호 중 **어느 하나의 첫 발동**(신호별 GAP dedup).
            평가창 실측 25.2건/일.
    발현    앵커 다음 봉 시가를 기준가로, **60분 안에 ±0.75×ATR** 최초 터치.
            그 분이 트리거이고 터치한 쪽이 상단/하단 터치(예측 대상 아님, 관측값).
            실측 25.0건/일 (앵커의 **99.3%** -- 15분 창의 88.6% 에서 올렸다).
            ⭐창을 넓혀도 기존(15분 안 발현) 사건의 성적은 시드 폭 안에서 동일하다
              (짝비교 -0.04/+0.45/+0.79pp). 얻는 것은 커버리지다.
...
## 사전등록 기대치 (발현창 60분 · 시드 20개 워크포워드 · 셔플 B=32)
전건(커버 100%, 25.0건/일)   VAL .5602  OOS .5875  HOLDOUT .5813
  셔플 귀무                       .5350      .5424          .5231
  초과                          +2.52pp    +4.52pp        +5.82pp   (판정 FLAG/FLAG/OK)
상위 절반(12.5건/일)          VAL .6118  OOS .6463  HOLDOUT .6314   초과 +7.95/+10.00/+10.52pp (전부 OK)
⭐시드 20/20 이 세 창 동시에 귀무 위. FAIL 0.
⭐**정확도는 셔플 귀무와 함께 읽는다.** 창마다 클래스 균형이 달라 원시 정확도끼리 비교하면 안 된다.
⚠️15분 창 기준선(.5643/.5934/.5823)은 **다른 모집단**이라 직접 비교하지 않는다.
...
DS = ROOT / "tmp/eth_breakout_nmove_20260909/dataset_nm60.parquet"
KL1 = ROOT / "binance_data/klines/ETHUSDT/ETHUSDT-1m-api.csv"
OUT = ROOT / "data/live/breakout_reversal_shadow_artifact"
ANCHOR, T_MULT, H, NMOVE_MIN = "first_fire", 0.75, 12, 60
...
CHUNK = 4000
RULE_ID = "breakout_reversal_ff075_w60_atr08_h1h_20260909"

# 사전등록 기대치 -- 시드 20개 단일시드 평균과 셔플 귀무 (research_eth_breakout_seed_robustness_20260908)
PREREG = {
    # 2026-09-09 시드 20개 워크포워드 실측 (research_eth_breakout_nmove60_validate_20260909)
    "cov100": {"acc": {"VAL": 0.5602, "OOS": 0.5875, "HOLDOUT_SPENT": 0.5813},
               "null": {"VAL": 0.5350, "OOS": 0.5424, "HOLDOUT_SPENT": 0.5231},
               "per_day": 25.0},
    "cov50":  {"acc": {"VAL": 0.6118, "OOS": 0.6463, "HOLDOUT_SPENT": 0.6314},
               "null": {"VAL": 0.5323, "OOS": 0.5463, "HOLDOUT_SPENT": 0.5262},
               "per_day": 12.5},
}
# 매매 규칙은 2026-09-08 부터 보류 상태다(브라켓 격자 0/196). 아티팩트를 다시 구울 때
# 이 기록이 사라지면 "왜 매매를 안 하나"의 근거가 없어지므로 직전 meta 에서 그대로 물려온다.
PREV_META = ROOT / "data/live/breakout_reversal_shadow_artifact/meta.json"
...
def build_xy():
    d = pd.read_parquet(DS)              # 이미 first_fire · T=0.75 단일 모집단
...
def main() -> int:
    ...
    ap.add_argument("--out", default=None, help="저장 위치(기본은 스테이징, 검증 후 교체한다)")
    a = ap.parse_args()
    global OUT
    OUT = Path(a.out) if a.out else ROOT / "tmp/eth_breakout_nmove_20260909/artifact_staging"
    import joblib
    ...
    prev_strategy = None
    if PREV_META.exists():
        prev_strategy = (json.loads(PREV_META.read_text()) or {}).get("strategy")
        print(f"직전 meta 에서 strategy 물려옴: {(prev_strategy or {}).get('status')}", flush=True)
    d, y, okm = build_xy()
    ...
    meta = {
        ...
        "emergence_window_min": NMOVE_MIN,   # ⭐러너가 이 값을 읽는다(코드 상수와 어긋나지 않게)
        "seeds": SEEDS, "features": feats,
        "n_train": int(len(fit)), "breakout_rate": float(y[fit].mean()),
        "train_span": [str(d["timestamp"].iloc[0]), str(d["timestamp"].iloc[-1])],
        "confidence_tiers": tiers, "prereg": PREREG, "strategy": prev_strategy,
