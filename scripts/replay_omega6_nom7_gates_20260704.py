#!/usr/bin/env python3
"""Pre-registered gate sweep for the m7-free Omega6 L2 retrain, validation-only
(2025-10-01..12-31). Grid intentionally modest (5 thresholds x 3 persistence x 4 tp x 3 sl x
3 cooldown = 540 configs) -- same order of magnitude as the original frozen-winner's "final"
round, not the hundreds-of-thousands-of-comparisons scale. If ANY variant passes all four
pre-registered gates (cost1 & cost3 PnL > 0, MDD >= -20% both tiers, trades >= 60, >= 3 months
with trades), a config from the CENTER of the passing region (not the single best score) will be
frozen and scored exactly once on the untouched 2026-03-02..06-30 window.
"""
...
TAPE_PATH = ROOT / "tmp/causal_regen_20260516/omega6_nom7_extended_decision_tape_20260704/tape.parquet"
OUT_DIR = ROOT / "tmp/causal_regen_20260516/omega6_nom7_gates_20260704"
...
def main() -> int:
    tape = load_tape()
    thresholds = [0.45, 0.50, 0.55, 0.58, 0.62, 0.65, 0.70]
    ...
    for thr, persist, tp_mult, sl_mult, cooldown in itertools.product(
        ...
        cfg = v2.VariantConfig(
            name=f"nom7_p{persist}_qt{thr}_tp{tp_mult}_sl{sl_mult}_cd{cooldown}",
            tp_mode="atr_scaled",
            tp_atr_mult=tp_mult,
            sl_atr_mult=sl_mult,
            sizing_mode="fixed",
    ...
    df.to_csv(OUT_DIR / "nom7_gate_ranking.csv", index=False)
    print(f"\ntotal: {len(df)}  gate_pass: {int(df['gate_pass'].sum())}", flush=True)
    print(df.head(25).to_string(index=False), flush=True)
    print(f"\nfull ranking: {OUT_DIR / 'nom7_gate_ranking.csv'}", flush=True)
    return 0
