...
MODEL_ID = "hf_v13_tree_vs_foundation_target_aware_lgbm_native_quant_tradefloor_v40_10_20260512"
EMBEDDING_CACHE_CONSUMER = "hf_v13_tree_vs_foundation_target_aware_lgbm_native_quant_v40_9_20260512"
...
DEFAULT_OUT_DIR = ROOT / "data/ensemble/supervised/hf_v13_tree_vs_foundation_target_aware_lgbm_native_quant_tradefloor_v40_10_20260512"
DEFAULT_REPORT = ROOT / "data/ensemble/reports/hf_v13_tree_vs_foundation_target_aware_lgbm_native_quant_tradefloor_v40_10_20260512_summary.json"
DEFAULT_AUDIT = ROOT / "data/ensemble/reports/hf_v13_tree_vs_foundation_target_aware_lgbm_native_quant_tradefloor_v40_10_20260512_audit.json"
DEFAULT_GRID = ROOT / "data/ensemble/reports/hf_v13_tree_vs_foundation_target_aware_lgbm_native_quant_tradefloor_v40_10_20260512_grid.csv"
...
def _nan_missing_audit_warnings(audit: dict[str, Any]) -> list[str]:
    warnings_out: list[str] = []
    for warning in list(audit.get("warnings", [])):
        if warning.startswith("missing_train_features_zero_filled:"):
            warnings_out.append(warning.replace("missing_train_features_zero_filled:", "missing_train_features_nan_preserved_for_lgbm:"))
        elif warning.startswith("missing_eval_features_zero_filled:"):
            warnings_out.append(warning.replace("missing_eval_features_zero_filled:", "missing_eval_features_nan_preserved_for_lgbm:"))
        else:
...
class ProjectionSpec:
    ...
    @property
    def name(self) -> str:
        return f"pls_macro{self.macro_dim}_micro{self.micro_dim}_{'dropmicro' if self.drop_raw_micro else 'keepmicro'}"
...
class TreeHParams:
    ...
    def name(self) -> str:
        ...
        smooth = str(self.path_smooth).replace(".", "p")
        extra = "et1" if self.extra_trees else "et0"
        return (
            f"lgbm_ne{self.n_estimators}_lr{lr}_leaves{self.num_leaves}"
            f"_d{self.max_depth}_child{self.min_child_samples}"
            f"_a{str(self.reg_alpha).replace('.', 'p')}_l{str(self.reg_lambda).replace('.', 'p')}"
            f"_sub{str(self.subsample).replace('.', 'p')}_col{str(self.colsample_bytree).replace('.', 'p')}"
            f"_ps{smooth}_{extra}"
...
def _classifier_hp(seed: int, hp: TreeHParams) -> Any:
    return LGBMClassifier(
        ...
        min_split_gain=float(hp.min_split_gain),
...
def _weighted_fit_classifier(model: Any, x: pd.DataFrame, y: np.ndarray, weights: np.ndarray) -> Any:
    if np.unique(y).size < 2:
        return None
    final_step = model.steps[-1][0] if hasattr(model, "steps") else None
    fit_kwargs = {f"{final_step}__sample_weight": weights} if final_step else {"sample_weight": weights}
...
def train_policy_with_hparams(
    ...
    *,
    ...
    quality_step = bundle["quality_model"].steps[-1][0] if hasattr(bundle["quality_model"], "steps") else None
    quality_kwargs = {f"{quality_step}__sample_weight": weights} if quality_step else {"sample_weight": weights}
    bundle["quality_model"].fit(x, np.asarray(y["quality"], dtype=np.float64), **quality_kwargs)
...
def _score_result(cost1: dict[str, Any], cost2: dict[str, Any], cost3: dict[str, Any]) -> float:
    trades = float(cost1.get("trades", 0.0))
    trades_day = float(cost1.get("trades_per_day", 0.0))
    trade_floor_penalty = 0.0
    if trades < 45.0:
        trade_floor_penalty += (45.0 - trades) * 5.0
    if trades_day < 0.45:
        trade_floor_penalty += (0.45 - trades_day) * 400.0
    return (
        float(cost1["pnl"])
...
def _encoded_hparam_grid() -> list[TreeHParams]:
    return [
        TreeHParams(260, 0.020, 31, -1, 120, 0.03, 0.30, 1.50, 0.80, 0.85, 1.0, True, 0.45),
        TreeHParams(320, 0.015, 31, -1, 160, 0.05, 0.80, 2.50, 0.70, 0.75, 3.0, True, 0.55),
        TreeHParams(360, 0.012, 63, -1, 180, 0.05, 1.00, 3.00, 0.70, 0.70, 5.0, True, 0.60),
        TreeHParams(280, 0.018, 63, -1, 120, 0.02, 0.50, 2.00, 0.80, 0.80, 7.0, True, 0.50),
        TreeHParams(420, 0.010, 63, -1, 220, 0.08, 1.50, 4.00, 0.65, 0.70, 10.0, True, 0.62),
        TreeHParams(240, 0.020, 31, -1, 180, 0.08, 1.00, 3.00, 0.70, 0.70, 5.0, False, 0.60),
        TreeHParams(300, 0.016, 63, -1, 140, 0.03, 0.60, 2.50, 0.78, 0.78, 3.0, True, 0.50),
        TreeHParams(300, 0.016, 31, -1, 100, 0.03, 0.40, 2.00, 0.82, 0.82, 2.0, True, 0.45),
        TreeHParams(340, 0.014, 63, -1, 160, 0.04, 0.80, 3.00, 0.75, 0.75, 4.0, True, 0.52),
        TreeHParams(260, 0.018, 31, -1, 90, 0.02, 0.30, 1.80, 0.85, 0.85, 1.0, True, 0.42),
    ]


def _projection_grid() -> list[ProjectionSpec]:
    return [
        ProjectionSpec(1, 1, True),
        ProjectionSpec(2, 2, False),
    ]
...
def main() -> int:
    ...
    x_train_lgbm = _preserve_source_missing_as_nan(x_train_base, train_df, list(x_train_base.columns))
    ...
    train_feat = _preserve_source_missing_as_nan(
        prepare_features(train_df, side_hint=0, close=_close(train_df), feature_cols=feature_cols),
        train_df,
        feature_cols,
    )
    val_feat = _preserve_source_missing_as_nan(
        prepare_features(val_df, side_hint=0, close=_close(val_df), feature_cols=feature_cols),
        val_df,
        feature_cols,
    )
    eval_feat = _preserve_source_missing_as_nan(
        prepare_features(eval_df, side_hint=0, close=_close(eval_df), feature_cols=feature_cols),
        eval_df,
        feature_cols,
    )
    ...
    micro_cols = [c for c in MICRO_PRIORITY if c in train_all.columns and c in eval_df.columns]
    if not micro_cols:
    ...
    train_macro = _extract_macro_embeddings(
        ...
        cache_path=_embedding_cache_path(
            ...
            extra_tag=f"csv={args.train_csv.name}|stride={args.train_stride}|split=train|consumer={EMBEDDING_CACHE_CONSUMER}",
        ),
    ...
    val_macro = _extract_macro_embeddings(
        ...
        cache_path=_embedding_cache_path(
            ...
            window_len=MACRO_LEN,
            extra_tag=f"csv={args.train_csv.name}|split=val|consumer={EMBEDDING_CACHE_CONSUMER}",
        ),
        batch_size=args.embed_batch,
    )
    eval_macro = _extract_macro_embeddings(
        ...
        cache_path=_embedding_cache_path(
            ...
            window_len=MACRO_LEN,
            extra_tag=f"csv={args.eval_csv.name}|split=eval|consumer={EMBEDDING_CACHE_CONSUMER}",
        ),
        batch_size=args.embed_batch,
    )
    ...
    print(f"[{MODEL_ID}] preparing target-aware projection full-parent tuning set", flush=True)
    residual_keep_cols = list(x_train_lgbm.columns)
    ...
    print(f"[{MODEL_ID}] tuning Chronos/Kairos target-aware projection full parent on 2025 Q4 validation", flush=True)
    hparams_grid = _encoded_hparam_grid()
    projection_grid = _projection_grid()
    grid_rows: list[dict[str, Any]] = []
    best_score = -1e18
    best_bundle: dict[str, Any] | None = None
    best_hp: TreeHParams | None = None
    best_spec: ProjectionSpec | None = None
    best_val: dict[str, Any] | None = None
    candidate_idx = 0
    for spec in projection_grid:
        train_macro_f, macro_pls = _fit_pls_scores(train_macro, proj_targets, train_macro, n_components=int(spec.macro_dim))
        val_macro_f = macro_pls.transform(val_macro).astype(np.float32)
        eval_macro_f = macro_pls.transform(eval_macro).astype(np.float32)
        train_micro_f, micro_pls = _fit_pls_scores(train_micro, proj_targets, train_micro, n_components=int(spec.micro_dim))
        val_micro_f = micro_pls.transform(val_micro).astype(np.float32)
        eval_micro_f = micro_pls.transform(eval_micro).astype(np.float32)

        raw_cols = [c for c in residual_keep_cols if (not spec.drop_raw_micro or c not in micro_cols)]
        x_train_full = x_train_lgbm.reset_index(drop=True).reindex(columns=raw_cols).copy()
        ...
        for hp in hparams_grid:
            candidate_idx += 1
            print(f"[{MODEL_ID}] candidate {candidate_idx}/{len(projection_grid) * len(hparams_grid)}: {spec.name} | {hp.name}", flush=True)
            ...
            val_cost1 = backtest_policy_frame(val_full, bundle, fee=cfg.fee, slip=cfg.slip)
            val_cost2 = backtest_policy_frame(val_full, bundle, fee=cfg.fee * 2.0, slip=cfg.slip * 2.0)
            val_cost3 = backtest_policy_frame(val_full, bundle, fee=cfg.fee * 3.0, slip=cfg.slip * 3.0)
            ...
            grid_rows.append(
                {
                    ...
                    **asdict(hp),
                    ...
                    "full_feature_count": int(x_train_full.shape[1]),
            ...
            if score > best_score:
                ...
                best_eval_full = eval_full.copy()

    if best_bundle is None or best_hp is None or best_spec is None or best_val is None:
        raise RuntimeError("target-aware full-parent tuning produced no valid candidate")

    print(f"[{MODEL_ID}] backtesting baseline vs tuned full selection", flush=True)
    full_val = best_val
    full_eval = {f"cost{k}": backtest_policy_frame(best_eval_full, best_bundle, fee=cfg.fee * k, slip=cfg.slip * k) for k in (1, 2, 3)}

    args.out_dir.mkdir(parents=True, exist_ok=True)
    with (args.out_dir / "baseline_tree_parent.pkl").open("wb") as f:
        pickle.dump(baseline_bundle, f)
    with (args.out_dir / "target_aware_full_bundle.pkl").open("wb") as f:
    ...
    comparison = {
        "validation": {
            ...
            "target_aware_full": full_val,
            "delta_cost1_pnl": float(full_val["cost1"]["pnl"] - baseline_val["cost1"]["pnl"]),
            "delta_cost1_mdd": float(full_val["cost1"]["mdd"] - baseline_val["cost1"]["mdd"]),
        },
        "oos_2026": {
            "baseline": baseline_eval,
            "target_aware_full": full_eval,
            "delta_cost1_pnl": float(full_eval["cost1"]["pnl"] - baseline_eval["cost1"]["pnl"]),
            "delta_cost1_mdd": float(full_eval["cost1"]["mdd"] - baseline_eval["cost1"]["mdd"]),
            "delta_cost2_pnl": float(full_eval["cost2"]["pnl"] - baseline_eval["cost2"]["pnl"]),
            "delta_cost3_pnl": float(full_eval["cost3"]["pnl"] - baseline_eval["cost3"]["pnl"]),
        },
        "selected_projection_name": best_spec.name,
        "selected_full_name": best_hp.name,
        "selected_projection_spec": asdict(best_spec),
        "selected_full_hparams": asdict(best_hp),
    ...
    warnings = _nan_missing_audit_warnings(audit)
    audit_for_report = dict(audit)
    audit_for_report["warnings"] = warnings
    final_audit = {
        "status": "pass" if not blocking else "fail",
        "blocking": blocking,
        "warnings": warnings,
        "selection_uses_2026": False,
        "feature_audit": audit_for_report,
        "comparison": comparison,
    }
    report = {
        "model_id": MODEL_ID,
        "design": "Baseline HGB tree versus target-aware projection LightGBM full parent. Baseline uses the original fixed HGB parent settings. The challenger removes KAN, projects Chronos-2 and Kairos_23m embeddings with train-only PLSRegression against action/quality-derived targets, appends those target-aware factors to a residual raw feature set, and trains all parent heads with LightGBM on that feature space. V40.10 keeps V40.9's LightGBM-native missing handling and quant controls, then adds a validation trade floor to avoid selecting extremely sparse low-MDD candidates. Projection spec and full-parent LightGBM hyperparameters are selected on 2025 Q4 validation.",
        "split_policy": "train=2025 Jan-Sep, validation=2025 Q4, OOS=2026 fixed",
        "chronos_model": CHRONOS_MODEL,
        "kairos_model": KAIROS_MODEL,
        "projection_method": "PLSRegression",
        "tree_backend": "LightGBM",
        "projection_targets": ["side_label", "signed_quality", "trade_flag"],
        ...
        "embedding_cache_consumer": EMBEDDING_CACHE_CONSUMER,
        "selection_trade_floor": {"min_validation_trades": 45, "min_validation_trades_day": 0.45},
        "training_meta": meta,
        "grid_size": len(projection_grid) * len(hparams_grid),
        "comparison": comparison,
        "audit": final_audit,
        "artifacts": {
            "baseline_bundle": str(args.out_dir / "baseline_tree_parent.pkl"),
            "target_aware_full_bundle": str(args.out_dir / "target_aware_full_bundle.pkl"),
