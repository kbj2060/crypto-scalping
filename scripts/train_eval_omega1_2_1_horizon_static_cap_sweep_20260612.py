...
MODEL_ID = "omega1_2_1_horizon_static_cap_sweep_20260612"
    ...
    *,
    ...
    for i in range(0, len(frame) - 2):
        if pos.side != 0:
            ...
            if not reason and route == "runner" and int(cap_bars) > 0:
                if int(i) - int(pos.entry_i) >= int(cap_bars) and unreal >= float(cap_min_unreal):
                    reason = "static_profit_cap_exit"
            if reason:
...
def main() -> int:
    ...
    configs = [{"rot_proba": 0.65, "cap_bars": 0, "cap_min_unreal": 0.0, "variant": "horizon_best_no_cap"}]
    for cap_bars in (2048, 2400, 2650, 2880):
            ...
            configs.append(
                {
                    ...
                    "variant": f"horizon_static_cap{cap_bars}_min{min_unreal:.2f}",
                }
            )
    ...
    ranking.to_csv(OUT_DIR / "horizon_static_cap_ranking.csv", index=False)
    balanced = ranking[
    ...
    balanced.to_csv(OUT_DIR / "horizon_static_cap_balanced.csv", index=False)
    ...
    report = {
        "model_id": MODEL_ID,
        "purpose": "Static profit cap on runner positions layered after current best horizon-router.",
        "rot_diag": diag,
        ...
        "artifacts": {
            "out_dir": str(OUT_DIR),
            "ranking": str(OUT_DIR / "horizon_static_cap_ranking.csv"),
            "balanced": str(OUT_DIR / "horizon_static_cap_balanced.csv"),
