...
from scripts.eval_hf_v13_frozen_v27_rule_exit_overlay_v31 import _grid as _v31_grid
from scripts.eval_hf_v13_frozen_v27_rule_exit_overlay_v31 import backtest as v31_backtest
from scripts.train_eval_hf_v13_jackpot_runner_v21_2 import CostRunnerConfig, _predict_cost_runner


MODEL_ID = "hf_v13_deep_alpha_global_local_scout_v36_20260512"
...
DEFAULT_OUT_DIR = ROOT / "data/ensemble/supervised/hf_v13_deep_alpha_global_local_scout_v36_20260512"
DEFAULT_REPORT = ROOT / "data/ensemble/reports/hf_v13_deep_alpha_global_local_scout_v36_20260512_summary.json"
DEFAULT_AUDIT = ROOT / "data/ensemble/reports/hf_v13_deep_alpha_global_local_scout_v36_20260512_audit.json"
DEFAULT_GRID = ROOT / "data/ensemble/reports/hf_v13_deep_alpha_global_local_scout_v36_20260512_grid.csv"
HORIZONS = (12, 24, 48)
...
class DeepAlphaTCN(nn.Module):
    ...
    def forward(self, seq: torch.Tensor) -> torch.Tensor:
        ...
        local_ctx = h[:, :, -1]
        return self.head(torch.cat([global_ctx, local_ctx], dim=1))
...
def _train_model(ds: dict[str, np.ndarray], norm: dict[str, np.ndarray], *, epochs: int) -> DeepAlphaTCN:
    ...
    print(f"[v36] device={device} cuda={torch.cuda.is_available()}", flush=True)
    if torch.cuda.is_available():
        print(f"[v36] gpu={torch.cuda.get_device_name(0)}", flush=True)
    model = DeepAlphaTCN(x.shape[-1]).to(device)
    pin = bool(torch.cuda.is_available())
    loader = DataLoader(TensorDataset(torch.from_numpy(x), torch.from_numpy(y)), batch_size=512, shuffle=True, pin_memory=pin)
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) == 1 or (epoch + 1) % 10 == 0 or (epoch + 1) == epochs:
            print(f"[v36] epoch {epoch + 1:03d}/{epochs} loss={loss_sum / max(batches, 1):.6f}", flush=True)
...
def _predict_all(model: DeepAlphaTCN, df: pd.DataFrame, seq_cols: list[str], norm: dict[str, np.ndarray]) -> np.ndarray:
    ...
    pad = np.zeros((v23.SEQ_LEN - 1, arr.shape[1]), dtype=np.float32)
    padded = np.vstack([pad, arr])
    windows = np.lib.stride_tricks.sliding_window_view(padded, window_shape=v23.SEQ_LEN, axis=0)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="V27 deep alpha candidate expansion sleeve.")
...
def main() -> int:
    args = parse_args()
    print("[v36] loading parent/jackpot/data", flush=True)
    ...
    print(f"[v36] building train set seq_cols={len(seq_cols)}", flush=True)
    train_ds = _build_train_set(train, seq_cols, fee=float(base["fee"]), slip=float(base["slip"]), stride=3)
    norm = _normalizer(train_ds["seq"])
    print(f"[v36] training global+local scout snapshots={len(train_ds['target'])} epochs={args.epochs}", flush=True)
    model = _train_model(train_ds, norm, epochs=args.epochs)
    print("[v36] predicting validation/OOS utilities", flush=True)
    ...
    for cfg in _v31_grid():
        print(f"[v36] v31 validation {cfg.name}", flush=True)
        v1 = v31_backtest(val, bundle, jackpot_model, add_cfg, val_q, cfg, fee=float(base["fee"]), slip=float(base["slip"]), cost_mult=1.0, decisions=val_dec)
        v2 = v31_backtest(val, bundle, jackpot_model, add_cfg, val_q, cfg, fee=float(base["fee"]), slip=float(base["slip"]), cost_mult=2.0, decisions=val_dec)
        v3 = v31_backtest(val, bundle, jackpot_model, add_cfg, val_q, cfg, fee=float(base["fee"]), slip=float(base["slip"]), cost_mult=3.0, decisions=val_dec)
        ...
        if best is None or row["selection_score"] > best["selection_score"]:
            best = row
    selected = type(_v31_grid()[0])(**best["config"])
    metrics: dict[str, Any] = {}
    ledgers: dict[str, str] = {}
    for mult in (1, 2, 3):
        r = v31_backtest(eval_df, bundle, jackpot_model, add_cfg, eval_q, selected, fee=float(base["fee"]), slip=float(base["slip"]), cost_mult=float(mult), decisions=eval_dec, record=(mult == 1))
        if mult == 1:
    ...
    model_path = args.out_dir / "v36_deep_alpha_global_local_scout.pt"
    torch.save({"model_id": MODEL_ID, "architecture": "tcn_global_avg_plus_last_step_concat", "state_dict": model.state_dict(), "seq_cols": seq_cols, "norm": norm, "selected_v31_config": asdict(selected), "add_config": asdict(add_cfg), "parent_model": str(args.parent_model), "jackpot_model": str(args.jackpot_model)}, model_path)
    ...
    audit = {"status": "pass" if not blocking else "fail", "verdict": verdict, "blocking": blocking, "warnings": warnings, "selection_uses_2026": False, "selection_window": "2025-10-01..2025-12-31", "oos_window": "2026 fixed OOS only after selection", "policy": "deep_alpha_global_local_scout_v36_with_v31_exit", "v21_2_preserved": True, "v31_exit_overlay": True, "deep_sleeve_only_when_parent_cash": True, "forbidden_sequence_columns": forbidden_cols, "train_snapshot_count": int(len(train_ds["target"])), "target_mean": np.mean(train_ds["target"], axis=0), "feature_audit": feature_audit, "selected_config": asdict(selected), "metrics": metrics}
    report = {"model_id": MODEL_ID, "design": "V36 replaces V27 AdaptiveAvgPool1d with parallel Global Average + Last Step concat pooling. It trains a new deep scout entry model, then evaluates it through the V31 rule-based dynamic exit overlay.", "parent_model": str(args.parent_model), "jackpot_model": str(args.jackpot_model), "model": str(model_path), "feature_manifest": str(manifest_path), "split_policy": "Train 2025 Jan-Sep; select V31 overlay thresholds on 2025 Oct-Dec; evaluate fixed 2026 OOS only after selection.", "selected_config": asdict(selected), "selection_result": best, "metrics": metrics, "audit": audit, "artifacts": {"model": str(model_path), "manifest": str(manifest_path), "report": str(args.report_out), "audit": str(args.audit_out), "grid": str(args.grid_out), "ledgers": ledgers}}
