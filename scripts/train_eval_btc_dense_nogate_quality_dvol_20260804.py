"""New-axis cheap falsification test (Deribit DVOL, per
project-btc-deribit-dvol-data-acquired-20260804.md): identical architecture/labels/thresholds/
split as scripts/train_eval_btc_dense_nogate_quality_20260804.py (the existing closed-line
baseline, results in tmp/btc_dense_nogate_quality_20260804.csv), with ONLY the feature set
changed: causalfix_final's 98 cols (post-EXCLUDE_COLS) + 7 DVOL-derived cols from
data/splits/year_oos/btc_dvol_features_20260804.parquet
(scripts/build_btc_dvol_features_20260804.py).

Purpose: isolate whether options-market-implied-volatility data has ANY incremental signal for
BTC before building anything more elaborate -- same "cheapest falsification first" principle used
for the (now-closed) cross-sectional panel axis. If this doesn't beat the 98-col-only baseline,
don't invest further in this axis without a different hypothesis.
"""
import sys
import time
...
OUT_CSV = ROOT / "tmp/btc_dense_nogate_quality_dvol_20260804.csv"
...
def main():
    ...
    frame = frame.merge(dvol, on="timestamp", how="left")

    dvol_cols = [c for c in dvol.columns if c != "timestamp"]
    feat_cols = base_feat_cols + dvol_cols
    print(f"feat_cols: {len(base_feat_cols)} base (causalfix_final) + {len(dvol_cols)} DVOL = {len(feat_cols)}")
    n = len(frame)
    ...
    for name, horizon, tp_mult, sl_mult in CONFIGS:
        ...
        n_dvol_in_top20 = sum(1 for c in imp.head(20).index if c in dvol_cols)
        print(f"  top 15 features (long): {list(imp.head(15).index)}")
        print(f"  DVOL features in top 20 by importance: {n_dvol_in_top20}/20")
    ...
    print(f"\n=== [DVOL] Configs with VAL AND OOS both positive (n>=15 each side): {len(both)} ===")
    if len(both):
    ...
    if BASELINE_CSV.exists():
        ...
        merged = out.merge(base, on=["config", "split", "thresh"], suffixes=("_dvol", "_baseline"))
        merged["oos_improvement_pct"] = merged["mean_net_pct_dvol"] - merged["mean_net_pct_baseline"]
        print("\n=== head-to-head: DVOL vs 98-col-only baseline (same config/split/thresh) ===")
        print(merged[["config", "split", "thresh", "n_trades_baseline", "mean_net_pct_baseline",
                       "n_trades_dvol", "mean_net_pct_dvol", "oos_improvement_pct"]]
