...
MODEL_ID = "alpha3_dsac_exit_owner_v4_20260515"
OUT_DIR = ROOT / "data/ensemble/supervised/alpha3_dsac_exit_owner_v4_20260515"
MODEL_OUT = OUT_DIR / "dsac_exit_owner_v4.pt"
REPORT_OUT = ROOT / "data/ensemble/reports/alpha3_dsac_exit_owner_v4_20260515_summary.json"
AUDIT_OUT = ROOT / "data/ensemble/reports/alpha3_dsac_exit_owner_v4_20260515_audit.json"
GRID_OUT = ROOT / "data/ensemble/reports/alpha3_dsac_exit_owner_v4_20260515_grid.csv"
DATASET_OUT = ROOT / "data/ensemble/reports/alpha3_dsac_exit_owner_v4_20260515_dataset.json"
...
def _expanded_targets(base_y: np.ndarray, base_actions: list[str], x_full: np.ndarray, feature_names: list[str]) -> np.ndarray:
    ...
    exit_context = profit_lock | near_sl | ((exit_adv >= 0.0040) & (giveback >= 0.0040))
    partial_context = strong_profit & (giveback >= 0.0040) & mature
    twap_context = (unreal >= 0.026) & (mfe >= 0.034) & (giveback >= 0.0060) & mature

    market = baseline - 0.0060 - 0.025 * np.maximum(unreal, 0.0)
    maker0 = exit0 - 0.0018
    maker2 = exit2 - 0.0020
    maker4 = exit4 - 0.0028
    close50_market = 0.35 * market + 0.65 * hold - 0.0100 - 0.080 * mae
    close50_maker2 = 0.35 * maker2 + 0.65 * hold - 0.0080 - 0.070 * mae
    twap_maker2 = 0.18 * maker2 + 0.82 * hold - 0.0100 - 0.025 * hold_norm
    hold_guard = hold - np.maximum(0.0, baseline - hold) * 0.03 - 0.010 * giveback - 0.002 * hold_norm

    invalid = np.full_like(hold, -1e6, dtype=np.float32)
    market = np.where(near_sl | (giveback >= 0.014), market, invalid)
    ...
    *,
    ...
    if action_name in {"close_50_market", "close_50_maker2", "twap_maker2"}:
        ...
        if unreal < 0.018 or mfe < 0.024 or giveback < 0.0040 or hold < 3:
            return "hold_with_guard"
        if action_name == "twap_maker2" and not (unreal >= 0.026 and mfe >= 0.034 and giveback >= 0.0060):
            return "close_50_maker2"
...
def main() -> int:
    ...
    for mode_i, mode in enumerate(FEATURE_MODES):
        ...
        model_out = OUT_DIR / f"dsac_exit_owner_v4_{mode}.pt"
    ...
    experiments = [
        ...
        {
            "name": f"alpha3_dsac_exit_owner_v4::{selected_mode}::{selected_policy.name}",
    ...
    report = {
        ...
        "design": {
            ...
            "limitations": "No L2/orderbook/queue inputs or labels by request. Maker fills use existing OHLC immediate-limit proxy; partial/TWAP rewards are synthetic approximations and event-gated to avoid churn.",
    ...
    audit = {
        ...
        "notes": [
            ...
            "Partial close, TWAP, market close, and hold-with-guard actions are included with stricter event-gated reward masks and runtime gates; partial/TWAP labels are synthetic approximations from DP close/hold targets.",
