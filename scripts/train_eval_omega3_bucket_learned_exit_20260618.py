...
MODEL_ID = "omega3_bucket_learned_exit_fixedlev2_20260618"
...
def _label_exit_now(arrays: dict[str, np.ndarray], pos: Any, i: int, active: np.ndarray, fee_eff: float, slip_eff: float) -> int:
    ...
    deep_adverse = current_unreal < -0.75 * float(pos.stop_loss)
    return int((giveback_risk and no_upside) or deep_adverse)
    ...
    *,
    ...
    for i in range(0, len(frame) - 2):
        if pos.side != 0:
            ...
            rows.append(_position_features(arrays, pos, i, active, risk, fee_eff, slip_eff))
...
def main() -> int:
    ...
    report = {
        ...
        "method": "Bucket fallback entry/risk model is kept fixed. Hard fallback SL/TP exits are removed. A position-aware exit classifier uses SL/TP distance/progress, unrealized PnL, hold age, and market features to decide learned_exit; parent_takeover and max_hold remain hard safety exits.",
