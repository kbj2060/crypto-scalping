#!/usr/bin/env python3
"""Tier0 피쳐(22+rsi=23개)를 ZDC(지그재그 방향확인) 라벨에 결합.

build_eth_5m_v_rebound_multitrigger_features_tier0_20260831.py(기존 giveback 라벨용)와 완전히
동일한 피쳐 구성/공식을 그대로 재사용(build_indicator_frame() import, sweep_penetration_atr
계산 등 전부 무변경) -- 유일한 차이는 라벨 소스(LABEL_CSV)와 라벨 관련 컬럼(giveback의
fast_move_atr_mult/giveback_ratio/outcome 대신 ZDC의 pivot_type/extreme_idx/confirm_idx/
hit/outcome)뿐. Tier0 피쳐 자체는 트리거 population 레벨이라 어떤 정답라벨을 쓰든 무관.

Run with the quant_ai conda env (torch is a transitive dependency of compute_indicators):
  /home/kbj20/anaconda3/envs/quant_ai/bin/python scripts/build_eth_5m_v_rebound_multitrigger_zigzag_direction_features_tier0_20260901.py
"""
...
FEATURE_COLUMNS = _tier0.FEATURE_COLUMNS + ["rsi"]  # 기존 giveback판과 정확히 동일한 23개
...
def main() -> int:
    ...
    out_path = OUT_DIR / "eth_5m_v_rebound_multitrigger_zigzag_direction_features_tier0.csv"
