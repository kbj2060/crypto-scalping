...
MODEL_ID = "omega1_2_true_3head_tabm_trendscan_20260620"
LABEL_DIR = ROOT / "tmp/causal_regen_20260516/trend_scanning_action_labels_20260531"
OUT_DIR = ROOT / "tmp/causal_regen_20260516" / MODEL_ID


def _add_trendscan_labels(year: int) -> pd.DataFrame:
    ...
    if invalid:
        raise RuntimeError(f"{path} invalid trend-scanning classes: {invalid}")
    return out


def main() -> int:
    if not LABEL_DIR.exists():
        raise RuntimeError(f"missing trend-scanning label dir: {LABEL_DIR}")
    direction_base._add_labels = _add_trendscan_labels
