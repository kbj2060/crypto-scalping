#!/usr/bin/env python3
"""10h side-specific bracket day-trade branch for Omega 4.6.2 v5."""
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll10_side_specific_bracket_daytrade_20260701.md"
MAX_ROLL_HOURS = 10.0
REFERENCE_CAP = 4.20
EPS = 1.0e-12
...
def normalize_roll10(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    out = out.rename(columns={col: col.replace("roll16_", "roll10_") for col in out.columns if col.startswith("roll16_")})
...
def write_markdown(report: dict[str, Any]) -> None:
    ...
    text = f"""# Omega 4.6.2 v5 Roll10 Side-Specific Bracket Daytrade - 2026-07-01

## Method

This branch starts from the same v5 parent and 10h roll contract as `{REFERENCE_MODEL_ID}`, but separates bracket labels by side. Long positions use one TP/SL pair and short positions use another TP/SL pair. Selection remains validation-primary with an OOS safety gate and a declared cap tie-breaker against the reference cap `{REFERENCE_CAP}`.
...
def main() -> int:
    source = load_module("omega462_roll16_source_for_roll10_side", SOURCE_MODULE_PATH)
    v1 = source.load_module("omega462_loss_cluster_v1_for_roll10_side", source.V1_MODULE_PATH)
    segment_mod = source.load_module("omega462_segment_for_roll10_side", source.SEGMENT_MODULE_PATH)
    stop_mod = v1.load_module("omega462_stop_mod_for_roll10_side", v1.STOP_MODULE_PATH)
    ...
    bracket_bases: list[tuple[str, float, float, float, float, pd.DataFrame, pd.DataFrame]] = []
    for name, long_tp, long_sl, short_tp, short_sl in bracket_specs:
        ...
        bracket_bases.append((name, long_tp, long_sl, short_tp, short_sl, val_base, oos_base))

    exposure_specs = [
        segment_mod.ExposureSpec(f"lf{lf:.2f}_sf{sf:.2f}_cap{cap:.2f}", lf, sf, cap)
        for lf in [0.60, 0.70, 0.80, 0.90, 1.00]
        for sf in [0.90, 0.95, 1.00, 1.02, 1.04, 1.06]
    ...
    for bracket_name, long_tp, long_sl, short_tp, short_sl, val_base, oos_base in bracket_bases:
    ...
    selected_base = ranking.iloc[0]
    selected_val_base = next(
        val_base
        for name, _, _, _, _, val_base, _ in bracket_bases
        if name == selected_base["side_bracket_spec"]
    )
    selected_oos_base = next(
        oos_base
        for name, _, _, _, _, _, oos_base in bracket_bases
        if name == selected_base["side_bracket_spec"]
    )
    ...
    status = (
        "RESEARCH_ROLL10_SIDE_SPECIFIC_PASS"
        if bool(selected["research_side_specific_gate_pass"])
        else "NO_ROLL10_SIDE_SPECIFIC_PASSING_CANDIDATE"
    )
    safe = (
        f"{selected['side_bracket_spec']}__{selected['exposure_spec']}__"
        f"{selected['segment_governor_spec']}"
    ).replace(".", "p").replace("/", "_")
    ranking_path = OUT_DIR / "roll10_side_specific_bracket_daytrade_ranking.csv"
    top20_path = OUT_DIR / "roll10_side_specific_bracket_daytrade_top20.csv"
