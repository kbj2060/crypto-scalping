"""ETH Omega4.6.1 FULL greedy router, both components JM-regime3, thresholds chosen to MATCH the
original wide24 activation rate (h48qual 0.4%, zig075 6.1%) rather than VAL PnL -- h48qual=q065
(nonzero_side 0.28%), zig075=q080 (nonzero_side 5.35%). Tests whether preserving JM's stability
advantage while keeping the SAME sparse operating frequency as live recovers the router-level loss
found when thresholds were VAL-PnL-selected (h48qual q055, zig075 q040).
"""
...
OUT_DIR = ROOT / "tmp/eth_greedy_router_regime_jmlam4_matched_20260809"
START, END = "2026-01-01", "2026-02-28"

JM_H48QUAL_DIR = ROOT / "tmp/causal_regen_20260516/omega4_3head_parent72_loose_entry_quality_20260620_regime_jmlam4_20260809_extgrid"
JM_H48QUAL_SIDECAR = ROOT / "tmp/causal_regen_20260516/omega4_2_trade_risk_sidecar_20260622_regime_jmlam4_q065_matched_20260809/risk_sidecar.pkl"
JM_ZIG075_DIR = ROOT / "tmp/causal_regen_20260516/omega4_3head_parent72_loose_entry_quality_20260620_zig075_regime_jmlam4_20260809_extgrid"
JM_ZIG075_SIDECAR = ROOT / "tmp/causal_regen_20260516/omega4_2_trade_risk_sidecar_20260622_zig075_regime_jmlam4_q080_matched_20260809/risk_sidecar.pkl"
...
def run_router(tag: str, use_jm: bool, common_ts: pd.Series) -> dict:
    ...
    if use_jm:
        ...
        h48qual_pred_csv = JM_H48QUAL_DIR / "oos_predictions_q065.csv"
...
def main() -> int:
    ...
    jm_h48_ts = set(pd.to_datetime(pd.read_csv(JM_H48QUAL_DIR / "oos_predictions_q065.csv", usecols=["timestamp"])["timestamp"]))
    ...
    result_jm = run_router("jmlam4_matched", use_jm=True, common_ts=common_ts_series)
    out = {
        "window": [START, END],
        "common_bars": len(common_ts),
        "note": "Both components JM regime3, thresholds chosen to MATCH original activation rate (h48qual q065 ~0.28%, zig075 q080 ~5.35%), not VAL PnL.",
        "baseline_wide24_full_router": result_baseline,
        "jmlam4_matched_full_router": result_jm,
    }
