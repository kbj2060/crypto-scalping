"""ETH regime3-current classifier built with a Statistical Jump Model (k=3, lambda=4) instead of the
live 12-state sticky HMM, using the EXACT SAME wide24 feature panel and ADX/slope/BB "sensitive"
label mode as scripts/experiment_regime3_current_hmm_wide24_20260529.py, so the swap isolates the
model architecture (HMM -> JM) and holds the feature engineering fixed, per the user's request.

Mirrors that script's own fit protocol: fit ONCE on the 2024 file only (train), then causally
transform 2024/2025/2026 forward-only with the frozen jump-model centroids -- no future leakage.

JM's causal_decode only yields a hard per-bar state path. To produce the same soft
bull_prob/bear_prob/chop_prob/confidence/entropy/margin contract the live sidecar expects, this
converts the online DP's running cost V_t(s) into a soft per-bar distribution via
softmax(-(V_t(s) - min_s V_t(s)) / temperature). The DP's switch clamp (V <= min(V) + lambda every
step) bounds the cross-state spread of V to roughly lambda, so temperature=lambda was picked by a
direct empirical sweep (temp in {0.1,0.25,0.5,1,2,4} against a 20k-bar sample) that matches the
live HMM's actual confidence distribution (mean 0.670/std 0.132) almost exactly at temp=lambda=4
(mean 0.673/std 0.098) -- not a first-principles calibration, but a deliberately matched one, not
the unexamined feature-dim guess this docstring originally used (that gave confidence stuck at
~0.33, i.e. uninformative). Those soft state-probabilities are then mapped to bull/bear/chop via the SAME
state_class_matrix / class_proba trick the wide24 script uses to turn its 12 HMM states into 3
classes, just with JM's 3 states as input, using the identical ADX/slope/BB rule labels for the
mapping fit. Output columns use the wide24 naming contract directly (no separate maskedname step).
"""
...
from scripts.experiment_regime3_current_hmm_wide24_20260529 import (  # noqa: E402
    CLASSES3, DEFAULT_TRAIN_2024, DEFAULT_TRANSFORMS, FEATURE_SETS, LABEL_CONFIGS,
...
LABEL_MODE = "balancedish_adx16_slope15_bb012"  # == "sensitive" prefix_stem, the live label mode
...
REPORT_PATH = ROOT / "data/ensemble/reports/eth_regime3_current_jm_lam4_20260809_report.json"
TAG = "jmlam4_20260809"
...
def main() -> None:
    ...
    # Calibrated empirically (not the feature-dim heuristic from the docstring): the online DP's
    # cross-state V spread is bounded by ~lambda (the switch clamp), and temperature=lambda gives a
    # confidence distribution (mean~0.67, std~0.10) closely matching the live HMM's actual spread
    # (mean 0.670, std 0.132) -- see the calibration sweep in this session, temp in {0.1..4.0}.
    temperature = LAMBDA
    ...
    # Sort states by mean forward 12-bar log-return of member bars (JM literature convention:
    # lowest mean return = bear, middle = chop, highest = bull), computed on the 2024 fit window only.
    ...
    class_order_idx = {"bear": bear_i, "chop": chop_i, "bull": bull_i}
    ...
    payload = {
        "model_id": f"eth_regime3_current_jm_{TAG}",
        "classes": CLASSES3,
    ...
    model_path = OUT_DIR / f"eth_regime3_current_jm_{TAG}_2024.joblib"
    ...
    for src in DEFAULT_TRANSFORMS:
        ...
        year = src.stem.split("_")[-1] if src.stem.split("_")[-1].isdigit() else None
        if year is None:
            # training_features_2026_rebuilt -> pull the 4-digit year token
            for tok in src.stem.split("_"):
                if tok.isdigit() and len(tok) == 4:
                    year = tok
                    break
        out_path = OUT_DIR / f"eth_regime3_current_hmm_{TAG}_{year}_maskedname.csv"
