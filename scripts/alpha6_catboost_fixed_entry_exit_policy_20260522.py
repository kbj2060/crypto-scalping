...
from scripts.alpha6_catboost_fixed_barrier_policy_20260522 import (  # noqa: E402
    DEFAULT_FEATURE_CSV,
    DEFAULT_LABEL_DIR,
    DEFAULT_SPEC_DIR,
    PolicyConfig,
...
MODEL_ID = "alpha6_catboost_fixed_entry_exit_policy_20260522"
DEFAULT_OUT_DIR = ROOT / "tmp/causal_regen_20260516/alpha6_catboost_fixed_entry_exit_policy_20260522"
...
def _build_exit_dataset(
    ...
    *,
    ...
    if max_samples > 0 and len(trade_idx) > max_samples:
        ...
        trade_idx.sort()
    notional = float(cfg.fixed_notional)
    for j in trade_idx:
        idx = int(valid[j])
        side = 1 if int(y["action"][j]) == 1 else -1
        atr = float(pd.to_numeric(frame.get("atr14_pct", 0.003), errors="coerce").fillna(0.003).iloc[idx])
        tp = float(np.clip(cfg.fixed_tp_atr * atr, cfg.tp_min, cfg.tp_max))
        sl = float(np.clip(cfg.fixed_sl_atr * atr, cfg.sl_min, cfg.sl_max))
        horizon = min(int(cfg.fixed_max_hold_bars), len(frame) - idx - 2)
        if horizon <= 1:
            continue
        entry = close[idx]
        ...
        for k in range(1, terminal + 1, max(1, int(step))):
            ...
            continue_score = future_best - 0.45 * future_adverse - 0.05 * (terminal - k) / max(float(horizon), 1.0)
...
def _backtest_fixed_entry_exit(
    frame: pd.DataFrame,
    x_val: np.ndarray,
    dec: pd.DataFrame,
    ...
    *,
    ...
    def enter(i: int, new_side: int, row: pd.Series) -> None:
        nonlocal side, entry, entry_equity, exposure, hold, tp, sl, cash, exposure_sum, long_entries, short_entries, mae, mfe, missed_entries
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Alpha6 fixed-barrier entry with separate exit layer.")
    ...
    ap.add_argument("--fixed-tp-atr", type=float, default=2.2)
    ap.add_argument("--fixed-sl-atr", type=float, default=1.5)
    ap.add_argument("--fixed-max-hold-bars", type=int, default=24)
    ap.add_argument("--fixed-cooldown-bars", type=int, default=3)
    ap.add_argument("--efficient-tp-bars", type=int, default=0)
    ap.add_argument("--min-net-edge", type=float, default=0.00045)
    ...
    print(
        f"[alpha6-v4] variant={args.variant} train_rows={len(train)} val_rows={len(val)} labels={len(valid)} features={len(model_features)} use_pca={use_pca}",
        flush=True,
    )
    entry_models = _fit_models(x_entry, y, args, train.iloc[valid].reset_index(drop=True))
    ...
    print(f"[alpha6-v4] exit_samples={len(y_exit)} close_rate={np.mean(y_exit):.3f}", flush=True)
    ...
    for eth in _threshold_grid(dec, args.entry_thresholds):
        for xth in exit_thresholds:
            bt = {
                f"cost{m}": _backtest_fixed_entry_exit(
                    val,
                    x_val,
                    dec,
                    exit_model,
                    ...
                    state_horizon=int(cfg.fixed_max_hold_bars),
