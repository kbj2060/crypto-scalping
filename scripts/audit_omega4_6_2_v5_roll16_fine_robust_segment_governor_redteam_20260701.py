#!/usr/bin/env python3
"""Red-team audit for Omega 4.6.2 v5 roll16 fine robust branch."""
...
RANKING = OUT_DIR / "roll16_fine_robust_segment_governor_ranking.csv"
EVAL_SCRIPT = ROOT / "scripts/eval_omega4_6_2_v5_roll16_fine_robust_segment_governor_20260701.py"
PRIOR_RUNTIME_AUDIT = (
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll16_fine_robust_segment_governor_redteam_20260701.md"
TOL = 1.0e-8
...
def main() -> None:
    audit = load_module("omega462_loss_cluster_audit_for_roll16_fine_robust", AUDIT_V1_PATH)
    segment_audit = load_module("omega462_segment_audit_for_roll16_fine_robust", AUDIT_SEGMENT_PATH)
    ...
    audit.add_check(
        checks,
        "selection_scope_discloses_fine_robust_and_oos_gate",
        report.get("selection_scope") == "validation_fine_robust_branch_with_oos_safety_gate; fresh_holdout_required",
        "research_blocker",
    ...
    audit.add_check(
        checks,
        "fine_robust_selection_rule_pass",
        selected.get("fine_robust_branch_rule_pass") is True
    ...
    audit.add_check(
        checks,
        "improves_old_robust_pnl_without_longer_hold",
    ...
    verdict = (
        "FULL_LIVE_PASS"
        if full_live_pass
        else "RESEARCH_ROLL16_FINE_ROBUST_PASS_FULL_LIVE_BLOCKED"
        if research_pass
        else "REDTEAM_FAIL"
    )
    payload = {
        "audit_id": "omega4_6_2_v5_roll16_fine_robust_segment_governor_redteam_20260701",
