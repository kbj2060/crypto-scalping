#!/usr/bin/env python3
"""Red-team audit for Omega 4.6.2 v5 roll10 side-specific fine valmax branch."""
...
RANKING = OUT_DIR / "roll10_side_specific_fine_valmax_ranking.csv"
EVAL_SCRIPT = ROOT / "scripts/eval_omega4_6_2_v5_roll10_side_specific_fine_valmax_20260701.py"
PRIOR_RUNTIME_AUDIT = (
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll10_side_specific_fine_valmax_redteam_20260701.md"
REFERENCE_CAP = 4.20
TOL = 1.0e-8
...
def write_markdown(payload: dict[str, Any]) -> None:
    ...
    lines = [
        "# Omega 4.6.2 v5 Roll10 Side-Specific Fine Valmax Red-Team Audit - 2026-07-01",
        "",
        ...
        f"- Roll max hold: `{selected['roll10_max_hours']}`",
        f"- Long TP/SL: `{selected['roll10_long_tp_move']}` / `{selected['roll10_long_sl_move']}`",
        f"- Short TP/SL: `{selected['roll10_short_tp_move']}` / `{selected['roll10_short_sl_move']}`",
        "",
...
def main() -> None:
    audit = load_module("omega462_loss_cluster_audit_for_roll10_fine_valmax", AUDIT_V1_PATH)
    segment_audit = load_module("omega462_segment_audit_for_roll10_fine_valmax", AUDIT_SEGMENT_PATH)
    ...
    audit.add_check(
        checks,
        "selection_tiebreaker_declared",
        report.get("selection_tiebreaker")
        == f"validation ties prefer exposure cap nearest the reference cap {REFERENCE_CAP:.2f}; OOS metrics are not ordering keys",
        "research_blocker",
        {"observed": report.get("selection_tiebreaker")},
    )
    audit.add_check(
        checks,
        "selected_fine_valmax_gate_pass",
        selected.get("research_fine_valmax_gate_pass") is True,
        "research_blocker",
        {"observed": selected.get("research_fine_valmax_gate_pass")},
    )
    audit.add_check(
        checks,
        "selected_reference_cap_tiebreak",
        abs(float(selected.get("cap_reference_distance_abs", 999.0))) <= TOL,
        "research_blocker",
        {"cap_reference_distance_abs": selected.get("cap_reference_distance_abs")},
    )
    ...
    audit.add_check(
        checks,
        "roll10_hold_contract",
        validation["max_hold_hours"] <= 10.0 and oos["max_hold_hours"] <= 10.0,
        "research_blocker",
    ...
    audit.add_check(
        checks,
        "improves_reference_validation_pnl_and_hold_with_oos_safety",
    ...
    audit.add_check(
        ...
        float(selected["roll10_max_hours"]) == 10.0
        and float(selected["roll10_long_tp_move"]) > 0.0
        and float(selected["roll10_long_sl_move"]) > 0.0
        and float(selected["roll10_short_tp_move"]) > 0.0
        and float(selected["roll10_short_sl_move"]) > 0.0
        and {"roll10_bracket_tp", "roll10_bracket_sl"}.issubset(set(validation_df["reason"].astype(str))),
        "research_blocker",
        {
            "long_tp": selected.get("roll10_long_tp_move"),
            "long_sl": selected.get("roll10_long_sl_move"),
            "short_tp": selected.get("roll10_short_tp_move"),
            "short_sl": selected.get("roll10_short_sl_move"),
        },
    )
    ...
    verdict = (
        "FULL_LIVE_PASS"
        if full_live_pass
        else "RESEARCH_ROLL10_FINE_VALMAX_PASS_FULL_LIVE_BLOCKED"
        if research_pass
        else "REDTEAM_FAIL"
    )
    payload = {
        "audit_id": "omega4_6_2_v5_roll10_side_specific_fine_valmax_redteam_20260701",
