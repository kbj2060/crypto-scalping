#!/usr/bin/env python3
"""일리아스1 h48qual pinned102 parent(L2 전체: direction+quality+exit head, encoder 포함)를
다른 시드로 재학습 -- N>=5 시드 확장(dual always-benchmark/N=5 재평가용, zig075의
train_eth_ilias1_zig075_pinned102_seed_variant_20260818.py와 쌍을 이룸). 원본
(.../zigzagfix_06_h48_quality_noctx_padded_e2_fulltrain_exit30k_pinned102_20260818,
seed=260620 -- report.json에 seed 필드가 없어 이 스크립트와 마찬가지로 CLI default를
그대로 썼다고 재구성)의 report.json에 기록된 실제 설정을 그대로 복제, seed와 out-suffix만
바꾼다:
  ...
  --quality-mode quality_label_action  (zig075의 same_as_direction과 다름 -- h48qual 고유)
  ...
  --quality-thresholds 0.35,0.40,...,0.95 (원본 ranking_by_validation_pnl의 threshold
    목록과 정확히 일치, zig075의 0.55-0.95 9개와 다른 0.35-0.95 13개 그리드)
...
if __name__ == "__main__":
    ...
    out_suffix = f"zigzagfix_06_h48_quality_noctx_padded_e2_fulltrain_exit30k_pinned102_20260818_seedvariant_{known.seed}"
