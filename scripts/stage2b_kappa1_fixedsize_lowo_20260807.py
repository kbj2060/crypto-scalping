...
OUT = ROOT / "docs/experiments/btc_kappa1_stage2b_fixedsize_lowo_results_20260807.json"
...
LEVERAGE = 3
MIN_TRADES_Q = 15
BOUNDS_LO = np.array([0.0, 0.0, 0.0, 1.0, 0.5])
BOUNDS_HI = np.array([1.0, 1.0, 2.0, 6.0, 3.0])
CMA_ITERS = 60
...
class Backtester:
    ...
    def run(self, params: np.ndarray, decision_quarters: set[str] | None) -> dict:
        w_flow, w_evt, thr, tp_mult, sl_mult = params
        ...
        for i in self.candidates:
            ...
            margin = 0.30  # amendment #2: sizing frozen to kill the size->0 degenerate escape
            if side == 1:
...
def optimize(bt: Backtester, train_quarters: list[str], seed: int) -> np.ndarray:
    ...
    x0 = rng.uniform(0.2, 0.8, size=5)
    es = cma.CMAEvolutionStrategy(x0, 0.25, {
        "bounds": [[0.0] * 5, [1.0] * 5], "seed": seed, "maxiter": CMA_ITERS, "verbose": -9})
...
def main() -> None:
    ...
    report = {
        ...
        "amendment": "stage2b: sizing frozen at margin_fraction=0.30 after stage2a optimizer collapsed size to ~0 (degenerate worst-quarter escape); recorded post-hoc 2026-08-07",
        "folds": folds,
