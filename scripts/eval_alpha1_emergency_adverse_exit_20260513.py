...
MODEL_ID = "alpha1_emergency_adverse_exit_20260513"
OUT_DIR = ROOT / "data/ensemble/supervised/alpha1_emergency_adverse_exit_20260513"
REPORT_OUT = ROOT / "data/ensemble/reports/alpha1_emergency_adverse_exit_20260513_summary.json"
AUDIT_OUT = ROOT / "data/ensemble/reports/alpha1_emergency_adverse_exit_20260513_audit.json"
GRID_OUT = ROOT / "data/ensemble/reports/alpha1_emergency_adverse_exit_20260513_grid.csv"
...
class Runtime:
    ...
    min_hold: int
    mae_trigger: float
    unreal_trigger: float
    adverse_flow_th: float
    opp_utility_th: float
    require_both: bool
    cooldown_after: int
    flow_th: float = 0.10
    fee_mult: float = 0.60
...
def _configs() -> list[Runtime]:
    rows = [Runtime("alpha1_4_identity", "none", 999, 99.0, -99.0, 99.0, 99.0, False, 0)]
    i = 0
    for scope in ("deep", "all"):
        for min_hold in (1, 2, 3):
            for mae in (0.008, 0.012, 0.018):
                for adverse in (0.18, 0.28, 0.40):
                    for opp in (0.010, 0.014, 0.020):
                        rows.append(Runtime(f"emergency_{scope}_{i}", scope, min_hold, mae, -mae * 0.55, adverse, opp, True, 6))
                        rows.append(Runtime(f"emergency_or_{scope}_{i}", scope, min_hold, mae, -mae * 0.75, adverse * 1.25, opp * 1.25, False, 6))
                        i += 1
    return rows
...
def _score(c1: dict[str, Any], c2: dict[str, Any], c3: dict[str, Any]) -> float:
    ...
    return float(c1["pnl"] + 0.38 * c2["pnl"] + 0.22 * c3["pnl"] - 0.85 * max(mdd - 30.0, 0.0) - 0.22 * mdd)


def _scope(owner: str, scope: str) -> bool:
    if scope == "deep":
...
def _adverse_flow(row: pd.Series, side: int) -> float:
    direction = float(np.sign(side))
    flow = (
        0.50 * _num(row, "net_taker_ratio", 0.0)
        + 0.25 * _num(row, "taker_acceleration", 0.0)
        + 0.15 * _num(row, "ofi_acceleration", 0.0)
        + 0.10 * _num(row, "trade_intensity", 0.0)
    )
    return float(-direction * flow)


def _route_variant(rt: Runtime) -> v45.LayerVariant:
    return v45.LayerVariant(
        rt.name,
        "alpha1_5_emergency_adverse_exit",
        alpha1.ALPHA1_CFG,
...
def _emergency_exit(df: pd.DataFrame, deep_q: np.ndarray, i: int, pos: int, owner: str, hold: int, unreal: float, mae: float, rt: Runtime) -> bool:
    if not _scope(owner, rt.scope) or hold < rt.min_hold:
        return False
    if unreal > rt.unreal_trigger or abs(mae) < rt.mae_trigger:
        return False
    row = df.iloc[i]
    adverse = _adverse_flow(row, pos) >= rt.adverse_flow_th
    ql, qs = float(deep_q[i, 0]), float(deep_q[i, 1])
    opp = (qs if pos > 0 else ql) >= rt.opp_utility_th
    return bool(adverse and opp) if rt.require_both else bool(adverse or opp)


def backtest_emergency(
    df: pd.DataFrame,
    bundle: dict[str, Any],
    ...
    *,
    ...
    for i in range(0, len(df) - 2):
        ...
        if pos != 0:
            ...
            hold = i - entry_idx
            reason = ""
            if _emergency_exit(df, deep_q, i, pos, owner, hold, unreal, mae, rt):
                reason = f"{owner}_emergency_adverse_exit"
            ...
            elif owner == "v21_2" and not reason:
            ...
            if reason:
                ...
                pos = 0
                owner = ""
                cooldown = max(int(next_cooldown), int(rt.cooldown_after if "emergency" in reason else 0))
                next_cooldown = 0
...
def _metrics(df, bundle, jackpot_model, add_cfg, q, dec, rt: Runtime, fee: float, slip: float) -> dict[str, Any]:
    return {
        f"cost{mult}": backtest_emergency(df, bundle, jackpot_model, add_cfg, q, rt, fee=fee, slip=slip, cost_mult=float(mult), decisions=dec)
        for mult in (1, 2, 3)
    }
...
def main() -> int:
    ...
    for rt in _configs():
        ...
        rows.append({**asdict(rt), "selection_score": score, "val_cost1_pnl": vm["cost1"]["pnl"], "val_cost1_mdd": vm["cost1"]["mdd"], "val_cost1_trades": vm["cost1"]["trades"], "val_cost2_pnl": vm["cost2"]["pnl"], "val_cost3_pnl": vm["cost3"]["pnl"], "val_emergency_exits": sum(v for k, v in vm["cost1"]["exits"].items() if "emergency" in k)})
        if score > best_score:
            best_score = score
            selected = rt
    ...
    manifest_path = OUT_DIR / "alpha1_5_emergency_adverse_exit_manifest.json"
    manifest_path.write_text(json.dumps({"model_id": MODEL_ID, "selected_config": asdict(selected), "parent_frozen": True, "v27_frozen": True, "v31_exit_frozen": False, "v21_2_frozen": True}, indent=2, ensure_ascii=False, default=_json_default), encoding="utf-8")
    ...
    report = {
        "model_id": MODEL_ID,
        "design": "Alpha1.5 MDD research: alpha1.4 execution proxy plus local emergency adverse-flow exit after entry. No entry blocking; parent, V27 entry, and V21.2 model are frozen.",
