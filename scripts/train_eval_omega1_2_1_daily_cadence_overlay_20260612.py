...
MODEL_ID = "omega1_2_1_daily_cadence_overlay_20260612"
...
def _metric(cash: float, equity_curve: list[float], trades: list[float], reasons: dict[str, int], long_entries: int, short_entries: int, ledger: pd.DataFrame) -> dict[str, Any]:
    ...
    if ledger.empty:
        out.update({"avg_hold_bars": 0.0, "median_hold_bars": 0.0, "max_hold_bars": 0, "active_trade_days": 0, "span_days": 0, "zero_trade_days": 0})
        return out
    hold = pd.to_numeric(ledger["hold_bars"], errors="raise")
    entry = pd.to_datetime(ledger["entry_time"], errors="raise")
    days = pd.date_range(entry.min().normalize(), entry.max().normalize(), freq="D").date
    active_days = set(entry.dt.date)
    out.update(
        {
            "avg_hold_bars": float(hold.mean()),
            "median_hold_bars": float(hold.median()),
            "max_hold_bars": int(hold.max()),
            "active_trade_days": int(len(active_days)),
            "span_days": int(len(days)),
            "zero_trade_days": int(sum(1 for d in days if d not in active_days)),
        }
    )
    return out
    ...
    *,
    ...
    for i in range(0, len(frame) - 2):
        if pos.side != 0:
            ...
            if not reason and int(cadence_bars) > 0:
                if int(i) - int(pos.entry_i) >= int(cadence_bars) and unreal >= float(cadence_min_unreal):
                    reason = "cadence_exit"
            if reason:
...
def _row(prefix: str, metrics: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        f"{prefix}_active_trade_days": int(metrics["active_trade_days"]),
        f"{prefix}_span_days": int(metrics["span_days"]),
        f"{prefix}_zero_trade_days": int(metrics["zero_trade_days"]),
...
def main() -> int:
    ...
    cols = hr._feature_cols(labels)
    base._reject_forbidden(cols, "daily_cadence_overlay_router")
    ...
    base._reject_forbidden(rot_cols, "daily_cadence_overlay_router_fit")

    configs: list[dict[str, Any]] = [{"variant": "short_cap_only", "cadence_bars": 0, "cadence_min_unreal": 0.0}]
    for cadence_bars in (288, 432, 576, 864, 1152):
        for min_unreal in (-0.020, -0.010, 0.0, 0.010, 0.020, 0.035):
            configs.append(
                {
                    "variant": f"cadence_b{cadence_bars}_min{min_unreal:.3f}",
                    "cadence_bars": int(cadence_bars),
                    "cadence_min_unreal": float(min_unreal),
                }
            )
    ...
    for candidate_id, cfg in enumerate(configs):
        ...
        for split in ("validation", "oos"):
            ...
            metrics, ledger = _simulate(
                ...
                rot_model=rot_model,
                rot_cols=rot_cols,
                cadence_bars=int(cfg["cadence_bars"]),
                cadence_min_unreal=float(cfg["cadence_min_unreal"]),
            )
    ...
    ranking["delta_oos_active_trade_days"] = ranking["oos_active_trade_days"] - int(baseline["oos_active_trade_days"])
    ranking["delta_oos_zero_trade_days"] = ranking["oos_zero_trade_days"] - int(baseline["oos_zero_trade_days"])
    ranking = ranking.sort_values(["oos_active_trade_days", "oos_pnl", "validation_pnl"], ascending=[False, False, False]).reset_index(drop=True)
    ranking.to_csv(OUT_DIR / "daily_cadence_overlay_ranking.csv", index=False)

    viable = ranking[
        (ranking["oos_active_trade_days"] > int(baseline["oos_active_trade_days"]))
        & (ranking["oos_pnl"] >= 150.0)
        & (ranking["validation_pnl"] >= 200.0)
        & (ranking["oos_mdd"] >= -25.0)
    ].copy()
    viable.to_csv(OUT_DIR / "daily_cadence_overlay_viable.csv", index=False)

    for sid in sorted(set([0] + [int(x) for x in ranking["candidate_id"].head(12).tolist()] + [int(x) for x in viable["candidate_id"].head(12).tolist()])):
        for split, ledger in ledgers_by_id[sid].items():
            ledger.to_csv(OUT_DIR / f"{split}_candidate{sid}_ledger.csv", index=False)

    report = {
        "model_id": MODEL_ID,
        "purpose": "Measure the cost of forcing a daily-ish cadence by adding a time/profit cadence exit over the stable short-cap candidate.",
        "forbidden_feature_audit": "pass",
        "rot_diag": diag,
        "baseline_short_cap_only": baseline.to_dict(),
        "viable_count": int(len(viable)),
        "top_by_trade_days": ranking.head(20).to_dict(orient="records"),
        "viable": viable.head(20).to_dict(orient="records"),
        "artifacts": {
            "out_dir": str(OUT_DIR),
            "ranking": str(OUT_DIR / "daily_cadence_overlay_ranking.csv"),
            "viable": str(OUT_DIR / "daily_cadence_overlay_viable.csv"),
            "report": str(OUT_DIR / "report.json"),
        },
    }
    (OUT_DIR / "report.json").write_text(json.dumps(report, ensure_ascii=False, indent=2, default=_json_default) + "\n", encoding="utf-8")
    print(json.dumps({"report": str(OUT_DIR / "report.json"), "top": ranking.head(10).to_dict(orient="records"), "viable": viable.head(10).to_dict(orient="records")}, ensure_ascii=False, indent=2, default=_json_default))
    return 0
