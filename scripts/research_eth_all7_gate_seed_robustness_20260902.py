...
OUT_DIR = ROOT / "tmp/eth_all7_gate_seed_robustness_20260902"
TOPK = json.loads((ROOT / "tmp/eth_all7_gated_ensemble_clean_20260902/contract.json").read_text())["priority_order"]
from research_eth_all7_gated_ensemble_clean_regime_20260902 import BUILT, FIRE_CACHE  # noqa: E402
...
def main() -> int:
    ...
    for n in TOPK:
        if n in BUILT:
            b = BUILT[n]
            cfgs[n] = {"sl": b["sl"], "arm": b["arm"], "trail": b["trail"],
                       "horizon": b["horizon"], "src": FIRE_CACHE / f"{n}.csv"}
        else:
            sl, arm, tr = st.loc[n, "val_only"].replace("SL", "").replace("ARM", "").replace("Tr", "").split("/")
            cfgs[n] = {"sl": float(sl), "arm": float(arm), "trail": float(tr),
                       "horizon": SIGNALS[n]["horizon"], "src": ROOT / SIGNALS[n]["fires"]}
