#!/usr/bin/env python3
"""Fine-grained refinement around the most promising region found so far:
persistence=4, quality_threshold=0.65, TP=12x/SL=5x ATR, cooldown=12
(cost1 +9.17%, MDD -14.40%, cost3 -2.50%, MDD -17.42% -- closest to clearing gates yet).
...
OUT_DIR = ROOT / "tmp/causal_regen_20260516/omega6_v2_refine_20260704"
...
def main() -> int:
    ...
    for thr, persist, tp_mult, sl_mult, cooldown in itertools.product(
        ...
        cfg = v2.VariantConfig(
            name=f"ref_p{persist}_qt{thr}_tp{tp_mult}_sl{sl_mult}_cd{cooldown}",
            tp_mode="atr_scaled",
            tp_atr_mult=tp_mult,
            sl_atr_mult=sl_mult,
            sizing_mode="fixed",
    ...
    df.to_csv(OUT_DIR / "refine_variant_ranking.csv", index=False)
    ...
    print(f"\nfull ranking: {OUT_DIR / 'refine_variant_ranking.csv'}", flush=True)
    return 0
