...
MODEL_ID = "alpha3_dsac_exit_owner_v2_20260515"
OUT_DIR = ROOT / "data/ensemble/supervised/alpha3_dsac_exit_owner_v2_20260515"
MODEL_OUT = OUT_DIR / "dsac_exit_owner_v2.pt"
REPORT_OUT = ROOT / "data/ensemble/reports/alpha3_dsac_exit_owner_v2_20260515_summary.json"
AUDIT_OUT = ROOT / "data/ensemble/reports/alpha3_dsac_exit_owner_v2_20260515_audit.json"
GRID_OUT = ROOT / "data/ensemble/reports/alpha3_dsac_exit_owner_v2_20260515_grid.csv"
DATASET_OUT = ROOT / "data/ensemble/reports/alpha3_dsac_exit_owner_v2_20260515_dataset.json"
...
def _expanded_targets(base_y: np.ndarray, base_actions: list[str], x_full: np.ndarray, feature_names: list[str]) -> np.ndarray:
    ...
    market = baseline - 0.0012
    maker0 = exit0 - 0.0002
    maker2 = exit2 - 0.0004
    maker4 = exit4 - 0.0008
    close50_market = 0.52 * market + 0.48 * hold - 0.0005 - 0.025 * mae
    close50_maker2 = 0.52 * maker2 + 0.48 * hold - 0.0004 - 0.020 * mae
    twap_maker2 = 0.34 * maker2 + 0.66 * hold - 0.0007 - 0.010 * hold_norm
    hold_guard = hold - np.maximum(0.0, baseline - hold) * 0.15 - 0.060 * giveback - 0.012 * hold_norm
    y = np.stack(
        [
            hold_guard,
            market,
...
def main() -> int:
    ...
    dataset_summary = {
        **dataset_meta,
        ...
        "reward_design": {
            ...
            "partial_close": "52% realized close value plus 48% hold continuation with MAE penalty.",
            "twap_maker2": "34% maker2 close value plus 66% hold continuation with time penalty.",
            "hold_with_guard": "hold continuation penalized by exit advantage, giveback, and hold age.",
        },
    }
    ...
    for mode_i, mode in enumerate(FEATURE_MODES):
        ...
        model_out = OUT_DIR / f"dsac_exit_owner_v2_{mode}.pt"
    ...
    experiments = [
        ...
        {
            "name": f"alpha3_dsac_exit_owner_v2::{selected_mode}::{selected_policy.name}",
    ...
    report = {
        ...
        "design": {
            ...
            "feature_design": "Feature sweep uses Alpha3 exit-owner analysis: state16 raw, state16+topbase20, state16+base PLS8, and state16+topbase20+base PLS8.",
            "limitations": "No L2/orderbook/queue inputs or labels by request. Maker fills use existing OHLC immediate-limit proxy; partial/TWAP rewards are synthetic approximations from full-close DP targets.",
    ...
    audit = {
        ...
        "notes": [
            ...
            "Partial close, TWAP, market close, and hold-with-guard actions are included; partial/TWAP labels are synthetic approximations from DP close/hold targets.",
