#!/usr/bin/env python3
"""git HEAD(커밋 e36908d 기준, 2026-08-19)의
scripts/train_eval_omega4_3head_parent72_loose_entry_quality_20260620.py를 그대로 스냅샷한
파일 -- 작업트리의 그 파일은 2026-08-18 exit_head pos_tp/pos_sl/pos_notional 버그수정이
미커밋 상태로 적용돼 있어(git diff HEAD로 확인), import하면 수정본이 로드된다. 이 스냅샷은
실제 라이브 h48qual/zig075 번들(FINAL_GOVERNOR_OMEGA4_6_1_H48QUAL/ZIG075_BUNDLE_PATH,
2026-06-29/30 학습)을 만들어낸 정확한 코드 상태를 보존해, "라이브로 승격된 그 모델 자체가
시드에 강건한가"를 재현학습으로 검증하기 위한 용도. scripts/eth_live_promotion_seed_
robustness_h48qual_seed_variant_20260819.py / _zig075_seed_variant_20260819.py가 이 모듈을
import해서 쓴다. 수정본(버그수정) 재학습은 이미 일리아스1 축에서 N=5+1시드로 완료됨
(REJECTED_SIGN_MISMATCH, seed-robust) -- 이 스냅샷은 그것과 다른 질문(원본 코드 자체의
시드 강건성)에 답하기 위한 것이다."""
