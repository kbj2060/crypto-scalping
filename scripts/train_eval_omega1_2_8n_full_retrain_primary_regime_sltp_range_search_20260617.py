...
MODEL_ID = "omega1_2_8n_full_retrain_primary_regime_sltp_range_search_20260617"
...
PRIMARY_RISK_PROFILES: dict[str, tuple[tuple[str, float, float], tuple[str, float, float], tuple[str, float, float]]] = {
    "flat_base_052_028": (
        ("base", 0.052, 0.028),
        ("wide", 0.052, 0.028),
        ("runner", 0.052, 0.028),
    ),
    "compact_045_024__052_028__065_034": (
        ("base", 0.045, 0.024),
        ("wide", 0.052, 0.028),
        ("runner", 0.065, 0.034),
    ),
    "moderate_052_028__058_031__065_034": (
        ("base", 0.052, 0.028),
        ("wide", 0.058, 0.031),
        ("runner", 0.065, 0.034),
    ),
    "eight_h_like_052_028__065_034__078_042": (
        ("base", 0.052, 0.028),
        ("wide", 0.065, 0.034),
        ("runner", 0.078, 0.042),
    ),
    "tp_extend_sl_base_052_028__065_028__078_028": (
        ("base", 0.052, 0.028),
        ("wide", 0.065, 0.028),
        ("runner", 0.078, 0.028),
    ),
    "tp_extend_sl_step_052_028__065_031__078_034": (
        ("base", 0.052, 0.028),
        ("wide", 0.065, 0.031),
        ("runner", 0.078, 0.034),
    ),
    "loose_sl_052_034__065_042__078_052": (
        ("base", 0.052, 0.034),
        ("wide", 0.065, 0.042),
        ("runner", 0.078, 0.052),
    ),
...
def _metric_row(candidate: str, family: str, cfg_id: int | None, cal_q: float, ev_min: float, utility_min: float | None, margin_min: float | None, val_m: dict[str, Any], oos_m: dict[str, Any], base_val: dict[str, Any], base_oos: dict[str, Any]) -> dict[str, Any]:
    ...
    row["primary_profile"] = str(val_m.get("primary_profile", "parent_original"))
    ...
    row["val_primary_regime_counts"] = val_m.get("primary_regime_counts", {})
    row["oos_primary_regime_counts"] = oos_m.get("primary_regime_counts", {})
...
def _primary_regime_name(i: int, vol: np.ndarray, q60: float, q85: float) -> str:
    v = float(vol[int(i)])
    if v >= float(q85):
        return "runner"
    if v >= float(q60):
...
def _profile_risk(profile: tuple[tuple[str, float, float], tuple[str, float, float], tuple[str, float, float]], regime: str) -> tuple[float, float]:
    for name, take_profit, stop_loss in profile:
        if name == regime:
            return float(take_profit), float(stop_loss)
    raise RuntimeError(f"missing primary regime in profile: {regime}")


def _primary_profile_row(row: pd.Series, take_profit: float, stop_loss: float) -> pd.Series:
    out = row.copy()
    out["take_profit"] = float(take_profit)
    out["stop_loss"] = abs(float(stop_loss))
    return out


def _metrics_with_primary_profile_and_fallback(
    frame: pd.DataFrame,
    dec: pd.DataFrame,
    risk: sleeve.FallbackRisk,
    profile_name: str,
    profile: tuple[tuple[str, float, float], tuple[str, float, float], tuple[str, float, float]],
    ...
    *,
    ...
    for i in range(0, len(frame) - 2):
        ...
        if bool(active[i]):
            ...
            if side != 0:
                regime = _primary_regime_name(i, vol, q60, q85)
                take_profit, stop_loss = _profile_risk(profile, regime)
                primary_row = _primary_profile_row(row, take_profit, stop_loss)
                cash, pos, entered = sleeve._open_position(cash, arrays, i, side, "primary", None, primary_row, fee_eff, slip_eff)
                if entered:
                    key = f"{profile_name}:{regime}_tp{take_profit:.4f}_sl{stop_loss:.4f}"
                    primary_entries += 1
    ...
    tp = np.asarray(primary_tp, dtype=np.float64)
    sl = np.asarray(primary_sl, dtype=np.float64)
    notional = np.asarray(primary_notional, dtype=np.float64)
    return {
        ...
        "primary_regime_counts": primary_counts,
        "primary_avg_tp": float(np.mean(tp)) if len(tp) else 0.0,
        "primary_avg_sl": float(np.mean(sl)) if len(sl) else 0.0,
        "primary_avg_tp_price_move": float(np.mean(tp / np.maximum(notional, 1.0e-12))) if len(tp) else 0.0,
        "primary_avg_sl_price_move": float(np.mean(sl / np.maximum(notional, 1.0e-12))) if len(sl) else 0.0,
    }
...
def main() -> int:
    ...
    diagnostics: dict[str, Any] = {
        "mode": "full_retrain_parent_numeric_cash_sleeve_primary_regime_sltp_range_search",
        "baseline_model_id": BASELINE_ID,
        "parent_artifact": meta["parent_dir"],
        "risk": asdict(RISK),
        "risk_semantics": "fallback risk remains omega1_2_8b baseline; primary row notional/leverage/max_hold are preserved while primary take_profit/stop_loss are replaced by validation-vol regime profile values",
        "vol_regime": {"source": "past close pct_change rolling std, window=96, min_periods=24", "validation_q60": vol_quantiles[0], "validation_q85": vol_quantiles[1]},
        "primary_risk_profiles": PRIMARY_RISK_PROFILES,
    ...
    for cal_q in (0.50, 0.65, 0.80):
        ...
        for ev_min in (0.001, 0.002, 0.003, 0.004):
            ...
            for profile_name, profile in PRIMARY_RISK_PROFILES.items():
                val_m = _metrics_with_primary_profile_and_fallback(val_payload["frame"], val_payload["dec"], RISK, profile_name, profile, val_ev_a, val_ev_c, 0.0, fee=fee, slip=slip, cost_mult=3.0, vol_quantiles=vol_quantiles)
                oos_m = _metrics_with_primary_profile_and_fallback(oos_payload["frame"], oos_payload["dec"], RISK, profile_name, profile, oos_ev_a, oos_ev_c, 0.0, fee=fee, slip=slip, cost_mult=3.0, vol_quantiles=vol_quantiles)
            ...
            for cfg_id, (uvl, uvs, uol, uos) in utility_preds.items():
                for utility_min in (-0.001, 0.0, 0.001, 0.002):
                    for margin_min in (0.0, 0.001, 0.002):
                        ...
                        for profile_name, profile in PRIMARY_RISK_PROFILES.items():
                            val_m = _metrics_with_primary_profile_and_fallback(val_payload["frame"], val_payload["dec"], RISK, profile_name, profile, val_a, val_c, 0.0, fee=fee, slip=slip, cost_mult=3.0, vol_quantiles=vol_quantiles)
                            oos_m = _metrics_with_primary_profile_and_fallback(oos_payload["frame"], oos_payload["dec"], RISK, profile_name, profile, oos_a, oos_c, 0.0, fee=fee, slip=slip, cost_mult=3.0, vol_quantiles=vol_quantiles)
    ...
    report = {
        ...
        "method": "Full-retrained 3-head parent artifact is used as parent. Cash sleeve EV lower-bound and numeric utility agreement/veto regressors are newly fit on validation full-retrain features. Fallback risk remains the omega1_2_8b baseline contract. Primary position take_profit/stop_loss ranges are grid-searched by validation-vol regime while preserving parent row notional/leverage/max_hold.",
