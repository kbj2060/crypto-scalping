...
MODEL_ID = "hf_v13_deep_alpha_attention_scout_v35_20260512"
...
DEFAULT_OUT_DIR = ROOT / "data/ensemble/supervised/hf_v13_deep_alpha_attention_scout_v35_20260512"
DEFAULT_REPORT = ROOT / "data/ensemble/reports/hf_v13_deep_alpha_attention_scout_v35_20260512_summary.json"
DEFAULT_AUDIT = ROOT / "data/ensemble/reports/hf_v13_deep_alpha_attention_scout_v35_20260512_audit.json"
DEFAULT_GRID = ROOT / "data/ensemble/reports/hf_v13_deep_alpha_attention_scout_v35_20260512_grid.csv"
HORIZONS = (12, 24, 48)
...
class FeatureWiseAttention(nn.Module):
    def __init__(self, seq_dim: int, reduction: int = 4) -> None:
        super().__init__()
        hidden = max(8, seq_dim // reduction)
    ...
    def forward(self, seq: torch.Tensor) -> torch.Tensor:
        gate = self.net(seq.mean(dim=1))
        return seq * gate.unsqueeze(1)
...
class TemporalAdditiveAttention(nn.Module):
    def __init__(self, hidden: int) -> None:
        ...
        self.recent_bias = nn.Parameter(torch.linspace(-0.35, 0.35, v23.SEQ_LEN).view(1, v23.SEQ_LEN, 1))

    def forward(self, seq: torch.Tensor) -> torch.Tensor:
        scores = self.score(seq) + self.recent_bias[:, -seq.shape[1] :, :]
...
class DeepAlphaTCN(nn.Module):
    def __init__(self, seq_dim: int, hidden: int = 72) -> None:
        super().__init__()
        self.feature_attention = FeatureWiseAttention(seq_dim)
        self.net = nn.Sequential(
        ...
        self.temporal_attention = TemporalAdditiveAttention(hidden)
        self.head = nn.Sequential(nn.Linear(hidden, 96), nn.GELU(), nn.Dropout(0.10), nn.Linear(96, 2))

    def forward(self, seq: torch.Tensor) -> torch.Tensor:
        seq = self.feature_attention(seq)
        h = self.net(seq.transpose(1, 2)).transpose(1, 2)
        return self.head(self.temporal_attention(h))
...
def main() -> int:
    args = parse_args()
    print("[v35] loading parent/jackpot/data", flush=True)
    ...
    print(f"[v35] building train set seq_cols={len(seq_cols)}", flush=True)
    train_ds = _build_train_set(train, seq_cols, fee=float(base["fee"]), slip=float(base["slip"]), stride=3)
    norm = _normalizer(train_ds["seq"])
    print(f"[v35] training attention scout snapshots={len(train_ds['target'])} epochs={args.epochs}", flush=True)
    model = _train_model(train_ds, norm, epochs=args.epochs)
    print("[v35] predicting validation/OOS utilities", flush=True)
    ...
    for cfg in _grid():
        print(f"[v35] validation {cfg.name}", flush=True)
    ...
    model_path = args.out_dir / "v35_deep_alpha_attention_scout.pt"
    torch.save({"model_id": MODEL_ID, "architecture": "feature_attention_tcn_temporal_attention", "state_dict": model.state_dict(), "seq_cols": seq_cols, "norm": norm, "selected_config": asdict(selected), "add_config": asdict(add_cfg), "parent_model": str(args.parent_model), "jackpot_model": str(args.jackpot_model)}, model_path)
