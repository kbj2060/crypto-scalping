...
MODEL_ID = "omega1_2_8o_full_retrain_primary_notional_scale_fallback_8h_20260617"
...
PRIMARY_NOTIONAL_SCALES = (1.00, 1.15, 1.30, 1.50, 1.75, 2.00)
VOL_RISKS = (
...
def _primary_notional_scaled_row(row: pd.Series, scale: float) -> pd.Series:
    out = row.copy()
    base_notional = float(row.get("notional_exposure", 0.0) or 0.0)
    base_leverage = float(row.get("leverage", 1.0) or 1.0)
    out["notional_exposure"] = base_notional * float(scale)
    out["leverage"] = base_leverage * float(scale)
    return out
    ...
    *,
    ...
    for i in range(0, len(frame) - 2):
        ...
        if bool(active[i]):
            ...
            if side != 0:
                primary_row = _primary_notional_scaled_row(row, primary_notional_scale)
    ...
    return {
        ...
        "primary_notional_scale": float(primary_notional_scale),
...
def _metric_row(candidate: str, family: str, cfg_id: int | None, cal_q: float, ev_min: float, utility_min: float | None, margin_min: float | None, val_m: dict[str, Any], oos_m: dict[str, Any], base_val: dict[str, Any], base_oos: dict[str, Any]) -> dict[str, Any]:
    ...
    row["primary_notional_scale"] = float(val_m.get("primary_notional_scale", 1.0) or 1.0)
...
def main() -> int:
    ...
    diagnostics: dict[str, Any] = {
        "mode": "full_retrain_parent_numeric_cash_sleeve_with_primary_notional_scale_and_fallback_vol_sltp",
        ...
        "primary_notional_scales": list(PRIMARY_NOTIONAL_SCALES),
        "primary_notional_policy": "primary row take_profit/stop_loss/max_hold are preserved; primary notional_exposure and leverage are multiplied by the scale to narrow account-PnL price lines",
        "vol_normalized_sltp": {
    ...
    for cal_q in (0.50, 0.65, 0.80):
        ...
        for ev_min in (0.001, 0.002, 0.003, 0.004):
            ...
            for primary_scale in PRIMARY_NOTIONAL_SCALES:
                val_m = _metrics_with_vol_sltp(val_payload["frame"], val_payload["dec"], val_ev_a, val_ev_c, 0.0, fee=fee, slip=slip, cost_mult=3.0, vol_quantiles=vol_quantiles, primary_notional_scale=primary_scale)
                oos_m = _metrics_with_vol_sltp(oos_payload["frame"], oos_payload["dec"], oos_ev_a, oos_ev_c, 0.0, fee=fee, slip=slip, cost_mult=3.0, vol_quantiles=vol_quantiles, primary_notional_scale=primary_scale)
                ev_name = f"full_retrain_ev_cal{cal_q:.2f}_ev{ev_min:.3f}_pns{primary_scale:.2f}"
            ...
            for cfg_id, (uvl, uvs, uol, uos) in utility_preds.items():
                for utility_min in (-0.001, 0.0, 0.001, 0.002):
                    for margin_min in (0.0, 0.001, 0.002):
                        ...
                        for primary_scale in PRIMARY_NOTIONAL_SCALES:
                            cand = f"{base_cand}_pns{primary_scale:.2f}"
                            val_m = _metrics_with_vol_sltp(val_payload["frame"], val_payload["dec"], val_a, val_c, 0.0, fee=fee, slip=slip, cost_mult=3.0, vol_quantiles=vol_quantiles, primary_notional_scale=primary_scale)
                            oos_m = _metrics_with_vol_sltp(oos_payload["frame"], oos_payload["dec"], oos_a, oos_c, 0.0, fee=fee, slip=slip, cost_mult=3.0, vol_quantiles=vol_quantiles, primary_notional_scale=primary_scale)
    ...
    report = {
        ...
        "method": "Full-retrained 3-head parent artifact is used as parent. Primary row notional_exposure/leverage scale is grid-searched to narrow account-PnL price lines while preserving primary TP/SL thresholds. Cash sleeve EV lower-bound and numeric utility agreement/veto regressors are newly fit on validation full-retrain features. Fallback uses omega1_2_8h volatility SLTP buckets.",
