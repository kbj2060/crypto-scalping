"""Build the BTC zigzag exit-layer dataset (2026-08-10).

Session context: the raw zigzag oracle label is a hard +-1 switch that flips instantly at
each confirmed pivot -- no gradient near the boundary, no near-zero region, hard to learn.
This script replaces it with a proper exit-layer training row instead of trying to soften
the switch itself:

  X (causal, live-computable, no look-ahead):
    bars_since_last_pivot   -- time elapsed since the last CONFIRMED pivot (counts up,
                                resets to 0 at each new pivot)
    eff_ratio_2h_trailing   -- Kaufman-style efficiency ratio (net / gross log-price move)
                                over the PAST EFF_WINDOW bars only

  y (oracle, forward-looking, label-only -- never a live input):
    exit_now_vs_hold        -- 1 if holding HORIZON_BARS further, in the direction of the
                                current zigzag wave, would not even clear round-trip cost;
                                0 if it would. Compares "lock in $0 now" against the actual
                                forward-realized, cost-aware return.

theta=2% was chosen over the frozen theta=0.5% detector specifically for this exit layer:
at 0.5% median wave duration is 1.3h, which would fire an "exit imminent" signal roughly
every hour -- far more often than this project's live models actually reverse, and the
same failure pattern documented for trailing stops (cuts long-hold winners short). At 2%
median wave duration is 12.9h, matching the long-hold timescale of the live models.

Both X columns must stay strictly backward-looking. The zigzag pivot detector itself is
look-ahead by design (it only knows a pivot happened once price has moved threshold%
past it) and may only ever feed the y target, per this project's Fresh-Forward rule --
never a feature.
"""
...
OUT_DIR = ROOT / "tmp/btc_zigzag_exit_layer_20260810"

THETA = 0.02          # zigzag reversal threshold defining a "wave" (see docstring)
EFF_WINDOW = 24        # bars (2h on 5m data) for the trailing efficiency-ratio feature
HORIZON_BARS = 48      # bars (4h) forward comparison horizon for the exit-now-vs-hold target
FEE, SLIP, COST_MULT = 0.0005, 0.0002, 3.0
COST = 2 * (FEE + SLIP) * COST_MULT   # 0.42% round trip, this project's standard convention
...
def zigzag_oracle(close: np.ndarray, threshold: float = 0.04) -> tuple[np.ndarray, list[int]]:
    """Retrospective zigzag wave segmentation. Copied verbatim from
    scripts/test_statistical_jump_model_regimes_20260808.py:140 to avoid pulling in that
    module's training-only dependency chain (lightgbm etc.).

    Uses future information BY DESIGN -- may only be used to build the y target below,
...
def exit_now_vs_hold_target(close: np.ndarray, direction: np.ndarray, horizon: int, cost: float) -> np.ndarray:
    ...
    y[side == 0] = np.nan  # no wave direction assigned yet (before the first pivot)
    return y
