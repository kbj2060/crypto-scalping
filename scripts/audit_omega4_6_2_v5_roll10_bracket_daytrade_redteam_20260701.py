#!/usr/bin/env python3
"""Red-team audit for Omega 4.6.2 v5 roll10 bracket day-trade branch."""
...
RANKING = OUT_DIR / "roll10_bracket_daytrade_ranking.csv"
EVAL_SCRIPT = ROOT / "scripts/eval_omega4_6_2_v5_roll10_bracket_daytrade_20260701.py"
PRIOR_RUNTIME_AUDIT = (
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll10_bracket_daytrade_redteam_20260701.md"
TOL = 1.0e-8
...
def write_markdown(payload: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- TP/SL: `{selected['roll10_tp_move']}` / `{selected['roll10_sl_move']}`",
        "",
...
def main() -> None:
    audit = load_module("omega462_loss_cluster_audit_for_roll10", AUDIT_V1_PATH)
    segment_audit = load_module("omega462_segment_audit_for_roll10", AUDIT_SEGMENT_PATH)
    ...
    audit.add_check(
        ...
        "research_blocker",
        {"observed": report.get("reference_model_id")},
    )
    audit.add_check(
        checks,
        ...
        "research_blocker",
        {"observed": report.get("parent_model_id")},
    )
    audit.add_check(
        checks,
    ...
    audit.add_check(
        ...
        == "validation ties prefer the minimum exposure cap at or above 4.0; OOS metrics are not ordering keys",
        "research_blocker",
        {"observed": report.get("selection_tiebreaker")},
    )
    audit.add_check(
        checks,
        "selected_cap_floor_tiebreak_pass",
        selected.get("cap_floor_tiebreak_pass") is True
        and abs(float(selected.get("cap_floor_distance_abs", 999.0))) <= TOL,
        "research_blocker",
        {
            "cap_floor_tiebreak_pass": selected.get("cap_floor_tiebreak_pass"),
            "cap_floor_distance_abs": selected.get("cap_floor_distance_abs"),
        },
    )
    audit.add_check(
        checks,
        "selected_daytrade_gate_pass",
        selected.get("research_daytrade_gate_pass") is True,
        "research_blocker",
        {"observed": selected.get("research_daytrade_gate_pass")},
    )
    ...
    audit.add_check(
        checks,
        "reduces_hold_vs_reference",
        validation["avg_hold_hours"] < reference["validation_avg_hold_hours"]
    ...
    audit.add_check(
        ...
        and float(selected["roll10_tp_move"]) > 0.0
        and float(selected["roll10_sl_move"]) > 0.0
        and {"roll10_bracket_tp", "roll10_bracket_sl"}.issubset(set(validation_df["reason"].astype(str))),
        "research_blocker",
        {"tp": selected.get("roll10_tp_move"), "sl": selected.get("roll10_sl_move")},
    )
    ...
    verdict = (
        "FULL_LIVE_PASS"
        if full_live_pass
        else "RESEARCH_ROLL10_DAYTRADE_PASS_FULL_LIVE_BLOCKED"
        if research_pass
        else "REDTEAM_FAIL"
    )
    payload = {
        "audit_id": "omega4_6_2_v5_roll10_bracket_daytrade_redteam_20260701",
