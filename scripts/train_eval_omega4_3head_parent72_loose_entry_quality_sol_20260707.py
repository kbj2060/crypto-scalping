#!/usr/bin/env python3
"""SOL copy of train_eval_omega4_3head_parent72_loose_entry_quality_20260620.py.

Imports the SOL `omega` module (train_eval_omega1_2_tabm_diffusion_risk_sol_
20260707.py) instead of the ETH one. The only structural change vs the ETH
...
(omega.TABM_2025) that this SOL build does not have. That block is only
...
independent_entry_hold_offsets mode is NOT supported by this SOL copy.
"""
...
MODEL_ID = "sol_omega4_3head_parent72_loose_entry_quality_20260707"
LABEL_DIR = ROOT / "tmp/causal_regen_20260516/sol_zigzag_action_labels_20260707"
...
def _prepare_frames(
    *,
    ...
    # No SOL analogue of the frozen upstream Omega1.2 TabM prediction file
    ...
    train_fixed = None
    if disable_tp_sl:
        raise RuntimeError("disable_tp_sl requires train_fixed, which this SOL copy does not build")
...
def _build_exit_dataset_entry_label_terminal_giveback(
    ...
    *,
    ...
    leverage/pos_exposure -- pass both explicitly, or both None. None is the ONLY option today:
    this function runs during the bundle's OWN original parent+exit_head training, before any risk
    sidecar for THIS bundle can exist (the sidecar is trained afterward, from this bundle's own
    predictions). None falls back to the fixed BASE_TEMPLATE constant, recorded transparently as
    risk_sizing_source in the returned diagnostics rather than silently claiming real sizing.
    2026-08-18 fix, see docs/experiments/eth_odyssey4_exit_head_liveatr_barrier_and_label_reaudit_
    20260818.md -- ported from the byte-identical ETH original's fix
    ...
    while i < last_i:
        ...
        if not filled or end_i < entry_i:
            skipped_segments += 1
            continue
        # Real per-segment risk sizing (or the fixed-constant fallback, see risk_sizing_source
        # above) -- locked in at the signal bar, matching live's entry_decision (sized once per
        # trade, held fixed for its whole duration).
...
def main() -> int:
    ...
    ap.add_argument("--out-suffix", default="sol_e4_train30k_exit12k")
    ...
    ap.add_argument("--base-feature-contract-bundle", type=Path, default=None)
    ...
    if args.base_feature_contract_bundle is not None:
        contract_path = Path(args.base_feature_contract_bundle)
        if not contract_path.exists():
            raise FileNotFoundError(contract_path)
        contract_bundle = torch.load(contract_path, map_location="cpu", weights_only=False)
        contract_cols = list(contract_bundle["base_cols"])
        missing_contract_cols = [col for col in contract_cols if col not in base_cols]
        if missing_contract_cols:
            raise RuntimeError(
                f"SOL base feature contract missing {len(missing_contract_cols)} columns: "
                f"{missing_contract_cols[:20]}"
            )
        base_cols = contract_cols
    ...
    # risk_margin/risk_leverage=None: this call trains the bundle's OWN original exit head, before
    # any risk sidecar for THIS bundle can exist (the sidecar is trained afterward, from this
    # bundle's own predictions) -- see _build_exit_dataset_entry_label_terminal_giveback's
    # docstring. 2026-08-18, docs/experiments/eth_odyssey4_exit_head_liveatr_barrier_and_label_
    # reaudit_20260818.md.
    ...
    report = {
        ...
        "baseline_model": "sol_omega4_3head_parent72_loose_entry_quality_sol_20260707",
        "label_dir": str(args.direction_label_dir),
        "label_contract": frames["label_contract"],
        "design": "SOL replication of Omega4 3-head TabM (h48qual/zig075 architecture). Direction head and Quality head can use independent offline label targets. No label diagnostic columns are model inputs.",
        "quality_target_rule": {
        ...
        "input_contract": {
            ...
            "base_feature_contract_bundle": (
                str(args.base_feature_contract_bundle)
                if args.base_feature_contract_bundle is not None
                else None
            ),
