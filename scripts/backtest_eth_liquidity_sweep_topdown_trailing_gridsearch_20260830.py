#!/usr/bin/env python3
"""Trailing-stop cost-gate grid search for liquidity_sweep top/down metalabel (final label config
H=30/GAP=12/K=4.0), using this repo's STANDARD backtest engine
(core.causal_futures_backtest.simulate_single_position, ATR-trailing-stop exit mode) -- the same
engine/convention taker_delta_z_climax and short_term_return_z's own trailing-stop cost-gate used
(backtest_taker_and_str_z_trailing_stop_standard_engine_20260830.py), NOT a scratchpad simulation.

Per this project's default policy (feedback_trailing_stop_default_costgate_logic_20260830):
trailing stop is the FIRST exit structure tried, not fixed TP/SL. Tests ALL raw H=30/GAP=12/K=4.0
fires (bottom=long, top=short), unconditional on the trained model's own probability -- matching
taker/short_term_return_z's own standard-engine precedent (tests whether the underlying liquidity_
sweep EVENT itself, which is what the dashboard net_score/votes actually use, has a cost-gate-
passing edge with a smart exit -- independent of whether the metalabel classifier's own predicted-
positive subset would do even better, a separate question not tested here).

SL grid informed by research_eth_liquidity_sweep_topdown_slrace_diagnostic_20260830.py's win-rate-
before-stopout curve (SL=1.5x->22.0% stopped, SL=4.0x->4.5% stopped). HOLDOUT (2026-04-01+)
entirely excluded -- single-touch policy, not yet earned.
"""
...
def log(msg: str) -> None:
    print(f"[liq_sweep_topdown_trailing_gridsearch] {msg}", flush=True)
