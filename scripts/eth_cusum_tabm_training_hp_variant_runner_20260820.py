#!/usr/bin/env python3
"""CUSUM+TB 라벨로 hp변형(lr↓+warmup+wd↑) TabM 학습 -- 사용자 지시로 DC(①)에서 이미 검증한
hp변형을 CUSUM(②)에도 적용.

`eth_directional_change_tabm_training_variant_runner_20260820.py`와 완전히 동일한 배선
(canonicaldata+단일모델+`eth_directional_change_tabm_training_variants_20260820.fit_expert_omega4_variant`)
이고 바뀌는 건 라벨 디렉토리 하나뿐 -- DC dense-cashfill 대신 CUSUM dense-cashfill
(`scripts/build_eth_directional_change_dense_cashfill_labels_20260819.py`를 `--sparse-dir`만
바꿔 재사용해 오늘 새로 만듦, `tmp/eth_cusum_triple_barrier_labels_dense_cashfill_20260820/`)."""
...
if __name__ == "__main__":
    ...
    parent_script._fit_expert_omega4 = _make_fit_fn("hp")

    out_suffix = f"cusum_dense_cashfill_unified_single_model_hp_seed{known.seed}_20260820"
    sys.argv = [sys.argv[0], *BASE_ARGS, "--seed", str(known.seed), "--out-suffix", out_suffix]
    print(f"stage=start seed={known.seed} variant=hp label=CUSUM out_suffix={out_suffix}", flush=True)
    t0 = time.time()
