...
MODEL_ID = "hf_v13_jackpot_runner_5x_v21_3_20260511"
...
DEFAULT_OUT_DIR = ROOT / "data/ensemble/supervised/hf_v13_jackpot_runner_5x_v21_3_20260511"
DEFAULT_REPORT = ROOT / "data/ensemble/reports/hf_v13_jackpot_runner_5x_v21_3_20260511_summary.json"
DEFAULT_AUDIT = ROOT / "data/ensemble/reports/hf_v13_jackpot_runner_5x_v21_3_20260511_audit.json"
DEFAULT_GRID = ROOT / "data/ensemble/reports/hf_v13_jackpot_runner_5x_v21_3_20260511_grid.csv"
...
def _grid() -> list[CostRunnerConfig]:
    ...
    i = 0
    for max_entry in (3.0, 3.5, 4.0, 4.5, 5.0):
        for add_frac in (0.20, 0.25, 0.30):
            for max_mult in (1.35, 1.50, 1.75):
                for bad_cap in (0.35, 0.50):
                    rows.append(CostRunnerConfig(f"v21_3_5x_jackpot_{i}", 99.0, 99.0, 0.60, -0.006, 0.20, 0.015, bad_cap, 0.004, 3, add_frac, 0.0, max_mult, max_entry, 5.0, 0.30))
                    i += 1
    rows.append(CostRunnerConfig("v21_3_parent_noop", 99.0, 99.0, 1.0, 99.0, 99.0, 99.0, 0.0, 99.0, 999, 0.0, 0.0, 1.0, 99.0, 5.0, 99.0))
    return rows
...
def _fit_cost_runner(frame: pd.DataFrame, bundle: dict[str, Any], *, fee: float, slip: float) -> dict[str, Any]:
    ...
    return {"regressor": reg, "q10_regressor": q10, "q90_regressor": q90, "classifier": clf, "jackpot_classifier": jackpot_clf, "bad_classifier": bad_clf, "cost3_classifier": cost3_clf, "feature_cols": list(x.columns), "snapshot_count": int(len(x)), "positive_rate": float((y > 0.0).mean()), "jackpot_rate": float((y2 >= 0.025).mean()), "bad_rate": float((y2 <= -0.004).mean()), "cost3_positive_rate": float((y3 > 0.0).mean()), "target_mean": float(y.mean()), "target_p10": float(np.quantile(y, 0.10)), "target_cost2_p90": float(np.quantile(y2, 0.90)), "target_p75": float(np.quantile(y, 0.75)), "target_p95": float(np.quantile(y, 0.95))}
...
def _predict_cost_runner(model: dict[str, Any], x: pd.DataFrame) -> tuple[float, float, float, float, float, float, float]:
    ...
    q10 = float(model["q10_regressor"].predict(xx)[0])
    q90 = float(model["q90_regressor"].predict(xx)[0])
    p = _positive_prob(model["classifier"], xx)
    p_jackpot = _positive_prob(model["jackpot_classifier"], xx)
    p_bad = _positive_prob(model["bad_classifier"], xx)
    p_cost3 = _positive_prob(model["cost3_classifier"], xx)
    return edge, p, q10, q90, p_jackpot, p_bad, p_cost3
...
def backtest(df: pd.DataFrame, bundle: dict[str, Any], model: dict[str, Any], cfg: CostRunnerConfig, *, fee: float, slip: float, cost_mult: float = 1.0, decisions: pd.DataFrame | None = None, record: bool = False) -> dict[str, Any]:
    ...
    for i in range(0, len(df) - 2):
        ...
        if pos != 0:
            ...
            if not reason and not add_done and cfg.full_add_frac > 0.0 and unreal >= cfg.min_unrealized and hold >= cfg.min_bars_since_entry and dd_abs <= cfg.dd_block:
                ...
                edge, p, q10, q90, p_jackpot, p_bad, p_cost3 = _predict_cost_runner(model, x)
                cap = min(parent_notional * cfg.max_total_mult, cfg.max_total_notional)
                action_frac = 0.0
                is_jackpot = p_jackpot >= cfg.jackpot_p and q90 >= cfg.jackpot_q90 and p_bad <= cfg.bad_cap and p_cost3 >= 0.40
                if is_jackpot:
                    action_frac = cfg.full_add_frac
                delta = max(0.0, min(parent_notional * action_frac, cap - notional))
                if delta > 1e-12:
                    ...
                    lane = "jackpot" if is_jackpot else ("full" if action_frac == cfg.full_add_frac else "half")
                    runner_actions["jackpot_add" if lane == "jackpot" else ("full_add" if lane == "full" else "half_add")] = runner_actions.get("jackpot_add" if lane == "jackpot" else ("full_add" if lane == "full" else "half_add"), 0) + 1
                    if record and open_record is not None:
                        open_record.update({"add_on_timestamp": str(df["timestamp"].iloc[fill_i]), "add_on_delta_notional": float(delta), "add_on_price": float(add_px), "cost_runner_edge": float(edge), "cost_runner_prob": float(p), "cost_runner_q10": float(q10), "cost_runner_q90": float(q90), "p_jackpot": float(p_jackpot), "p_bad_addon": float(p_bad), "p_cost3_survive": float(p_cost3), "add_action": lane, "add_fee_pct": float(fee_eff * delta * 100.0)})
                else:
        ...
        entry_equity = cash
        entry_idx = i
        parent_notional = min(float(dec.notional_exposure), cfg.max_entry_notional, cfg.max_total_notional)
        notional = parent_notional
...
def main() -> int:
    ...
    model_path = args.out_dir / "v21_3_5x_jackpot_runner.pkl"
    ...
    pd.DataFrame([{**{f"cfg_{k}": v for k, v in r["config"].items()}, "score": r["selection_score"], "val_pnl": r["validation_cost1"]["pnl"], "val_mdd": r["validation_cost1"]["mdd"], "val_trades": r["validation_cost1"]["trades"], "val_c2_pnl": r["validation_cost2"]["pnl"], "val_c3_pnl": r["validation_cost3"]["pnl"], "val_add": r["validation_cost1"]["runner_actions"].get("add_on", 0), "val_full": r["validation_cost1"]["runner_actions"].get("full_add", 0), "val_half": r["validation_cost1"]["runner_actions"].get("half_add", 0), "val_reject": r["validation_cost1"]["runner_actions"].get("reject", 0)} for r in rows]).to_csv(args.grid_out, index=False)
    blocking: list[str] = []
    warnings: list[str] = []
    ...
    if max(float(metrics[k]["avg_notional"]) for k in ("cost1", "cost2", "cost3")) > 5.0 + 1e-9:
        blocking.append("avg_notional_exceeds_5x_cap")
    audit = {"status": "pass" if not blocking else "fail", "verdict": "promote" if not blocking and metrics["cost1"]["pnl"] > 139.40707050411612 and metrics["cost2"]["pnl"] > 0.0 else "iterate", "blocking": blocking, "warnings": warnings, "selection_uses_2026": False, "selection_window": "2025-10-01..2025-12-31", "oos_window": "2026 fixed OOS only after selection", "model_meta": {k: v for k, v in model.items() if k not in {"regressor", "q10_regressor", "q90_regressor", "classifier", "jackpot_classifier", "bad_classifier", "cost3_classifier", "feature_cols"}}, "feature_audit": feature_audit, "selected_config": asdict(selected), "metrics": metrics}
    report = {"model_id": MODEL_ID, "design": "v21.3 5x jackpot-only expansion. V21.2 jackpot gate is preserved while max entry notional, add-on fraction, and total multiplier are selected on 2025 validation with an absolute 5x notional cap.", "base_model": str(args.model), "model": str(model_path), "split_policy": "Tail runner trained on 2025 Jan-Sep; config selected on 2025 Oct-Dec; 2026 fixed OOS not used for selection.", "selected_config": asdict(selected), "selection_result": best, "metrics": metrics, "audit": audit, "artifacts": {"model": str(model_path), "report": str(args.report_out), "audit": str(args.audit_out), "grid": str(args.grid_out), "ledgers": ledgers}}
