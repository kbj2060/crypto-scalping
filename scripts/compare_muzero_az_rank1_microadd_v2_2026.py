...
MODEL_ID = "muzero_az_rank1_microadd_v2_2026"
DEFAULT_REPORT_OUT = ROOT / "data/ensemble/reports/muzero_az_rank1_microadd_v2_2026.json"
DEFAULT_MODEL_DIR = ROOT / "data/ensemble/supervised/muzero_az_rank1_microadd_v2"
...
MICROADD_NOTIONAL_CAPS = (0.05, 0.075, 0.10, 0.15, 0.20)
...
def _microadd_grid_configs(args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    for margin in (0.10, 0.15, 0.20, 0.30):
        for agreement in (2, 3):
            for survival in (0.54, 0.58, 0.62, 0.70):
                for edge_floor in (0.0, 0.0005, 0.0010, 0.0020):
                    for max_cvar_loss in (0.008, 0.012, 0.018):
...
def _selection_score(candidate: dict[str, Any], baseline: dict[str, Any], cost3_candidate: dict[str, Any], cost3_baseline: dict[str, Any]) -> float:
    ...
    return float(pnl_delta - mdd_penalty + 8.0 * tpd_delta + microadd_bonus - 0.35 * cost3_degradation)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Compare current-top MuZero/AZ Stage2+AZ Exit with alpha-preserving defensive + micro-add sleeve v2.")
    ...
    p.add_argument("--max-grid-configs", type=int, default=160)
...
def main() -> int:
    ...
    joblib.dump(
        ...
        args.model_dir / "rank1_microadd_v2.pkl",
    )
    selector_payload = {
    ...
    report = {
        ...
        "note": "Frozen current-top MuZero/AZ Stage2+AZ Exit baseline plus defensive monotonic active-row sleeve and deterministic current-bar-vote micro-adds only when sequential baseline replay is flat.",
        ...
        "eval": {
            ...
            "rank1_microadd_v2": candidate_eval,
        ...
        "calibration_audit": {"selector_artifact": str(args.model_dir / "threshold_selector.json"), "model_artifact": str(args.model_dir / "rank1_microadd_v2.pkl")},
        ...
        "limitations": ["Funding, liquidation, and maintenance-margin path are not first-class in this loop-2 implementation."],
    }
