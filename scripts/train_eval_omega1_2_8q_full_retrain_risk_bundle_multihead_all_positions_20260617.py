...
MODEL_ID = "omega1_2_8q_full_retrain_risk_bundle_multihead_all_positions_20260617"
OUT_DIR = ROOT / "tmp/causal_regen_20260516" / MODEL_ID
RISK = sleeve.FallbackRisk("risk_bundle_multihead_base_n081_l2_h192", 0.052, 0.028, 0.81, 2.0, 192)
BASELINE_ID = "omega1_2_true_3head_tabm_20260603_full_retrain_cash_alpha43_20260608"
SLTP_BUCKETS = (
    sleeve.FallbackRisk("base_tp052_sl028_n081_l2_h192", 0.052, 0.028, 0.81, 2.0, 192),
    sleeve.FallbackRisk("tight_notional_tp052_sl028_n105_l259_h192", 0.052, 0.028, 1.05, 2.5925925926, 192),
    sleeve.FallbackRisk("balanced_tp065_sl034_n095_l235_h192", 0.065, 0.034, 0.95, 2.3456790123, 192),
    sleeve.FallbackRisk("runner_tp078_sl042_n081_l2_h192", 0.078, 0.042, 0.81, 2.0, 192),
    sleeve.FallbackRisk("fast_cut_tp040_sl020_n081_l2_h96", 0.040, 0.020, 0.81, 2.0, 96),
    sleeve.FallbackRisk("tight_fast_tp040_sl020_n105_l259_h96", 0.040, 0.020, 1.05, 2.5925925926, 96),
)
RISK_LABEL_UTILITY_CFG = {"stop_penalty": 0.003, "mae_penalty": 0.20, "time_penalty": 0.0}
UTILITY_CFGS = (
...
def _risk_utility(d: dict[str, Any], risk: sleeve.FallbackRisk, cfg: dict[str, float]) -> float:
    adverse = abs(min(float(d["mae"]), 0.0))
    time_frac = min(float(d["bars"]) / float(max(int(risk.max_hold_bars), 1)), 1.0)
    return float(d["net"]) - float(cfg["stop_penalty"]) * int(d["stop"]) - float(cfg["mae_penalty"]) * adverse - float(cfg["time_penalty"]) * time_frac


def _bucket_label_table(payload: dict[str, Any]) -> tuple[pd.DataFrame, dict[str, Any]]:
    frame = payload["frame"].reset_index(drop=True)
    active = omega._active(payload["dec"].reset_index(drop=True))
    rows: list[dict[str, Any]] = []
    cash_idx = np.flatnonzero(~active)
    for row_id, i in enumerate(cash_idx):
        if row_id % 1000 == 0:
            print(json.dumps({"stage": "bucket_labels", "row": int(row_id), "total_cash": int(len(cash_idx))}, ensure_ascii=True), flush=True)
        if i >= len(frame) - max(int(r.max_hold_bars) for r in SLTP_BUCKETS) - 3:
            continue
        ...
        for side_name, side in (("long", 1), ("short", -1)):
            ...
            nets: list[float] = []
            stops: list[int] = []
            for risk in SLTP_BUCKETS:
                d = _simulate_side_detail(payload, int(i), side, risk)
                utils.append(_risk_utility(d, risk, RISK_LABEL_UTILITY_CFG))
                nets.append(float(d["net"]))
                stops.append(int(d["stop"]))
            best = int(np.argmax(np.asarray(utils, dtype=np.float64)))
            row[f"{side_name}_bucket"] = best
            row[f"{side_name}_bucket_utility"] = float(utils[best])
            row[f"{side_name}_bucket_net"] = float(nets[best])
            row[f"{side_name}_bucket_stop"] = int(stops[best])
        rows.append(row)
    labels = pd.DataFrame(rows)
    diag = {
        "valid_cash_rows": int(len(labels)),
        "utility_cfg": dict(RISK_LABEL_UTILITY_CFG),
        "buckets": [asdict(r) for r in SLTP_BUCKETS],
        "long_bucket_counts": labels["long_bucket"].value_counts().sort_index().to_dict() if len(labels) else {},
        "short_bucket_counts": labels["short_bucket"].value_counts().sort_index().to_dict() if len(labels) else {},
    }
    return labels, diag
...
def _fit_predict_bucket_head(
    ...
    *,
    ...
    y = np.full(len(x_val), int(default_bucket), dtype=np.int64)
    y[idx] = labels[col].to_numpy(dtype=np.int64)
    val_pred = np.full(len(x_val), int(default_bucket), dtype=np.int64)
    folds_meta: list[dict[str, Any]] = []
    for fold_id, (tr, va) in enumerate(_chron_folds(idx)):
        clf = HistGradientBoostingClassifier(max_iter=120, learning_rate=0.035, max_leaf_nodes=9, l2_regularization=2.0, random_state=int(seed + fold_id * 10))
        clf.fit(x_val.iloc[tr].to_numpy(dtype=np.float64), y[tr])
        pred = clf.predict(x_val.iloc[va].to_numpy(dtype=np.float64)).astype(np.int64)
        val_pred[va] = np.clip(pred, 0, len(SLTP_BUCKETS) - 1)
        folds_meta.append({"fold": int(fold_id), "train_rows": int(len(tr)), "val_rows": int(len(va)), "classes": [int(c) for c in clf.classes_]})

    clf = HistGradientBoostingClassifier(max_iter=120, learning_rate=0.035, max_leaf_nodes=9, l2_regularization=2.0, random_state=int(seed + 101))
    clf.fit(x_val.iloc[idx].to_numpy(dtype=np.float64), y[idx])
    oos_pred = np.clip(clf.predict(x_oos.to_numpy(dtype=np.float64)).astype(np.int64), 0, len(SLTP_BUCKETS) - 1)
    diag = {
        "target_col": col,
        "default_bucket": int(default_bucket),
        "val_counts": pd.Series(val_pred).value_counts().sort_index().to_dict(),
        "oos_counts": pd.Series(oos_pred).value_counts().sort_index().to_dict(),
        "folds": folds_meta,
    }
    return val_pred, oos_pred, diag


def _fit_bucket_heads(x_val: pd.DataFrame, x_oos: pd.DataFrame, labels: pd.DataFrame) -> tuple[dict[str, np.ndarray], dict[str, np.ndarray], dict[str, Any]]:
    val: dict[str, np.ndarray] = {}
    oos: dict[str, np.ndarray] = {}
    diag: dict[str, Any] = {}
    for name, col, seed in (("long_bucket", "long_bucket", 283101), ("short_bucket", "short_bucket", 283201)):
        val[name], oos[name], diag[name] = _fit_predict_bucket_head(x_val, x_oos, labels, col, seed=seed, default_bucket=1)
    return val, oos, diag


def _bucket_risk_for_row(side: int, i: int, preds: dict[str, np.ndarray]) -> sleeve.FallbackRisk:
    key = "long_bucket" if int(side) > 0 else "short_bucket"
    bucket = int(np.clip(int(preds[key][int(i)]), 0, len(SLTP_BUCKETS) - 1))
    return SLTP_BUCKETS[bucket]


def _open_position_with_risk(
    cash: float,
...
def _metrics_with_bucket_sltp(
    frame: pd.DataFrame,
    dec: pd.DataFrame,
    risk_preds: dict[str, np.ndarray],
    ...
    *,
    ...
    for i in range(0, len(frame) - 2):
        ...
        if bool(active[i]):
            ...
            if side != 0:
                row_risk = _bucket_risk_for_row(side, i, risk_preds)
                cash, pos, entered = _open_position_with_risk(cash, arrays, i, side, "primary", row_risk, fee_eff, slip_eff)
                if entered:
                    primary_entries += 1
                    long_entries += int(side > 0)
                    short_entries += int(side < 0)
                    primary_bucket_counts[row_risk.name] = primary_bucket_counts.get(row_risk.name, 0) + 1
                    primary_tp.append(float(row_risk.take_profit))
                    primary_sl.append(float(row_risk.stop_loss))
                    primary_notional.append(float(row_risk.notional))
                    primary_leverage.append(float(row_risk.leverage))
        else:
            ...
            if side_action in (sleeve.ACTION_LONG, sleeve.ACTION_SHORT) and conf >= float(threshold):
                ...
                if entered:
                    ...
                    long_entries += int(side > 0)
                    short_entries += int(side < 0)
                    fallback_bucket_counts[row_risk.name] = fallback_bucket_counts.get(row_risk.name, 0) + 1
    ...
    return {
        ...
        "primary_avg_tp": float(np.mean(primary_tp)) if primary_tp else 0.0,
        "primary_avg_sl": float(np.mean(primary_sl)) if primary_sl else 0.0,
        "primary_avg_notional": float(np.mean(primary_notional)) if primary_notional else 0.0,
        "primary_avg_leverage": float(np.mean(primary_leverage)) if primary_leverage else 0.0,
        "primary_avg_tp_price_move": float(np.mean(primary_tp_arr / np.maximum(primary_notional_arr, 1.0e-12))) if primary_tp else 0.0,
        "primary_avg_sl_price_move": float(np.mean(primary_sl_arr / np.maximum(primary_notional_arr, 1.0e-12))) if primary_sl else 0.0,
        ...
        "fallback_bucket_counts": fallback_bucket_counts,
    }
...
def _metric_row(candidate: str, family: str, cfg_id: int | None, cal_q: float, ev_min: float, utility_min: float | None, margin_min: float | None, val_m: dict[str, Any], oos_m: dict[str, Any], base_val: dict[str, Any], base_oos: dict[str, Any]) -> dict[str, Any]:
    ...
    row["val_primary_bucket_counts"] = val_m.get("primary_bucket_counts", {})
    row["oos_primary_bucket_counts"] = oos_m.get("primary_bucket_counts", {})
...
def main() -> int:
    ...
    bucket_labels, bucket_diag = _bucket_label_table(val_payload)
    val_bucket_preds, oos_bucket_preds, bucket_fit_diag = _fit_bucket_heads(x_val, x_oos, bucket_labels)
    ...
    diagnostics: dict[str, Any] = {
        "mode": "full_retrain_parent_numeric_cash_sleeve_with_risk_bundle_multihead_all_positions",
        ...
        "risk_bundles": [asdict(r) for r in SLTP_BUCKETS],
        "risk_bundle_semantics": "multihead chooses a bundled TP/SL/notional/leverage/max_hold contract for both primary and fallback positions",
        ...
        "bucket_labels": bucket_diag,
        "bucket_heads": bucket_fit_diag,
        "ev_labels": ev_diag,
    }
    ...
    for cal_q in (0.50, 0.65, 0.80):
        ...
        for ev_min in (0.001, 0.002, 0.003, 0.004):
            ...
            val_m = _metrics_with_bucket_sltp(val_payload["frame"], val_payload["dec"], val_bucket_preds, val_ev_a, val_ev_c, 0.0, fee=fee, slip=slip, cost_mult=3.0)
            oos_m = _metrics_with_bucket_sltp(oos_payload["frame"], oos_payload["dec"], oos_bucket_preds, oos_ev_a, oos_ev_c, 0.0, fee=fee, slip=slip, cost_mult=3.0)
            ...
            for cfg_id, (uvl, uvs, uol, uos) in utility_preds.items():
                for utility_min in (-0.001, 0.0, 0.001, 0.002):
                    for margin_min in (0.0, 0.001, 0.002):
                        ...
                        val_m = _metrics_with_bucket_sltp(val_payload["frame"], val_payload["dec"], val_bucket_preds, val_a, val_c, 0.0, fee=fee, slip=slip, cost_mult=3.0)
                        oos_m = _metrics_with_bucket_sltp(oos_payload["frame"], oos_payload["dec"], oos_bucket_preds, oos_a, oos_c, 0.0, fee=fee, slip=slip, cost_mult=3.0)
