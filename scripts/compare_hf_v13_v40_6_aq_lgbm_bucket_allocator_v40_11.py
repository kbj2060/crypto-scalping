...
MODEL_ID = "hf_v13_tree_vs_foundation_aq_lgbm_bucket_allocator_v40_11_20260512"
...
DEFAULT_OUT_DIR = ROOT / "data/ensemble/supervised/hf_v13_tree_vs_foundation_aq_lgbm_bucket_allocator_v40_11_20260512"
DEFAULT_REPORT = ROOT / "data/ensemble/reports/hf_v13_tree_vs_foundation_aq_lgbm_bucket_allocator_v40_11_20260512_summary.json"
DEFAULT_AUDIT = ROOT / "data/ensemble/reports/hf_v13_tree_vs_foundation_aq_lgbm_bucket_allocator_v40_11_20260512_audit.json"
DEFAULT_GRID = ROOT / "data/ensemble/reports/hf_v13_tree_vs_foundation_aq_lgbm_bucket_allocator_v40_11_20260512_grid.csv"
...
def _stage2_feature_frame(
    ...
    *,
    ...
    return pd.concat([aq_reset, ctx], axis=1)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "V40.11: v40.6 action/quality parent + LightGBM bucket allocator. "
            "Compares AQ-only and AQ+encoded-context stage2 inputs with OOF stage1 predictions."
        )
    )
...
def main() -> int:
    ...
    aq_hp = TreeHParams(220, 0.040, 31, 0.08, 0.35)
    print(f"[{MODEL_ID}] training action/quality-only parent", flush=True)
    aq_bundle = _train_aq_model(x_train_full, y, cfg=cfg, hp=aq_hp, random_state=int(args.seed), feature_cols=list(x_train_full.columns))
    ...
    val_aq = _aq_outputs(aq_bundle, val_full)
    eval_aq = _aq_outputs(aq_bundle, eval_full)
    context_cols = list(x_train_full.columns)

    print(f"[{MODEL_ID}] selecting LightGBM bucket allocator on 2025 Q4", flush=True)
    grid_rows: list[dict[str, Any]] = []
    best: dict[str, Any] | None = None
    stage2_modes = ("aq_only", "aq_plus_context")
    candidate_total = len(stage2_modes) * len(_bucket_hp_grid())
    candidate_idx = 0
    for mode in stage2_modes:
        ...
        for hp in _bucket_hp_grid():
            candidate_idx += 1
            print(f"[{MODEL_ID}] bucket candidate {candidate_idx}/{candidate_total}: {mode} | {hp.name}", flush=True)
            bucket_bundle = _fit_bucket_allocator(
                stage2_train_x,
                y_oof,
                cfg=cfg,
                hp=hp,
            ...
            val_dec = _predict_two_stage(aq_bundle, bucket_bundle, val_full)
            ...
            row = {
                ...
                **asdict(hp),
                ...
                "stage2_train_rows": int(len(stage2_train_x)),
                "stage2_trade_rows": int(np.sum(np.asarray(y_oof["action"]) != ACTION_CASH)),
    ...
    eval_dec = _predict_two_stage(aq_bundle, selected_bucket, eval_full)
    ...
    audit = {
        ...
        "selection_window": "2025 Q4 bucket allocator HP selection",
        "oos_window": "2026 fixed OOS only after selection",
        "stage1": "v40_6 encoded action/quality HGB only",
        "stage2": "LightGBM bucket allocator selected between AQ-only and AQ+encoded-context features",
    ...
    report = {
        "model_id": MODEL_ID,
        "design": "Two-stage v40.6 parent split. Stage 1 keeps Chronos/Kairos PLS encoded HGB but only trains action and quality heads. Stage 2 trains one LightGBM bucket allocator family for notional, leverage, TP, SL, max_hold, and cooldown. Stage 2 training uses forward OOF action/quality predictions to avoid same-row stacking leakage and compares AQ-only versus AQ+encoded-context inputs.",
        ...
        "stage2_oof_trade_rows": int(np.sum(np.asarray(y_oof["action"]) != ACTION_CASH)),
    ...
    print(json.dumps({"report": str(args.report_out), "audit": str(args.audit_out), "grid": str(args.grid_out), "selected_bucket": asdict(best["hp"]), "metrics": metrics, "audit_status": audit["status"]}, ensure_ascii=False, default=_json_default))
    return 0
