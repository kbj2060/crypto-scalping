#!/usr/bin/env python3
"""Red-team audit for Omega 4.6.2 v5 roll8 side-specific PnL-tilt branch."""
...
RANKING = OUT_DIR / "roll8_side_specific_pnl_tilt_ranking.csv"
EVAL_SCRIPT = ROOT / "scripts/eval_omega4_6_2_v5_roll8_side_specific_pnl_tilt_20260701.py"
PRIOR_RUNTIME_AUDIT = (
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll8_side_specific_pnl_tilt_redteam_20260701.md"
...
def write_markdown(payload: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- Roll max hold: `{selected['roll8_max_hours']}`",
        f"- Long TP/SL: `{selected['roll8_long_tp_move']}` / `{selected['roll8_long_sl_move']}`",
        f"- Short TP/SL: `{selected['roll8_short_tp_move']}` / `{selected['roll8_short_sl_move']}`",
        f"- Validation avg-hold tolerance: `{VALIDATION_AVG_HOLD_TOLERANCE_HOURS}h`",
        "",
...
def main() -> None:
    audit = load_module("omega462_loss_cluster_audit_for_roll8_pnl_tilt", AUDIT_V1_PATH)
    segment_audit = load_module("omega462_segment_audit_for_roll8_pnl_tilt", AUDIT_SEGMENT_PATH)
    ...
    audit.add_check(
        ...
        == "validation_primary_roll8_pnl_tilt_with_oos_safety_gate; fresh_holdout_required",
        "research_blocker",
    ...
    audit.add_check(
        ...
        == "among research-gated pnl-tilt variants, sort by validation_pnl, validation_avg_hold, cap distance to 4.20; OOS is not an ordering key",
        "research_blocker",
        {"observed": report.get("selection_rule")},
    )
    audit.add_check(
        checks,
        "selected_roll8_pnl_tilt_gate_pass",
        selected.get("research_roll8_pnl_tilt_gate_pass") is True,
        "research_blocker",
        {"observed": selected.get("research_roll8_pnl_tilt_gate_pass")},
    )
    ...
    audit.add_check(
        checks,
        "roll8_pnl_tilt_hold_contract",
        validation["max_hold_hours"] <= 8.0
        and oos["max_hold_hours"] <= 8.0
        and validation["avg_hold_hours"]
        <= reference["validation_avg_hold_hours"] + VALIDATION_AVG_HOLD_TOLERANCE_HOURS + TOL
    ...
    audit.add_check(
        checks,
        "improves_reference_pnl_with_oos_hold_improvement",
    ...
    audit.add_check(
        ...
        float(selected["roll8_max_hours"]) == 8.0
        and float(selected["roll8_long_tp_move"]) > 0.0
        and float(selected["roll8_long_sl_move"]) > 0.0
        and float(selected["roll8_short_tp_move"]) > 0.0
        and float(selected["roll8_short_sl_move"]) > 0.0
        and {"roll8_bracket_tp", "roll8_bracket_sl"}.issubset(set(validation_df["reason"].astype(str))),
        "research_blocker",
        {
            "long_tp": selected.get("roll8_long_tp_move"),
            "long_sl": selected.get("roll8_long_sl_move"),
            "short_tp": selected.get("roll8_short_tp_move"),
            "short_sl": selected.get("roll8_short_sl_move"),
        },
    )
    ...
    verdict = (
        "FULL_LIVE_PASS"
        if full_live_pass
        else "RESEARCH_ROLL8_PNL_TILT_PASS_FULL_LIVE_BLOCKED"
        if research_pass
        else "REDTEAM_FAIL"
    )
    payload = {
        "audit_id": "omega4_6_2_v5_roll8_side_specific_pnl_tilt_redteam_20260701",
