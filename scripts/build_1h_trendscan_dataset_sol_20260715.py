"""SOL port of build_1h_trendscan_dataset_20260705.py (Sigma3 dataset builder). resample_1h /
compute_features are copied verbatim (no numba dependency). The trend-scanning core
(_trend_scan_fast, normally numba-JIT'd in build_trend_scanning_action_labels_20260531.py) is
reimplemented here fully vectorized in numpy/pandas -- importing the numba module directly fails
in this venv (numba requires numpy<2.3, this venv has numpy 2.3.5, a known conflict noted in
project memory for the Omega risk-sidecar pickles). Numerically equivalent OLS-identity math
(SS_res = SS_tot - beta*S_xy), verified against the numba version's formulas.

This is the first stage of porting Sigma6 (1h trend-scanning signal + Sigma5 trailing-stop +
Regime3 not-chop filter) to SOL -- the strongest-generalizing verified labeling approach in the
project (ETH OOS +45.9%/+16.6%), chosen after RL/DP labeling failed three ways this session.
"""
...
OUT_DIR = ROOT / "tmp/causal_regen_20260516/sigma3_1h_trendscan_sol_20260715"
SRC_FILES = {
...
def trend_scan_fast(values: np.ndarray, windows: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    CAUSALITY FIX 2026-08-04: the previous pandas-rolling version shifted each window's rolling
    sum backward (`sum_y[:n-L+1] = sum_y_full[L-1:]`, and the matching `s1` shift), which assigns
    the window ending at L-1 to output index 0 -- i.e. index j received the sum of
    values[j:j+L], reading up to L-1 bars INTO THE FUTURE relative to j. Confirmed via direct
    empirical recomputation against stored ts_t_value on saved BTC/ETH outputs. Rewritten using
    sliding_window_view with an explicit end-of-window destination index (see
...
def main() -> int:
    ...
    for year, path in SRC_FILES.items():
        ...
        out_path = OUT_DIR / f"sigma3_1h_sol_{year}.parquet"
