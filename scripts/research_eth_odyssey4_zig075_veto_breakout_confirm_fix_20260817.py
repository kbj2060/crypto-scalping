#!/usr/bin/env python3
"""RESEARCH ONLY -- candidate fix for the zig075 SHORT/sustained-uptrend veto's ranging-regime
...
=== Design discipline (matched to the ORIGINAL detector's own documented discipline in
research_eth_omega461_regime_aware_exit_head_uptrend_guard_20260814.py's module docstring
Step 1-4 -- read in full before writing this, not re-derived from memory) ===
...
DETECTOR_PERCENTILE=0.90 is a round, unswept decile convention. This script's fix candidate is
held to the SAME bar:

Hypothesis for the misfire mechanism: dual_momentum's rolling-week-positive-fraction score can
stay high during a range-bound "local bounce" (price recovering toward the top of an existing
range) purely because the trailing-1-week return is positive, WITHOUT price ever making a new
multi-week high -- a genuine sustained uptrend, by contrast, naturally keeps setting new highs
while momentum stays persistently positive. Fix: AND the existing (unmodified) momentum condition
with a breakout-confirmation condition that reuses the SAME WEEK_BARS=2016 window already in the
original detector (no new lookback parameter) and needs no threshold of its own (no new tunable
number): `close_t >= rolling_max(close, WEEK_BARS)_t`, i.e. "this bar's close is a new >=1-week
high". This is the closest possible companion to the original design -- same window, zero new
knobs.

=== Verification protocol (pre-registered before reading any window's own result) ===
...
OUT_DIR = ROOT / "tmp/causal_regen_20260516/eth_odyssey4_zig075_veto_breakout_confirm_fix_20260817"
N_SEEDS = 20
...
def _rolling_close_high_confirmed(base_csv: Path) -> pd.DataFrame:
    """Mirrors guard._rolling_dual_momentum_score's exact read/sort/dedup discipline. New column:
    True iff this bar's close is >= the trailing WEEK_BARS-bar rolling max of close (i.e. a new
    >=1-week high) -- zero new tunable parameters, same window as the original detector."""
    ...
    rolling_max = close.rolling(WEEK_BARS, min_periods=WEEK_BARS).max()
    frame["breakout_confirmed"] = close >= rolling_max
...
def main() -> int:
    ...
    log("=== stage=detector_build (v1 unmodified, v2 = v1 AND breakout_confirmed) ===")
