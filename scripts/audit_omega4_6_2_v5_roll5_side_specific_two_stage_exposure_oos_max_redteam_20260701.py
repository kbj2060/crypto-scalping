#!/usr/bin/env python3
"""Red-team audit for Omega 4.6.2 v5 roll5 OOS-max branch."""
...
ROLL5_EVAL_PATH = ROOT / "scripts/eval_omega4_6_2_v5_roll5_side_specific_two_stage_exposure_hold_compressed_20260701.py"
...
RANKING = OUT_DIR / "roll5_oos_max_ranking.csv"
TOP20 = OUT_DIR / "roll5_oos_max_top20.csv"
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll5_side_specific_two_stage_exposure_oos_max_redteam_20260701.md"
TOL = 1.0e-8
...
def main() -> None:
    audit = load_module("omega462_loss_cluster_audit_for_roll5_oos_max", AUDIT_HELPER_PATH)
    roll5_eval = load_module("omega462_roll5_eval_for_oos_max_audit", ROLL5_EVAL_PATH)
    ...
    missing = {
        ...
        for name, path in {
            ...
            "ranking": RANKING,
            "top20": TOP20,
            "roll5_eval_script": ROLL5_EVAL_PATH,
    ...
    audit.add_check(
        ...
        report.get("selection_scope") == "roll5_validation_nearmax_oos_max; fresh_holdout_required",
        "research_blocker",
    ...
    audit.add_check(
        checks,
        "selected_gate_pass",
        selected.get("research_roll5_oos_max_gate_pass") is True
        and selected.get("research_roll5_hold_compressed_gate_pass") is True,
        "research_blocker",
        {
            "oos_max": selected.get("research_roll5_oos_max_gate_pass"),
            "base_gate": selected.get("research_roll5_hold_compressed_gate_pass"),
        },
    )
    ...
    audit.add_check(
        ...
        and oos["mdd"] >= -20.0
        and validation["max_hold_hours"] <= 5.0
        and oos["max_hold_hours"] <= 5.0
    ...
    verdict = (
        "FULL_LIVE_PASS"
        if full_live_pass
        else "RESEARCH_ROLL5_OOS_MAX_PASS_FULL_LIVE_BLOCKED"
        if research_pass
        else "REDTEAM_FAIL"
    )
    payload = {
        "audit_id": "omega4_6_2_v5_roll5_side_specific_two_stage_exposure_oos_max_redteam_20260701",
