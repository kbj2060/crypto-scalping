"""Stage 0.5 GO/NO-GO test (docs/btc_panel_crossasset_architecture_design_20260804.md):
identical architecture/labels/thresholds/split as
scripts/train_eval_btc_dense_nogate_quality_20260804.py (the existing closed-line baseline,
results in tmp/btc_dense_nogate_quality_20260804.csv), with ONLY the feature set changed:
causalfix_final's 114 cols + the 16 new panel market-state cols from
data/splits/year_oos/btc_panel_marketstate_features_20260804.parquet (Stage 0.5,
scripts/build_btc_panel_marketstate_features_20260804.py).

Purpose: isolate whether the new panel-derived cross-sectional data has ANY incremental signal
for BTC before building any new model/backbone. If this doesn't beat the 114-col-only baseline,
the panel/transformer architecture (Stage 1+) is not built -- see design doc section 4 GO/NO-GO
criteria.
"""
import sys
import time
...
MARKETSTATE_PATH = ROOT / "data/splits/year_oos/btc_panel_marketstate_features_20260804.parquet"
FUNDING_OI_PATH = ROOT / "data/splits/year_oos/btc_panel_funding_oi_features_20260804.parquet"
OUT_CSV = ROOT / "tmp/btc_dense_nogate_quality_marketstate_20260804.csv"
...
def main():
    ...
    marketstate = pd.read_parquet(MARKETSTATE_PATH)
    funding_oi = pd.read_parquet(FUNDING_OI_PATH)
    frame = frame.merge(marketstate, on="timestamp", how="left").merge(funding_oi, on="timestamp", how="left")

    marketstate_cols = [c for c in marketstate.columns if c != "timestamp"] + \
                       [c for c in funding_oi.columns if c != "timestamp"]
    feat_cols = base_feat_cols + marketstate_cols
    print(f"feat_cols: {len(base_feat_cols)} base (causalfix_final) + {len(marketstate_cols)} market-state = {len(feat_cols)}")
    n = len(frame)
    ...
    for name, horizon, tp_mult, sl_mult in CONFIGS:
        ...
        n_marketstate_in_top20 = sum(1 for c in imp.head(20).index if c in marketstate_cols)
        print(f"  top 15 features (long): {list(imp.head(15).index)}")
        print(f"  market-state features in top 20 by importance: {n_marketstate_in_top20}/20")
    ...
    print(f"\n=== [market-state] Configs with VAL AND OOS both positive (n>=15 each side): {len(both)} ===")
    if len(both):
    ...
    # --- head-to-head vs the 114-col-only baseline ---
    if BASELINE_CSV.exists():
        base = pd.read_csv(BASELINE_CSV)
        merged = out.merge(base, on=["config", "split", "thresh"], suffixes=("_marketstate", "_baseline"))
        merged["oos_improvement_pct"] = merged["mean_net_pct_marketstate"] - merged["mean_net_pct_baseline"]
        print("\n=== head-to-head: market-state vs 114-col-only baseline (same config/split/thresh) ===")
        print(merged[["config", "split", "thresh", "n_trades_baseline", "mean_net_pct_baseline",
                       "n_trades_marketstate", "mean_net_pct_marketstate", "oos_improvement_pct"]]
