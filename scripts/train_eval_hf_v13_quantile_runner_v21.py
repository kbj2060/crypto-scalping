...
MODEL_ID = "hf_v13_quantile_runner_v21_20260511"
...
DEFAULT_OUT_DIR = ROOT / "data/ensemble/supervised/hf_v13_quantile_runner_v21_20260511"
DEFAULT_REPORT = ROOT / "data/ensemble/reports/hf_v13_quantile_runner_v21_20260511_summary.json"
DEFAULT_AUDIT = ROOT / "data/ensemble/reports/hf_v13_quantile_runner_v21_20260511_audit.json"
DEFAULT_GRID = ROOT / "data/ensemble/reports/hf_v13_quantile_runner_v21_20260511_grid.csv"
...
def _grid() -> list[CostRunnerConfig]:
    ...
    i = 0
    for full_edge, half_edge in ((0.010, 0.002), (0.010, 0.004), (0.015, 0.002), (0.006, 0.004), (0.020, 0.006)):
        for p_th in (0.55, 0.65):
            for q10_floor in (-0.012, -0.006, 0.0):
                rows.append(CostRunnerConfig(f"v21_quantile_runner_{i}", full_edge, half_edge, p_th, q10_floor, 0.004, 3, 0.25, 0.125, 1.35, 2.75, 0.30))
                i += 1
    rows.append(CostRunnerConfig("v21_parent_noop", 99.0, 99.0, 1.0, 99.0, 99.0, 999, 0.0, 0.0, 1.0, 99.0, 99.0))
    return rows
...
def _fit_cost_runner(frame: pd.DataFrame, bundle: dict[str, Any], *, fee: float, slip: float) -> dict[str, Any]:
    ...
    return {"regressor": reg, "q10_regressor": q10, "classifier": clf, "feature_cols": list(x.columns), "snapshot_count": int(len(x)), "positive_rate": float((y > 0.0).mean()), "target_mean": float(y.mean()), "target_p10": float(np.quantile(y, 0.10)), "target_p75": float(np.quantile(y, 0.75)), "target_p95": float(np.quantile(y, 0.95))}


def _predict_cost_runner(model: dict[str, Any], x: pd.DataFrame) -> tuple[float, float, float]:
    ...
    for i in range(0, len(df) - 2):
        ...
        if pos != 0:
            ...
            if not reason and not add_done and cfg.full_add_frac > 0.0 and unreal >= cfg.min_unrealized and hold >= cfg.min_bars_since_entry and dd_abs <= cfg.dd_block:
                ...
                edge, p, q10 = _predict_cost_runner(model, x)
                cap = parent_notional * cfg.max_total_mult
                action_frac = 0.0
                if edge >= cfg.full_edge_th and p >= cfg.p_th and q10 >= cfg.q10_floor:
                    action_frac = cfg.full_add_frac
                elif edge >= cfg.half_edge_th and p >= cfg.p_th and q10 >= min(cfg.q10_floor, -0.006):
                ...
                if delta > 1e-12:
                    ...
                    runner_actions["full_add" if action_frac == cfg.full_add_frac else "half_add"] = runner_actions.get("full_add" if action_frac == cfg.full_add_frac else "half_add", 0) + 1
                    if record and open_record is not None:
                        open_record.update({"add_on_timestamp": str(df["timestamp"].iloc[fill_i]), "add_on_delta_notional": float(delta), "add_on_price": float(add_px), "cost_runner_edge": float(edge), "cost_runner_prob": float(p), "cost_runner_q10": float(q10), "add_action": "full" if action_frac == cfg.full_add_frac else "half", "add_fee_pct": float(fee_eff * delta * 100.0)})
                else:
...
def main() -> int:
    ...
    model_path = args.out_dir / "v21_quantile_runner.pkl"
