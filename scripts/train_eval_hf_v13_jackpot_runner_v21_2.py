...
MODEL_ID = "hf_v13_jackpot_runner_v21_2_20260511"
...
DEFAULT_OUT_DIR = ROOT / "data/ensemble/supervised/hf_v13_jackpot_runner_v21_2_20260511"
DEFAULT_REPORT = ROOT / "data/ensemble/reports/hf_v13_jackpot_runner_v21_2_20260511_summary.json"
DEFAULT_AUDIT = ROOT / "data/ensemble/reports/hf_v13_jackpot_runner_v21_2_20260511_audit.json"
DEFAULT_GRID = ROOT / "data/ensemble/reports/hf_v13_jackpot_runner_v21_2_20260511_grid.csv"
...
def _grid() -> list[CostRunnerConfig]:
    ...
    i = 0
    for jackpot_p, jackpot_q90 in ((0.20, 0.015), (0.25, 0.015), (0.35, 0.015), (0.25, 0.020), (0.35, 0.020), (0.25, 0.025), (0.35, 0.025)):
        for bad_cap in (0.35, 0.50):
            rows.append(CostRunnerConfig(f"v21_2_jackpot_runner_{i}", 99.0, 99.0, 0.60, -0.006, jackpot_p, jackpot_q90, bad_cap, 0.004, 3, 0.20, 0.0, 1.35, 2.75, 0.30))
            i += 1
    rows.append(CostRunnerConfig("v21_2_parent_noop", 99.0, 99.0, 1.0, 99.0, 99.0, 99.0, 0.0, 99.0, 999, 0.0, 0.0, 1.0, 99.0, 99.0))
    return rows
...
def main() -> int:
    ...
    model_path = args.out_dir / "v21_2_jackpot_runner.pkl"
    ...
    report = {"model_id": MODEL_ID, "design": "v21.2 jackpot-only add-on runner. Robust/half lanes are disabled; add-ons require jackpot probability, q90 upside, bad-add-on cap, and cost3 survival gate.", "base_model": str(args.model), "model": str(model_path), "split_policy": "Tail runner trained on 2025 Jan-Sep; config selected on 2025 Oct-Dec; 2026 fixed OOS not used for selection.", "selected_config": asdict(selected), "selection_result": best, "metrics": metrics, "audit": audit, "artifacts": {"model": str(model_path), "report": str(args.report_out), "audit": str(args.audit_out), "grid": str(args.grid_out), "ledgers": ledgers}}
