"""Build a BTC feature panel + TB trade-outcome labels with the IDENTICAL pipeline used for the
ETH/SOL regime analysis (FeatureEngineer + corrected TB label constants), for the BTC
regime-conditioned entry line (2026-08-08, screen rank #1).

Secondary cross-asset series: ETH (for ETH-primary and SOL-primary panels the secondary is BTC;
for BTC-primary the other major is ETH). NOTE: FeatureEngineer's secondary columns are NAMED
close_btc/volume_btc/quote_volume_btc by contract — here they CONTAIN ETH data; features like
btc_corr_60 therefore mean corr-to-ETH in this panel. Cosmetic naming kept so the identical
pipeline runs untouched.

Outputs:
  data/splits/year_oos/btc_features_2024_2026_regimeline.csv
  data/splits/year_oos/btc_5m_tripbarrier_tradeoutcome_labels_regimeline_20260808.parquet
"""
...
SECONDARY_KLINE_PATH = ROOT / "binance_data/klines/ETHUSDT/ETHUSDT-5m-api.csv"
METRICS_DIR = ROOT / "binance_data/metrics"
FUNDING_DIR = ROOT / "binance_data/funding_rate_other"
FEATURES_OUT = ROOT / "data/splits/year_oos/btc_features_2024_2026_regimeline.csv"
LABELS_OUT = ROOT / "data/splits/year_oos/btc_5m_tripbarrier_tradeoutcome_labels_regimeline_20260808.parquet"
START = pd.Timestamp("2024-06-01")

PRIMARY_COLS = ["timestamp", "open", "high", "low", "close", "volume", "quote_volume",
                "trades", "taker_buy_base", "taker_buy_quote",
                "sum_open_interest_value", "sum_toptrader_long_short_ratio",
                "count_long_short_ratio", "last_funding_rate"]
SECONDARY_COLS = ["timestamp", "close_btc", "volume_btc", "quote_volume_btc"]
...
def main() -> int:
    ...
    sec = pd.read_csv(SECONDARY_KLINE_PATH, usecols=["timestamp", "close", "volume", "quote_volume"], low_memory=False)
    sec["timestamp"] = pd.to_datetime(sec["timestamp"])
    sec = sec.rename(columns={"close": "close_btc", "volume": "volume_btc", "quote_volume": "quote_volume_btc"})
    sec = sec.sort_values("timestamp").drop_duplicates("timestamp").reset_index(drop=True)
    ...
    raw = raw.merge(sec, on="timestamp", how="left")
    ...
    fe = FeatureEngineer()
    feats = fe.process(raw[PRIMARY_COLS].copy(), raw[SECONDARY_COLS].copy())
    ...
    print(f"wrote {LABELS_OUT}; CASH/LONG/SHORT = {int(counts.get(0,0))}/{int(counts.get(1,0))}/{int(counts.get(2,0))}", flush=True)
    return 0
