#!/usr/bin/env python3
"""BTC 증거신호 경제성게이트 통과의 **적대적 검증** -- 진입 타이밍이 무작위보다 나은가.

## 왜 필요한가

인덱스 오프셋 버그(108바)를 고치자 0/672 -> 208셀 통과로 뒤집혔다. 통과 자체는 이제
계산이 맞지만, **트레일링스톱은 진입이 무작위여도 높은 승률을 만든다**는 게 이 저장소의
기록된 사실이다(ETH orthogonal_combo: 승률 91~96%인데 무작위 진입도 83~85%).
그러므로 "통과했다"는 것만으로는 부족하고 **무작위 진입 대비 우위**를 봐야 한다.

특히 `demarker_extreme`은 hit률 0.90으로 사실상 상시 발동에 가까워 이 검정이 결정적이다.

## 검정 3종

  A. ⭐**무작위 진입 귀무분포(단일 셀, B=200)** -- 실제 통과 셀(ARM>=1.0 최선)을 고정하고,
     같은 개수/같은 롱숏 비율의 진입 시점을 **무작위로** 뽑아 같은 브래킷을 돌린다.
     실제 성과가 이 분포의 95번째 백분위 위여야 "타이밍에 정보가 있다".
  B. **무작위 진입 귀무분포(격자 전체, B=20)** -- 통과 셀 개수 자체가 우연인지.
     96셀 중 몇 개가 우연히 통과하는지의 분포를 만든다.
...
REPORT = ROOT / "data/research/btc_evidence_signals_costgate_20260902/report.json"
OUT = ROOT / "data/research/btc_evidence_signals_costgate_20260902/random_entry_null.json"
B_CELL, B_GRID, SEED = 200, 20, 20260902


def log(m): print(f"[rand-null] {m}", flush=True)
...
def random_fires(frame, n_long, n_short, rng):
    ...
    pick = rng.choice(len(f), size=n_long + n_short, replace=False)
    sub = f.iloc[np.sort(pick)][["timestamp", "atr", "close"]].reset_index(drop=True)
...
def main() -> int:
    ...
    out = {"B_cell": B_CELL, "B_grid": B_GRID, "seed": SEED, "signals": {}}
    ...
    for k, v2 in out["signals"].items():
        log(f"  {k:<26} A(VAL/OOS) {v2['A_random_entry']['val_pctile']:>5.1f}/"
            f"{v2['A_random_entry']['oos_pctile']:>5.1f}%  "
            f"B {v2['B_pass_count']['pctile']:>5.1f}%  "
            f"{'✅' if v2['passed'] else '❌'}")
