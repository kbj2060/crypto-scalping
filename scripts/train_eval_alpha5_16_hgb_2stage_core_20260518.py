...
MODEL_ID = "alpha5_16_hgb_2stage_core_20260518"
...
DEFAULT_OUT_DIR = ROOT / "tmp/causal_regen_20260516/alpha5_16_hgb_2stage_core_20260518"
...
def _entry_target(
    ...
    *,
    ...
    mask = (action != 0) & tp_first & (consensus >= float(consensus_min)) & (edge_gap >= float(edge_gap_min))
...
def _direction_target(frame: pd.DataFrame) -> np.ndarray:
    action = pd.to_numeric(frame["label_action"], errors="coerce").fillna(0).to_numpy(np.int64)
    return (action == 1).astype(np.int64)
...
def _direction_weights(frame: pd.DataFrame) -> np.ndarray:
    y = _direction_target(frame)
    base = pd.to_numeric(frame["label_sample_weight"], errors="coerce").fillna(0.0).to_numpy(np.float64)
    edge_gap = pd.to_numeric(frame["meta_edge_gap"], errors="coerce").fillna(0.0).to_numpy(np.float64)
...
def _stage_specs() -> list[dict[str, HGBSpec]]:
    ...
    return [
        {"name": "stage2_v1", "entry": specs["regularized"], "direction": specs["deeper"]},
        {"name": "stage2_v2", "entry": specs["deeper"], "direction": specs["regularized"]},
        {"name": "stage2_v3", "entry": specs["regularized"], "direction": specs["regularized"]},
    ]
...
def main() -> None:
    p = argparse.ArgumentParser(description="Train 2-stage HGB action selector on Alpha5.13 regime4_core.")
    ...
    p.add_argument("--entry-require-profitable", action="store_true", default=True)
    p.add_argument("--entry-no-require-profitable", dest="entry_require_profitable", action="store_false")
    p.add_argument("--entry-require-selected", action="store_true", default=True)
    p.add_argument("--entry-no-require-selected", dest="entry_require_selected", action="store_false")
    p.add_argument("--max-hold-bars", type=int, default=96)
    p.add_argument("--unit-exposure", type=float, default=1.0)
    p.add_argument("--fee", type=float, default=0.0005)
    p.add_argument("--slip", type=float, default=0.0002)
    p.add_argument("--seed", type=int, default=51601)
    args = p.parse_args()
    ...
    if not entry_cols or not direction_cols:
        raise ValueError("feature subset selection failed")
    ...
    target_kwargs = {
        "consensus_min": float(args.entry_consensus_min),
        "edge_gap_min": float(args.entry_edge_gap_min),
        "require_profitable": bool(args.entry_require_profitable),
        "require_selected": bool(args.entry_require_selected),
    }
    entry_mask_train = _entry_target(train_fit, **target_kwargs) == 1
    entry_mask_val = _entry_target(val_df, **target_kwargs) == 1
    entry_mask_oos = _entry_target(oos_df, **target_kwargs) == 1

    train_dir = train_fit.loc[entry_mask_train].reset_index(drop=True)
    x_train_direction = _x(train_dir, direction_cols)
    ...
    y_direction = _direction_target(train_dir)
    ...
    w_direction = _direction_weights(train_dir)
    ...
    print(json.dumps({
        ...
        "rows": {"train_fit": int(len(train_fit)), "train_direction": int(len(train_dir)), "validation": int(len(val_df)), "oos": int(len(oos_df))},
        ...
        "entry_positive_ratio_val": float(np.mean(entry_mask_val)),
        "entry_positive_ratio_oos": float(np.mean(entry_mask_oos)),
    ...
    for i, stage in enumerate(stages, start=1):
        ...
        direction_model = _fit_hgb(x_train_direction, y_direction, w_direction, stage["direction"], int(args.seed + i * 100 + 2))

        p_entry_val = _binary_proba(entry_model, x_val_entry)
        p_long_val = _binary_proba(direction_model, x_val_direction)
        ...
        p_long_oos = _binary_proba(direction_model, x_oos_direction)
        ...
        artifact = args.out_dir / f"{stage['name']}_alpha5_16_hgb_2stage_core.joblib"
        joblib.dump({
            "model_id": MODEL_ID,
            "baseline_fixed": {"track": "regime4_core", "single_hgb": "deeper"},
            "entry_feature_cols": entry_cols,
            "direction_feature_cols": direction_cols,
            "models": {"entry": entry_model, "direction": direction_model},
    ...
    summary = {
        "model_id": MODEL_ID,
        "design": "2-stage action-only HGB on regime4_core: stage1 entry-vs-wait, stage2 long-vs-short with split feature spaces.",
        "baseline_fixed": {"track": "regime4_core", "single_hgb": "deeper"},
        "experiments": rows,
        "best": best,
        "top10": sorted(rows, key=lambda r: float(r["validation"]["score"]), reverse=True)[:10],
    }
    summary_path = args.out_dir / "alpha5_16_hgb_2stage_core_summary.json"
    grid_path = args.out_dir / "alpha5_16_hgb_2stage_core_grid.csv"
