...
MODEL_ID = "alpha3_tabnet_parent_v1_20260514"
OUT_DIR = ROOT / "data/ensemble/supervised/alpha3_tabnet_parent_v1_20260514"
REPORT_OUT = ROOT / "data/ensemble/reports/alpha3_tabnet_parent_v1_20260514_summary.json"
AUDIT_OUT = ROOT / "data/ensemble/reports/alpha3_tabnet_parent_v1_20260514_audit.json"
GRID_OUT = ROOT / "data/ensemble/reports/alpha3_tabnet_parent_v1_20260514_grid.csv"
...
class MultiTaskTabNetParent(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        action_hidden = self.action_trunk(shared)
        ...
        param_hidden = self.param_trunk(torch.cat([shared, scaled_action_hidden], dim=-1))
        return {
            "action": action_logits,
            "quality": self.quality_head(param_hidden).squeeze(-1),
            "notional": self.notional_head(param_hidden),
            "leverage": self.leverage_head(param_hidden),
            "take_profit": self.take_profit_head(param_hidden),
            "stop_loss": self.stop_loss_head(param_hidden),
            "max_hold": self.max_hold_head(param_hidden),
            "cooldown": self.cooldown_head(param_hidden),
...
def _loss(
    ...
    *,
    ...
    task = _homo_loss([action_loss, quality_loss, *bucket_losses], model.log_vars)
    loss = task + 1e-3 * out["mask_entropy"] + 0.05 * _contradiction_penalty(out, model.buckets)
    return loss, {
        "action": float(action_loss.detach().cpu()),
        "quality": float(quality_loss.detach().cpu()),
        "bucket": float(torch.stack(bucket_losses).mean().detach().cpu()),
        "mix": float(mix),
    ...
    *,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for xb, yb, sb in train_loader:
            ...
            out = model(xb)
            loss, parts = _loss(model, out, yb, sb, action_weight=action_weight, epoch=epoch, warmup_epochs=warmup_epochs)
            loss.backward()
...
def _runtime_grid() -> list[RuntimeConfig]:
    ...
    for conf in (0.38, 0.46, 0.54, 0.62, 0.70):
        for q in (-0.015, 0.0, 0.015):
            for unc in (0.035, 0.060, 0.090):
                for scale, cap in ((0.75, 1.80), (0.90, 2.30), (1.00, 2.75), (1.10, 3.10)):
                    for expected in (False, True):
                        rows.append(RuntimeConfig(f"tabnet_c{conf:.2f}_q{q:.3f}_u{unc:.3f}_s{scale:.2f}_cap{cap:.2f}_{'exp' if expected else 'arg'}", conf, q, unc, scale, cap, expected))
    return rows
...
def main() -> int:
    p = argparse.ArgumentParser(description="Alpha3 parent replacement with multi-task TabNet.")
    ...
    model = MultiTaskTabNetParent(len(feature_cols), buckets)
    ...
    if args.quick:
        grid = [
            r
            for r in grid
            if r.confidence_floor in (0.46, 0.54, 0.62)
            and r.quality_floor in (0.0,)
            and r.uncertainty_max in (0.060, 0.090)
            and r.notional_scale in (0.90, 1.00)
    ...
    model_path = OUT_DIR / "alpha3_tabnet_parent_v1.pt"
    torch.save(
        {
            "model_id": MODEL_ID,
    ...
    report = {
        "model_id": MODEL_ID,
        "design": "Alpha3 parent replacement using Multi-Task TabNet backbone. Frozen Alpha3 downstream layers are preserved; only parent decision frame is replaced.",
        "baseline": {"name": "alpha3_hgb_parent", "metrics": baseline_metrics, "score": _score(baseline_metrics)},
        "candidate": {"name": "alpha3_tabnet_parent_v1", "selected_runtime": asdict(best_rt), "metrics": tabnet_metrics, "score": _score(tabnet_metrics)},
        "training": training,
        "audit": audit,
