...
# --- 20260724 reduced80 fork ---------------------------------------------------------------
# Fork of train_eval_omega4_3head_parent72_loose_entry_quality_20260620.py (unmodified original
# preserved). Only addition: an optional --base-cols-allowlist-file flag that restricts the
# auto-derived base_cols to an explicit column list (research feature-reduction experiment,
# see docs discussion 20260724). Nothing else in the pipeline is changed. If omitted, behavior
...
def _build_exit_dataset_entry_label_path_optimal(
    ...
    *,
    ...
    original's fix (train_eval_omega4_3head_parent72_loose_entry_quality_20260620.py). The
    per-segment `row_notional` also feeds `_exit_fill_net`'s cash simulation that derives exit_net/
    edge/label, unchanged in scope -- that's a legitimate backtest-sizing assumption, not the
    train/inference feature-parity bug this fix targets."""
...
def _build_exit_dataset_entry_label_terminal_giveback(
    ...
    *,
    ...
    constant, recorded as risk_sizing_source. 2026-08-18 fix, see docs/experiments/eth_odyssey4_
    exit_head_liveatr_barrier_and_label_reaudit_20260818.md -- ported from the byte-identical ETH
    original's fix (train_eval_omega4_3head_parent72_loose_entry_quality_20260620.py)."""
    required = {"timestamp", "zigzag_action", "open", "high", "low", "close"}
    missing = sorted(required - set(frame.columns))
    if missing:
...
def main() -> int:
    ...
    ap.add_argument("--base-cols-allowlist-file", type=Path, default=None, help="Optional path to a text file (one column name per line) restricting base_cols to this explicit set. Every listed column must already be present in the auto-derived feature_cols, or this errors out.")
    ...
    ap.add_argument("--cfg-dropout", type=float, default=None, help="Override parent.CFG.dropout (regularization sweep knob). Default: leave parent module default unchanged.")
    ap.add_argument("--cfg-weight-decay", type=float, default=None, help="Override parent.CFG.weight_decay (regularization sweep knob).")
    ap.add_argument("--cfg-lr", type=float, default=None, help="Override parent.CFG.lr.")
    ap.add_argument("--cfg-patience", type=int, default=None, help="Override parent.CFG.patience.")
    args = ap.parse_args()

    if any(v is not None for v in (args.cfg_dropout, args.cfg_weight_decay, args.cfg_lr, args.cfg_patience)):
        from dataclasses import replace as _dc_replace

        _overrides: dict[str, Any] = {}
        if args.cfg_dropout is not None:
            _overrides["dropout"] = float(args.cfg_dropout)
        if args.cfg_weight_decay is not None:
            _overrides["weight_decay"] = float(args.cfg_weight_decay)
        if args.cfg_lr is not None:
            _overrides["lr"] = float(args.cfg_lr)
        if args.cfg_patience is not None:
            _overrides["patience"] = int(args.cfg_patience)
        parent.CFG = _dc_replace(parent.CFG, **_overrides)
    ...
    allowlist_removed_cols: list[str] = []
    if args.base_cols_allowlist_file is not None:
        allow_path = Path(args.base_cols_allowlist_file)
        allowlist = [ln.strip() for ln in allow_path.read_text(encoding="utf-8").splitlines() if ln.strip()]
        missing = sorted(set(allowlist) - set(base_cols))
        if missing:
            raise RuntimeError(f"--base-cols-allowlist-file requests columns not present in auto-derived base_cols: {missing}")
        allowlist_removed_cols = [c for c in base_cols if c not in set(allowlist)]
        base_cols = [c for c in base_cols if c in set(allowlist)]
    ...
    # risk_margin/risk_leverage=None: bootstrap training, no sidecar for either function's bundle
    # can exist yet -- see each function's docstring.
    ...
    report = {
        ...
        "input_contract": {
            ...
            "base_cols_allowlist_file": str(args.base_cols_allowlist_file) if args.base_cols_allowlist_file is not None else None,
            "base_cols_allowlist_removed_cols": allowlist_removed_cols,
            "base_cols": base_cols,
        },
