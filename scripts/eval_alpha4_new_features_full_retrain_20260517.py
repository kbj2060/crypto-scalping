...
from scripts import train_eval_hf_v13_deep_alpha_candidate_expansion_v27 as v27train  # noqa: E402
...
MODEL_ID = "alpha4_new_features_full_retrain_20260517"
BASE_PARENT = ROOT / "data/ensemble/supervised/hf_v13_clean_regime_repro_20260511/v13_clean_regime_h288.pkl"
DEFAULT_TRAIN_CSV = ROOT / "tmp/causal_regen_20260516/ai_feature_combo_grid/trade_candidates_2025_patchtst__tide__dlinear.csv"
DEFAULT_EVAL_CSV = ROOT / "tmp/causal_regen_20260516/ai_feature_combo_grid/trade_candidates_2026_patchtst__tide__dlinear.csv"
ALPHA4_ROOT = ROOT / "tmp/causal_regen_20260516/alpha4_new_features_full_retrain_20260517"
OUT_DIR = ALPHA4_ROOT / "artifacts"
REPORT_OUT = ALPHA4_ROOT / "alpha4_new_features_full_retrain_summary.json"
AUDIT_OUT = ALPHA4_ROOT / "alpha4_new_features_full_retrain_audit.json"
GRID_OUT = ALPHA4_ROOT / "alpha4_new_features_full_retrain_grid.csv"
FAST_MODE = False
...
ALPHA4_EXTRA_PARENT_FEATURES = ("tp_sl_action_score", "tp_sl_path_edge")
OLD_CLEAN_PREFIX = "clean_regime_2024_unsup_v4_"
REGIME4_PREFIXES = ("clean_regime4_2024_unsup_v1_", "regime4_pred_")


def _regime4_cols(train: pd.DataFrame, eval_df: pd.DataFrame) -> list[str]:
    common = set(train.columns) & set(eval_df.columns)
    return sorted(c for c in common if c.startswith(REGIME4_PREFIXES))


def _drop_old_clean(cols: list[str]) -> list[str]:
    return [c for c in cols if not c.startswith(OLD_CLEAN_PREFIX)]


def _allow_regime4_audit(audit: dict[str, Any]) -> dict[str, Any]:
    out = copy.deepcopy(audit)
    blocking: list[str] = []
    for item in list(out.get("blocking", [])):
        if not str(item).startswith("forbidden_feature_cols:"):
            blocking.append(item)
            continue
        cols = [c for c in str(item).split(":", 1)[1].split(",") if c]
        remaining = [c for c in cols if not c.startswith(REGIME4_PREFIXES)]
        if remaining:
            blocking.append("forbidden_feature_cols:" + ",".join(remaining))
    out["blocking"] = blocking
    out["status"] = "pass" if not blocking else "fail"
    return out
...
def _train_alpha4_v27(
    *,
    ...
    print(f"[{MODEL_ID}] retraining V27-style deep alpha utility TCN epochs={epochs}", flush=True)
    seq_cols = _drop_retrain_features(v27train._seq_cols(train_all))
    train_ds = v27train._build_train_set(train_df, seq_cols, fee=fee, slip=slip, stride=3)
    norm = v27train._normalizer(train_ds["seq"])
    model = v27train._train_model(train_ds, norm, epochs=int(epochs))
    payload = {
        "model_id": MODEL_ID,
        "layer": "alpha4_v27_style_deep_alpha_utility",
        "state_dict": model.state_dict(),
        "seq_cols": seq_cols,
        "norm": norm,
        "train_snapshot_count": int(len(train_ds["target"])),
        "target_mean": np.mean(train_ds["target"], axis=0),
        "target_p90": np.quantile(train_ds["target"], 0.90, axis=0),
        "removed_legacy_features": sorted(DROP_RETRAIN_FEATURES),
    }
    out_dir.mkdir(parents=True, exist_ok=True)
    model_path = out_dir / "alpha4_v27_style_deep_alpha_utility.pt"
    torch.save(payload, model_path)
    manifest_path = out_dir / "alpha4_v27_style_feature_manifest.json"
    manifest = {
        "model": str(model_path),
        "seq_len": int(v27train.v23.SEQ_LEN),
        "seq_cols": seq_cols,
        "train_snapshot_count": int(len(train_ds["target"])),
    }
...
def main() -> int:
    ...
    p = argparse.ArgumentParser(description="Train Alpha4 from the causal/new feature frame: parent, V27-style deep utility, teacher gate, and V21.2 runner.")
    ...
    p.add_argument("--v27-epochs", type=int, default=80)
    ...
    feature_cols = _drop_old_clean(_drop_retrain_features(list(parent_ref["feature_cols"])))
    for col in ALPHA4_EXTRA_PARENT_FEATURES:
        if col in train_all.columns and col in eval_df.columns and col not in feature_cols:
            feature_cols.append(col)
    for col in _regime4_cols(train_all, eval_df):
    ...
    contract_cols = _drop_old_clean(_drop_retrain_features(_feature_cols(train_all, eval_df)))
    for col in _regime4_cols(train_all, eval_df):
        if col not in contract_cols:
            contract_cols.append(col)
    audit_base = _allow_regime4_audit(_audit_contract(train_all, eval_df, feature_cols))
    ...
    frozen_v27_payload, frozen_v27_model = v31._load_v27(v31.DEFAULT_V27)
    ...
    val_q = v31._predict_all(frozen_v27_model, val_df, frozen_v27_payload["seq_cols"], frozen_v27_payload["norm"])
    eval_q = v31._predict_all(frozen_v27_model, eval_df, frozen_v27_payload["seq_cols"], frozen_v27_payload["norm"])
    ...
    v27_payload, v27_model, v27_meta = _train_alpha4_v27(
        train_all=train_all,
        train_df=train_df,
        out_dir=OUT_DIR / "alpha4_v27_style_deep_alpha_utility",
        fee=fee,
        slip=slip,
        epochs=int(args.v27_epochs),
    )
    ...
    audit = {
        ...
        "selection_window": "2025-10-01..2025-12-31",
        "oos_window": "2026 fixed OOS only after parent/downstream selection",
        "alpha4_layers_retrained": {
            "parent": True,
            "v27_style_deep_alpha_utility": True,
            "teacher_gate": True,
            "v21_2_runner": True,
            "v31_rule_overlay": "reselected_fixed_alpha3_limit_close_contract",
        },
    ...
    report = {
        "model_id": MODEL_ID,
        "design": "Alpha4 new-feature full retrain. Parent candidates are trained with the original base h288 label recipe and evaluated with the Alpha3 margin110 runtime overlay. A V27-style TCN utility model is retrained on the causal/new feature frame, then every parent candidate retrains teacher gate and V21.2 runner. Teacher runtime and runner config are selected on 2025Q4; fixed 2026 OOS uses the corrected Alpha3 limit-close execution contract.",
        "experiments": experiments,
        "audit": audit,
        "artifacts": {"out_dir": str(OUT_DIR), "v27": v27_meta, "report": str(REPORT_OUT), "audit": str(AUDIT_OUT), "grid": str(GRID_OUT)},
    }
    REPORT_OUT.write_text(json.dumps(report, indent=2, ensure_ascii=False, default=_json_default), encoding="utf-8")
    AUDIT_OUT.write_text(json.dumps(audit, indent=2, ensure_ascii=False, default=_json_default), encoding="utf-8")
