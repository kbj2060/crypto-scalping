#!/usr/bin/env python3
"""RESEARCH ONLY -- cheap_gate for the BatchEnsemble k-reduction candidate.

See docs/experiments/eth_odyssey4_dl_reference_deep_analysis_20260816.md Section 2/4 and memory
eth_odyssey4_batchensemble_collapse_and_quality_head_duplication_20260816: a real deployed zig075
bundle's k=8 ensemble members were measured to have collapsed to one function (pairwise top-
confidence correlation 0.997-0.999, argmax unanimity 96-97% vs an independent-baseline ~0.05-0.17%),
and a fresh canonical-config training run showed the diversity gates (input_scale/expert_scale)
barely move from their random-init spread even after real training. Open question left there: does
this actually cost anything on direction_balanced_accuracy, or is k=8 "free" regardless of collapse?

This script is the CHEAP_GATE step: same seed/expert/CFG/plain-CE loss as the real canonical script
(A1's GCE port was reverted, N=5 seed, 4/5 worse -- plain CE is the real baseline), same TRUE
102(+13pos)=115-feature live pipeline (not the 185-feature proxy -- C2's proxy-pipeline result
flipped when re-verified on true features, so this candidate uses true features from the start), k
varied over {1, 2, 4, 8} with everything else held fixed. If direction_balanced_accuracy is
materially unchanged (or better) at k<8, that is real evidence the k=8 ensemble machinery is
functionally inert for this label/architecture and a real N>=5-seed comparison is warranted. If
direction_balanced_accuracy degrades as k shrinks, that is a valid negative result -- collapsed
members can still matter (e.g. via the dropout-driven per-member noise averaging acting as implicit
regularization even without genuine functional diversity) and this closes the candidate without
further experiment.

Single seed, single expert (bull, for consistency with A1/C1/C2/other cheap_gates this session).
"""
...
OUT_DIR = ROOT / "tmp/causal_regen_20260516/eth_odyssey4_batchensemble_k_reduction_cheap_gate_20260816"
EXPERT = "bull"
SEED = 260816
EPOCHS = 28
K_VALUES = [1, 2, 4, 8]  # 8 = canonical baseline, included for an apples-to-apples in-run comparison


def log(msg: str) -> None:
    print(f"[k_reduction_cheap_gate] {msg}", flush=True)


def _fit_with_k(x_dir, y_dir, route_frame, x_exit, y_exit, exit_route_frame, *, expert_idx: int, seed: int, epochs: int, device: torch.device, k: int) -> dict[str, Any]:
    """canon._fit_expert_3head's exact training loop (plain CE, matching the real canonical script),
    with ONE change: CFG.k replaced. Also tracks direction_balanced_accuracy and
    quality_balanced_accuracy per epoch (canon._fit_expert_3head doesn't track either)."""
    torch.manual_seed(int(seed) + int(expert_idx))
    np.random.seed(int(seed) + int(expert_idx))
    cfg = replace(canon.CFG, k=int(k))
    ...
    for epoch in range(int(epochs)):
        ...
        with torch.no_grad():
            ...
            qual_bacc = float(balanced_accuracy_score(vy.cpu().numpy(), qual_pred))
        curve.append({"epoch": epoch + 1, "val_loss": round(vloss, 5), "direction_bacc": round(dir_bacc, 5), "quality_bacc": round(qual_bacc, 5)})
        if vloss + 1.0e-6 < best_loss:
            ...
            best_components = {
                ...
                "quality_balanced_accuracy": qual_bacc,
            }
            best_epoch = epoch + 1
            stale = 0
        else:
...
def main() -> int:
    ...
    log(f"=== stage=prepare_frames (true 115-feature pipeline) expert={EXPERT} seed={SEED} k_values={K_VALUES} ===")
    ...
    report: dict[str, Any] = {
        "design": "BatchEnsemble k-reduction cheap_gate -- k in {1,2,4,8}, single seed/expert, plain-CE canonical loss, true 115-feature pipeline.",
        "seed": SEED,
        "expert": EXPERT,
    ...
    for k in K_VALUES:
        res = _fit_with_k(x_train, y_train, train_raw, x_exit, y_exit, frame_exit, expert_idx=expert_idx, seed=SEED, epochs=EPOCHS, device=device, k=k)
        report["runs"][f"k_{k}"] = res
        bc = res["best_components"]
        log(
            f"  k={k} n_params={res['n_params']} early_stop_epoch={res['early_stop_epoch']} "
            f"best_val_loss={res['best_validation_loss']:.4f} dir_bacc={bc['direction_balanced_accuracy']:.4f} "
            f"qual_bacc={bc['quality_balanced_accuracy']:.4f} ({res['train_seconds']}s)"
        )

    baseline = report["runs"]["k_8"]
    verdict = {}
    for k in K_VALUES:
        if k == 8:
            continue
        r = report["runs"][f"k_{k}"]
        verdict[f"k_{k}_vs_k_8"] = {
            "n_params_ratio": round(r["n_params"] / baseline["n_params"], 4),
            "early_stop_epoch_delta": r["early_stop_epoch"] - baseline["early_stop_epoch"],
            "best_val_loss_delta": r["best_validation_loss"] - baseline["best_validation_loss"],
            "dir_bacc_delta": r["best_components"]["direction_balanced_accuracy"] - baseline["best_components"]["direction_balanced_accuracy"],
            "qual_bacc_delta": r["best_components"]["quality_balanced_accuracy"] - baseline["best_components"]["quality_balanced_accuracy"],
        }
    report["verdict"] = verdict
    for k, v in verdict.items():
        log(f"=== VERDICT {k}: params={v['n_params_ratio']:.1%} dir_bacc_delta={v['dir_bacc_delta']:+.4f} qual_bacc_delta={v['qual_bacc_delta']:+.4f} val_loss_delta={v['best_val_loss_delta']:+.4f} ===")
