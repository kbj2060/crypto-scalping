...
import train_eval_omega1_2_mamba_sac_coordinator_20260603 as old_coord  # noqa: E402
import train_eval_omega1_2_tabm_3head_20260603 as threehead  # noqa: E402
import train_eval_omega1_2_tabm_diffusion_risk_20260603 as omega  # noqa: E402
import train_omega1_regime3_expert_direction_head_volpca_20260602 as hard  # noqa: E402


MODEL_ID = "omega1_2_mamba_sac_3head_feature_coordinator_20260604"
OUT_DIR = ROOT / "tmp/causal_regen_20260516" / MODEL_ID
DEFAULT_3HEAD_DIR = ROOT / "tmp/causal_regen_20260516/omega1_2_true_3head_tabm_20260603_final_tp_sl_on_e28_exit30k_q080"
...
ACTION_GRID = [
    CoordAction(0.0, 0.0, 0.05),
    *[
        CoordAction(1.0, mult, sl)
        for mult in (0.50, 0.75, 1.00, 1.20)
        for sl in (0.020, 0.035, 0.050, 0.070)
    ],
]

FAST_ACTION_GRID = [
    CoordAction(0.0, 0.0, 0.05),
    *[CoordAction(1.0, mult, sl) for mult in (0.60, 1.00) for sl in (0.035, 0.065)],
]
...
def _action_to_vec(a: CoordAction) -> np.ndarray:
    return np.asarray(
        [
            float(a.enter),
            float(a.exposure_mult) / 1.20,
            (float(a.emergency_sl) - 0.020) / (0.070 - 0.020),
        ],
        dtype=np.float32,
    )
...
def _vec_to_action(v: np.ndarray) -> CoordAction:
    ...
    return CoordAction(
        enter=float(x[0]),
        exposure_mult=float(x[1] * 1.20),
        emergency_sl=float(0.020 + x[2] * (0.070 - 0.020)),
    )
...
def _apply_norm(df: pd.DataFrame, norm: dict[str, Any]) -> np.ndarray:
    ...
    if missing:
        raise RuntimeError(f"3-head Mamba-SAC state missing columns: {missing[:20]}")
...
def _load_3head_payloads(threehead_dir: Path) -> dict[str, Any]:
    path = threehead_dir / "true_3head_tabm_bundle.pt"
    if not path.exists():
...
def _predict_3head_frame(frame: pd.DataFrame, bundle: dict[str, Any], *, quality_threshold: float, device: torch.device, oof: bool) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    src = threehead._prediction_output(frame, direction, quality, threshold=float(quality_threshold), prefix=prefix)
    src[f"{prefix}_exit_p_hold_feature_only"] = exit_prob[:, 0]
    src[f"{prefix}_exit_p_exit_feature_only"] = exit_prob[:, 1]
    src[f"{prefix}_exit_edge_feature_only"] = exit_prob[:, 1] - exit_prob[:, 0]
    return x, src


def _prepare_frames(threehead_dir: Path, *, quality_threshold: float, device: torch.device) -> dict[str, Any]:
    ...
    oos_df = eval_df.reset_index(drop=True)
    bundle = _load_3head_payloads(threehead_dir)
    _x_train, train_src = _predict_3head_frame(train_df, bundle, quality_threshold=quality_threshold, device=device, oof=True)
    _x_val, val_src = _predict_3head_frame(val_df, bundle, quality_threshold=quality_threshold, device=device, oof=True)
    _x_oos, oos_src = _predict_3head_frame(oos_df, bundle, quality_threshold=quality_threshold, device=device, oof=False)
    train_dec = omega._to_fixed_decisions(train_src, oof=True)
    val_dec = omega._to_fixed_decisions(val_src, oof=True)
    oos_dec = omega._to_fixed_decisions(oos_src, oof=False)
    s_train = omega._build_state_frame(train_df, train_src, train_dec, oof=True, feature_cols=feature_cols)
    s_val = omega._build_state_frame(val_df, val_src, val_dec, oof=True, feature_cols=feature_cols)
    s_oos = omega._build_state_frame(oos_df, oos_src, oos_dec, oof=False, feature_cols=feature_cols)
    ...
    return {
        ...
        "threehead_dir": str(threehead_dir),
    }
...
def _simulate_action(
    ...
    *,
    ...
    notional = float(np.clip(float(row.get("notional_exposure", 0.0) or 0.0) * float(coord.exposure_mult), 0.0, 1.20))
    leverage = float(row.get("leverage", 1.0) or 1.0)
    if notional <= 0.0:
        return 0.0, {"active": 0, "exit_reason": "zero_exposure", "exit_i": int(signal_i), "net": 0.0}
    cash = 1.0 - entry_fee * notional
    ...
    for i in range(entry_i, last_i + 1):
        ...
        if unreal <= -abs(float(coord.emergency_sl)):
            ...
            exit_i = int(i)
            break
        if stop_loss > 0.0 and unreal <= -stop_loss:
            _, exit_fill, exit_fee, _ = omega._try_execution(arrays, int(i), side, entry=False, fee_base=fee_eff, slip_base=slip_eff)
    ...
    *,
    ...
    for idx in active_idx:
        ...
        for action in action_grid:
            reward, meta = _simulate_action(frame, arrays, dec, int(idx), action, fee=fee, slip=slip, cost_mult=cost_mult, max_sim_bars=int(max_sim_bars))
        ...
        keep = [skip] if float(local[0][0]) <= float(min_trade_edge) else [x for x in local[:4] if float(x[0]) > float(min_trade_edge)]
    ...
    if not xs:
        raise RuntimeError("empty 3-head feature coordinator dataset")
    r = np.asarray(rewards, dtype=np.float32)
    return (
        OfflineData(np.asarray(xs, dtype=np.float32), np.asarray(acts, dtype=np.float32), r, np.asarray(weights, dtype=np.float32)),
        {
            ...
            "counterfactual_exit_reasons": reasons,
        },
    )
...
class CoordinatorActor(nn.Module):
    def __init__(self, input_dim: int, d_model: int = 96, emb_dim: int = 96) -> None:
        super().__init__()
        self.enc = old_coord.MambaEncoder(input_dim, d_model, emb_dim)
        self.head = nn.Sequential(nn.Linear(emb_dim, 96), nn.SiLU(), nn.Linear(96, 3))
...
class CoordinatorCritic(nn.Module):
    def __init__(self, input_dim: int, d_model: int = 96, emb_dim: int = 96) -> None:
        super().__init__()
        self.enc = old_coord.MambaEncoder(input_dim, d_model, emb_dim)
        self.q = nn.Sequential(nn.Linear(emb_dim + 3, 128), nn.SiLU(), nn.Linear(128, 64), nn.SiLU(), nn.Linear(64, 1))
...
def _train(data: OfflineData, *, device: torch.device, steps: int, batch_size: int, lr: float) -> tuple[CoordinatorActor, dict[str, Any]]:
    ...
    for step in range(1, int(steps) + 1):
        ...
        w_b = w_b.to(device)
        q_loss = torch.nn.functional.smooth_l1_loss(q1(seq_b, act_b), rew_b) + torch.nn.functional.smooth_l1_loss(q2(seq_b, act_b), rew_b)
        opt_q.zero_grad(set_to_none=True)
        q_loss.backward()
        torch.nn.utils.clip_grad_norm_(list(q1.parameters()) + list(q2.parameters()), 3.0)
        opt_q.step()
        pred = actor(seq_b)
        q_val = torch.minimum(q1(seq_b, pred), q2(seq_b, pred))
        bc = (((pred - act_b) ** 2).mean(dim=1) * w_b).sum() / torch.clamp(w_b.sum(), min=1.0)
...
def _policy_actions(actor: CoordinatorActor, seq: np.ndarray, *, device: torch.device) -> np.ndarray:
    ...
    for start in range(0, len(seq), 2048):
        out.append(actor(torch.from_numpy(seq[start : start + 2048]).to(device)).detach().cpu().numpy())
    return np.concatenate(out, axis=0).astype(np.float32)


def _replay(frame: pd.DataFrame, seq: np.ndarray, dec: pd.DataFrame, actor: CoordinatorActor, *, fee: float, slip: float, cost_mult: float, device: torch.device, max_sim_bars: int) -> dict[str, Any]:
    ...
    while i < len(frame) - 2:
        ...
        _reward, meta = _simulate_action(frame, arrays, dec, i, coord, fee=fee, slip=slip, cost_mult=cost_mult, max_sim_bars=int(max_sim_bars))
...
def main() -> int:
    ...
    ap.add_argument("--threehead-dir", type=Path, default=DEFAULT_3HEAD_DIR)
    ...
    ap.add_argument("--min-trade-edge", type=float, default=0.002)
    ...
    frames = _prepare_frames(Path(args.threehead_dir), quality_threshold=float(args.quality_threshold), device=device)
    fee, slip = omega._load_fee_slip()
    state_cols = [c for c in frames["s_train"].columns if c != "timestamp"]
    bad = [c for c in state_cols if "clean_regime4" in c or "regime4_pred" in c or "tp_sl_action_score" in c]
    if bad:
        raise RuntimeError(f"forbidden state columns passed audit: {bad[:20]}")
    norm = _fit_norm(frames["s_train"][state_cols])
    seq_train = _rolling_sequences(_apply_norm(frames["s_train"][state_cols], norm), int(args.seq_len))
    seq_val = _rolling_sequences(_apply_norm(frames["s_val"][state_cols], norm), int(args.seq_len))
    seq_oos = _rolling_sequences(_apply_norm(frames["s_oos"][state_cols], norm), int(args.seq_len))
    grid = FAST_ACTION_GRID if bool(args.fast_grid) else ACTION_GRID
    ...
    actor, train_diag = _train(data, device=device, steps=int(args.steps), batch_size=int(args.batch_size), lr=float(args.lr))
    val_metrics = _replay(frames["val_df"], seq_val, frames["val_dec"], actor, fee=fee, slip=slip, cost_mult=float(args.cost_mult), device=device, max_sim_bars=int(args.eval_max_sim_bars))
    oos_metrics = _replay(frames["oos_df"], seq_oos, frames["oos_dec"], actor, fee=fee, slip=slip, cost_mult=float(args.cost_mult), device=device, max_sim_bars=int(args.eval_max_sim_bars))
    report = {
        "model_id": MODEL_ID,
        "design": "3-head TabM Direction/Quality/Exit is frozen. Exit Head output is state feature only; it is never thresholded as an immediate exit trigger. Mamba-SAC chooses entry veto, exposure multiplier, and emergency SL. TP/SL barriers remain the accounting exits.",
        "threehead_dir": str(args.threehead_dir),
        "quality_threshold": float(args.quality_threshold),
        "state_columns": list(norm["columns"]),
        "seq_len": int(args.seq_len),
        "action": {"dims": ["enter_gate", "exposure_multiplier", "emergency_sl"], "grid_size_for_counterfactual": len(grid), "fast_grid": bool(args.fast_grid)},
        "training": {
        ...
        "artifacts": {"out_dir": str(out_dir), "report": str(out_dir / "report.json"), "model": str(out_dir / "mamba_sac_3head_feature_coordinator.pt")},
    }
    torch.save({"actor_state_dict": actor.state_dict(), "normalizer": norm, "seq_len": int(args.seq_len), "state_columns": list(norm["columns"])}, out_dir / "mamba_sac_3head_feature_coordinator.pt")
