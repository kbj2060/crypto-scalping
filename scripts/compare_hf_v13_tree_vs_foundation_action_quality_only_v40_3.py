...
MODEL_ID = "hf_v13_tree_vs_foundation_action_quality_only_v40_3_20260512"
...
DEFAULT_OUT_DIR = ROOT / "data/ensemble/supervised/hf_v13_tree_vs_foundation_action_quality_only_v40_3_20260512"
DEFAULT_REPORT = ROOT / "data/ensemble/reports/hf_v13_tree_vs_foundation_action_quality_only_v40_3_20260512_summary.json"
DEFAULT_AUDIT = ROOT / "data/ensemble/reports/hf_v13_tree_vs_foundation_action_quality_only_v40_3_20260512_audit.json"
DEFAULT_GRID = ROOT / "data/ensemble/reports/hf_v13_tree_vs_foundation_action_quality_only_v40_3_20260512_grid.csv"
...
def main() -> int:
    ...
    print(f"[{MODEL_ID}] preparing mixed action-quality tuning set", flush=True)
    ...
    print(f"[{MODEL_ID}] tuning encoded action-quality heads on 2025 Q4 validation", flush=True)
    ...
    for idx, hp in enumerate(hparams_grid):
        print(f"[{MODEL_ID}] mixed candidate {idx + 1}/{len(hparams_grid)}: {hp.name}", flush=True)
        ...
        val_decisions = predict_policy_frame_mixed(bundle, baseline_bundle, val_feat, val_enc)
    ...
    if best_bundle is None or best_hp is None or best_val is None:
        raise RuntimeError("mixed action-quality tuning produced no valid candidate")

    print(f"[{MODEL_ID}] backtesting baseline vs tuned mixed selection", flush=True)
    mixed_val = best_val
    eval_decisions = predict_policy_frame_mixed(best_bundle, baseline_bundle, eval_feat, eval_enc)
    ...
    report = {
        "model_id": MODEL_ID,
        "design": "Baseline tree versus mixed parent. Baseline uses the original fixed HGB parent settings. The challenger uses Chronos-2 macro PCA-128, Kairos_23m micro PCA-32, and QuantFastKAN state-32 encodings only for the action/quality heads, while keeping the baseline tree bucket heads for notional, leverage, TP, SL, hold, and cooldown. Encoded action-quality HGB hyperparameters are selected on 2025 Q4 validation.",
