#!/usr/bin/env python3
"""Sigma3 variance-reduction: train 5 HGB seeds with FIXED iterations (no per-seed early-stop,
which caused the 143-vs-400 iteration divergence and severe seed instability in the initial
2-seed sweep) and AVERAGE their class probabilities into ONE deterministic ensemble tape.

This is a principled bias-unchanged / variance-reduced follow-up, not a new architecture. It is
the FINAL validation look before the one-shot: whatever the pre-registered 27-config grid says
on the ensemble tape, the untouched 2026-03-02..06-30 window is scored at most once (only if a
config passes), and the search is not expanded further regardless.
"""
...
def main() -> int:
    ...
    tape = pd.DataFrame({
        ...
        "primary_action": final_action, "primary_side": side, "primary_expert": "sigma3ens",
    ...
    out_path = OUT_DIR / "tape_ensemble.parquet"
