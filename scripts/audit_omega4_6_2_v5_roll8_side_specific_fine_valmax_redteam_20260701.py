#!/usr/bin/env python3
"""Red-team audit for Omega 4.6.2 v5 roll8 side-specific fine branch."""
...
RANKING = OUT_DIR / "roll8_side_specific_fine_valmax_ranking.csv"
EVAL_SCRIPT = ROOT / "scripts/eval_omega4_6_2_v5_roll8_side_specific_fine_valmax_20260701.py"
PRIOR_RUNTIME_AUDIT = (
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll8_side_specific_fine_valmax_redteam_20260701.md"
TOL = 1.0e-8
...
def main() -> None:
    audit = load_module("omega462_loss_cluster_audit_for_roll8_fine", AUDIT_V1_PATH)
    segment_audit = load_module("omega462_segment_audit_for_roll8_fine", AUDIT_SEGMENT_PATH)
    ...
    audit.add_check(
        ...
        == "validation_primary_roll8_hold_compression_with_oos_safety_gate; fresh_holdout_required",
        "research_blocker",
    ...
    audit.add_check(
        ...
        == "among research-gated roll8 variants, sort by validation_pnl, validation_avg_hold, cap distance to 4.20; OOS is not an ordering key",
        "research_blocker",
        {"observed": report.get("selection_rule")},
    )
    audit.add_check(
        checks,
        "selected_roll8_gate_pass",
        selected.get("research_roll8_gate_pass") is True,
        "research_blocker",
        {"observed": selected.get("research_roll8_gate_pass")},
    )
    ...
    verdict = (
        "FULL_LIVE_PASS"
        if full_live_pass
        else "RESEARCH_ROLL8_FINE_PASS_FULL_LIVE_BLOCKED"
        if research_pass
        else "REDTEAM_FAIL"
    )
    payload = {
        "audit_id": "omega4_6_2_v5_roll8_side_specific_fine_valmax_redteam_20260701",
