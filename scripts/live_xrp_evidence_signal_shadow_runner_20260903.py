...
BTC 섀도우 러너의 **수정 완료본**(4모드 해상 + 조기확정 규칙 + expired 분리 + 신호봉 ATR)을
자산 상수와 확정 셀만 바꿔 포팅했다. BTC에서 터진 결함 4종이 이미 다 고쳐진 버전이다.
"""
...
_SIG = importlib.import_module("live_xrp_evidence_signal_metalabel_20260903")
compute_xrp_evidence_signals = _SIG.compute_xrp_evidence_signals
SYMBOL = _SIG.SYMBOL

STATE = ROOT / "data/live/xrp_evidence_signal_shadow_state.json"
...
# 신호별 HIT 판정 규격 (동결 컨텍스트 리포트의 asset_params와 동일 출처)
...
HIT_SPEC = {
    # ⭐XRP 확정 셀 -- 각 신호 자신의 라벨 정의에서 왔다(BTC/ETH와 전부 다르다).
    #   근거: docs/experiments/xrp_evidence_signal_and_regime_20260903.md 3~5단계
    "demarker_extreme":         {"horizon": 2,  "k": 1.5, "mode": "touch"},
    "kalman_deviation_meanrev": {"horizon": 5,  "k": 2.0, "mode": "touch"},
    "short_term_return_z":      {"horizon": 12, "k": 1.5, "mode": "touch_mae_capped",
                                 "k_loss_mult": 2.0},
    # ⚠️giveback -- 해상에 **18봉(2xH)** 필요. H로 착각하면 절반 시점에 잘못 확정한다.
    "taker_delta_climax":       {"horizon": 9,  "k": 1.5, "mode": "touch_giveback_sustained",
                                 "full_window": 18, "giveback_ceiling": 0.20},
    "orthogonal_combo":         {"horizon": 8,  "k": 2.0, "mode": "touch"},
}
HOLDOUT_AUC = {"demarker_extreme": 0.6759, "kalman_deviation_meanrev": 0.6223,
               "short_term_return_z": 0.6132, "taker_delta_climax": 0.6091,
               "orthogonal_combo": 0.5599}
...
def log(m: str) -> None:
    print(f"[xrp-evshadow {datetime.now(timezone.utc):%m-%d %H:%M:%S}] {m}", flush=True)
...
def report(s: dict[str, Any]) -> None:
    ...
    ctx = json.loads((ROOT / "data/labels/xrp_5m_evidence_signal_live_contexts_20260903"
