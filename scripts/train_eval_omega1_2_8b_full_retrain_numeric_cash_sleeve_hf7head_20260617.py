...
MODEL_ID = "omega1_2_8b_full_retrain_numeric_cash_sleeve_hf7head_20260617_dynamic_risk"
        ...
        rows.append(
            {
                ...
                "long_leverage": float(long_risk.leverage),
                "short_leverage": float(short_risk.leverage),
            }
        )
    labels = pd.DataFrame(rows)
    diag = {
        ...
        "long_leverage_min": float(labels["long_leverage"].min()) if len(labels) else float(default_risk.leverage),
        "long_leverage_max": float(labels["long_leverage"].max()) if len(labels) else float(default_risk.leverage),
        "short_leverage_min": float(labels["short_leverage"].min()) if len(labels) else float(default_risk.leverage),
        "short_leverage_max": float(labels["short_leverage"].max()) if len(labels) else float(default_risk.leverage),
    }
    return labels, diag
...
def _risk_from_predictions(
    ...
    if int(side) > 0:
        ...
        leverage = float(risk_pred["long_leverage"][int(i)])
    else:
        ...
        leverage = float(risk_pred["short_leverage"][int(i)])
    tp = float(np.clip(tp, float(PARENT_RISK_TP_MIN), float(PARENT_RISK_TP_MAX)))
    sl = float(np.clip(abs(sl), float(PARENT_RISK_SL_MIN), float(PARENT_RISK_SL_MAX)))
    notional = float(np.clip(notional, float(PARENT_RISK_NOTIONAL_MIN), float(PARENT_RISK_NOTIONAL_MAX)))
    leverage = float(np.clip(leverage, float(PARENT_RISK_LEVERAGE_MIN), float(PARENT_RISK_LEVERAGE_MAX)))
...
def main() -> int:
    ...
    val_leverage_p, val_leverage_s, oos_leverage_p, oos_leverage_s, risk_leverage_diag = _fit_predict_risk_head(
        x_val,
        x_oos,
        risk_labels,
        "long_leverage",
        "short_leverage",
        seed=282401,
        clip_min=float(PARENT_RISK_LEVERAGE_MIN),
        clip_max=float(PARENT_RISK_LEVERAGE_MAX),
    )
    val_risk_pred = {
        ...
        "long_leverage": val_leverage_p,
        "short_leverage": val_leverage_s,
    }
    oos_risk_pred = {
        ...
        "long_leverage": oos_leverage_p,
        "short_leverage": oos_leverage_s,
    }
    ...
    diagnostics: dict[str, Any] = {
        ...
        "risk_label_bounds": {
            ...
            "long_leverage": {"min": float(PARENT_RISK_LEVERAGE_MIN), "max": float(PARENT_RISK_LEVERAGE_MAX)},
            "short_leverage": {"min": float(PARENT_RISK_LEVERAGE_MIN), "max": float(PARENT_RISK_LEVERAGE_MAX)},
        },
        "risk_fit": {
            "tp": risk_tp_diag,
            "stop_loss": risk_sl_diag,
            "notional": risk_notional_diag,
            "leverage": risk_leverage_diag,
        },
