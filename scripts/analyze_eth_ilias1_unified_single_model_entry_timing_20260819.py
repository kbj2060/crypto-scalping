#!/usr/bin/env python3
"""레짐 무관 단일모델(bull 1개 학습, bear/chop은 가중치 복사)로 재학습한 zig075(seed260620,
trial12 레시피, q080)의 진입타이밍 rank를 3-expert baseline(trial12, LONG=0.636/SHORT=0.463)
대비 확인."""
...
BUNDLE_PATH = ROOT / "tmp/causal_regen_20260516/omega4_3head_parent72_loose_entry_quality_20260620_trial12_unified_single_model_20260819/true_3head_tabm_bundle.pt"
...
def main() -> int:
    ...
    print()
    print("=== SUMMARY: unified single-model (seed260620, q080) ===", flush=True)
    print(f"LONG:  n={len(long_arr)} mean={long_arr.mean():.4f} median={np.median(long_arr):.4f}  (trial12 3-expert baseline was 0.636)", flush=True)
    print(f"SHORT: n={len(short_arr)} mean={short_arr.mean():.4f} median={np.median(short_arr):.4f}  (trial12 3-expert baseline was 0.463)", flush=True)
