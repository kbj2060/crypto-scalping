...
# ────────────────────────────────────────────────────────────── 평가
def lift_at(score: np.ndarray, y: np.ndarray, cov: float = FIRE) -> tuple[float, int, float]:
...
def null_lift(y: np.ndarray, rng, cov: float = FIRE, B: int = 400) -> tuple[float, float]:
...
def main() -> int:
    ...
    ap.add_argument("--gate", choices=("volexp", "volexpq", "atr", "bbw", "none"), default="volexp",
