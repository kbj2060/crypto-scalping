#!/usr/bin/env python3
"""사용자 요청 신규 축(레짐피쳐 제거 + 2024+2025 학습데이터 확장) TabM 학습 CLI 러너.
`eth_directional_change_tabm_training_unified_single_model_20260819.py`와 동일한
단일모델(MoE 라우팅 무력화) monkeypatch, canonicaldata 래퍼만
`eth_candidate_2024_2025_noregime_canonicaldata_20260820.py`로 교체. 라벨은 DC(①)
dense-cashfill(2024/2025/2026 전부 있음, 이번 축의 1차 검증 라벨로 DC를 씀 -- CUSUM도
같은 방식으로 재사용 가능하나 이번엔 범위 밖)."""
...
import eth_candidate_2024_2025_noregime_canonicaldata_20260820 as canon  # noqa: E402
...
if __name__ == "__main__":
    ...
    out_suffix = f"dc_2024_2025_noregime_unified_single_model_seed{known.seed}_20260820"
    sys.argv = [sys.argv[0], *BASE_ARGS, "--seed", str(known.seed), "--out-suffix", out_suffix]
    print(f"stage=start seed={known.seed} out_suffix={out_suffix} mode=2024_2025_noregime", flush=True)
    t0 = time.time()
