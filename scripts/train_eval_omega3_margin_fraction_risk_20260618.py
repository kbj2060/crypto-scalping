...
MODEL_ID = "omega3_full_retrain_margin_fraction_risk_20260618"
OUT_DIR = ROOT / "tmp/causal_regen_20260516" / MODEL_ID
PARENT_RISK_TP_MIN = 0.0015
PARENT_RISK_TP_MAX = 0.2500
PARENT_RISK_SL_MIN = 0.0010
PARENT_RISK_SL_MAX = 0.2000
PARENT_RISK_NOTIONAL_MIN = 0.02
PARENT_RISK_NOTIONAL_MAX = 1.80
PARENT_RISK_LEVERAGE_MIN = 1.0
PARENT_RISK_LEVERAGE_MAX = 6.0
PARENT_RISK_MARGIN_FRACTION_MIN = PARENT_RISK_NOTIONAL_MIN / PARENT_RISK_LEVERAGE_MAX
PARENT_RISK_MARGIN_FRACTION_MAX = PARENT_RISK_NOTIONAL_MAX / PARENT_RISK_LEVERAGE_MIN
...
def _risk_from_path_row(row: pd.Series, prefix: str, default: sleeve.FallbackRisk) -> sleeve.FallbackRisk:
    ...
    risk_scale = 1.0 + 0.65 * rr_signal + 0.20 * float(net_signal) - 0.10 * float(time_signal)
    lev_scale = 1.0 + 0.35 * rr_signal + 0.12 * float(net_signal) - 0.06 * float(time_signal)

    notional = float(np.clip(float(default.notional) * float(np.clip(risk_scale, 0.10, 2.20)), float(PARENT_RISK_NOTIONAL_MIN), float(PARENT_RISK_NOTIONAL_MAX)))
    leverage = float(np.clip(float(default.leverage) * float(np.clip(lev_scale, 0.20, 4.0)), float(PARENT_RISK_LEVERAGE_MIN), float(PARENT_RISK_LEVERAGE_MAX)))
    margin_fraction = float(np.clip(notional / max(leverage, 1.0e-12), float(PARENT_RISK_MARGIN_FRACTION_MIN), float(PARENT_RISK_MARGIN_FRACTION_MAX)))
    leverage = float(np.clip(notional / max(margin_fraction, 1.0e-12), float(PARENT_RISK_LEVERAGE_MIN), float(PARENT_RISK_LEVERAGE_MAX)))
...
def _risk_label_table(path_labels: pd.DataFrame, default_risk: sleeve.FallbackRisk) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    for row_id, path_row in enumerate(path_labels.to_dict("records")):
        ...
        rows.append(
            {
                "i": int(i),
                "long_take_profit": float(long_risk.take_profit),
                "short_take_profit": float(short_risk.take_profit),
                "long_stop_loss": float(long_risk.stop_loss),
                "short_stop_loss": float(short_risk.stop_loss),
                "long_notional": float(long_risk.notional),
                "short_notional": float(short_risk.notional),
                "long_margin_fraction": float(long_risk.notional) / max(float(long_risk.leverage), 1.0e-12),
                "short_margin_fraction": float(short_risk.notional) / max(float(short_risk.leverage), 1.0e-12),
            }
        )
    labels = pd.DataFrame(rows)
    diag = {
        ...
        "long_tp_min": float(labels["long_take_profit"].min()) if len(labels) else float(default_risk.take_profit),
        "long_tp_max": float(labels["long_take_profit"].max()) if len(labels) else float(default_risk.take_profit),
        "short_tp_min": float(labels["short_take_profit"].min()) if len(labels) else float(default_risk.take_profit),
        "short_tp_max": float(labels["short_take_profit"].max()) if len(labels) else float(default_risk.take_profit),
        "long_sl_min": float(labels["long_stop_loss"].min()) if len(labels) else float(default_risk.stop_loss),
        "long_sl_max": float(labels["long_stop_loss"].max()) if len(labels) else float(default_risk.stop_loss),
        "short_sl_min": float(labels["short_stop_loss"].min()) if len(labels) else float(default_risk.stop_loss),
        "short_sl_max": float(labels["short_stop_loss"].max()) if len(labels) else float(default_risk.stop_loss),
        ...
        "long_margin_fraction_min": float(labels["long_margin_fraction"].min()) if len(labels) else float(default_risk.notional / max(default_risk.leverage, 1.0e-12)),
        "long_margin_fraction_max": float(labels["long_margin_fraction"].max()) if len(labels) else float(default_risk.notional / max(default_risk.leverage, 1.0e-12)),
        "short_margin_fraction_min": float(labels["short_margin_fraction"].min()) if len(labels) else float(default_risk.notional / max(default_risk.leverage, 1.0e-12)),
        "short_margin_fraction_max": float(labels["short_margin_fraction"].max()) if len(labels) else float(default_risk.notional / max(default_risk.leverage, 1.0e-12)),
    }
    return labels, diag
...
def _risk_from_predictions(
    ...
    if int(side) > 0:
        tp = float(risk_pred["long_take_profit"][int(i)])
        sl = float(risk_pred["long_stop_loss"][int(i)])
        notional = float(risk_pred["long_notional"][int(i)])
        margin_fraction = float(risk_pred["long_margin_fraction"][int(i)])
    else:
        tp = float(risk_pred["short_take_profit"][int(i)])
        sl = float(risk_pred["short_stop_loss"][int(i)])
        notional = float(risk_pred["short_notional"][int(i)])
        margin_fraction = float(risk_pred["short_margin_fraction"][int(i)])
    tp = float(np.clip(tp, float(PARENT_RISK_TP_MIN), float(PARENT_RISK_TP_MAX)))
    sl = float(np.clip(abs(sl), float(PARENT_RISK_SL_MIN), float(PARENT_RISK_SL_MAX)))
    notional = float(np.clip(notional, float(PARENT_RISK_NOTIONAL_MIN), float(PARENT_RISK_NOTIONAL_MAX)))
    margin_fraction = float(np.clip(margin_fraction, float(PARENT_RISK_MARGIN_FRACTION_MIN), float(PARENT_RISK_MARGIN_FRACTION_MAX)))
    leverage = float(np.clip(notional / max(margin_fraction, 1.0e-12), float(PARENT_RISK_LEVERAGE_MIN), float(PARENT_RISK_LEVERAGE_MAX)))
    return sleeve.FallbackRisk("pred_dynamic", tp, sl, notional, leverage, int(default_risk.max_hold_bars))
...
def main() -> int:
    ...
    val_notional_p, val_notional_s, oos_notional_p, oos_notional_s, risk_notional_diag = _fit_predict_risk_head(
        x_val,
        x_oos,
        risk_labels,
        "long_notional",
        "short_notional",
        seed=282301,
        clip_min=float(PARENT_RISK_NOTIONAL_MIN),
        clip_max=float(PARENT_RISK_NOTIONAL_MAX),
    )
    val_margin_p, val_margin_s, oos_margin_p, oos_margin_s, risk_margin_diag = _fit_predict_risk_head(
        x_val,
        x_oos,
        risk_labels,
        "long_margin_fraction",
        "short_margin_fraction",
        seed=282401,
        clip_min=float(PARENT_RISK_MARGIN_FRACTION_MIN),
        clip_max=float(PARENT_RISK_MARGIN_FRACTION_MAX),
    )
    val_risk_pred = {
        "long_take_profit": val_tp_p,
        "short_take_profit": val_tp_s,
        "long_stop_loss": val_sl_p,
        "short_stop_loss": val_sl_s,
        "long_notional": val_notional_p,
        "short_notional": val_notional_s,
        "long_margin_fraction": val_margin_p,
        "short_margin_fraction": val_margin_s,
    }
    oos_risk_pred = {
        "long_take_profit": oos_tp_p,
        "short_take_profit": oos_tp_s,
        "long_stop_loss": oos_sl_p,
        "short_stop_loss": oos_sl_s,
        "long_notional": oos_notional_p,
        "short_notional": oos_notional_s,
        "long_margin_fraction": oos_margin_p,
    ...
    diagnostics: dict[str, Any] = {
        ...
        "risk_label_bounds": {
            "long_take_profit": {"min": float(PARENT_RISK_TP_MIN), "max": float(PARENT_RISK_TP_MAX)},
            "short_take_profit": {"min": float(PARENT_RISK_TP_MIN), "max": float(PARENT_RISK_TP_MAX)},
            "long_stop_loss": {"min": float(PARENT_RISK_SL_MIN), "max": float(PARENT_RISK_SL_MAX)},
            "short_stop_loss": {"min": float(PARENT_RISK_SL_MIN), "max": float(PARENT_RISK_SL_MAX)},
            ...
            "long_margin_fraction": {"min": float(PARENT_RISK_MARGIN_FRACTION_MIN), "max": float(PARENT_RISK_MARGIN_FRACTION_MAX)},
            "short_margin_fraction": {"min": float(PARENT_RISK_MARGIN_FRACTION_MIN), "max": float(PARENT_RISK_MARGIN_FRACTION_MAX)},
        },
        "risk_fit": {
            "tp": risk_tp_diag,
            "stop_loss": risk_sl_diag,
            "notional": risk_notional_diag,
            "margin_fraction": risk_margin_diag,
        },
    ...
    report = {
        ...
        "method": "Full-retrained 3-head parent artifact is used as parent. Cash sleeve EV lower-bound and numeric utility agreement/veto regressors are newly fit on validation full-retrain features. Fallback risk targets are derived row-wise from validation path simulations using a fixed base risk, then the regressors predict TP/SL/notional/margin_fraction dynamically at inference. Leverage is derived as notional / margin_fraction and clipped to futures risk bounds. Additional probe settings run with relaxed EV/utility minima to force fallback activation for sensitivity checks.",
