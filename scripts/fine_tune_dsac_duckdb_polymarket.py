...
DEFAULT_OUT_CKPT = ROOT / "data/ensemble/ckpt/fine_tuned_dsac_agents_polymarket.pth"
DEFAULT_OUT_REPORT = ROOT / "data/ensemble/metrics/fine_tuned_dsac_polymarket_report.json"
...
def _load_duck(cfg: Cfg) -> pd.DataFrame:
    ...
    m["ts"] = pd.to_datetime(m["ts"], utc=True).dt.tz_convert(cfg.tz).dt.floor("min")
    if not t.empty:
        t["ts"] = pd.to_datetime(t["ts"], utc=True).dt.tz_convert(cfg.tz).dt.floor("min")
...
def _fetch_polymarket_features_range(ts_min: pd.Timestamp, ts_max: pd.Timestamp, tz: str) -> pd.DataFrame:
    ...
    for d in days:
        ...
        p_mode = pd.to_numeric(f.get("mode_prob", 0.0), errors="coerce").fillna(0.0).to_numpy(dtype=np.float64)
        w_tgt = pd.to_numeric(f.get("weighted_target", 0.0), errors="coerce").fillna(0.0).to_numpy(dtype=np.float64)
        ...
        for i, r in f.iterrows():
            ...
            vals1 = [float(d1.get(lb, 0.0) or 0.0) for lb in top3]
            vals3 = [float(d3.get(lb, 0.0) or 0.0) for lb in top3]
            if vals1:
                j = int(np.argmax(np.abs(vals1)))
                d1_signed[i] = float(vals1[j])
                d1_abs[i] = float(abs(vals1[j]))
            if vals3:
                d3_abs[i] = float(max(abs(x) for x in vals3))

        g = pd.DataFrame(
            {
                "ts": pd.to_datetime(f["ts"], utc=True).dt.tz_convert(tz).dt.floor("min"),
                "poly_mode_prob": p_mode,
                "poly_weighted_target": w_tgt,
                "poly_d1_abs": d1_abs,
                "poly_d3_abs": d3_abs,
...
def _build_df(cfg: Cfg) -> pd.DataFrame:
    ...
    poly = _fetch_polymarket_features_range(ts0, ts1, cfg.tz)
    if len(poly):
        base = base.copy()
        poly = poly.copy()
        base["ts_key"] = pd.to_datetime(base["ts"]).astype("int64")
        poly["ts_key"] = pd.to_datetime(poly["ts"]).astype("int64")
        base = pd.merge_asof(
            base.sort_values("ts_key"),
            poly.sort_values("ts_key").drop(columns=["ts"]),
            on="ts_key",
            direction="nearest",
            tolerance=int(pd.Timedelta(minutes=2).value),
        )
        base = base.drop(columns=["ts_key"])
...
def _feature_columns(df: pd.DataFrame) -> list[str]:
    ...
    # 폴리마켓 피처를 DSAC 상태에 직접 주입
    df["poly_mode_prob"] = df["poly_mode_prob"].fillna(0.0)
    df["poly_weighted_gap"] = df["poly_weighted_gap"].fillna(0.0)
    df["poly_d1_abs"] = df["poly_d1_abs"].fillna(0.0)
    df["poly_d3_abs"] = df["poly_d3_abs"].fillna(0.0)
    df["poly_d1_signed"] = df["poly_d1_signed"].fillna(0.0)
    ...
    cols = [
        ...
        # engineered (8)
        "liq_imbalance", "nif_x_obi", "tox_x_collapse",
        "poly_mode_prob", "poly_weighted_gap", "poly_d1_abs", "poly_d3_abs", "poly_d1_signed",
    ]
    for c in cols:
        if c not in df.columns:
            df[c] = 0.0
...
def run(cfg: Cfg, out_ckpt: Path, out_report: Path) -> dict:
    ...
    n = len(df)
    n_test = max(120, int(n * cfg.test_ratio))
    n_val = max(120, int(n * cfg.val_ratio))
    ...
    out_ckpt = _unique_output_path(out_ckpt)
    out_report = _unique_output_path(out_report)
...
def main() -> None:
    p = argparse.ArgumentParser(description="Fine-tune DSAC with injected Polymarket features (no overwrite)")
    p.add_argument("--symbol", default="ETHUSDT")
    p.add_argument("--days", type=int, default=10)
    p.add_argument("--horizon-min", type=int, default=30)
    p.add_argument("--epochs", type=int, default=6)
    p.add_argument("--updates-per-epoch", type=int, default=350)
    ...
    p.add_argument("--out-ckpt", default=str(DEFAULT_OUT_CKPT))
    ...
    out = run(cfg, Path(args.out_ckpt), Path(args.out_report))
