...
MODEL_ID = "omega1_2_8k_full_retrain_numeric_cash_sleeve_continuous_vol_risk_all_positions_20260617"
OUT_DIR = ROOT / "tmp/causal_regen_20260516" / MODEL_ID
RISK = sleeve.FallbackRisk("continuous_vol_risk_all_positions_label_base_n081_l2_h192", 0.052, 0.028, 0.81, 2.0, 192)
...
def _metrics_with_vol_sltp(
    ...
    *,
    ...
    for i in range(0, len(frame) - 2):
        ...
        if bool(active[i]):
            ...
            if side_action in (sleeve.ACTION_LONG, sleeve.ACTION_SHORT) and conf >= float(threshold):
                ...
                row_risk = _vol_risk_for_row(i, vol, q60, q85)
                cash, pos, entered = _open_position_with_risk(cash, arrays, i, side, "fallback", row_risk, fee_eff, slip_eff)
                if entered:
                    fallback_entries += 1
                    long_entries += int(side > 0)
                    short_entries += int(side < 0)
    ...
    return {
        ...
        "primary_avg_tp_price_move": float(np.mean(np.asarray(primary_tp) / np.maximum(np.asarray(primary_notional), 1.0e-12))) if primary_tp else 0.0,
        "primary_avg_sl_price_move": float(np.mean(np.asarray(primary_sl) / np.maximum(np.asarray(primary_notional), 1.0e-12))) if primary_sl else 0.0,
...
def main() -> int:
    ...
    diagnostics: dict[str, Any] = {
        "mode": "full_retrain_parent_numeric_cash_sleeve_with_continuous_vol_risk_all_positions",
        ...
        "continuous_vol_risk": {
            ...
            "application": "primary and fallback positions; parent row TP/SL/leverage is replaced during metrics evaluation",
    ...
    report = {
        ...
        "method": "Full-retrained 3-head parent artifact is used as parent. Cash sleeve EV lower-bound and numeric utility agreement/veto regressors are newly fit on validation full-retrain features and evaluated on fixed 2026 OOS. Primary and fallback positions use continuous volatility-normalized account-PnL TP/SL and leverage instead of discrete risk templates.",
