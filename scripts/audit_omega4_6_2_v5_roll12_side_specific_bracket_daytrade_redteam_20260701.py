#!/usr/bin/env python3
"""Red-team audit for Omega 4.6.2 v5 roll12 side-specific bracket day-trade branch."""
...
RANKING = OUT_DIR / "roll12_side_specific_bracket_daytrade_ranking.csv"
EVAL_SCRIPT = ROOT / "scripts/eval_omega4_6_2_v5_roll12_side_specific_bracket_daytrade_20260701.py"
PRIOR_RUNTIME_AUDIT = (
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll12_side_specific_bracket_daytrade_redteam_20260701.md"
REFERENCE_CAP = 4.20
TOL = 1.0e-8
...
def main() -> None:
    audit = load_module("omega462_loss_cluster_audit_for_roll12_side", AUDIT_V1_PATH)
    segment_audit = load_module("omega462_segment_audit_for_roll12_side", AUDIT_SEGMENT_PATH)
    ...
    audit.add_check(
        checks,
        "improves_reference_pnl_with_max_hold_not_worse",
    ...
    verdict = (
        "FULL_LIVE_PASS"
        if full_live_pass
        else "RESEARCH_ROLL12_SIDE_SPECIFIC_PASS_FULL_LIVE_BLOCKED"
        if research_pass
        else "REDTEAM_FAIL"
    )
    payload = {
        "audit_id": "omega4_6_2_v5_roll12_side_specific_bracket_daytrade_redteam_20260701",
