#!/usr/bin/env python3
"""12h bracket day-trade branch for Omega 4.6.2 v5."""
...
MODEL_ID = "omega4_6_2_v5_roll12_bracket_daytrade_20260701"
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll12_bracket_daytrade_20260701.md"
MAX_ROLL_HOURS = 12.0
...
def normalize_roll12(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    rename = {col: col.replace("roll16_", "roll12_") for col in out.columns if col.startswith("roll16_")}
...
def write_markdown(report: dict[str, Any]) -> None:
    ...
    text = f"""# Omega 4.6.2 v5 Roll12 Bracket Daytrade - 2026-07-01
...
def main() -> int:
    source = load_module("omega462_roll16_source_for_roll12", SOURCE_MODULE_PATH)
    v1 = source.load_module("omega462_loss_cluster_v1_for_roll12", source.V1_MODULE_PATH)
    segment_mod = source.load_module("omega462_segment_for_roll12", source.SEGMENT_MODULE_PATH)
    stop_mod = v1.load_module("omega462_stop_mod_for_roll12", v1.STOP_MODULE_PATH)
    ...
    exposure_specs = [
        ...
        segment_mod.ExposureSpec("long100_short110_cap430", 1.00, 1.10, 4.30),
    ]
    governors = [
    ...
    selected_val_base = normalize_roll12(
        source.split_roll_bracket(
            val_parent,
            train_market,
            segment_mod,
            MAX_ROLL_HOURS,
            float(selected["roll12_tp_move"]),
            float(selected["roll12_sl_move"]),
        )
    )
    selected_oos_base = normalize_roll12(
        source.split_roll_bracket(
            oos_parent,
            eval_market,
            segment_mod,
    ...
    status = (
        "RESEARCH_ROLL12_DAYTRADE_PASS"
        if bool(selected["research_daytrade_gate_pass"])
        else "NO_ROLL12_DAYTRADE_PASSING_CANDIDATE"
    )
    safe = (
    ...
    ranking_path = OUT_DIR / "roll12_bracket_daytrade_ranking.csv"
    top20_path = OUT_DIR / "roll12_bracket_daytrade_top20.csv"
        ...
        json.dumps(
            {
                ...
                "selected_tp": selected["roll12_tp_move"],
                "selected_sl": selected["roll12_sl_move"],
                "selected_validation": {
