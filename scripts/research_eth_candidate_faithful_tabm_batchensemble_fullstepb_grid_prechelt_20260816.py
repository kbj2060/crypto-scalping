#!/usr/bin/env python3
"""RESEARCH ONLY -- corrected checkpoint-selection criterion for the full_R_S_B_embed[quarter]
grid (docs/experiments/eth_candidate_faithful_tabm_batchensemble_20260816.md). The prior run
(fullstepb_grid_scheduled) used direction_val_loss (whatever loss the model trains with, including
GCE) with plain patience=20 -- found via literature research to have TWO fixable problems:

1. **GCE is mathematically bounded** (`L_q=(1-p^q)/q in [0,1/q]`, Zhang & Sabuncu arXiv:1805.07836)
   so its validation loss saturates early even while accuracy keeps improving, exhausting patience
   prematurely (confirmed empirically: GCE runs' selection gap to the true accuracy peak was ~2x
   plain-CE runs' gap). Fix, confirmed as the GCE paper's OWN practice (it selects checkpoints by
   "maximum validation accuracy", not by its own L_q loss): decouple the SELECTION criterion from
   the TRAINING loss. This script always uses plain (unbounded) CE for the selection criterion,
   regardless of which loss (CE or GCE) the model is being trained with.
2. **Class-weighting mismatch**: direction_val_loss was sample-weighted (frequency-weighted across
   classes) while direction_balanced_accuracy is class-weighted (equal per class). Fix: the
   selection-criterion CE here is weighted by the SAME class-balance weights (`dir_w`) already used
   for the training loss, aligning its weighting scheme with balanced_accuracy's.
3. **Patience-counter noise sensitivity**: Prechelt ("Early Stopping -- But When?", 1998, full text
   read) documents this exact problem on real data (his own validation curve has 16 local minima
   before true overfitting, "no obvious rule" to pick the true one) and recommends STRIP-based
   checking over raw per-epoch patience: evaluate in k=5-epoch strips, stop after s=4 CONSECUTIVE
   worsening strips (his own best speed/quality tradeoff, the "UP_4" criterion) -- implemented here
   instead of a naive per-epoch patience counter. Explicitly NOT using EMA/smoothing of the metric --
   researched and found to have no literature grounding (Prechelt never uses it; strip-averaging is
   the citable alternative).

Same architecture (full_R_S_B_embed[quarter], 109,836 params), same cosine LR schedule (2e-4->2e-6,
T_max=60), same 2x3 grid (CE vs GCE x AdamW vs RAdam vs AdaBelief), same expert=bull/seed=260816 --
ONLY the selection/stopping criterion changed, so this is a clean, isolated re-test of that one fix.
"""
...
base = stepb.base
hard = stepb.hard
OUT_DIR = ROOT / "tmp/causal_regen_20260516/eth_candidate_faithful_tabm_batchensemble_fullstepb_grid_prechelt_20260816"
SEED = 260816
EXPERT = "bull"
LR_MAX = 2.0e-4
LR_MIN = 2.0e-6
MAX_EPOCHS = 60
STRIP_LEN = 5     # Prechelt's k
UP_S = 4          # Prechelt's UP_4 -- best speed/quality tradeoff in his own experiments
QUARTER = {"hidden": 96, "d_embed": 4, "n_bins": 8}
GCE_Q = combo.GCE_Q  # 0.7 -- training loss only, NEVER used for selection now


def log(msg: str) -> None:
    print(f"[faithful_tabm_fullstepb_prechelt] {msg}", flush=True)
...
def _fit_grid_cell_prechelt(x_dir, y_dir, route_frame, x_exit, y_exit, exit_route_frame, *, expert_idx, seed, device, cfg, opt_factory, use_gce):
    ...
    def selection_val_loss_and_bacc():
        """Always plain CE (never GCE), weighted by the same class-balance weights as training --
        decoupled from the training loss per the GCE paper's own selection practice, and weight-
        aligned with balanced_accuracy per the class-weighting-mismatch fix."""
        model.eval()
        with torch.no_grad():
            vo = model(vx_t)
...
def main() -> int:
    ...
    log(f"=== stage=prepare_frames (light, shared) expert={EXPERT} lr={LR_MAX}->{LR_MIN} cosine max_epochs={MAX_EPOCHS} "
        f"strip_len={STRIP_LEN} up_s={UP_S} selection=plain_class_balanced_CE config=quarter{QUARTER} ===")
    ...
    report: dict[str, Any] = {
        "design": "full_R_S_B_embed[quarter], cosine schedule + Prechelt UP_4 strip-based stopping on plain class-balanced CE selection criterion (decoupled from training loss), 2 losses x 3 optimizers grid.",
        "seed": SEED, "expert": EXPERT, "lr_max": LR_MAX, "lr_min": LR_MIN, "max_epochs": MAX_EPOCHS, "strip_len": STRIP_LEN, "up_s": UP_S, "config": QUARTER, "cells": {},
    }
    t0 = time.time()
    for loss_name, use_gce in (("CE", False), ("GCE", True)):
        for opt_name, opt_factory in OPTIMIZERS.items():
            ...
            result = _fit_grid_cell_prechelt(x_train, y_train, train_raw, x_exit, y_exit, frame_exit, expert_idx=expert_idx, seed=SEED, device=device, cfg=cfg, opt_factory=opt_factory, use_gce=use_gce)
    ...
    log("=== stage=summary (ranked by selected_bacc -- i.e. what a real training run would actually deliver) ===")
    ...
    log("  reference (previous criterion, direction_val_loss + plain patience=20): AdamW+CE selected=0.5543, AdaBelief+CE selected=0.5517, RAdam+CE selected=0.5430, "
        "RAdam+GCE selected=0.5191, AdaBelief+GCE selected=0.5172, AdamW+GCE selected=0.5171")
