...
import train_eval_omega1_2_1_tabm_7head_risk_20260611 as hf7  # noqa: E402
import train_eval_omega1_2_tabm_diffusion_risk_20260603 as omega  # noqa: E402


MODEL_ID = "omega1_2_8b_full_retrain_numeric_cash_sleeve_hf7head_20260617_relaxed"
OUT_DIR = ROOT / "tmp/causal_regen_20260516" / MODEL_ID
HF7_BUNDLE_PATH = ROOT / "tmp/causal_regen_20260516" / "omega1_2_1_regime3_7head_tabm_risk_20260611" / "regime3_7head_tabm_bundle.pt"
HF7_THRESHOLD = 0.50
HF7_DEVICE = "cpu"
...
def _load_hf7_bundle() -> dict[str, Any]:
    if not HF7_BUNDLE_PATH.exists():
        raise RuntimeError(f"missing HF7 bundle: {HF7_BUNDLE_PATH}")
    bundle = torch.load(HF7_BUNDLE_PATH, map_location="cpu", weights_only=False)
    expected = {"models", "base_cols", "pos_cols", "buckets"}
    missing = sorted(expected - set(bundle.keys()))
    if missing:
        raise RuntimeError(f"HF7 bundle missing keys: {missing}")
    return bundle


def _hf7_risk_table(frame: pd.DataFrame, bundle: dict[str, Any], *, prefix: str) -> pd.DataFrame:
    x = hf7._base_input(frame, list(bundle["base_cols"]))
    device = torch.device(HF7_DEVICE)
    preds = {expert: hf7._predict_payload(bundle["models"][expert], x, device=device) for expert in tuple(bundle["models"].keys())}
    route = hf7.hard._route_id(frame)
    direction = hf7._routed(preds, route, "direction", 3)
    quality = hf7._routed(preds, route, "quality", 3)
    risk_probs = {
        "tp": hf7._routed(preds, route, "tp", len(bundle["buckets"]["tp"])),
        "sl": hf7._routed(preds, route, "sl", len(bundle["buckets"]["sl"])),
        "margin": hf7._routed(preds, route, "margin", len(bundle["buckets"]["margin"])),
        "leverage": hf7._routed(preds, route, "leverage", len(bundle["buckets"]["leverage"])),
        "max_hold": hf7._routed(preds, route, "max_hold", len(bundle["buckets"]["max_hold"])),
    }
    src = hf7._prediction_output(frame, direction, quality, risk_probs, threshold=float(HF7_THRESHOLD), prefix=prefix)
    keep = src[[
        f"{prefix}_take_profit",
        f"{prefix}_stop_loss",
        f"{prefix}_notional_exposure",
        f"{prefix}_leverage",
        f"{prefix}_max_hold_bars",
    ]].copy()
    keep.insert(0, "i", np.arange(len(src), dtype=np.int64))
    return keep


def _risk_from_hf7_row(row: pd.Series, default: sleeve.FallbackRisk) -> sleeve.FallbackRisk:
    tp = float(row.get("hf7_take_profit", default.take_profit))
    sl = float(row.get("hf7_stop_loss", default.stop_loss))
    notional = float(row.get("hf7_notional_exposure", default.notional))
    leverage = float(row.get("hf7_leverage", default.leverage))
    max_hold = int(row.get("hf7_max_hold_bars", default.max_hold_bars))
    if not np.isfinite(tp) or tp <= 0.0:
        tp = float(default.take_profit)
    if not np.isfinite(sl) or sl < 0.0:
        sl = abs(float(default.stop_loss))
    if not np.isfinite(notional) or notional <= 0.0:
        notional = float(default.notional)
    if not np.isfinite(leverage) or leverage <= 0.0:
        leverage = float(default.leverage)
    if not np.isfinite(max_hold):
        max_hold = int(default.max_hold_bars)
    notional = float(np.clip(notional, 0.01, 4.0))
    leverage = float(np.clip(leverage, 1.0, 6.0))
    max_hold = int(np.clip(int(max_hold), 0, 1024))
    return sleeve.FallbackRisk("hf7_dynamic", tp, sl, notional, leverage, max_hold)
...
def _path_label_table(payload: dict[str, Any], risk_rows: pd.DataFrame, default_risk: sleeve.FallbackRisk) -> tuple[pd.DataFrame, dict[str, Any]]:
    frame = payload["frame"].reset_index(drop=True)
    if len(risk_rows) != len(frame):
        raise RuntimeError("HF7 risk row count mismatch in path label table")
    active = omega._active(payload["dec"].reset_index(drop=True))
    rows: list[dict[str, Any]] = []
    ...
    for row_id, i in enumerate(cash_idx):
        ...
        if i >= len(frame) - int(default_risk.max_hold_bars) - 3:
            continue
        row_risk = _risk_from_hf7_row(risk_rows.loc[int(i)], default_risk)
...
def _metrics_with_hf7_risk(
    ...
    *,
    ...
    if len(risk_rows) != len(frame):
        raise RuntimeError("HF7 risk row count mismatch in metrics")
    arrays = sleeve._arrays(frame)
    active = omega._active(dec)
    ...
    for i in range(0, len(frame) - 2):
        ...
        if bool(active[i]):
            ...
            if side_action in (sleeve.ACTION_LONG, sleeve.ACTION_SHORT) and conf >= float(threshold):
                ...
                row_risk = _risk_from_hf7_row(risk_rows.iloc[int(i)], default_risk)
...
def main() -> int:
    ...
    hf7_bundle = _load_hf7_bundle()
    val_hf7 = _hf7_risk_table(val_payload["frame"], hf7_bundle, prefix="hf7")
    oos_hf7 = _hf7_risk_table(oos_payload["frame"], hf7_bundle, prefix="hf7")
    ...
    path_labels, path_diag = _path_label_table(val_payload, val_hf7, RISK)
    ...
    diagnostics: dict[str, Any] = {
        ...
        "hf7_bundle": {
            "path": str(HF7_BUNDLE_PATH),
            "threshold": float(HF7_THRESHOLD),
            "device": str(HF7_DEVICE),
            "path_rows": int(len(val_hf7)),
            "oos_path_rows": int(len(oos_hf7)),
        },
        "risk": asdict(RISK),
    ...
    for cal_q in (0.50, 0.65, 0.80):
        ...
        for ev_min in (0.001, 0.002, 0.003, 0.004):
            ...
            val_m = _metrics_with_hf7_risk(
                val_payload["frame"], val_payload["dec"], val_hf7, val_ev_a, val_ev_c, RISK, 0.0, fee=fee, slip=slip, cost_mult=3.0
            )
            oos_m = _metrics_with_hf7_risk(
                oos_payload["frame"], oos_payload["dec"], oos_hf7, oos_ev_a, oos_ev_c, RISK, 0.0, fee=fee, slip=slip, cost_mult=3.0
            )
            ...
            for cfg_id, (uvl, uvs, uol, uos) in utility_preds.items():
                for utility_min in (-0.001, 0.0, 0.001, 0.002):
                    for margin_min in (0.0, 0.001, 0.002):
                        ...
                        val_m = _metrics_with_hf7_risk(
                            val_payload["frame"], val_payload["dec"], val_hf7, val_a, val_c, RISK, 0.0, fee=fee, slip=slip, cost_mult=3.0
                        )
                        oos_m = _metrics_with_hf7_risk(
                            oos_payload["frame"], oos_payload["dec"], oos_hf7, oos_a, oos_c, RISK, 0.0, fee=fee, slip=slip, cost_mult=3.0
                        )
            ...
            for probe_ev_min in PROBE_EV_MINS:
                ...
                val_m = _metrics_with_hf7_risk(
                    val_payload["frame"], val_payload["dec"], val_hf7, val_ev_probe_a, val_ev_probe_c, RISK, 0.0, fee=fee, slip=slip, cost_mult=3.0
                )
                oos_m = _metrics_with_hf7_risk(
                    oos_payload["frame"], oos_payload["dec"], oos_hf7, oos_ev_probe_a, oos_ev_probe_c, RISK, 0.0, fee=fee, slip=slip, cost_mult=3.0
                )
    ...
    report = {
        ...
        "method": "Full-retrained 3-head parent artifact is used as parent. Cash sleeve EV lower-bound and numeric utility agreement/veto regressors are newly fit on validation full-retrain features. Fallback risk inputs are per-row from HF7 risk template (tp/sl/notional/leverage/max_hold). Additional probe settings run with relaxed EV/utility minima to force fallback activation for sensitivity checks.",
