#!/usr/bin/env python3
"""Fine exposure upgrade for Omega 4.6.2 v5 roll8 side-specific branch."""
...
MODEL_ID = "omega4_6_2_v5_roll8_side_specific_fine_exposure_20260701"
REFERENCE_MODEL_ID = "omega4_6_2_v5_roll8_side_specific_fine_valmax_20260701"
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll8_side_specific_fine_exposure_20260701.md"
REFERENCE_CAP = 4.20
EPS = 1.0e-12
...
def write_markdown(report: dict[str, Any]) -> None:
    ...
    text = f"""# Omega 4.6.2 v5 Roll8 Side-Specific Fine Exposure - 2026-07-01
...
- Research gate pass: `{selected["research_roll8_fine_exposure_gate_pass"]}`
...
def main() -> int:
    roll8 = load_module("omega462_roll8_for_fine_exposure", ROLL8_PATH)
    base = roll8.load_module("omega462_roll9_base_for_roll8_fine_exposure", roll8.BASE_PATH)
    source = base.load_module("omega462_roll16_source_for_roll8_fine_exposure", base.SOURCE_MODULE_PATH)
    v1 = source.load_module("omega462_loss_cluster_v1_for_roll8_fine_exposure", source.V1_MODULE_PATH)
    segment_mod = source.load_module("omega462_segment_for_roll8_fine_exposure", source.SEGMENT_MODULE_PATH)
    stop_mod = v1.load_module("omega462_stop_mod_for_roll8_fine_exposure", v1.STOP_MODULE_PATH)
    ...
    bracket_name = "fine8_fast"
    long_tp, long_sl, short_tp, short_sl = 0.0200, 0.0300, 0.0250, 0.0400
    ...
    exposure_specs = [
        ...
        for lf in [0.75, 0.78, 0.80, 0.82, 0.85, 0.88, 0.90]
        for sf in [0.93, 0.95, 0.97, 0.975, 0.98, 0.99, 1.00, 1.02]
        for cap in [4.00, 4.20, 4.30, 4.50]
    ]
    governors = [
    ...
    for exposure in exposure_specs:
        for governor in governors:
            ...
            row["validation_roll8_fine_exposure_gate_pass"] = validation_gate
            row["oos_safety_gate_pass"] = oos_gate
            row["research_roll8_fine_exposure_gate_pass"] = validation_gate and oos_gate
            row["cap_reference_distance_abs"] = abs(float(row["exposure_cap_notional"]) - REFERENCE_CAP)
            rows.append(row)

    ranking = pd.DataFrame(rows).sort_values(
        [
            "research_roll8_fine_exposure_gate_pass",
            "validation_pnl",
    ...
    status = (
        "RESEARCH_ROLL8_SIDE_SPECIFIC_FINE_EXPOSURE_PASS"
        if bool(selected["research_roll8_fine_exposure_gate_pass"])
        else "NO_ROLL8_SIDE_SPECIFIC_FINE_EXPOSURE_PASSING_CANDIDATE"
    )
    safe = (
        f"{selected['side_bracket_spec']}__{selected['exposure_spec']}__"
        f"{selected['segment_governor_spec']}"
    ).replace(".", "p").replace("/", "_")
    ranking_path = OUT_DIR / "roll8_side_specific_fine_exposure_ranking.csv"
    top20_path = OUT_DIR / "roll8_side_specific_fine_exposure_top20.csv"
    ...
    report = {
        ...
        "selection_scope": "validation_primary_roll8_fine_exposure_with_oos_safety_gate; fresh_holdout_required",
        "selection_rule": f"among research-gated fine-exposure variants, sort by validation_pnl, validation_avg_hold, cap distance to {REFERENCE_CAP:.2f}; OOS is not an ordering key",
        ...
        json.dumps(
            {
                ...
                "selected_validation": {
                    ...
                    "gate": bool(selected["validation_roll8_fine_exposure_gate_pass"]),
                },
                "selected_oos": {
