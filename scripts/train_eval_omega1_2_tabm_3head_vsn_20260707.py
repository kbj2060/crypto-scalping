...
MODEL_ID = "omega1_2_true_3head_tabm_vsn_20260707"
...
class VariableSelectionGate(nn.Module):
    """VSN-style self-gating feature-selection layer (Temporal Fusion Transformer-inspired),
    applied to the raw input vector before it enters the TabM ensemble backbone. Deliberately
    lightweight (one small MLP producing per-feature sigmoid gates, not the full per-variable
    GRN+softmax design) given the small trade-count regime this model operates in. Motivation:
    2026-07-06/07 session found h48qual's own 102 raw input features carry mostly noise for that
    component (its own trades are net-negative both windows) -- a learned per-bar feature gate
    might suppress irrelevant inputs instead of feeding the whole vector uniformly into the
    ensemble, as the un-gated TabM currently does.
    """
    ...
    def __init__(self, n_features: int, hidden: int) -> None:
        ...
        # zero-init the last layer so gate starts at sigmoid(0)=0.5 -> *2.0 = 1.0 (identity at init)
...
class ThreeHeadTabM(nn.Module):
    def __init__(self, n_features: int, *, cfg: ThreeHeadConfig = CFG) -> None:
        ...
        self.vsn = VariableSelectionGate(self.n_features, int(cfg.vsn_hidden)) if bool(cfg.use_vsn) else None
...
def main() -> int:
    ...
    ap.add_argument("--use-vsn", action="store_true")
    args = ap.parse_args()

    global CFG
    if bool(args.use_vsn):
        CFG = ThreeHeadConfig(use_vsn=True)
