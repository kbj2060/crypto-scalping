...
MODEL_ID = "omega3_tabm_4head_atr_triple_barrier_risk_20260619"
OUT_DIR = ROOT / "tmp/causal_regen_20260516" / MODEL_ID

RISK_COLS = ["tp_atr_mult", "sl_atr_mult"]
RISK_BOUNDS = {
    "tp_atr_mult": (1.0, 3.0),
    "sl_atr_mult": (0.6, 1.5),
}
TP_PRICE_MOVE_BOUNDS = (0.008, 0.050)
SL_PRICE_MOVE_BOUNDS = (0.006, 0.035)
TP_ATR_MULT_CANDIDATES = np.asarray([1.0, 1.5, 2.0, 2.5, 3.0], dtype=np.float32)
SL_ATR_MULT_CANDIDATES = np.asarray([0.6, 0.8, 1.0, 1.2, 1.5], dtype=np.float32)
NOTIONAL_BUCKETS = np.asarray([0.2, 0.5, 1.0, 2.0, 3.0], dtype=np.float32)
EDGE_BUCKET_THRESHOLDS = np.asarray([0.0010, 0.0040, 0.0080, 0.0150], dtype=np.float64)
...
def _atr_pct(frame: pd.DataFrame, *, period: int = 14) -> np.ndarray:
    ...
    atr = pd.Series(tr).rolling(int(period), min_periods=1).mean().to_numpy(dtype=np.float64)
    return np.clip(atr / np.maximum(close, 1.0e-12), 1.0e-6, 1.0)


def _price_moves_from_atr_mult(frame: pd.DataFrame, risk: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    atr_pct = _atr_pct(frame)
    r = np.asarray(risk, dtype=np.float64)
    tp_mult = np.clip(r[:, 0], RISK_BOUNDS["tp_atr_mult"][0], RISK_BOUNDS["tp_atr_mult"][1])
    sl_mult = np.clip(r[:, 1], RISK_BOUNDS["sl_atr_mult"][0], RISK_BOUNDS["sl_atr_mult"][1])
    tp_price = np.clip(tp_mult * atr_pct, TP_PRICE_MOVE_BOUNDS[0], TP_PRICE_MOVE_BOUNDS[1])
    sl_price = np.clip(sl_mult * atr_pct, SL_PRICE_MOVE_BOUNDS[0], SL_PRICE_MOVE_BOUNDS[1])
    return atr_pct, tp_price, sl_price


def _first_touch_net(
    *,
    side: int,
    entry: float,
    high: np.ndarray,
    ...
    if int(side) not in (omega.ACTION_LONG, omega.ACTION_SHORT):
        raise RuntimeError(f"invalid triple-barrier side: {side}")
    for offset, (hi, lo) in enumerate(zip(high, low), start=1):
        if int(side) == omega.ACTION_LONG:
            hit_sl = float(lo) <= entry * (1.0 - float(sl_price_move))
            hit_tp = float(hi) >= entry * (1.0 + float(tp_price_move))
            if hit_sl:
                move = -abs(float(sl_price_move)) - 2.0 * float(slip_eff)
                return move * float(notional) - 2.0 * float(fee_eff) * float(notional), "stop_loss", int(offset)
            if hit_tp:
                move = float(tp_price_move) - 2.0 * float(slip_eff)
                return move * float(notional) - 2.0 * float(fee_eff) * float(notional), "take_profit", int(offset)
        else:
            hit_sl = float(hi) >= entry * (1.0 + float(sl_price_move))
            hit_tp = float(lo) <= entry * (1.0 - float(tp_price_move))
            if hit_sl:
                move = -abs(float(sl_price_move)) - 2.0 * float(slip_eff)
                return move * float(notional) - 2.0 * float(fee_eff) * float(notional), "stop_loss", int(offset)
            if hit_tp:
                move = float(tp_price_move) - 2.0 * float(slip_eff)
                return move * float(notional) - 2.0 * float(fee_eff) * float(notional), "take_profit", int(offset)
    close_last = float((high[-1] + low[-1]) * 0.5)
    if int(side) == omega.ACTION_LONG:
        move = (close_last - entry) / max(entry, 1.0e-12) - 2.0 * float(slip_eff)
    else:
        move = (entry - close_last) / max(entry, 1.0e-12) - 2.0 * float(slip_eff)
    return move * float(notional) - 2.0 * float(fee_eff) * float(notional), "vertical", int(len(high))


def _build_price_risk_labels(
    frame: pd.DataFrame,
    y: np.ndarray,
    *,
    horizon: int,
    ...
    for n_seen, i in enumerate(active_idx, start=1):
        if n_seen == 1 or n_seen % 10000 == 0:
            print(json.dumps({"stage": "build_atr_triple_barrier_labels", "seen": int(n_seen), "total": int(len(active_idx))}, ensure_ascii=True), flush=True)
        entry_i = int(i) + 1
        ...
        if not np.isfinite(entry) or entry <= 0.0:
            continue
        atr_i = float(atr_pct[int(i)])
        if not np.isfinite(atr_i) or atr_i <= 0.0:
            continue
        high_path = arrays["high"][entry_i : end_i + 1]
        low_path = arrays["low"][entry_i : end_i + 1]
        best: tuple[float, float, float, str, int] | None = None
        for tp_mult in TP_ATR_MULT_CANDIDATES:
            tp_price = float(np.clip(float(tp_mult) * atr_i, TP_PRICE_MOVE_BOUNDS[0], TP_PRICE_MOVE_BOUNDS[1]))
            for sl_mult in SL_ATR_MULT_CANDIDATES:
                sl_price = float(np.clip(float(sl_mult) * atr_i, SL_PRICE_MOVE_BOUNDS[0], SL_PRICE_MOVE_BOUNDS[1]))
                net, reason, offset = _first_touch_net(
                    side=int(y[i]),
                    entry=entry,
                    high=high_path,
                    low=low_path,
                    tp_price_move=tp_price,
                    sl_price_move=sl_price,
                    notional=1.0,
                    fee_eff=fee_eff,
                    slip_eff=slip_eff,
                )
                candidate = (float(net), float(tp_mult), float(sl_mult), str(reason), int(offset))
                if best is None or candidate[0] > best[0]:
                    best = candidate
        if best is None:
            continue
        best_net, best_tp_mult, best_sl_mult, reason, offset = best
        if reason != "take_profit" or best_net <= 0.0:
            bucket = 0
        else:
            bucket = int(np.searchsorted(EDGE_BUCKET_THRESHOLDS, float(best_net), side="right"))
        risk[int(i)] = np.asarray([best_tp_mult, best_sl_mult], dtype=np.float32)
        notional_bucket[int(i)] = int(bucket)
        usable[int(i)] = True
        best_nets.append(float(best_net))
        touches[str(reason)] = int(touches.get(str(reason), 0) + 1)
        hold_offsets.append(int(offset))
        tp_moves.append(float(np.clip(best_tp_mult * atr_i, TP_PRICE_MOVE_BOUNDS[0], TP_PRICE_MOVE_BOUNDS[1])))
        sl_moves.append(float(np.clip(best_sl_mult * atr_i, SL_PRICE_MOVE_BOUNDS[0], SL_PRICE_MOVE_BOUNDS[1])))
        notionals.append(float(NOTIONAL_BUCKETS[int(bucket)]))
    unit = _risk_to_unit(risk)
    diag = {
        "label_mode": "atr_scaled_triple_barrier_first_touch",
        "horizon": int(horizon),
        "atr_period": 14,
        "active_rows": int(active.sum()),
        "usable_rows": int(usable.sum()),
        "tp_atr_mult_candidates": TP_ATR_MULT_CANDIDATES.tolist(),
        "sl_atr_mult_candidates": SL_ATR_MULT_CANDIDATES.tolist(),
        "tp_atr_mult_mean": float(np.mean(risk[usable, 0])) if bool(usable.any()) else 0.0,
        "sl_atr_mult_mean": float(np.mean(risk[usable, 1])) if bool(usable.any()) else 0.0,
        "tp_price_move_mean": float(np.mean(tp_moves)) if tp_moves else 0.0,
        "sl_price_move_mean": float(np.mean(sl_moves)) if sl_moves else 0.0,
        "atr_pct_mean": float(np.mean(atr_pct[usable])) if bool(usable.any()) else 0.0,
        ...
        "first_touch_reasons": touches,
        "best_net_mean": float(np.mean(best_nets)) if best_nets else 0.0,
        "edge_bucket_thresholds": EDGE_BUCKET_THRESHOLDS.tolist(),
        "hold_offset_mean": float(np.mean(hold_offsets)) if hold_offsets else 0.0,
...
def _apply_price_risk(frame: pd.DataFrame, dec: pd.DataFrame, risk: np.ndarray, notional_bucket: np.ndarray) -> pd.DataFrame:
    ...
    atr_pct, tp_price, sl_price = _price_moves_from_atr_mult(frame, risk)
    ...
    out.loc[active, "atr_pct"] = atr_pct[active]
    out.loc[active, "tp_atr_mult"] = np.asarray(risk, dtype=np.float64)[:, 0][active]
    out.loc[active, "sl_atr_mult"] = np.asarray(risk, dtype=np.float64)[:, 1][active]
    ...
    out.loc[~active, "stop_loss"] = 0.0
    out.loc[~active, "atr_pct"] = atr_pct[~active]
    out.loc[~active, "tp_atr_mult"] = 0.0
    out.loc[~active, "sl_atr_mult"] = 0.0
...
def main() -> int:
    ...
    print(json.dumps({"stage": "prepare_frames_start", "model_id": MODEL_ID}, ensure_ascii=True), flush=True)
    frames = fast_frames._prepare_frames_fast(disable_tp_sl=False)
    print(json.dumps({"stage": "prepare_frames_done"}, ensure_ascii=True), flush=True)
    ...
    train_raw_for_labels = train_fit_frame
    x_train = threehead._base_input(train_raw, base_cols)
    y_train_full = train_raw["zigzag_action"].to_numpy(dtype=np.int64)
    y_train_label = train_raw_for_labels["zigzag_action"].to_numpy(dtype=np.int64)
    print(json.dumps({"stage": "risk_label_start", "train_rows": int(len(train_raw_for_labels)), "active_rows": int(((y_train_label == omega.ACTION_LONG) | (y_train_label == omega.ACTION_SHORT)).sum())}, ensure_ascii=True), flush=True)
    y_risk, y_notional_bucket, risk_mask, risk_diag = _build_price_risk_labels(
        train_raw_for_labels,
        y_train_label,
        horizon=int(args.risk_label_horizon),
        fee=fee,
        slip=slip,
        cost_mult=float(args.cost_mult),
    )
    print(json.dumps({"stage": "risk_label_done", "diag": risk_diag}, ensure_ascii=True, default=_json_default), flush=True)
    if int(args.max_train_rows) > 0:
        limit = int(args.max_train_rows)
        x_train = x_train.iloc[:limit].reset_index(drop=True)
        y_train = y_train_full[:limit]
        y_risk = y_risk[:limit]
    ...
    def predict_frame(frame: pd.DataFrame, *, threshold: float, oof: bool) -> tuple[pd.DataFrame, np.ndarray, np.ndarray]:
        ...
        return _apply_price_risk(frame, dec, risk, notional_bucket), risk, notional_bucket
    ...
    for q in thresholds:
        ...
        reports[name] = {
            ...
            "validation_risk_distribution": {
                "tp_atr_mult_mean": float(np.mean(val_risk[:, 0])),
                "sl_atr_mult_mean": float(np.mean(val_risk[:, 1])),
            ...
            "oos_risk_distribution": {
                "tp_atr_mult_mean": float(np.mean(oos_risk[:, 0])),
                "sl_atr_mult_mean": float(np.mean(oos_risk[:, 1])),
    ...
    torch.save(
        {
            ...
            "risk_cols": RISK_COLS,
            "risk_bounds": RISK_BOUNDS,
            "tp_price_move_bounds": TP_PRICE_MOVE_BOUNDS,
            "sl_price_move_bounds": SL_PRICE_MOVE_BOUNDS,
            "notional_buckets": NOTIONAL_BUCKETS.tolist(),
        },
        OUT_DIR / "omega3_4head_atr_triple_barrier_tabm_bundle.pt",
    )
    report = {
        "model_id": MODEL_ID,
        "status": "atr_triple_barrier_4head_risk_eval",
        "design": "Parent TabM with shared encoder and four heads: direction, quality, exit, risk. Direction keeps zigzag_action labels. Risk branch is trained from ATR-scaled triple-barrier first-touch labels and outputs tp_atr_mult/sl_atr_mult plus a notional bucket classifier. Leverage and margin are removed from the learned risk contract.",
        "risk_contract": {
            "outputs": RISK_COLS + ["notional_bucket"],
            "bounds": RISK_BOUNDS,
            "tp_price_move_bounds": TP_PRICE_MOVE_BOUNDS,
            "sl_price_move_bounds": SL_PRICE_MOVE_BOUNDS,
            "tp_price_move": "clip(tp_atr_mult * atr_pct, tp_min, tp_max)",
            "sl_price_move": "clip(sl_atr_mult * atr_pct, sl_min, sl_max)",
            "notional_buckets": NOTIONAL_BUCKETS.tolist(),
            "notional_quality_coupling": "disabled; notional label bucket is assigned from path edge only",
