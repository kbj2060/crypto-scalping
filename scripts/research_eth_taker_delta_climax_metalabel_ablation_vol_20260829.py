#!/usr/bin/env python3
"""Ablation for taker_delta_z_climax v4 (research_eth_taker_delta_climax_metalabel_tabpfn_
20260829.py, cluster-anchored MFE/2h/2.0xATR label, VAL/OOS/HOLDOUT AUC 0.622/0.608/0.650):
permutation importance found atr_percentile_864 (+0.035) alone contributed ~5x the next feature,
and together with atr_pct (+0.007) and realized_vol_ratio (+0.006) the 3 volatility-regime
features accounted for the largest chunk of the top-10. Does the AUC survive without them, or was
most of v4's signal just "we're in a high-volatility regime, big moves are more likely regardless
of anything about this specific fire"?

Reuses the already-built v4 feature CSV (cluster-anchored, no rebuild needed -- the expensive
indicator-frame construction already happened once). Same TRAIN(<2025-09-01)/VAL(2025-09-01~
2025-12-31)/OOS(2026-01-01~2026-03-31)/HOLDOUT(2026-04-01~) split, same 4 seeds, full-23-feature
config re-run alongside the ablated-20-feature config as an internal consistency check (full
should reproduce v4's original report.json numbers almost exactly).
"""
...
CSV_PATH = ROOT / "data/labels/eth_5m_taker_delta_climax_metalabel_20260829/eth_5m_taker_delta_climax_metalabel_features.csv"
