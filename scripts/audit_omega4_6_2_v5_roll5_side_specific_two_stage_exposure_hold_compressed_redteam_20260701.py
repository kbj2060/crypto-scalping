#!/usr/bin/env python3
"""Red-team audit for Omega 4.6.2 v5 roll5 hold-compressed branch."""
...
EVAL_MODULE_PATH = ROOT / "scripts/eval_omega4_6_2_v5_roll5_side_specific_two_stage_exposure_hold_compressed_20260701.py"
...
RANKING = OUT_DIR / "roll5_two_stage_exposure_hold_compressed_ranking.csv"
TOP20 = OUT_DIR / "roll5_two_stage_exposure_hold_compressed_top20.csv"
PRIOR_RUNTIME_AUDIT = (
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll5_side_specific_two_stage_exposure_hold_compressed_redteam_20260701.md"
TOL = 1.0e-8
...
def write_markdown(payload: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- Max roll hold: `{selected['roll5_max_hours']}h`",
        "",
    ...
    lines.extend(
        [
            ...
            "## Replay Checks",
            "",
            f"- Validation replay: `{payload['roll5_replay']['validation']['pass']}`",
            f"- OOS replay: `{payload['roll5_replay']['oos']['pass']}`",
            "",
            "## Artifacts",
            "",
...
def main() -> None:
    audit = load_module("omega462_loss_cluster_audit_for_roll5_hold_compressed", AUDIT_HELPER_PATH)
    eval_mod = load_module("omega462_roll5_hold_compressed_eval_for_audit", EVAL_MODULE_PATH)
    ...
    audit.add_check(
        ...
        == "validation_primary_roll5_two_stage_veto_exposure_overlay_with_oos_safety_gate; fresh_holdout_required",
        "research_blocker",
    ...
    audit.add_check(
        checks,
        "selected_gate_pass",
        selected.get("research_roll5_hold_compressed_gate_pass") is True,
        "research_blocker",
        {"observed": selected.get("research_roll5_hold_compressed_gate_pass")},
    )
    ...
    verdict = (
        "FULL_LIVE_PASS"
        if full_live_pass
        else "RESEARCH_ROLL5_HOLD_COMPRESSED_PASS_FULL_LIVE_BLOCKED"
        if research_pass
        else "REDTEAM_FAIL"
    )
    payload = {
        "audit_id": "omega4_6_2_v5_roll5_side_specific_two_stage_exposure_hold_compressed_redteam_20260701",
        ...
        "checks": checks,
        "roll5_replay": {"validation": val_replay, "oos": oos_replay},
