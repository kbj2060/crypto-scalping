#!/usr/bin/env python3
"""SINGLE final HOLDOUT (2026-04-01+) exposure for the orthogonal_combo trailing-stop cost-gate --
final config SL=4.0xATR/ARM=0.5xATR/Trail=0.1xATR, picked from
backtest_eth_orthogonal_combo_trailing_gridsearch_20260831.py (73/96 combos passed VAL+OOS, this
was the #1 combo by min(VAL,OOS)=+9.36bp) + backtest_eth_orthogonal_combo_trailing_optimistic_
crosscheck_20260831.py (robust to both intrabar-ordering conventions, ~1.0-1.3bp spread, same
ranking either way). Companion to the classification HOLDOUT already touched in
research_eth_orthogonal_combo_metalabel_ablation_20260830.py (AUC 0.7245) -- together these are the
ONE holdout exposure for this model; do not re-run with a different SL/ARM/Trail after seeing this
result.
"""
...
def log(msg: str) -> None:
    print(f"[orthogonal_combo_trailing_HOLDOUT] {msg}", flush=True)
...
def main() -> int:
    ...
    log(f"(for reference: VAL was +9.36bp/win91.5%, OOS was +15.13bp/win96.0% at this exact config)")
    return 0
