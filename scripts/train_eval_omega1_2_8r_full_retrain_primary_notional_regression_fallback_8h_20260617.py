...
MODEL_ID = "omega1_2_8r_full_retrain_primary_notional_regression_fallback_8h_20260617"
OUT_DIR = ROOT / "tmp/causal_regen_20260516" / MODEL_ID
RISK = sleeve.FallbackRisk("vol_normalized_sltp_n081_l2_h192", 0.052, 0.028, 0.81, 2.0, 192)
BASELINE_ID = "omega1_2_true_3head_tabm_20260603_full_retrain_cash_alpha43_20260608"
PRIMARY_NOTIONAL_GRID = (0.81, 0.95, 1.05, 1.20, 1.35, 1.50)
PRIMARY_NOTIONAL_MIN = min(PRIMARY_NOTIONAL_GRID)
PRIMARY_NOTIONAL_MAX = max(PRIMARY_NOTIONAL_GRID)
VOL_RISKS = (
    sleeve.FallbackRisk("vol_base_tp052_sl028_n081_h192", 0.052, 0.028, 0.81, 2.0, 192),
    sleeve.FallbackRisk("vol_wide_tp065_sl034_n081_h192", 0.065, 0.034, 0.81, 2.0, 192),
    sleeve.FallbackRisk("vol_runner_tp078_sl042_n081_h192", 0.078, 0.042, 0.81, 2.0, 192),
)
NOTIONAL_LABEL_UTILITY_CFG = {"stop_penalty": 0.003, "mae_penalty": 0.20, "time_penalty": 0.0}
UTILITY_CFGS = (
...
def _primary_notional_row(row: pd.Series, notional: float) -> pd.Series:
    out = row.copy()
    out["notional_exposure"] = float(np.clip(float(notional), PRIMARY_NOTIONAL_MIN, PRIMARY_NOTIONAL_MAX))
    return out


def _simulate_primary_side_detail(payload: dict[str, Any], i: int, side: int, notional: float) -> dict[str, Any]:
    frame = payload["frame"].reset_index(drop=True)
    dec = payload["dec"].reset_index(drop=True)
    arrays = sleeve._arrays(frame)
    row = _primary_notional_row(dec.iloc[int(i)], notional)
    fee_eff = float(payload["fee"]) * 3.0
    slip_eff = float(payload["slip"]) * 3.0

    cash = 1.0
    cash, pos, entered = sleeve._open_position(cash, arrays, int(i), int(side), "primary", None, row, fee_eff, slip_eff)
    if not entered:
        return {"net": -1.0e-6, "mae": 0.0, "mfe": 0.0, "stop": 0, "bars": 0, "reason": "entry_miss"}

    max_hold = int(pos.max_hold_bars)
    max_i = len(frame) - 2 if max_hold <= 0 else min(len(frame) - 2, int(pos.entry_i) + max_hold)
    mfe = 0.0
    mae = 0.0
    exit_i = max_i
    reason = "forced_end" if max_hold <= 0 else "max_hold"
    for j in range(int(pos.entry_i), max_i + 1):
        px = float(arrays["close"][j])
        raw = (px * (1.0 - slip_eff) - pos.entry_price) / max(pos.entry_price, 1.0e-12) if side > 0 else (pos.entry_price - px * (1.0 + slip_eff)) / max(pos.entry_price, 1.0e-12)
        unreal = raw * float(pos.notional)
        ...
        if pos.take_profit > 0.0 and unreal >= float(pos.take_profit):
            reason = "take_profit"
            break
        if pos.stop_loss > 0.0 and unreal <= -abs(float(pos.stop_loss)):
            reason = "stop_loss"
            break
        if max_hold > 0 and int(j) - int(pos.entry_i) >= max_hold:
            reason = "max_hold"
            break
...
def _notional_utility(d: dict[str, Any], cfg: dict[str, float], max_hold: int) -> float:
    adverse = abs(min(float(d["mae"]), 0.0))
    time_frac = 0.0 if int(max_hold) <= 0 else min(float(d["bars"]) / float(max(int(max_hold), 1)), 1.0)
    return float(d["net"]) - float(cfg["stop_penalty"]) * int(d["stop"]) - float(cfg["mae_penalty"]) * adverse - float(cfg["time_penalty"]) * time_frac


def _primary_notional_label_table(payload: dict[str, Any]) -> tuple[pd.DataFrame, dict[str, Any]]:
    frame = payload["frame"].reset_index(drop=True)
    dec = payload["dec"].reset_index(drop=True)
    active = omega._active(dec)
    rows: list[dict[str, Any]] = []
    active_idx = np.flatnonzero(active)
    for row_id, i in enumerate(active_idx):
        if row_id % 250 == 0:
            print(json.dumps({"stage": "primary_notional_labels", "row": int(row_id), "total_primary": int(len(active_idx))}, ensure_ascii=True), flush=True)
        if i >= len(frame) - 3:
            continue
        max_hold = int(dec.iloc[int(i)].get("max_hold_bars", 0) or 0)
        row: dict[str, Any] = {"i": int(i)}
        for side_name, side in (("long", 1), ("short", -1)):
            utils: list[float] = []
            nets: list[float] = []
            reasons: list[str] = []
            for notional in PRIMARY_NOTIONAL_GRID:
                d = _simulate_primary_side_detail(payload, int(i), side, float(notional))
                utils.append(_notional_utility(d, NOTIONAL_LABEL_UTILITY_CFG, max_hold))
                nets.append(float(d["net"]))
                reasons.append(str(d["reason"]))
            best = int(np.argmax(np.asarray(utils, dtype=np.float64)))
            row[f"{side_name}_notional"] = float(PRIMARY_NOTIONAL_GRID[best])
            row[f"{side_name}_notional_utility"] = float(utils[best])
            row[f"{side_name}_notional_net"] = float(nets[best])
            row[f"{side_name}_notional_reason"] = reasons[best]
        rows.append(row)
    labels = pd.DataFrame(rows)
    diag = {
        "valid_primary_rows": int(len(labels)),
        "utility_cfg": dict(NOTIONAL_LABEL_UTILITY_CFG),
        "notional_grid": list(PRIMARY_NOTIONAL_GRID),
        "long_notional_counts": labels["long_notional"].value_counts().sort_index().to_dict() if len(labels) else {},
        "short_notional_counts": labels["short_notional"].value_counts().sort_index().to_dict() if len(labels) else {},
        "long_reason_counts": labels["long_notional_reason"].value_counts().sort_index().to_dict() if len(labels) else {},
        "short_reason_counts": labels["short_notional_reason"].value_counts().sort_index().to_dict() if len(labels) else {},
    }
    return labels, diag
...
def _metrics_with_vol_sltp(
    ...
    *,
    ...
    primary_notional_pred: dict[str, np.ndarray] | None,
    primary_notional_policy: str,
) -> dict[str, Any]:
    arrays = sleeve._arrays(frame)
    active = omega._active(dec)
    ...
    primary_tp: list[float] = []
    primary_sl: list[float] = []
    primary_notional: list[float] = []
    primary_leverage: list[float] = []
    ...
    for i in range(0, len(frame) - 2):
        ...
        if bool(active[i]):
            ...
            if side != 0:
                if primary_notional_pred is None:
                    primary_row = row
                else:
                    key = "long_notional" if side > 0 else "short_notional"
                    row_notional = float(primary_notional_pred[key][int(i)])
                    primary_row = _primary_notional_row(row, row_notional)
                cash, pos, entered = sleeve._open_position(cash, arrays, i, side, "primary", None, primary_row, fee_eff, slip_eff)
                if entered:
                    primary_entries += 1
                    long_entries += int(side > 0)
                    short_entries += int(side < 0)
                    primary_tp.append(float(pos.take_profit))
                    primary_sl.append(float(pos.stop_loss))
                    primary_notional.append(float(pos.notional))
                    primary_leverage.append(float(pos.leverage))
        else:
    ...
    primary_tp_arr = np.asarray(primary_tp, dtype=np.float64)
    primary_sl_arr = np.asarray(primary_sl, dtype=np.float64)
    primary_notional_arr = np.asarray(primary_notional, dtype=np.float64)
    return {
        ...
        "primary_notional_policy": str(primary_notional_policy),
        "primary_avg_tp": float(np.mean(primary_tp_arr)) if len(primary_tp_arr) else 0.0,
        "primary_avg_sl": float(np.mean(primary_sl_arr)) if len(primary_sl_arr) else 0.0,
        "primary_avg_notional": float(np.mean(primary_notional_arr)) if len(primary_notional_arr) else 0.0,
        "primary_avg_leverage": float(np.mean(primary_leverage)) if primary_leverage else 0.0,
        "primary_avg_tp_price_move": float(np.mean(primary_tp_arr / np.maximum(primary_notional_arr, 1.0e-12))) if len(primary_tp_arr) else 0.0,
        "primary_avg_sl_price_move": float(np.mean(primary_sl_arr / np.maximum(primary_notional_arr, 1.0e-12))) if len(primary_sl_arr) else 0.0,
...
def _fit_predict_hgb_notional(
    ...
    *,
    ...
    idx = labels["i"].to_numpy(dtype=np.int64)
    y_long = np.full(len(x_val), 0.81, dtype=np.float64)
    y_short = np.full(len(x_val), 0.81, dtype=np.float64)
    y_long[idx] = labels["long_notional"].to_numpy(dtype=np.float64)
    y_short[idx] = labels["short_notional"].to_numpy(dtype=np.float64)
    val_long = np.full(len(x_val), 0.81, dtype=np.float64)
    val_short = np.full(len(x_val), 0.81, dtype=np.float64)
    folds_meta: list[dict[str, Any]] = []
    for fold_id, (tr, va) in enumerate(_chron_folds(idx)):
        ml = HistGradientBoostingRegressor(max_iter=120, learning_rate=0.035, max_leaf_nodes=9, l2_regularization=2.0, random_state=int(seed + fold_id * 10 + 1))
        ms = HistGradientBoostingRegressor(max_iter=120, learning_rate=0.035, max_leaf_nodes=9, l2_regularization=2.0, random_state=int(seed + fold_id * 10 + 2))
        ml.fit(x_val.iloc[tr].to_numpy(dtype=np.float64), y_long[tr])
        ms.fit(x_val.iloc[tr].to_numpy(dtype=np.float64), y_short[tr])
    ...
    ml = HistGradientBoostingRegressor(max_iter=120, learning_rate=0.035, max_leaf_nodes=9, l2_regularization=2.0, random_state=int(seed + 101))
    ms = HistGradientBoostingRegressor(max_iter=120, learning_rate=0.035, max_leaf_nodes=9, l2_regularization=2.0, random_state=int(seed + 102))
    x_train = x_val.iloc[idx].to_numpy(dtype=np.float64)
    ml.fit(x_train, y_long[idx])
    ms.fit(x_train, y_short[idx])
    ...
    val = {"long_notional": np.clip(val_long, PRIMARY_NOTIONAL_MIN, PRIMARY_NOTIONAL_MAX), "short_notional": np.clip(val_short, PRIMARY_NOTIONAL_MIN, PRIMARY_NOTIONAL_MAX)}
    oos = {"long_notional": np.clip(oos_long, PRIMARY_NOTIONAL_MIN, PRIMARY_NOTIONAL_MAX), "short_notional": np.clip(oos_short, PRIMARY_NOTIONAL_MIN, PRIMARY_NOTIONAL_MAX)}
    diag = {
        "model": "HistGradientBoostingRegressor side-specific heads",
        "target_cols": ["long_notional", "short_notional"],
        "folds": folds_meta,
        "val_mean": {k: float(np.mean(v)) for k, v in val.items()},
        "oos_mean": {k: float(np.mean(v)) for k, v in oos.items()},
    }
    return val, oos, diag


class NotionalTabM(nn.Module):
    def __init__(self, n_features: int, *, k: int = 8, hidden: int = 128, layers: int = 3, dropout: float = 0.08) -> None:
        super().__init__()
        self.k = int(k)
        ...
        self.expert_scale = nn.ParameterList(nn.Parameter(torch.randn(self.k, int(hidden)) * 0.03 + 1.0) for _ in range(max(0, int(layers) - 1)))
        self.norms = nn.ModuleList(nn.LayerNorm(int(hidden)) for _ in range(max(0, int(layers))))
        self.dropout = nn.Dropout(float(dropout))
        self.long_head = nn.Linear(int(hidden), 1)
        self.short_head = nn.Linear(int(hidden), 1)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        h = self.encode(x)
        long = self.long_head(h).squeeze(-1)
        short = self.short_head(h).squeeze(-1)
        return torch.stack([long, short], dim=-1)


def _notional_to_unit(y: np.ndarray) -> np.ndarray:
    return ((np.asarray(y, dtype=np.float32) - float(PRIMARY_NOTIONAL_MIN)) / (float(PRIMARY_NOTIONAL_MAX) - float(PRIMARY_NOTIONAL_MIN))).astype(np.float32)


def _unit_to_notional(y: np.ndarray) -> np.ndarray:
    return np.clip(np.asarray(y, dtype=np.float64), 0.0, 1.0) * (float(PRIMARY_NOTIONAL_MAX) - float(PRIMARY_NOTIONAL_MIN)) + float(PRIMARY_NOTIONAL_MIN)
...
def _standardize_fit(x: pd.DataFrame) -> tuple[np.ndarray, dict[str, Any]]:
    ...
    if not np.isfinite(out).all():
        raise RuntimeError("non-finite standardized TabM training matrix")
...
def _standardize_apply(x: pd.DataFrame, scaler: dict[str, Any]) -> np.ndarray:
    if list(x.columns) != list(scaler["columns"]):
        raise RuntimeError("TabM notional feature column contract mismatch")
    arr = x.to_numpy(dtype=np.float32)
...
def _train_tabm_notional(x_train: np.ndarray, y_train: np.ndarray, *, seed: int, epochs: int = 80) -> tuple[NotionalTabM, dict[str, Any]]:
    random.seed(int(seed))
    np.random.seed(int(seed))
    torch.manual_seed(int(seed))
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = NotionalTabM(x_train.shape[1]).to(device)
    opt = torch.optim.AdamW(model.parameters(), lr=2.0e-3, weight_decay=2.0e-4)
    ...
    for _epoch in range(int(epochs)):
        ...
        for xb, yb in loader:
            ...
            yb = yb.to(device)
            pred = model(xb)
            loss = torch.nn.functional.smooth_l1_loss(pred.mean(dim=1), yb)
            opt.zero_grad(set_to_none=True)
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 2.0)
            opt.step()
            total += float(loss.detach().cpu()) * int(len(xb))
            count += int(len(xb))
        losses.append(total / max(count, 1))
    return model, {"device": str(device), "epochs": int(epochs), "final_loss": float(losses[-1]) if losses else None}


@torch.no_grad()
def _predict_tabm_notional(model: NotionalTabM, x: np.ndarray) -> np.ndarray:
    device = next(model.parameters()).device
    model.eval()
    chunks: list[np.ndarray] = []
    for start in range(0, len(x), 8192):
        xb = torch.from_numpy(x[start : start + 8192].astype(np.float32)).to(device)
        pred = model(xb).mean(dim=1).detach().cpu().numpy()
        chunks.append(pred)
    return np.concatenate(chunks, axis=0) if chunks else np.zeros((0, 2), dtype=np.float32)


def _fit_predict_tabm_notional(
    x_val: pd.DataFrame,
    x_oos: pd.DataFrame,
    labels: pd.DataFrame,
    *,
    ...
    idx = labels["i"].to_numpy(dtype=np.int64)
    y = np.column_stack([
        _notional_to_unit(labels["long_notional"].to_numpy(dtype=np.float64)),
        _notional_to_unit(labels["short_notional"].to_numpy(dtype=np.float64)),
    ]).astype(np.float32)
    val_long = np.full(len(x_val), 0.81, dtype=np.float64)
    val_short = np.full(len(x_val), 0.81, dtype=np.float64)
    folds_meta: list[dict[str, Any]] = []
    for fold_id, (tr_pos, va_pos) in enumerate(_chron_folds(idx)):
        tr_mask = np.isin(idx, tr_pos)
        x_fit, scaler = _standardize_fit(x_val.iloc[tr_pos])
        model, train_diag = _train_tabm_notional(x_fit, y[tr_mask], seed=int(seed + fold_id * 10), epochs=60)
        pred = _unit_to_notional(_predict_tabm_notional(model, _standardize_apply(x_val.iloc[va_pos], scaler)))
        val_long[va_pos] = pred[:, 0]
        val_short[va_pos] = pred[:, 1]
        folds_meta.append({"fold": int(fold_id), "train_rows": int(len(tr_pos)), "val_rows": int(len(va_pos)), **train_diag})

    x_fit, scaler = _standardize_fit(x_val.iloc[idx])
    model, train_diag = _train_tabm_notional(x_fit, y, seed=int(seed + 101), epochs=80)
    oos_pred = _unit_to_notional(_predict_tabm_notional(model, _standardize_apply(x_oos, scaler)))
    val = {"long_notional": np.clip(val_long, PRIMARY_NOTIONAL_MIN, PRIMARY_NOTIONAL_MAX), "short_notional": np.clip(val_short, PRIMARY_NOTIONAL_MIN, PRIMARY_NOTIONAL_MAX)}
    oos = {"long_notional": np.clip(oos_pred[:, 0], PRIMARY_NOTIONAL_MIN, PRIMARY_NOTIONAL_MAX), "short_notional": np.clip(oos_pred[:, 1], PRIMARY_NOTIONAL_MIN, PRIMARY_NOTIONAL_MAX)}
    diag = {
        "model": "TabM shared encoder with long/short notional regression heads",
        "target_cols": ["long_notional", "short_notional"],
        "folds": folds_meta,
        "final_train": train_diag,
        "val_mean": {k: float(np.mean(v)) for k, v in val.items()},
        "oos_mean": {k: float(np.mean(v)) for k, v in oos.items()},
    }
    return val, oos, diag
...
def _metric_row(candidate: str, family: str, cfg_id: int | None, cal_q: float, ev_min: float, utility_min: float | None, margin_min: float | None, val_m: dict[str, Any], oos_m: dict[str, Any], base_val: dict[str, Any], base_oos: dict[str, Any]) -> dict[str, Any]:
    ...
    row["primary_notional_policy"] = str(val_m.get("primary_notional_policy", "parent"))
    row["val_primary_avg_tp"] = float(val_m.get("primary_avg_tp", 0.0) or 0.0)
    row["val_primary_avg_sl"] = float(val_m.get("primary_avg_sl", 0.0) or 0.0)
    row["oos_primary_avg_tp"] = float(oos_m.get("primary_avg_tp", 0.0) or 0.0)
    row["oos_primary_avg_sl"] = float(oos_m.get("primary_avg_sl", 0.0) or 0.0)
    row["val_primary_avg_notional"] = float(val_m.get("primary_avg_notional", 0.0) or 0.0)
    row["oos_primary_avg_notional"] = float(oos_m.get("primary_avg_notional", 0.0) or 0.0)
    row["val_primary_avg_leverage"] = float(val_m.get("primary_avg_leverage", 0.0) or 0.0)
    row["oos_primary_avg_leverage"] = float(oos_m.get("primary_avg_leverage", 0.0) or 0.0)
    row["val_primary_avg_tp_price_move"] = float(val_m.get("primary_avg_tp_price_move", 0.0) or 0.0)
    row["val_primary_avg_sl_price_move"] = float(val_m.get("primary_avg_sl_price_move", 0.0) or 0.0)
    row["oos_primary_avg_tp_price_move"] = float(oos_m.get("primary_avg_tp_price_move", 0.0) or 0.0)
    row["oos_primary_avg_sl_price_move"] = float(oos_m.get("primary_avg_sl_price_move", 0.0) or 0.0)
...
def main() -> int:
    ...
    primary_notional_labels, primary_notional_diag = _primary_notional_label_table(val_payload)
    val_hgb_notional, oos_hgb_notional, hgb_notional_diag = _fit_predict_hgb_notional(x_val, x_oos, primary_notional_labels, seed=284000)
    val_tabm_notional, oos_tabm_notional, tabm_notional_diag = _fit_predict_tabm_notional(x_val, x_oos, primary_notional_labels, seed=284500)
    primary_notional_policies = {
        "parent_notional": (None, None),
        "hgb_notional_regression": (val_hgb_notional, oos_hgb_notional),
        "tabm_notional_multihead": (val_tabm_notional, oos_tabm_notional),
    }
    ...
    diagnostics: dict[str, Any] = {
        "mode": "full_retrain_parent_numeric_cash_sleeve_with_primary_notional_regression_and_fallback_vol_sltp",
        ...
        "primary_notional_grid": list(PRIMARY_NOTIONAL_GRID),
        "primary_notional_policy": "primary row take_profit/stop_loss/leverage/max_hold are preserved; only primary notional_exposure is replaced by clipped regression output",
        "vol_normalized_sltp": {
            ...
            "buckets": [asdict(r) for r in VOL_RISKS],
        },
        ...
        "primary_notional_labels": primary_notional_diag,
        "hgb_notional_head": hgb_notional_diag,
        "tabm_notional_head": tabm_notional_diag,
        "ev_labels": ev_diag,
    }
    ...
    for cal_q in (0.50, 0.65, 0.80):
        ...
        for ev_min in (0.001, 0.002, 0.003, 0.004):
            ...
            for policy_name, (val_notional, oos_notional) in primary_notional_policies.items():
                val_m = _metrics_with_vol_sltp(val_payload["frame"], val_payload["dec"], val_ev_a, val_ev_c, 0.0, fee=fee, slip=slip, cost_mult=3.0, vol_quantiles=vol_quantiles, primary_notional_pred=val_notional, primary_notional_policy=policy_name)
                oos_m = _metrics_with_vol_sltp(oos_payload["frame"], oos_payload["dec"], oos_ev_a, oos_ev_c, 0.0, fee=fee, slip=slip, cost_mult=3.0, vol_quantiles=vol_quantiles, primary_notional_pred=oos_notional, primary_notional_policy=policy_name)
                ev_name = f"full_retrain_ev_cal{cal_q:.2f}_ev{ev_min:.3f}_{policy_name}"
            ...
            for cfg_id, (uvl, uvs, uol, uos) in utility_preds.items():
                for utility_min in (-0.001, 0.0, 0.001, 0.002):
                    for margin_min in (0.0, 0.001, 0.002):
                        ...
                        base_cand = f"full_retrain_ev_cal{cal_q:.2f}_ev{ev_min:.3f}_numcfg{cfg_id}_u{utility_min:.3f}_m{margin_min:.3f}"
                        diagnostics[f"{base_cand}_filter"] = {"validation": val_filter, "oos": oos_filter}
                        for policy_name, (val_notional, oos_notional) in primary_notional_policies.items():
                            cand = f"{base_cand}_{policy_name}"
                            val_m = _metrics_with_vol_sltp(val_payload["frame"], val_payload["dec"], val_a, val_c, 0.0, fee=fee, slip=slip, cost_mult=3.0, vol_quantiles=vol_quantiles, primary_notional_pred=val_notional, primary_notional_policy=policy_name)
                            oos_m = _metrics_with_vol_sltp(oos_payload["frame"], oos_payload["dec"], oos_a, oos_c, 0.0, fee=fee, slip=slip, cost_mult=3.0, vol_quantiles=vol_quantiles, primary_notional_pred=oos_notional, primary_notional_policy=policy_name)
    ...
    report = {
        ...
        "method": "Full-retrained 3-head parent artifact is used as parent. Primary row notional_exposure is selected by side-specific notional regression heads while preserving primary TP/SL/leverage/max_hold. Cash sleeve EV lower-bound and numeric utility agreement/veto regressors are newly fit on validation full-retrain features. Fallback uses omega1_2_8h volatility SLTP buckets.",
