"""Build and publish the exact source-stable v5 one-minute feature stream."""
...
DEFAULT_OUTPUT = ROOT / "data/live/eth_micro_scalp_v5_features_1m.csv"
...
def _stream_contract(
    ...
    base_names = v5.v4.SOURCE_STABLE_FEATURES
    micro_names = v5.v3.core.MICRO_FEATURES
    names = ["timestamp", "close", *base_names, *micro_names]
    publish = stream[
    ...
    required = ["close", *base_names, "micro_available", "book_available"]
    required_finite = bool(
        np.isfinite(numeric[required].to_numpy(dtype=np.float64)).all()
    )
    micro_payload = [name for name in micro_names if name.startswith("micro_")]
    book_payload = [name for name in micro_names if name.startswith("book_")]
...
def build(
    ...
    if lookback_days < 7 or lookback_days > 29:
        ...
        pd.Timestamp.utcnow().tz_localize(None).floor("min")
    ...
    checkpoint = torch.load(v5.MODEL_PATH, map_location="cpu", weights_only=False)
    if checkpoint.get("model_id") != v5.MODEL_ID:
        raise RuntimeError("v5 model id mismatch")
    if checkpoint.get("activation_allowed") is not False or checkpoint["policy"]["enabled"]:
        raise RuntimeError("v5 artifact safety contract mismatch")
    if tuple(checkpoint["base_feature_names"]) != tuple(v5.v4.SOURCE_STABLE_FEATURES):
        raise RuntimeError("v5 source-stable feature contract mismatch")
    if tuple(checkpoint["micro_feature_names"]) != tuple(v5.v3.core.MICRO_FEATURES):
        raise RuntimeError("v5 micro feature contract mismatch")
    session = requests.Session()
    session.headers.update(
        {"User-Agent": "crypto-scalping-v5-public-feature-observer/1.0"}
    )
    ...
    stable_indices = [
        v5.v3.core.BASE_FEATURES.index(name)
        for name in v5.v4.SOURCE_STABLE_FEATURES
    ]
    ...
    stream = full_stream[
        [
            "timestamp",
            "close",
            *v5.v4.SOURCE_STABLE_FEATURES,
            *v5.v3.core.MICRO_FEATURES,
        ]
    ].copy()
    ...
    report = {
        "schema_version": "eth_micro_scalp_v5.feature_stream_build.v1",
        "model_id": v5.MODEL_ID,
        "model_sha256": base._sha256(v5.MODEL_PATH),
        ...
        "failure_reason": None if published else "v5 parity or stream contract failed",
    }
