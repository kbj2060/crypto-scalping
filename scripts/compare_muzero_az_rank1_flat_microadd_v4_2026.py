...
MODEL_ID = "muzero_az_rank1_flat_microadd_v4_2026"
DEFAULT_REPORT_OUT = ROOT / "data/ensemble/reports/muzero_az_rank1_flat_microadd_v4_2026.json"
DEFAULT_MODEL_DIR = ROOT / "data/ensemble/supervised/muzero_az_rank1_flat_microadd_v4"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Compare current-top MuZero/AZ Stage2+AZ Exit with expanded flat-only micro-add sleeve v4.")
    ...
    p.add_argument("--max-grid-configs", type=int, default=800)
...
def main() -> int:
    ...
    for i, micro_cfg_row in enumerate(_microadd_grid_configs(args)):
        ...
        if float(val_micro_telemetry.get("microadd_entries_per_day", 0.0)) < 0.10:
            eligible = False
    ...
    joblib.dump(
        ...
        args.model_dir / "rank1_flat_microadd_v4.pkl",
    )
    selector_payload = {
    ...
    report = {
        ...
        "note": "Frozen current-top MuZero/AZ Stage2+AZ Exit baseline preserved exactly; expanded deterministic current-bar-vote micro-adds can fire only when exact sequential baseline replay is flat.",
        ...
        "eval": {
            ...
            "rank1_flat_microadd_v4": candidate_eval,
        ...
        "calibration_audit": {"selector_artifact": str(args.model_dir / "threshold_selector.json"), "model_artifact": str(args.model_dir / "rank1_flat_microadd_v4.pkl")},
        ...
        "limitations": ["Funding, liquidation, and maintenance-margin path are not first-class in this loop-4 implementation."],
    }
