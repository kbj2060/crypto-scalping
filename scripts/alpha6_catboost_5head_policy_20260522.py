...
MODEL_ID = "alpha6_catboost_entry_risk_policy_20260522"
...
DEFAULT_OUT_DIR = ROOT / "tmp/causal_regen_20260516/alpha6_catboost_entry_risk_policy_20260522"
DEFAULT_LABEL_CACHE_DIR = ROOT / "tmp/causal_regen_20260516/alpha6_label_cache_20260522"
LIFECYCLE_CACHE_VERSION = "alpha6-lifecycle-v4"
...
class PolicyConfig:
    ...
    adverse_penalty: float = 1.80
    size_penalty: float = 0.030
    hold_penalty: float = 0.010
    turnover_bonus: float = 0.0009
    tp_min: float = 0.0015
    tp_max: float = 0.0120
    sl_min: float = 0.0008
    sl_max: float = 0.0100
...
def _apply_bucket_preset(cfg: PolicyConfig, preset: str) -> PolicyConfig:
    if str(preset).lower() == "fast":
        return replace(
            cfg,
            notional_buckets=(0.15, 0.25, 0.40),
            tp_atr_buckets=(1.2, 2.2, 3.0),
            sl_atr_buckets=(0.8, 1.5, 2.0),
        )
    return cfg


def _lifecycle_cache_path(
    *,
    cache_dir: Path,
    variant: str,
    train: pd.DataFrame,
...
def _build_lifecycle_labels(frame: pd.DataFrame, cfg: PolicyConfig, *, stride_bars: int, batch_size: int) -> tuple[np.ndarray, dict[str, np.ndarray], dict[str, Any]]:
    ...
    cost_unit = 2.0 * float(cfg.fee + cfg.slip)
    max_notional = max(max(cfg.notional_buckets), 1.0)
    for start in range(0, valid.size, int(batch_size)):
        ...
        tp_grid = [np.clip(float(tp_mult) * atr_now, cfg.tp_min, cfg.tp_max) for tp_mult in cfg.tp_atr_buckets]
        sl_grid = [np.clip(float(sl_mult) * atr_now, cfg.sl_min, cfg.sl_max) for sl_mult in cfg.sl_atr_buckets]
        for action, side in ((1, 1.0), (2, -1.0)):
            side_ret = raw_ret * side
            if side > 0:
                adverse_path = np.maximum(0.0, 1.0 - fut_low / entry_px[:, None])
            else:
                adverse_path = np.maximum(0.0, fut_high / entry_px[:, None] - 1.0)
            combo_cache: list[tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]] = []
            for ti, tp in enumerate(tp_grid):
                for si, sl in enumerate(sl_grid):
                    if side > 0:
                        tp_hit = fut_high >= entry_px[:, None] * (1.0 + tp[:, None])
                        sl_hit = fut_low <= entry_px[:, None] * (1.0 - sl[:, None])
                    else:
                        tp_hit = fut_low <= entry_px[:, None] * (1.0 - tp[:, None])
                        sl_hit = fut_high >= entry_px[:, None] * (1.0 + sl[:, None])
                    hit = tp_hit | sl_hit
                    has_hit = hit.any(axis=1)
                    first_hit = np.where(has_hit, hit.argmax(axis=1), h - 1).astype(np.int64)
                    path_mask = step_index[:, :h] <= first_hit[:, None]
                    ambiguous = tp_hit[row_idx, first_hit] & sl_hit[row_idx, first_hit]
                    win = tp_hit[row_idx, first_hit] & ~ambiguous
                    loss = sl_hit[row_idx, first_hit] | ambiguous
                    mae = np.max(np.where(path_mask, adverse_path, 0.0), axis=1)
                    combo_cache.append((tp, sl, first_hit, win, loss, mae))
            for ni, notional in enumerate(cfg.notional_buckets):
                for combo_i, (tp, sl, first_hit, win, loss, mae) in enumerate(combo_cache):
                    ti = combo_i // len(sl_grid)
                    si = combo_i % len(sl_grid)
                    time_decay = 1.0 / np.sqrt(first_hit.astype(np.float64) + 2.0)
                    edge = np.where(win, tp * time_decay, np.where(loss, -sl, 0.0))
                    pnl = edge * float(notional) - cost_unit * float(notional)
                    hold_frac = (first_hit.astype(np.float64) + 1.0) / max(float(h), 1.0)
                    score = (
                        pnl
                        - float(cfg.mae_penalty_lambda) * mae * float(notional)
                        - float(cfg.size_penalty) * (float(notional) / max_notional) ** 2
                        - float(cfg.hold_penalty) * hold_frac
                        + np.where(win, float(cfg.turnover_bonus) / np.maximum(first_hit.astype(np.float64) + 1.0, 1.0) ** 0.35, 0.0)
                    )
                    if action == 1:
                        take = score > best_long
                    else:
                        take = score > best_short
                    if not np.any(take):
                        continue
                    if action == 1:
                        best_long[take] = score[take]
                        best_long_idx["notional"][take] = int(ni)
                        best_long_idx["take_profit"][take] = int(ti)
                        best_long_idx["stop_loss"][take] = int(si)
                    else:
                        best_short[take] = score[take]
                        best_short_idx["notional"][take] = int(ni)
                        best_short_idx["take_profit"][take] = int(ti)
                        best_short_idx["stop_loss"][take] = int(si)
        choose_long = ((best_long - best_short) > float(cfg.direction_margin)) & (best_long > min_edge_now)
        choose_short = ((best_short - best_long) > float(cfg.direction_margin)) & (best_short > min_edge_now)
        y["action"][start:end] = np.where(choose_long, 1, np.where(choose_short, 2, 0)).astype(np.int64)
        y["notional"][start:end] = np.where(choose_long, best_long_idx["notional"], np.where(choose_short, best_short_idx["notional"], 0)).astype(np.int64)
        y["take_profit"][start:end] = np.where(choose_long, best_long_idx["take_profit"], np.where(choose_short, best_short_idx["take_profit"], 0)).astype(np.int64)
        y["stop_loss"][start:end] = np.where(choose_long, best_long_idx["stop_loss"], np.where(choose_short, best_short_idx["stop_loss"], 0)).astype(np.int64)
        chosen_score = np.where(choose_long, best_long, np.where(choose_short, best_short, float(cfg.cash_score)))
        y["quality"][start:end] = chosen_score
    meta = {
        "candidates": int(valid.size),
        "stride_bars": int(stride_bars),
        "max_train_horizon_bars": int(h),
        "removed_heads": ["max_hold", "cooldown"],
        "trained_heads": ["action", "quality", "notional", "take_profit", "stop_loss"],
        "labeling_basis": "alpha6_quality_filtered_path_scored",
...
def _load_or_build_lifecycle_labels(
    train: pd.DataFrame,
    cfg: PolicyConfig,
    *,
    stride_bars: int,
    ...
    if cache_dir is not None and not disable_cache:
        ...
        cache_path = _lifecycle_cache_path(
            cache_dir=cache_dir,
            variant=variant,
            train=train,
            cfg=cfg,
        ...
        if cache_path.exists():
            ...
            return payload["valid"], payload["y"], meta
    valid, y, meta = _build_lifecycle_labels(train, cfg, stride_bars=stride_bars, batch_size=batch_size)
    meta = dict(meta)
    meta["cache_hit"] = False
    if cache_path is not None:
        joblib.dump({"valid": valid, "y": y, "meta": meta}, cache_path)
        meta["cache_path"] = str(cache_path)
    return valid, y, meta
...
def _fit_models(x: np.ndarray, y: dict[str, np.ndarray], args: argparse.Namespace) -> dict[str, Any]:
    ...
    if np.unique(y["quality"]).size < 2:
        models["quality_model"] = _ConstantRegressor(float(y["quality"][0]) if len(y["quality"]) else 0.0)
    else:
        models["quality_model"] = CatBoostRegressor(**_regressor_params(args, args.seed + 99))
        models["quality_model"].fit(Pool(x, y["quality"], weight=weight))
    x_trade = x[trade]
    w_trade = weight[trade]
    for offset, key in enumerate(("notional", "take_profit", "stop_loss"), start=1):
        vals = y[key][trade]
...
def _predict_policy(models: dict[str, Any], x: np.ndarray, frame: pd.DataFrame, cfg: PolicyConfig) -> pd.DataFrame:
    ...
    conf = np.mean(np.vstack([action_conf, c1, c2, c3]), axis=0)
    return pd.DataFrame(
        {
            "action": action,
...
def _backtest(frame: pd.DataFrame, dec: pd.DataFrame, *, threshold: float, fee: float, slip: float) -> dict[str, Any]:
    close = pd.to_numeric(frame["close"], errors="coerce").ffill().to_numpy(dtype=np.float64)
    high = pd.to_numeric(frame["high"], errors="coerce").ffill().to_numpy(dtype=np.float64)
    low = pd.to_numeric(frame["low"], errors="coerce").ffill().to_numpy(dtype=np.float64)
    cash = 1.0
    ...
    def enter(i: int, new_side: int, row: pd.Series) -> None:
        nonlocal side, entry, entry_equity, exposure, hold, tp, sl, cash, exposure_sum, long_entries, short_entries
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Alpha6 CatBoost entry/risk policy without hold/cooldown heads.")
    ...
    ap.add_argument("--cash-score", type=float, default=PolicyConfig.cash_score)
    ...
    cfg = _apply_bucket_preset(replace(PolicyConfig(), cash_score=float(args.cash_score)), str(args.bucket_preset))
