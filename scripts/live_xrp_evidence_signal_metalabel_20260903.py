...
Usage:
    from live_btc_evidence_signal_metalabel_20260902 import compute_xrp_evidence_signals
    out = compute_xrp_evidence_signals()

## ⚠️XRP 포팅 (2026-09-03)

BTC 라이브 스코어러의 자산 상수 + **확정 셀**만 바꾼 포팅. 서빙 5종
(`liquidity_sweep` 0.4886 / `fib_extension_exhaustion` 0.4738은 HOLDOUT AUC 무작위 미만으로 제외).

⭐`FILL_SPEC`의 `mode`/`full_window`는 그 신호 자신의 라벨 정의에서 왔다.
`taker_delta_climax`는 `touch_giveback_sustained`라 **해상에 18봉(2xH)**이 필요하다 —
BTC에서 이걸 H로 착각해 절반 시점에 잘못 확정한 전례가 있다.
"""
...
def _load_models() -> dict[str, Any]:
    ...
    for name, info in _meta()["signals"].items():
        ...
        out[name] = {"clf": clf, "features": feats,
                     "train_hit_rate": info["hit_rate"], "params": info["asset_params"]}
    _MODELS = out
    return out
...
def _build_frame() -> pd.DataFrame | None:
    ...
    ⚠️`build_xrp_5m_evidence_signal_candidates_tier0_20260903.py`가 쓰는 것과 **같은 함수들**을
    ...
    # ⚠️후보 빌드(build_xrp_5m_evidence_signal_candidates_tier0_20260903.py:159~173)와
    # **동일한 공식**으로 파생 피쳐를 만든다. 여기가 어긋나면 학습/추론 패리티가 깨진다.
    bspec = importlib.util.spec_from_file_location(
        "btc_cand_build",
        ROOT / "scripts/build_xrp_5m_evidence_signal_candidates_tier0_20260903.py")
...
def compute_xrp_evidence_signals() -> dict[str, Any]:
...
# build_xrp_5m_evidence_signal_candidates_tier0_20260903.py는 NAMED_TRIGGERS에 "z"를 포함한
# 이름을 쓰는데, contexts_report.json(그리고 이를 그대로 쓰는 HIT_SPEC/HOLDOUT_AUC, 섀도우
# 러너)만 축약된 이름을 쓴다. 위 compute_xrp_evidence_signals()는 `col = f"{side}_{name}"`를
...
# ⭐XRP 확정 셀. mode/full_window는 그 신호 자신의 라벨 정의에서 온다
# (BTC에서 이걸 뭉갰다가 라이브 hit률이 2.6배 과대평가됐다).
FILL_SPEC = {
    "demarker_extreme":         {"k": 1.5, "horizon": 2,  "mode": "touch"},
    "kalman_deviation_meanrev": {"k": 2.0, "horizon": 5,  "mode": "touch"},
    "short_term_return_z":      {"k": 1.5, "horizon": 12, "mode": "touch_mae_capped",
                                 "k_loss_mult": 2.0},
    "taker_delta_climax":       {"k": 1.5, "horizon": 9,  "mode": "full_window",
                                 "full_window": 18},
...
def compute_xrp_evidence_signals_panel(history_bars: int = PANEL_HISTORY_BARS) -> dict[str, Any]:
    ...
    위 compute_xrp_evidence_signals()(섀도우 러너 전용, 그 모양에 의존하는 프로세스가 이미
...
if __name__ == "__main__":
    t0 = time.time()
    r = compute_xrp_evidence_signals()
