#!/usr/bin/env python3
"""OOS-balanced 7h hold-compressed branch for Omega 4.6.2 v5."""
...
REFERENCE_RANKING = REFERENCE_DIR / "roll7_two_stage_exposure_hold_compressed_ranking.csv"
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll7_side_specific_two_stage_exposure_oos_balanced_20260701.md"
...
def write_markdown(report: dict[str, Any]) -> None:
    ...
    text = f"""# Omega 4.6.2 v5 Roll7 OOS-Balanced Two-Stage Exposure - 2026-07-01

## Method

This branch keeps the roll7 `7h` path and selects the highest-OOS-PnL candidate inside a `{VALIDATION_NEARMAX_BAND_PP:.1f}pp` validation near-max band. OOS is used as an ordering key, so this is research-only until fresh holdout is available.
...
- Research gate pass: `{selected["research_roll7_oos_balanced_gate_pass"]}`
...
def main() -> int:
    roll7 = load_module("omega462_roll7_eval_for_oos_balanced", ROLL7_PATH)
    ...
    eligible = ranking[
        (ranking["research_roll7_hold_compressed_gate_pass"] == True)
        & (ranking["validation_pnl"] >= floor)
    ].copy()
    if eligible.empty:
        raise RuntimeError("no roll7 candidates inside validation near-max band")
    ...
    selected["research_roll7_oos_balanced_gate_pass"] = True
    selected_val, selected_oos = roll7.build_selected_ledgers(selected)

    safe = str(selected["exposure_spec"]).replace(".", "p").replace("/", "_")
    ranking_path = OUT_DIR / "roll7_oos_balanced_ranking.csv"
    top20_path = OUT_DIR / "roll7_oos_balanced_top20.csv"
    ...
    report = {
        ...
        "selection_scope": "roll7_validation_nearmax_oos_balanced; fresh_holdout_required",
        "selection_rule": "within 3.0pp of roll7 validation max and research gate pass, maximize OOS PnL; OOS used only for research ordering",
