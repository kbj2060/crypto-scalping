...
CHRONOS_PATH = ROOT / "data/splits/year_oos/btc_5m_chronos_feature_20260806.parquet"
...
def main() -> int:
    ...
    df = df.merge(chronos, on="timestamp", how="inner")
    df = df.dropna(subset=["zigzag_action"]).reset_index(drop=True)

    chronos_cols = ["chronos_q10_delta", "chronos_q50_delta", "chronos_q90_delta", "chronos_width", "chronos_mean_delta"]
    ...
    ranks = [list(imp.index).index(c) for c in chronos_cols]
    print(f"\nchronos feature ranks (0=top, out of {len(imp)}): {dict(zip(chronos_cols, ranks))}")
    print(imp.head(10).to_string())

    df[["timestamp", "pred"]].to_parquet(ROOT / "tmp/btc_1h_volregime_20260805/btc5m_layerB_chronos_pred.parquet", index=False)
