...
def simulate_profile(df: pd.DataFrame, ckpt_path: str, profile: str) -> dict:
    with tempfile.TemporaryDirectory(prefix=f"mdd_guard_{profile}_") as tmpdir:
        ...
        for i in range(60, len(df) - 1):
            ...
            if meta_router.pos is None:
                ...
                if abs_action < close_th:
                    ...
                    fa, kelly = 1, min(abs_action, max_kelly)
                if profile in {"circuit_breaker", "convex_guard", "tox_guard"} and live_unr <= -0.040:
                    fa, kelly = 0, 0.0
            else:
                live_unr = float(meta_router._net_pnl_frac(current_price))
                if abs_action < close_th:
            ...
            if fa in (1, 2):
                ...
                toxicity = _safe_float(last_row.get("shadow_toxicity_score", 0.0), 0.0)
                ...
                if profile == "plain_2x":
                    ...
                    if loss_streak >= 3:
                        ...
                        base_mult = 1.55
                    if drawdown >= 0.05:
                        base_mult = min(base_mult, 1.20)
                    if very_bad:
                        base_mult = min(base_mult, 1.15)
                elif profile == "tox_guard":
                    if toxicity >= 0.85 or queue_collapse >= 0.75:
                        base_mult = 1.00
                    elif toxicity >= 0.60 or queue_collapse >= 0.55:
                        base_mult = 1.25
                    elif bad:
                        base_mult = 1.50
                elif profile == "trend_smart":
                    if good and flow_good and conf > 0.62 and not very_bad:
                        base_mult = 2.20
                    elif aligned and conf > 0.55 and toxicity < 0.45:
                        base_mult = 1.80
                    elif bad:
                        base_mult = 1.35
                    if drawdown >= 0.04:
                        base_mult = min(base_mult, 1.25)
                else:
...
def main() -> None:
    ...
    profiles = ["plain_2x", "circuit_breaker", "convex_guard", "tox_guard", "trend_smart"]
    ...
    out_json = args.out_json or os.path.join(
        ...
        f"explore_mdd_guards_2x_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
