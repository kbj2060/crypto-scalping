...
MODEL_ID = "btc_omega4_2_trade_risk_sidecar_20260708"
...
def _replay_with_risk(
    ...
    *,
    ...
    # Default-off trailing stop (2026-08-07 re-validation of the G1 S3 lever on the promoted
    # swingtransition model; same convention as the ETH replay functions' trailing args --
    # leaving both None reproduces the original behaviour exactly). Once favorable excursion
    # through the PREVIOUS bar reaches trailing_activate * take_profit, exit when the current
    # bar's move falls trailing_trail * stop_loss behind that running favorable extreme. The
    # extreme excludes the current bar (using bar i's own move to set the stop bar i is tested
    # against would be intrabar lookahead -- G1's causality note).
    trailing_activate: float | None = None,
    trailing_trail: float | None = None,
    device: torch.device,
    ...
    for i in range(0, len(frame) - 2):
        if pos != 0:
            ...
            mfe_prev = mfe  # favorable extreme through bar i-1 only (trailing causality)
            mfe = max(mfe, move)
            mae = min(mae, move)
            eq = cash * (1.0 + unreal)
        else:
        ...
        if pos != 0:
            ...
            if take_profit > 0.0 and move >= take_profit:
                ...
                trailing_activate is not None
                and trailing_trail is not None
                and take_profit > 0.0
                and stop_loss > 0.0
                and mfe_prev >= float(trailing_activate) * take_profit
                and move <= mfe_prev - float(trailing_trail) * abs(stop_loss)
            ):
