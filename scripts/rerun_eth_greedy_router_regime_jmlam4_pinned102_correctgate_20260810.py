"""Re-run the trusted 2-month JM router comparison with sidecars retrained under the CORRECT real
production risk gate (notional 0.45-0.95, MDD floor -25.00%, matching the currently-live sidecars'
own selection_rule -- see tmp/causal_regen_20260516/omega4_2_trade_risk_sidecar_20260622_plus_t12_
livepass_{h48qual_q050,zig075_q075}_precomputed_20260630/report.json). Supersedes
rerun_eth_greedy_router_regime_jmlam4_pinned102_matched_20260809.py, whose sidecars used a
mistaken -8.00% MDD floor (the base script's argparse default, not what the live model was actually
built under) and had to be relaxed to notional 0.0/0.0 to produce any result at all.
"""
...
OUT_DIR = ROOT / "tmp/eth_greedy_router_regime_jmlam4_pinned102_correctgate_20260810"
...
JM_H48QUAL_SIDECAR = ROOT / "tmp/causal_regen_20260516/omega4_2_trade_risk_sidecar_20260622_pinned102_jmlam4_q070_correctgate_20260810/risk_sidecar.pkl"
...
JM_ZIG075_SIDECAR = ROOT / "tmp/causal_regen_20260516/omega4_2_trade_risk_sidecar_20260622_pinned102_jmlam4_q080_correctgate_20260810/risk_sidecar.pkl"
...
def main() -> int:
    ...
    result_jm = run_router("jmlam4_pinned102_correctgate", use_jm=True, common_ts=common_ts_series)
    out = {
        "window": [START, END],
        "common_bars": len(common_ts),
        "note": "CORRECTED-GATE: sidecars retrained under the real live MDD floor (-25%, not the mistaken -8%), notional 0.45-0.95 unchanged.",
        "baseline_wide24_full_router": result_baseline,
        "jmlam4_pinned102_correctgate_full_router": result_jm,
    }
