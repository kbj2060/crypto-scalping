#!/usr/bin/env python3
"""SINGLE final HOLDOUT (2026-04-01+) exposure for the fib_extension_exhaustion trailing-stop
cost-gate -- final config SL=3.5xATR/ARM=0.5xATR/Trail=0.1xATR, picked from
backtest_eth_fib_extension_exhaustion_trailing_gridsearch_20260831.py (9/96 combos passed VAL+OOS,
this was the #1 combo by min(VAL,OOS)=+3.00bp) + backtest_eth_fib_extension_exhaustion_trailing_
optimistic_crosscheck_20260831.py (robust, -1.23/+0.29bp spread) + backtest_eth_fib_extension_
exhaustion_trailing_random_entry_baseline_20260831.py (random entries: VAL-2.89bp/OOS-2.11bp, both
NEGATIVE, vs the real signal's VAL+15.15bp/OOS+3.00bp -- confirms the very high 88-93% win rate is
an ARM=0.5 exit-structure artifact, same as orthogonal_combo, but the avg-bp edge over random is
genuine). Companion to the classification HOLDOUT already touched in research_eth_fib_extension_
exhaustion_metalabel_final_20260831.py (AUC 0.6210) -- together these are the ONE holdout exposure
for this model; do not re-run with a different SL/ARM/Trail after seeing this result.
"""
...
HORIZON_BARS = 20
SL, ARM, TRAIL = 3.5, 0.5, 0.1
HOLDOUT_START = pd.Timestamp("2026-04-01")


def log(msg: str) -> None:
    print(f"[fib_ext_trailing_HOLDOUT] {msg}", flush=True)
...
def main() -> int:
    ...
    log(f"(for reference: VAL was +15.15bp/win93.2%, OOS was +3.00bp/win87.6% at this exact config; "
        f"random-entry baseline was VAL-2.89bp/OOS-2.11bp)")
    return 0
