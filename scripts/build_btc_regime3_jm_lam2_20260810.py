"""BTC regime3-current classifier, Statistical Jump Model k=3, lambda=2 (not lambda=4).
Direct fork of scripts/build_btc_regime3_jm_lam4_20260809.py -- only LAMBDA/TAG differ. lambda=2 was
identified by scripts/sweep_btc_regime3_jm_lambda_temperature_20260810.py as dominating lambda=4 on
detector-level balanced_accuracy across all three years (2024/2025/2026: +1.5-2pp) while retaining
reasonable persistence (median run 13-14 bars vs raw HMM's ~8 bars). This build produces the
downstream-ready CSVs to test whether that detector-level gain survives into the full parent+sidecar
retrain, per the user's request to push the lambda=2 finding through the same downstream pipeline
lambda=4 went through.
"""
...
REPORT_PATH = ROOT / "data/ensemble/reports/btc_regime3_current_jm_lam2_20260810_report.json"
TAG = "jmlam2_20260810"
...
def main() -> None:
    ...
    print(f"[2/4] fitting JM (k=3, lambda={LAMBDA}) on 2024 only")
    ...
    print(f"[report] in-sample (2024 fit window) balanced_accuracy: {fit_bal_acc:.4f}")
    ...
    report = {
        "model_id": payload["model_id"], "label_mode": LABEL_MODE, "prefix_stem": PREFIX_STEM,
        "feature_set": FEATURE_SET, "feature_cols": cols, "fit_source": str(DEFAULT_TRAIN_2024),
        "jm_lambda": LAMBDA, "jm_k": K, "jm_temperature": temperature,
        "state_order_bear_chop_bull": [bear_i, chop_i, bull_i],
        "state_mean_fwd12bar_logret": mean_ret_by_state,
        "fit_in_sample_balanced_accuracy": float(fit_bal_acc), "outputs": {},
    }
    ...
    for src in DEFAULT_TRANSFORMS:
        ...
        ev = {
            "rows": int(len(y)), "accuracy": float(acs(y, pred)), "balanced_accuracy": float(bas(y, pred)),
