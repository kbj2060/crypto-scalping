#!/usr/bin/env python3
"""정리된 133피쳐(리던던시 감사) DC 5시드 분석 -- 개별 조건부 방향정확도 + 5시드 딥앙상블,
DC 원본(158피쳐/동일 TRAIN_CSV/EVAL_CSV/epochs=2) 베이스라인과 비교. 피쳐셋 하나만 다른
단일변수 비교(OOS 행수도 51,746으로 원본과 동일 -- canonicaldata 래퍼를 그대로 재사용했으므로)."""
...
SEEDS = [10160857, 750183690, 439246437, 661223132, 513194591]
...
def _seed_dir(seed: int) -> Path:
    return ROOT / f"tmp/causal_regen_20260516/omega4_3head_parent72_loose_entry_quality_20260620_dc_pruned133_unified_single_model_seed{seed}_20260820"
...
def main() -> None:
    ...
    print(f"\n[pruned133 개별5시드] cond_acc: {min(accs):.1f}~{max(accs):.1f}% "
          f"(DC원본(158피쳐/동일설정) 베이스라인: 48.2~51.4%)", flush=True)
    ...
    print(f"\n[pruned133 5시드 딥앙상블] n_bars={ens['n_bars']:,} LONG%={ens['long_pct']:.1f}% "
          f"cond_acc={ens['cond_acc']:.1f}% pnl={ens['pnl']:+.2f} mdd={ens['mdd']:+.2f} "
          f"trades={ens['trades']} wr={ens['wr']:.3f}", flush=True)

    out_path = ROOT / "tmp/eth_dc_pruned133_analysis_20260820.json"
