...
def _controller(profile: str, side: str, row: pd.Series, balance: float, peak_balance: float, recent_pnls: list[float]) -> float:
    ...
    core = (
        1.15 * (1.0 if aligned else -0.35)
        + 1.20 * np.clip(conf - 0.50, -0.50, 0.50)
        + 0.85 * np.clip(flow_score, -1.0, 1.0)
        + 0.35 * np.clip(regime_conf - 0.50, -0.50, 0.50)
        - 1.00 * min(qwidth / 0.012, 1.5)
        - 0.80 * min(vol_z / 2.5, 1.5)
        - 0.85 * min(toxicity / 0.9, 1.5)
        - 0.65 * min(aftershock / 0.8, 1.5)
        - 0.55 * min(queue_collapse / 0.9, 1.5)
        - 0.95 * min(drawdown / 0.06, 1.5)
        - 0.45 * loss_streak
        - (0.55 if whipsaw else 0.0)
    )

    if profile == "controller_balanced":
        raw = core
        lev = 1.0 + 1.0 * _sigmoid(raw)
        lev = min(lev, 2.0)
        if drawdown >= 0.04:
            lev = min(lev, 1.45)
        if whipsaw and (qwidth > 0.009 or vol_z > 1.5):
            lev = min(lev, 1.10)
        return float(np.clip(lev, 1.0, 2.0))

    if profile == "controller_conservative":
        raw = core - 0.25
        lev = 1.0 + 0.85 * _sigmoid(raw)
        lev = min(lev, 1.85)
        if drawdown >= 0.03:
            lev = min(lev, 1.30)
        if loss_streak >= 2:
            lev = min(lev, 1.15)
        if toxicity > 0.7 or aftershock > 0.65:
            lev = 1.0
        return float(np.clip(lev, 1.0, 2.0))

    if profile == "controller_convex":
        raw = core + 0.10
        lev = 1.0 + 1.10 * _sigmoid(raw)
        lev = min(lev, 2.0)
        if conf > 0.66 and flow_score > 0.08 and aligned and drawdown < 0.02 and toxicity < 0.45 and qwidth < 0.006:
            lev = min(2.0, lev + 0.15)
        if drawdown >= 0.03:
            lev = min(lev, 1.40)
        if whipsaw:
            lev = min(lev, 1.05)
...
def simulate(df: pd.DataFrame, ckpt_path: str, profile: str) -> dict:
    with tempfile.TemporaryDirectory(prefix=f"lev_ctrl_{profile}_") as tmpdir:
        ...
        for i in range(60, len(df) - 1):
            ...
            if fa in (1, 2):
                ...
                lev_mult = _controller(profile, side, last_row, balance, peak_balance, recent_pnls)
...
def main() -> None:
    ...
    profiles = ["controller_conservative", "controller_balanced", "controller_convex"]
    results = [simulate(df, args.ckpt_path, p) for p in profiles]
    payload = {
    ...
    out_json = args.out_json or os.path.join(
        ...
        f"backtest_leverage_controller_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
    )
