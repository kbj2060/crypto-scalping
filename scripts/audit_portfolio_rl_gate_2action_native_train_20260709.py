#!/usr/bin/env python3
"""Audit native-trained 2-action portfolio RL gate."""
...
ART_DIR = ROOT / "tmp/causal_regen_20260516/portfolio_rl_gate_2action_native_train_20260709"
REPORT_PATH = ART_DIR / "report.json"
OUT_JSON = ART_DIR / "redteam_audit.json"
OUT_MD = ROOT / "docs/audits/portfolio_rl_gate_2action_native_train_redteam_20260709.md"
...
def main() -> int:
    ...
    if report.get("native_training_transition_coverage") != "rule_take_all_visited_validation_events_only":
        _issue(issues, "P1", "training_coverage", f"unexpected coverage {report.get('native_training_transition_coverage')}")
    _issue(issues, "P1", "promotion_grade", "native training used only 25 rule-visited validation events; promotion_grade remains false")

    ledgers = {
        "validation_rule_take_all": ART_DIR / "validation_rule_take_all_ledger.csv",
        "validation_native_rl_gate": ART_DIR / "validation_native_rl_gate_ledger.csv",
        "oos_rule_take_all": ART_DIR / "oos_extended_rule_take_all_ledger.csv",
        "oos_native_rl_gate": ART_DIR / "oos_extended_native_rl_gate_ledger.csv",
    }
    diag = {name: _audit_ledger(name, path, issues) for name, path in ledgers.items()}
    res = report["results"]
    if res["native_rl_gate"]["oos_extended"] == res["rule_take_all"]["oos_extended"]:
        _issue(issues, "P2", "oos_no_policy_effect", "native RL gate exactly matches rule_take_all on OOS")
    if res["native_rl_gate"]["oos_extended"]["mdd"] < -25.0:
        _issue(issues, "P1", "oos_mdd_budget", f"OOS MDD={res['native_rl_gate']['oos_extended']['mdd']:.2f}%")
    audit = {
        "audit_id": "portfolio_rl_gate_2action_native_train_redteam_20260709",
