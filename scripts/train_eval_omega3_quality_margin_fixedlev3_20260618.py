...
MODEL_ID = "omega3_quality_margin_fixedlev3_price_move_risk_20260618"
...
PARENT_RISK_MARGIN_FRACTION_MAX = PARENT_RISK_NOTIONAL_MAX / FIXED_LEVERAGE
...
def _risk_from_path_row(row: pd.Series, prefix: str, default: sleeve.FallbackRisk) -> sleeve.FallbackRisk:
    ...
    notional = float(np.clip(margin_fraction * leverage, float(PARENT_RISK_NOTIONAL_MIN), float(PARENT_RISK_NOTIONAL_MAX)))
...
def _risk_from_predictions(
    ...
    return sleeve.FallbackRisk("pred_quality_margin_fixedlev3", take_profit, stop_loss, notional, leverage, int(default_risk.max_hold_bars))
...
def main() -> int:
    ...
    diagnostics: dict[str, Any] = {
        ...
        "parent_risk_source": {
            ...
            "head_contract": "tp_price_move/sl_price_move/margin_fraction; leverage fixed at 3; notional and account TP/SL are derived",
        },
        "risk": {**asdict(RISK), "fixed_leverage": float(FIXED_LEVERAGE)},
