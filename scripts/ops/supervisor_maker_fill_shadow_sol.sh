#!/usr/bin/env bash
# SOL 메이커 체결 시뮬레이션(peg vs static) 워커의 크래시 재기동 래퍼 (2026-09-04).
#
...
#
# 결정시점 동기화 arm 사용 -- SOL은 microstructure.duckdb에 orderbook_decision_snapshots_sol이 있다.
#
...
for pid in $(pgrep -f "[m]aker_fill_shadow_worker.py"); do
  ...
  if [ "${sym:-ETHUSDT}" = "SOLUSDT" ]; then
    echo "[$(date -Iseconds)] SOL maker_fill이 이미 실행 중(pid $pid) -- 켜지 않는다." >&2
    exit 1
  fi
done

export PYTHONPATH="$ROOT${PYTHONPATH:+:$PYTHONPATH}"
export MAKER_SHADOW_SYMBOL="SOLUSDT"
export MAKER_SHADOW_SPACING_S="300"
export MAKER_SHADOW_TIMEOUT_S="120"
export MAKER_SHADOW_POLICIES="peg,static"
export MAKER_SHADOW_DB_PATH="$ROOT/data/live/maker_fill_shadow_sol.duckdb"
export MAKER_SHADOW_DECISION_TABLE="orderbook_decision_snapshots_sol"

exec "$ROOT/scripts/ops/_supervise.sh" \
  "maker_fill_shadow_worker.py(SOL)" \
  "$ROOT/data/live/.supervisor_maker_fill_shadow_sol.lock" \
  "$ROOT/logs/supervisor/maker_fill_shadow_sol" \
