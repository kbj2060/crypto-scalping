#!/usr/bin/env bash
# 진입 지정가 페이드 v4 섀도우 러너의 크래시 재기동 래퍼 (2026-09-04).
#
...
# 아직 돌고 있는데 이걸 켜면 같은 러너가 둘이 되어 **같은 상태파일(data/live/entry_limit_fade_v4_shadow_state.json)에 동시에 쓴다**.
...
RUNNER="scripts/live_eth_entry_limit_fade_shadow_runner_20260903.py"
if pgrep -f "[${RUNNER:0:1}]${RUNNER:1} --loop" >/dev/null 2>&1; then
  echo "[$(date -Iseconds)] 진입 지정가 페이드 v4: 러너가 이미 실행 중 -- supervisor를 켜지 않는다(중복 방지)." >&2
  exit 1
fi
...
exec "$ROOT/scripts/ops/_supervise.sh" \
  "live_eth_entry_limit_fade_shadow_runner_20260903.py" \
  "$ROOT/data/live/.supervisor_eth_entry_limit_fade_shadow.lock" \
  "$ROOT/logs/supervisor/eth_entry_limit_fade_shadow" \
