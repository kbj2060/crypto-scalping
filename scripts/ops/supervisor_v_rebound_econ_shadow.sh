#!/usr/bin/env bash
# V자반등 경제라벨 섀도우 러너의 크래시 재기동 래퍼 (2026-09-04).
#
...
# 아직 돌고 있는데 이걸 켜면 같은 러너가 둘이 되어 **같은 상태파일(data/live/v_rebound_econ_shadow_state.json)에 동시에 쓴다**.
...
RUNNER="scripts/live_eth_v_rebound_econ_shadow_runner_20260902.py"
if pgrep -f "[${RUNNER:0:1}]${RUNNER:1} --loop" >/dev/null 2>&1; then
  echo "[$(date -Iseconds)] V자반등 경제라벨: 러너가 이미 실행 중 -- supervisor를 켜지 않는다(중복 방지)." >&2
  exit 1
fi
...
exec "$ROOT/scripts/ops/_supervise.sh" \
  "live_eth_v_rebound_econ_shadow_runner_20260902.py" \
  "$ROOT/data/live/.supervisor_v_rebound_econ_shadow.lock" \
  "$ROOT/logs/supervisor/v_rebound_econ_shadow" \
