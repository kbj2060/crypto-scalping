#!/usr/bin/env bash
# XRP 메이커 체결 시뮬레이션(peg vs static) 워커의 크래시 재기동 래퍼 (2026-09-04).
#
...
#
# 결정시점 동기화 arm은 끈다 -- microstructure_xrp.duckdb에 orderbook_decision_snapshots 계열 테이블이 없다(2026-09-04 실측). 주 측정(5분 주기 peg vs static 체결비용)은 그대로 돈다.
#
...
for pid in $(pgrep -f "[m]aker_fill_shadow_worker.py"); do
  ...
  if [ "${sym:-ETHUSDT}" = "XRPUSDT" ]; then
    echo "[$(date -Iseconds)] XRP maker_fill이 이미 실행 중(pid $pid) -- 켜지 않는다." >&2
    exit 1
  fi
done

export PYTHONPATH="$ROOT${PYTHONPATH:+:$PYTHONPATH}"
export MAKER_SHADOW_SYMBOL="XRPUSDT"
export MAKER_SHADOW_SPACING_S="300"
export MAKER_SHADOW_TIMEOUT_S="120"
export MAKER_SHADOW_POLICIES="peg,static"
export MAKER_SHADOW_DB_PATH="$ROOT/data/live/maker_fill_shadow_xrp.duckdb"
export MAKER_SHADOW_DECISION_ENABLED="0"

exec "$ROOT/scripts/ops/_supervise.sh" \
  "maker_fill_shadow_worker.py(XRP)" \
  "$ROOT/data/live/.supervisor_maker_fill_shadow_xrp.lock" \
  "$ROOT/logs/supervisor/maker_fill_shadow_xrp" \
