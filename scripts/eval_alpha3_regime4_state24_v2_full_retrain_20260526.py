...
MODEL_ID = "alpha3_regime4_state24_v2_full_retrain_20260526"
...
OUT_DIR = ROOT / "data/ensemble/supervised/alpha3_regime4_state24_v2_full_retrain_20260526"
REPORT_OUT = ROOT / "data/ensemble/reports/alpha3_regime4_state24_v2_full_retrain_20260526_summary.json"
AUDIT_OUT = ROOT / "data/ensemble/reports/alpha3_regime4_state24_v2_full_retrain_20260526_audit.json"
GRID_OUT = ROOT / "data/ensemble/reports/alpha3_regime4_state24_v2_full_retrain_20260526_grid.csv"
...
def _merge_state24(base: pd.DataFrame, side: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
    base_norm = base.copy()
    side_norm = side.copy()
    base_norm["timestamp"] = pd.to_datetime(base_norm["timestamp"], utc=True, errors="coerce").dt.tz_convert(None)
    side_norm["timestamp"] = pd.to_datetime(side_norm["timestamp"], utc=True, errors="coerce").dt.tz_convert(None)
    base_norm = base_norm.dropna(subset=["timestamp"]).sort_values("timestamp").drop_duplicates("timestamp", keep="last").reset_index(drop=True)
    side_norm = side_norm.dropna(subset=["timestamp"]).sort_values("timestamp").drop_duplicates("timestamp", keep="last").reset_index(drop=True)
    v2_cols = [c for c in side.columns if c.startswith(TARGET_STATE24_PREFIX)]
    if not v2_cols:
        raise RuntimeError("state24 sidecar has no v2 columns")
    merged = base_norm.merge(side_norm[["timestamp", *v2_cols]], on="timestamp", how="left", suffixes=("", "__v2"))
    missing = [c for c in v2_cols if c not in merged.columns]
    if missing:
        raise RuntimeError(f"state24 merge failed; missing merged columns: {missing[:20]}")
    na_cols = [c for c in v2_cols if merged[c].isna().any()]
    if na_cols:
        raise RuntimeError(f"state24 merge produced NaN values: {na_cols[:20]}")
    return merged, v2_cols


def _build_feature_cols(cols: list[str], state24_v2_cols: list[str]) -> tuple[list[str], list[str]]:
...
def _safe_seq_cols(df: pd.DataFrame) -> list[str]:
    cols = [c for c in v27._seq_cols(df) if not c.startswith(CLEAN_PREFIX) and not c.startswith(SOURCE_STATE24_PREFIX)]
    bad = [
        c
        for c in cols
...
def main() -> int:
    ...
    feature_cols, removed_cols = _build_feature_cols(raw_cols, state24_v2_cols)
    ...
    report = {
        ...
        "feature_contract": {
            ...
            "source_sidecars": {"2025": str(SIDE_CLEAN4_2025), "2026": str(SIDE_CLEAN4_2026)},
        },
