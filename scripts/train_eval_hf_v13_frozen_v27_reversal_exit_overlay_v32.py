...
MODEL_ID = "hf_v13_frozen_v27_reversal_exit_overlay_v32_20260511"
...
DEFAULT_OUT_DIR = ROOT / "data/ensemble/supervised/hf_v13_frozen_v27_reversal_exit_overlay_v32_20260511"
DEFAULT_REPORT = ROOT / "data/ensemble/reports/hf_v13_frozen_v27_reversal_exit_overlay_v32_20260511_summary.json"
DEFAULT_AUDIT = ROOT / "data/ensemble/reports/hf_v13_frozen_v27_reversal_exit_overlay_v32_20260511_audit.json"
DEFAULT_GRID = ROOT / "data/ensemble/reports/hf_v13_frozen_v27_reversal_exit_overlay_v32_20260511_grid.csv"
SEQ_LEN = 72
...
def _overlay_grid() -> list[OverlayConfig]:
    return [
        OverlayConfig("v32_p055_hold1", 0.010, 0.004, 1.2, 12, 0.55, 1, 0.045, 0.022, 48),
        OverlayConfig("v32_p060_hold2", 0.010, 0.004, 1.2, 12, 0.60, 2, 0.045, 0.022, 48),
        OverlayConfig("v32_p065_hold3", 0.010, 0.004, 1.2, 12, 0.65, 3, 0.045, 0.022, 48),
        OverlayConfig("v32_p060_n1", 0.010, 0.004, 1.0, 12, 0.60, 2, 0.045, 0.022, 48),
        OverlayConfig("v32_p055_n1", 0.010, 0.004, 1.0, 12, 0.55, 1, 0.045, 0.022, 48),
        OverlayConfig("v32_p062_precision", 0.012, 0.005, 1.0, 12, 0.62, 2, 0.045, 0.022, 48),
    ]
...
def _predict_reversal_prob(model: Any, state: dict[str, float]) -> float:
...
def _reversal_label(frame: pd.DataFrame, i: int, side: int, horizon: int, vol_anchor: float) -> int:
    px0 = float(frame["close"].iloc[i])
    down_bar = max(vol_anchor, 0.0035)
    up_bar = max(vol_anchor * 1.10, 0.0030)
    for j in range(i + 1, min(i + horizon + 1, len(frame))):
        px = float(frame["close"].iloc[j])
        ret = (px - px0) / max(px0, 1e-12) if side > 0 else (px0 - px) / max(px0, 1e-12)
        if ret <= -down_bar:
            return 1
        if ret >= up_bar:
            return 0
...
def _collect_reversal_training(
    ...
    *,
    ...
    for i in range(0, len(frame) - 2):
        if pos != 0:
            ...
            if owner == "deep_alpha" and hold >= 1:
                state = _deep_state_row(frame, i, pos, edge, margin, hold, unreal, mfe, mae)
                rows.append(state)
                labels.append(_reversal_label(frame, i, pos, horizon=12, vol_anchor=state["vol_anchor"]))
            if reason:
                pos = 0
                owner = ""
    ...
    if not rows:
        raise RuntimeError("no reversal training rows")
    ...
    for i in range(0, len(df) - 2):
        ...
        if pos != 0:
            ...
            if owner == "deep_alpha" and hold >= cfg.min_hold:
                p_rev = _predict_reversal_prob(
                    overlay_model,
                    _deep_state_row(df, i, pos, entry_edge, entry_margin, hold, unreal, mfe, mae),
                )
                if p_rev >= cfg.reversal_p_th:
                    reason = "deep_alpha_reversal_overlay"
            if not reason:
                if take_profit > 0.0 and unreal >= take_profit:
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="V32 frozen V27 with reversal-probability exit overlay.")
...
def main() -> int:
    args = parse_args()
    print("[v32] loading models and data", flush=True)
    ...
    print("[v32] predicting frozen V27 deep utilities", flush=True)
    ...
    print("[v32] predicting frozen parent policy decisions", flush=True)
    ...
    print("[v32] collecting reversal labels", flush=True)
    ...
    print(f"[v32] fitting reversal model rows={len(x_train)} positive_rate={float(np.mean(y_train)):.4f}", flush=True)
    overlay_model.fit(x_train.loc[:, REVERSAL_FEATURES].to_numpy(dtype=np.float32), y_train)

    rows: list[dict[str, Any]] = []
    best: dict[str, Any] | None = None
    print("[v32] running validation grid", flush=True)
    for cfg in _overlay_grid():
        print(f"[v32] validation {cfg.name}", flush=True)
    ...
    print(f"[v32] selected {selected.name}; running 2026 fixed OOS", flush=True)
    for mult in (1, 2, 3):
    ...
    model_path = args.out_dir / "v32_reversal_exit_overlay.pkl"
    ...
    audit = {"status": "pass" if not blocking else "fail", "verdict": verdict, "blocking": blocking, "warnings": warnings, "selection_uses_2026": False, "selection_window": "2025-10-01..2025-12-31", "oos_window": "2026 fixed OOS only after selection", "policy": "frozen_v27_reversal_exit_overlay_v32", "v27_entry_frozen": True, "v21_2_preserved": True, "deep_sleeve_only_when_parent_cash": True, "feature_audit": feature_audit, "train_rows": int(len(x_train)), "positive_rate": float(np.mean(y_train)), "selected_config": asdict(selected), "metrics": metrics, "baseline_v27": {"cost1": V27_COST1, "cost2": V27_COST2, "cost3": V27_COST3}}
    report = {"model_id": MODEL_ID, "design": "V32 freezes the trained V27 entry model and V21.2 jackpot parent. A tabular reversal-probability classifier monitors open deep_alpha positions and closes them when reversal risk exceeds a threshold.", "selected_config": asdict(selected), "selection_result": best, "metrics": metrics, "audit": audit, "artifacts": {"model": str(model_path), "report": str(args.report_out), "audit": str(args.audit_out), "grid": str(args.grid_out), "ledgers": ledgers}}
