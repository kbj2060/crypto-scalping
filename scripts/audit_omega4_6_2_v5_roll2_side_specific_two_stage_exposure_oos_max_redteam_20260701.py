#!/usr/bin/env python3
"""Red-team audit for Omega 4.6.2 v5 roll2 OOS-max branch."""
...
MODEL_ID = "omega4_6_2_v5_roll2_side_specific_two_stage_exposure_oos_max_20260701"
...
RANKING = OUT_DIR / "roll2_oos_max_ranking.csv"
TOP20 = OUT_DIR / "roll2_oos_max_top20.csv"
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll2_side_specific_two_stage_exposure_oos_max_redteam_20260701.md"
TOL = 1.0e-8
...
def main() -> None:
    audit = load_module("omega462_loss_cluster_audit_for_roll2_oos_max", AUDIT_HELPER_PATH)
    roll2_eval = load_module("omega462_roll2_eval_for_oos_max_audit", ROLL2_EVAL_PATH)
    ...
    audit.add_check(
        ...
        report.get("selection_scope") == "roll2_validation_nearmax_oos_max; fresh_holdout_required",
        "research_blocker",
    ...
    audit.add_check(
        checks,
        "selected_gate_pass",
        selected.get("research_roll2_oos_max_gate_pass") is True
        and selected.get("research_roll2_hold_compressed_gate_pass") is True,
        "research_blocker",
        {
            "oos_max": selected.get("research_roll2_oos_max_gate_pass"),
    ...
    verdict = (
        "FULL_LIVE_PASS"
        if full_live_pass
        else "RESEARCH_ROLL2_OOS_MAX_PASS_FULL_LIVE_BLOCKED"
        if research_pass
        else "REDTEAM_FAIL"
    )
    payload = {
        "audit_id": "omega4_6_2_v5_roll2_side_specific_two_stage_exposure_oos_max_redteam_20260701",
