...
MODEL_ID = "hf_v13_margin110_deep_residual_scout_v28_20260511"
...
DEFAULT_OUT_DIR = ROOT / "data/ensemble/supervised/hf_v13_margin110_deep_residual_scout_v28_20260511"
DEFAULT_REPORT = ROOT / "data/ensemble/reports/hf_v13_margin110_deep_residual_scout_v28_20260511_summary.json"
DEFAULT_AUDIT = ROOT / "data/ensemble/reports/hf_v13_margin110_deep_residual_scout_v28_20260511_audit.json"
DEFAULT_GRID = ROOT / "data/ensemble/reports/hf_v13_margin110_deep_residual_scout_v28_20260511_grid.csv"
HORIZONS = (12, 24, 48)
MARGIN110_COST1 = 139.40707050411612
MARGIN110_COST2 = 103.01953110685385
MARGIN110_COST3 = 32.04895618355474
...
class DeepAlphaTCN(nn.Module):
    def __init__(self, seq_dim: int, hidden: int = 72) -> None:
        ...
        self.head = nn.Sequential(nn.Linear(hidden, 96), nn.GELU(), nn.Dropout(0.10), nn.Linear(96, 2))

    def forward(self, seq: torch.Tensor) -> torch.Tensor:
        return self.head(self.net(seq.transpose(1, 2)).squeeze(-1))
...
def _grid() -> list[DeepAlphaConfig]:
    ...
    i = 0
    for edge in (0.006, 0.008, 0.010):
        for margin in (0.0015, 0.0030):
            for notional in (0.5, 0.8, 1.2):
                rows.append(DeepAlphaConfig(f"v28_e{edge:.3f}_m{margin:.4f}_n{notional:.1f}", edge, margin, notional, 0.030, 0.018, 24, 6))
                i += 1
    rows.append(DeepAlphaConfig("v28_slow_high_edge", 0.010, 0.0040, 1.2, 0.045, 0.022, 48, 12))
    rows.append(DeepAlphaConfig("v28_precision_high_edge", 0.014, 0.0050, 1.2, 0.050, 0.022, 48, 12))
    return rows
    ...
    *,
    ...
    for i in range(0, len(df) - 2):
        ...
        if int(dec.action) != ACTION_CASH and int(dec.side) != 0:
            ...
            mfe = mae = 0.0
            actions["margin110_entry"] = actions.get("margin110_entry", 0) + 1
            if record:
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="V28 deep residual scout sleeve directly attached to margin110 parent.")
...
def main() -> int:
    ...
    for cfg in _grid():
        v1 = backtest(val, bundle, val_q, cfg, fee=float(base["fee"]), slip=float(base["slip"]), cost_mult=1.0, decisions=val_dec)
        v2 = backtest(val, bundle, val_q, cfg, fee=float(base["fee"]), slip=float(base["slip"]), cost_mult=2.0, decisions=val_dec)
        v3 = backtest(val, bundle, val_q, cfg, fee=float(base["fee"]), slip=float(base["slip"]), cost_mult=3.0, decisions=val_dec)
    ...
    for mult in (1, 2, 3):
        r = backtest(eval_df, bundle, eval_q, selected, fee=float(base["fee"]), slip=float(base["slip"]), cost_mult=float(mult), decisions=eval_dec, record=(mult == 1))
        if mult == 1:
    ...
    model_path = args.out_dir / "v28_margin110_deep_residual_scout.pt"
    torch.save({"model_id": MODEL_ID, "state_dict": model.state_dict(), "seq_cols": seq_cols, "norm": norm, "selected_config": asdict(selected), "parent_model": str(args.parent_model)}, model_path)
    ...
    if metrics["cost1"]["pnl"] <= MARGIN110_COST1:
    ...
    verdict = "promote" if not blocking and metrics["cost1"]["pnl"] > MARGIN110_COST1 and metrics["cost2"]["pnl"] > 0.0 and metrics["cost3"]["pnl"] > 0.0 else "iterate"
    audit = {"status": "pass" if not blocking else "fail", "verdict": verdict, "blocking": blocking, "warnings": warnings, "selection_uses_2026": False, "selection_window": "2025-10-01..2025-12-31", "oos_window": "2026 fixed OOS only after selection", "policy": "margin110_deep_residual_scout_v28", "margin110_parent_preserved": True, "deep_sleeve_only_when_parent_cash": True, "forbidden_sequence_columns": forbidden_cols, "train_snapshot_count": int(len(train_ds["target"])), "target_mean": np.mean(train_ds["target"], axis=0), "feature_audit": feature_audit, "selected_config": asdict(selected), "metrics": metrics}
    report = {"model_id": MODEL_ID, "design": "Margin110 Deep Residual Scout V28. A TCN predicts long/short unit utility from causal market sequences. The existing hf_v13_clean_regime_margin110 parent entries are preserved; the deep sleeve may open extra trades only when the margin110 parent is CASH.", "parent_model": str(args.parent_model), "model": str(model_path), "feature_manifest": str(manifest_path), "split_policy": "Train 2025 Jan-Sep; select thresholds/notional on 2025 Oct-Dec; evaluate fixed 2026 OOS only after selection.", "baseline_margin110": {"cost1": MARGIN110_COST1, "cost2": MARGIN110_COST2, "cost3": MARGIN110_COST3}, "selected_config": asdict(selected), "selection_result": best, "metrics": metrics, "audit": audit, "artifacts": {"model": str(model_path), "manifest": str(manifest_path), "report": str(args.report_out), "audit": str(args.audit_out), "grid": str(args.grid_out), "ledgers": ledgers}}
