#!/usr/bin/env python3
"""Fine validation-max 10h side-specific bracket branch for Omega 4.6.2 v5."""
...
HELPER_PATH = ROOT / "scripts/eval_omega4_6_2_v5_roll10_side_specific_bracket_daytrade_20260701.py"
MODEL_ID = "omega4_6_2_v5_roll10_side_specific_fine_valmax_20260701"
REFERENCE_MODEL_ID = "omega4_6_2_v5_roll10_side_specific_bracket_daytrade_20260701"
...
AUDIT_MD = ROOT / "docs/audits/omega4_6_2_v5_roll10_side_specific_fine_valmax_20260701.md"
REFERENCE_CAP = 4.20
EPS = 1.0e-12
...
def write_markdown(report: dict[str, Any]) -> None:
    ...
    text = f"""# Omega 4.6.2 v5 Roll10 Side-Specific Fine Valmax - 2026-07-01
...
- Long TP/SL: `{selected["roll10_long_tp_move"]:.4f}` / `{selected["roll10_long_sl_move"]:.4f}`
- Short TP/SL: `{selected["roll10_short_tp_move"]:.4f}` / `{selected["roll10_short_sl_move"]:.4f}`
- Research gate pass: `{selected["research_fine_valmax_gate_pass"]}`
...
def main() -> int:
    helper = load_module("omega462_roll10_side_helper_for_fine_valmax", HELPER_PATH)
    source = helper.load_module("omega462_roll16_source_for_roll10_fine_valmax", helper.SOURCE_MODULE_PATH)
    v1 = source.load_module("omega462_loss_cluster_v1_for_roll10_fine_valmax", source.V1_MODULE_PATH)
    segment_mod = source.load_module("omega462_segment_for_roll10_fine_valmax", source.SEGMENT_MODULE_PATH)
    stop_mod = v1.load_module("omega462_stop_mod_for_roll10_fine_valmax", v1.STOP_MODULE_PATH)
    ...
    bracket_specs = [
        ("fine10_valmax", 0.0200, 0.0450, 0.0250, 0.0400),
        ("fine10_fast", 0.0200, 0.0300, 0.0250, 0.0400),
        ("fine10_oos", 0.0250, 0.0350, 0.0400, 0.0400),
        ("fine10_oos2", 0.0200, 0.0300, 0.0400, 0.0400),
    ]
    ...
    for name, long_tp, long_sl, short_tp, short_sl in bracket_specs:
        val_base = helper.combine_sides(
            helper.split_side(source, segment_mod, val_long, train_market, long_tp, long_sl),
            helper.split_side(source, segment_mod, val_short, train_market, short_tp, short_sl),
            val_skipped,
        )
        oos_base = helper.combine_sides(
            helper.split_side(source, segment_mod, oos_long, eval_market, long_tp, long_sl),
            helper.split_side(source, segment_mod, oos_short, eval_market, short_tp, short_sl),
            oos_skipped,
        )
    ...
    exposure_specs = [
        ...
        for lf in [0.75, 0.85, 0.90, 0.95, 1.00]
        for sf in [1.00, 1.02, 1.04, 1.06]
        ...
        for exposure in exposure_specs:
            for governor in governors:
                ...
                row = {
                    ...
                    "roll10_max_hours": helper.MAX_ROLL_HOURS,
                    "roll10_long_tp_move": long_tp,
                    "roll10_long_sl_move": long_sl,
                    "roll10_short_tp_move": short_tp,
                    "roll10_short_sl_move": short_sl,
                    ...
                    **helper.flatten("validation", stop_mod.metrics(val_work)),
                    **helper.flatten("oos", stop_mod.metrics(oos_work)),
                }
                validation_gate = bool(
                    float(row["validation_pnl"]) > float(reference["validation_pnl"])
                    and float(row["validation_mdd"]) >= -20.0
                    and float(row["validation_max_hold_hours"]) <= helper.MAX_ROLL_HOURS + 1.0e-9
                ...
                oos_gate = bool(
                    ...
                    and float(row["oos_max_hold_hours"]) <= helper.MAX_ROLL_HOURS + 1.0e-9
                ...
                row["validation_fine_valmax_gate_pass"] = validation_gate
                row["oos_safety_gate_pass"] = oos_gate
                row["research_fine_valmax_gate_pass"] = validation_gate and oos_gate
    ...
    ranking = pd.DataFrame(rows).sort_values(
        [
            "research_fine_valmax_gate_pass",
            "selection_score",
            "validation_pnl",
    ...
    status = (
        "RESEARCH_ROLL10_SIDE_SPECIFIC_FINE_VALMAX_PASS"
        if bool(selected["research_fine_valmax_gate_pass"])
        else "NO_ROLL10_SIDE_SPECIFIC_FINE_VALMAX_PASSING_CANDIDATE"
    )
    safe = (
        f"{selected['side_bracket_spec']}__{selected['exposure_spec']}__"
        f"{selected['segment_governor_spec']}"
    ).replace(".", "p").replace("/", "_")
    ranking_path = OUT_DIR / "roll10_side_specific_fine_valmax_ranking.csv"
    top20_path = OUT_DIR / "roll10_side_specific_fine_valmax_top20.csv"
        ...
        json.dumps(
            {
                ...
                "selected_validation": {
                    ...
                    "gate": bool(selected["validation_fine_valmax_gate_pass"]),
                },
                "selected_oos": {
