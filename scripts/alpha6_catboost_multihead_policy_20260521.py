...
MODEL_ID = "alpha6_catboost_multihead_policy_20260521"
...
DEFAULT_OUT_DIR = ROOT / "tmp/causal_regen_20260516/alpha6_catboost_multihead_policy_20260521"
...
def _build_lifecycle_labels(frame: pd.DataFrame, cfg: PolicyConfig, *, stride_bars: int, batch_size: int) -> tuple[np.ndarray, dict[str, np.ndarray], dict[str, Any]]:
    ...
    for start in range(0, valid.size, int(batch_size)):
        ...
        atr_now = atr[idx]
        best = np.full(n, float(cfg.cash_score), dtype=np.float64)
        labels = {k: y[k][start:end].copy() for k in y}
        for action, side in ((1, 1.0), (2, -1.0)):
            ...
            for ni, notional in enumerate(cfg.notional_buckets):
                ...
                for ti, tp_mult in enumerate(cfg.tp_atr_buckets):
                    tp = np.clip(float(tp_mult) * atr_now, cfg.tp_min, cfg.tp_max)
                    for si, sl_mult in enumerate(cfg.sl_atr_buckets):
                        sl = np.clip(float(sl_mult) * atr_now, cfg.sl_min, cfg.sl_max)
                        hit = (exp_path >= tp[:, None]) | (exp_path <= -sl[:, None])
                        ...
                        for hi, hold in enumerate(cfg.max_hold_buckets):
                            hold_i = max(0, min(int(hold), h) - 1)
                            exit_i = np.minimum(first_hit, hold_i).astype(np.int64)
                            pnl = exp_path[row_idx, exit_i] - cost_unit * float(notional)
                            adverse = np.maximum(0.0, -cum_min[row_idx, exit_i])
                            hold_frac = (exit_i.astype(np.float64) + 1.0) / max(float(h), 1.0)
                            score = (
                                pnl
                            ...
                            for r in np.flatnonzero(take):
                                next_end = min(h, int(exit_i[r]) + 1 + 24)
                                if int(exit_i[r]) + 1 >= next_end:
                                    cool_i[r] = 0
                                    continue
                                next_slice = exp_path[r, int(exit_i[r]) + 1 : next_end]
                                reversal = max(0.0, -float(np.min(next_slice)))
                                continuation = max(0.0, float(np.max(next_slice)))
                                if continuation > reversal + 0.006:
                                    ci = 0
                                elif reversal > 0.018:
                                    ci = len(cfg.cooldown_buckets) - 1
                                elif reversal > 0.010:
                                    ci = min(len(cfg.cooldown_buckets) - 1, 3)
                                else:
                                    ci = min(len(cfg.cooldown_buckets) - 1, 1)
                                cool_i[r] = int(ci)
...
def _predict_policy(models: dict[str, Any], x: np.ndarray, frame: pd.DataFrame, cfg: PolicyConfig) -> pd.DataFrame:
    ...
    conf = np.mean(np.vstack([action_conf, c1, c2, c3, c4, c5]), axis=0)
    return pd.DataFrame(
        {
            "action": action,
...
def _backtest(frame: pd.DataFrame, dec: pd.DataFrame, *, threshold: float, fee: float, slip: float) -> dict[str, Any]:
    ...
    def enter(i: int, new_side: int, row: pd.Series) -> None:
        nonlocal side, entry, entry_equity, exposure, hold, max_hold, cooldown, tp, sl, cash, exposure_sum, long_entries, short_entries
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Alpha6 v2 CatBoost multi-head tabular lifecycle policy.")
    ...
    cfg = replace(PolicyConfig(), cash_score=float(args.cash_score))
